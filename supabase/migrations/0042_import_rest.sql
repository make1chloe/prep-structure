-- ─────────────────────────────────────────────────────────────
-- 0042 · 나머지 이관 — 반 · 출결 · 수납 · 상담 · 문의 · 할 일 · 공지 · 휴강
--
-- ⚠️ **반 이름을 안 가져온다** — 원장님: 「반 이름이 없다. **요일+시각이 곧 반**」.
--    옛 이름(「월수1」)은 요일·시각에서 저절로 지어지므로 **별명 칸에도 안 넣는다.**
-- ⚠️ `class_students` 에 **기간이 없다**(class_id·student_id 뿐).
--    계획대로 **시작일 = 이관일**로 박고 「지난 것은 모름」을 매핑표에 남긴다.
--    → 이래서 **이관일 이전 달 회차·수강료는 못 맞춘다.** 검증 4번의 그 자리다.
-- ─────────────────────────────────────────────────────────────
...
begin
  ...
  where a.status='makeup' and exists (select 1 from v2.students x where x.id=a.student_id)
    and a.student_id::text not in (select old_id from v2.import_skip where old_table='students')
  on conflict (student_id, on_date, of_date) do nothing;
