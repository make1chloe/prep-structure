...
-- ══ [일정 화면] v2.day_sheet 에 지각 「얼마나」를 담을 칸
-- 왜: ㉔ 은 「지각 예정에는 **얼마나**가 있다 — 10·20·30·60분, 또는 도착 시각」이라고 못 박았고 `lib/attend.js` 도 `lateMinutes()` 로 값을 다 따진다. 그런데 담을 칸이 없어 `LATE_MIN_COLUMN = null` 이고 `lateSaved: false` 로 나간다. 지금 화면은 「아직 저장되지 않습니다」를 그 자리에 띄우지만, 이 칸이 없으면 **「20분 늦어요」를 학부모께 못 보낸다.**
alter table v2.day_sheet add column if not exists late_min smallint
  check (late_min is null or (late_min >= 0 and late_min <= 600));
comment on column v2.day_sheet.late_min is
  '지각 「얼마나」(분). ⚠️ 도착 시각을 적었으면 lib/attend.js 의 lateMinutes() 가 분으로 바꿔 넣는다.
   ⚠️ null 은 0분이 아니라 「안 적었다」다 — 발송 문구가 그 둘을 갈라야 한다';

-- ⚠️ 이 칸이 생기면 lib/attend.js 의 LATE_MIN_COLUMN 에 'late_min' 을 적고
--    SQL_UPSERT 에 칸 하나를 더한다. 그 전까지는 저장된 척하지 않는다.
...
create table v2.exam_word (                       -- 한 줄 = 「이 낱말이 나오면 이 갈래다」
...
alter table v2.fee_rule
  add constraint fee_rule_target check (student_id is not null or class_id is not null);

-- ② 끝일이 시작일보다 앞설 수 없다 (앞서면 그 달 조회에서 조용히 사라진다)
alter table v2.fee_rule
  add constraint fee_rule_span check (to_date is null or to_date >= from_date);
...
-- ══ [교재 화면] 「실제로 지나간 구간」을 몇 %로 셈하는 한 벌
-- 왜: 계획이 「끝까지 끌어다 놓고 다 봤다를 막는다 — 실제로 지나간 구간을 세고 건너뛴 구간은 안 센다」고 못 박았는데, 겹치는 구간을 합쳐 길이로 나누는 판단이 `lib/` 에도 DB 에도 **한 벌도 없다**. 화면에서 세면 그날부터 규칙이 두 벌이 되므로 이 판에서는 안 셌다. ⚠️ 이 값은 **대략치이지 아이를 판단할 숫자가 아니다** — 함수 주석에 그대로 적어야 한다.
-- 0073_video_seen.sql
-- 겹치는 구간을 합쳐 「실제로 지나간 초」를 센다. 저장하지 않는다 (원칙 5).
...
returns table (secs int, pct int) language sql stable as $$
  with s as (
    select (x->>0)::int as a, (x->>1)::int as b
      from v2.video_view v, lateral unnest(v.spans) as x
     where v.video_id = p_video and v.student_id = p_student
  ),
  ordered as (select a, b, max(b) over (order by a rows between unbounded preceding and 1 preceding) as prev_max from s),
  grp as (select a, b, sum(case when prev_max is null or a > prev_max then 1 else 0 end) over (order by a) as g from ordered),
  merged as (select min(a) as a, max(b) as b from grp group by g)
  select coalesce(sum(b - a), 0)::int,
         case when (select seconds from v2.video where id = p_video) > 0
              then least(100, (coalesce(sum(b - a), 0) * 100 / (select seconds from v2.video where id = p_video)))::int
              else null end
    from merged
$$;
comment on function v2.video_seen(uuid, uuid) is '실제로 지나간 구간의 합(초)과 %. ⚠️ 대략치다 — 아이를 판단할 숫자가 아니다. 끝까지 끌어다 놓은 것은 구간이 안 생기므로 안 세어진다';
-- ⚠️ `v2.video_view.spans` 의 생김새를 못 확인했다(0줄이라 실측이 안 된다). `int4[]` 가 아니라
--    `[[시작,끝], …]` 꼴의 jsonb 여야 위 함수가 맞는다. 지금 칸은 ARRAY 이므로,
--    적는 쪽(아이 화면)이 서기 전에 **무엇을 담을지부터 못 박아야** 한다.
