"use server";
/** 학교별 표 06c 의 손 · 학원 사람만. 판단·쓰기는 lib/grid.js 한 벌(+ 새 표 · 삭제·복구·이름 · 칸 · 줄 · 셀 · 보드 축 · 따로 챙길 아이). 지우는 손이 없다(대전제-6). 단원 목록은 lib/exam unitsOf(06b 와 같은 것) */
...
const wrap = (fn) => act(fn, "학교별 표 06c");   // 손 한 벌은 lib/act.js · 삼키지 않고 서버 기록에 까닭을 남긴다(원칙-1)
...
export async function gridShareAct(id, on) { return wrap(async () => { const { sb } = await staff(); await setShare(sb, id, on); return {}; }); }   // (너) 아이·학부모 공개 스위치
