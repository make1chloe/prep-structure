...
function Head({ col, i, ctx }) {
  ...
  return <th className="colhd" data-g="col" data-col={col.id} data-type={col.type}>
    <div className="cn2"><span className="sgrip">⠿</span>{col.label}<span className="ord"><button type="button" {...icon("왼쪽으로")} disabled={pending || i === 0} data-act="col-left" onClick={() => run(() => colMoveAct(g.id, col.id, "left"), null)}>◀</button><button type="button" {...icon("오른쪽으로")} disabled={pending || i === cols.length - 1} data-act="col-right" onClick={() => run(() => colMoveAct(g.id, col.id, "right"), null)}>▶</button><button type="button" {...icon("칸 삭제")} disabled={pending} data-act="col-retire" onClick={() => run(() => colRetireAct(col.id), `${col.label} 칸을 내렸습니다(지우지 않았습니다)`)}>✕</button></span></div>
...
export default function Board({ d }) {
  ...
  return <>
    ...
    <div className="shtabs" data-g="tabs">{grids.map((x) => <button key={x.id} type="button" className="shtab" aria-pressed={g?.id === x.id} data-act="tab" data-grid={x.id} onClick={() => { setSel(x.id); setFocus(null); }}>{x.label} <i>{alive(x.rows_).length}</i></button>)}
      {retired.length > 0 && <button type="button" className="shtab" aria-pressed={showRetired} data-act="show-retired" onClick={() => setShowRetired(!showRetired)}>삭제한 표 <i>{retired.length}</i></button>}</div>
    ...
    {g && <div data-g="grid" data-grid={g.id}>
      ...
      {view === "table" && <div className="tblwrap" data-g="table" data-focus={focus ? "1" : "0"}><table style={{ minWidth: 700 }}><thead><tr><th style={{ minWidth: 150 }}>{g.rows === "school" ? "학교" : g.rows === "student" ? "학생" : "줄"}</th>{cols.map((col, i) => <Head key={col.id} col={col} i={i} ctx={ctx} />)}<th className="addc">+ 칸</th></tr></thead><tbody>
        {focusRows(rows, focus).map((r, i) => <tr key={r.id} data-g="row" data-row={r.id} className={focus === r.id ? "hi" : ""}>
          <td className="stu"><span className="sgrip">⠿</span><b>{rowTitle(r)}</b><small>{rowSub(r) || (g.rows === "school" ? "🏫" : "")} <button className="lnk" type="button" {...icon("이 줄만 크게")} data-act="row-focus" aria-pressed={focus === r.id} onClick={() => setFocus(focus === r.id ? null : r.id)}>▣</button> <button className="lnk" type="button" {...icon("위로")} disabled={pending || i === 0} data-act="row-up" onClick={() => run(() => rowMoveAct(g.id, r.id, "up"), null)}>▲</button> <button className="lnk" type="button" {...icon("아래로")} disabled={pending || i === rows.length - 1} data-act="row-down" onClick={() => run(() => rowMoveAct(g.id, r.id, "down"), null)}>▼</button> <button className="lnk" type="button" {...icon("줄 삭제")} disabled={pending} data-act="row-retire" onClick={() => run(() => rowRetireAct(r.id), `${rowTitle(r)} 줄을 내렸습니다(지우지 않았습니다)`)}>✕</button></small></td>
    ...
    <div className="savebar" style={{ marginTop: 8 }} data-g="bar">
      <span className="pill" data-g="bar-count">표 {c.grids}종 · 줄 {c.rows} · 칸 {c.cols}{c.retired ? ` · 삭제한 표 ${c.retired}` : ""}</span>
      <span className="spacer" />
    </div>
  </>;
}
