...
import { gridAddAct, gridRetireAct, gridReviveAct, gridRenameAct, colAddAct, colSetAct, colMoveAct, colRetireAct, boardColAct, rowAddAct, rowMoveAct, rowRetireAct, cellAct, watchAct, unitsAct } from "./actions.js";
import { TEMPLATES, COL_TYPES, PICK_OF, typeName, pickName, parseCol, cellText, toggleItem, addItem, checkText, rowTitle, rowSub, alive, liveOf, counts, jumpTargets, boardOf, nextOption, cardTitle, watchSummary, visibleGrids } from "@/lib/grid-plan";
  ...
  const { pending, save, val, setV, edit, setEdit, chk, setChk, chkNew, setChkNew, units, loadUnits, refs } = ctx;
    ...
    if (col.type === "checklist") { const key = `${r.id}|${col.id}`, list = Array.isArray(v) ? v : []; return <>
      ...
      {chk === key && <div className="task" data-g="chk" style={{ marginTop: 4, padding: 8 }}>
        {list.map((x, i) => <label key={i} className="ckl" style={{ display: "flex" }}><input type="checkbox" className="ck" checked={x.done} disabled={pending} onChange={() => save(r, col, toggleItem(list, i), `${x.name} — ${x.done ? "풀었습니다" : "✓"}`)} /> {x.name}</label>)}
...
function Head({ col, i, ctx }) {
  ...
  return <th className="colhd" data-g="col" data-col={col.id} data-type={col.type}>
    ...
    <select value={col.type} aria-label={`${col.label} 종류`} data-g="ctype" style={{ width: "auto", marginTop: 4 }} onChange={(x) => { const type = x.target.value; const options = type === "select" ? (Array.isArray(col.options) && col.options.length ? col.options : ["예정", "진행", "끝"]) : type === "pick" ? { of: "book" } : []; run(() => colSetAct(col.id, { label: col.label, type, options }), `${col.label} — ${typeName(type)}`); }}>{COL_TYPES.map(([k, nm]) => <option key={k} value={k}>{nm}{k === "select" && col.type === "select" ? ` · ${(col.options ?? []).join(" / ")}` : k === "pick" && col.type === "pick" ? ` · ${pickName(col.options?.of)}` : ""}</option>)}</select>
  </th>;
}
export default function Board({ d }) {
  ...
  const save = (r, col, v, what = null) => run(() => cellAct(r.id, col.id, v), what ?? `${rowTitle(r)} · ${col.label} — 저장`, () => setEdit((s) => { const n = { ...s }; delete n[`${r.id}|${col.id}`]; return n; }));
  ...
  return <>
    ...
    {g && <div data-g="grid" data-grid={g.id}>
      <div className="shtitle"><b data-g="grid-label">{g.label}</b><button className="lnk" type="button" data-act="rename" onClick={() => { const nm = window.prompt("표 이름", g.label); if (nm != null && nm.trim() && nm.trim() !== g.label) run(() => gridRenameAct(g.id, nm), "이름을 바꿨습니다"); }}>✎</button><span className="spacer" /><button className="del" type="button" disabled={pending} data-act="grid-retire" onClick={() => run(() => gridRetireAct(g.id), `${g.label} — 내렸습니다(되살릴 수 있습니다)`, () => setSel(null))}>표 삭제</button></div>
      ...
      {view === "table" && focus && (() => { const r = rows.find((x) => x.id === focus); return r ? <div className="card" data-g="focus"><div className="ctitle"><span className="cemo">▣</span>{rowTitle(r)} — 한 줄만 크게</div><div className="left">{cols.map((col) => <div className="lf" key={col.id}><span className="ln">·</span><div><b>{col.label}</b><small>{cellText(col, r.cells?.[col.id], refs) || "—"}</small></div><div style={{ flex: "1 1 220px" }}><Cell r={r} col={col} ctx={ctx} /></div></div>)}</div></div> : null; })()}
      {view === "board" && bd && <div className="nb-board" data-g="board">
        ...
        {bd.axis && bd.columns.map((col) => <div className="nb-col" key={col.key} data-g="bcol" data-key={col.key}><div className="nb-colh"><span className="nb-pill">{col.label}</span><span className="nb-cnt" data-g="bcount">{col.cards.length}</span></div>
          {col.cards.map((r) => <div className="nb-card" key={r.id} data-g="bcard" data-row={r.id}><span className="nb-title">{cardTitle(g, r, refs)}</span>
            ...
            <div className="wv" style={{ marginTop: 6, gap: 4 }}><button className="btn sm" type="button" disabled={pending || !nextOption(bd.axis, col.key, "left")} data-act="card-left" onClick={() => run(() => cellAct(r.id, bd.axis.id, nextOption(bd.axis, col.key, "left")), `${rowTitle(r)} → ${nextOption(bd.axis, col.key, "left")}`)}>◀</button><button className="btn sm" type="button" disabled={pending || !nextOption(bd.axis, col.key, "right")} data-act="card-right" onClick={() => run(() => cellAct(r.id, bd.axis.id, nextOption(bd.axis, col.key, "right")), `${rowTitle(r)} → ${nextOption(bd.axis, col.key, "right")}`)}>▶</button></div></div>)}
