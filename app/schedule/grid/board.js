...
function Cell({ r, col, ctx }) {
  const { pending, save, quick, val, setV, chk, setChk, chkNew, setChkNew, units, refs } = ctx;
    ...
    if (col.type === "checklist") { const key = `${r.id}|${col.id}`, list = Array.isArray(v) ? v : []; return <div className="wv" style={{ gap: 6, marginBottom: 0 }} data-g="chk">
      {list.map((x, i) => <span key={i} className="wv" style={{ gap: 2, marginBottom: 0 }} data-g="chk-item"><label className="ckl"><input type="checkbox" className="ck" checked={x.done} aria-label={`${rowTitle(r)} ${col.label} ${x.name}`} onChange={() => quick(r, col, toggleItem(list, i))} /> {x.name}</label><button type="button" className="lnk" disabled={pending} {...icon(`${x.name} 삭제`)} data-act="chk-remove" onClick={() => save(r, col, removeItem(list, i), `${x.name} · 뺐습니다`)}>✕</button></span>)}
      {chk === key
