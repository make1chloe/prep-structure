  ...
  return (
    <div className="card card-tight" style={{ marginTop: 10, background: "var(--surface-2)" }}>
      <div className="row" style={{ gap: 8, alignItems: "baseline", flexWrap: "wrap" }}>
        <b style={{ fontSize: 13 }}>학교별 시험 회차</b>
        <span className="hint" style={{ fontSize: 11.5 }}>{from} ~ {to}</span>
      ...
      <div className="stack" style={{ gap: 2 }}>
        {rows.map((r) => (
          <div className="unitrow" key={r.name} style={{ alignItems: "flex-start", flexWrap: "wrap" }}>
            <b style={{ fontSize: 12.5, minWidth: 78 }}>{shortName(r.name)}</b>
            ...
            {r.missed.length > 0 && (
              <span className="hint" style={{ flexBasis: "100%", color: "var(--amber)", fontSize: 11.5 }}>
