    ...
    {live.map((c) => { const s = sessionsOf(c, target), isOpen = open === c.id, cand = candidates(d.students, c.members); return (
      <div className="card" key={c.id} style={{ margin: "0 0 8px" }} data-g="class-row" data-class={c.id} data-open={isOpen ? "1" : "0"}>
        ...
        {isOpen && <>
          ...
          <div className="tags" style={{ margin: "4px 0 0" }} data-g="member-list">{c.members.map((m) => <span key={m.student_id} className="tag" data-g="member" data-student={m.student_id}><PickBox pick={pk} id={`m:${c.id}:${m.student_id}`} label={`${m.name} 고르기`} />{m.name}{m.grade ? ` ${m.grade}학년` : ""} <button type="button" className="btn sm gho" style={{ padding: "0 4px", minHeight: 0 }} disabled={pending} data-act="member-remove" {...icon(`${m.name} 빼기`)} onClick={() => run(() => removeAct(c.id, m.student_id, d.on), `${m.name} · 오늘부터 명단에서 뺐습니다`)}>✕</button></span>)}{!c.members.length && <span className="tag">아직 아무도 없습니다</span>}</div>
