/** 🏫 시험 회차 06b · 범위와 시험일(보드 B가 하던 일). 학교 × 학년 회차마다: 보는 아이 · 영어 시험일 · 범위(교재 단원) · 교재 보류(N주 전부터 · 끝나는 날 저절로) · 안 봄 · 숨김. 전국은 학교를 안 붙인다.
...
export default async function Exams() {
  ...
  if (!isStaff(me?.role)) return frame(<div className="card"><div className="ctitle"><span className="cemo">🗓️</span>학교 시험은 학원 사람의 화면입니다</div><p className="note">{me ? `${ROLE_NAME[me.role] ?? me.role} 계정입니다.` : "로그인이 필요합니다."}</p></div>);
  let d;
  try { const date = await today(sb); d = { date, board: await examBoard(sb, date) }; }
  catch (e) { { console.error("[화면] 학교 시험 못 엶:", e); return frame(<Oops what="학교 시험" e={e} />); } }
