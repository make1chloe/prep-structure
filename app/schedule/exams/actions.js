"use server";
/** 시험 회차 손 · 학원 사람만. 판단·쓰기는 lib/exam.js 한 벌(범위 · 안 봄 · 숨김 · 몇 주 전부터 · 지금 보류 · 풀기). 지우는 손이 없다(대전제-6) */
...
const wrap = (fn) => act(fn, "학교 시험 06b");   // 손 한 벌은 lib/act.js · 삼키지 않고 서버 기록에 까닭을 남긴다(원칙-1)
...
export async function hiddenManyAct(ids, hidden) { return wrap(async () => { const { sb } = await staff(); return setHiddenMany(sb, ids, hidden, await today(sb)); }); }   // (어28)-③ 고른 시험 숨기기·복구 한 번에
