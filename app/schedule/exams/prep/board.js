...
import MaterialModal from "@/app/_shell/materialmodal.js";   // (어94) 05 와 같은 「+ 자료」 양식 한 벌(원칙-1)
...
export default function Board({ d }) {
  ...
  const [add, setAdd] = useState(false); const [prog, setProg] = useState({}); const [revised, setRevised] = useState({});
    ...
    {e && <>
        ...
        {tree.groups.map((g) => <div className="mt1" key={g.source} data-g="mt1" data-source={g.source}>
          ...
          {g.materials.map((m) => { const left = (m.gives ?? []).filter((x) => !x.handed_at).length, nx = nextStep(m, todos, today); return <div className="mt2" key={m.id} data-g="mt2" data-material={m.id} data-state={m.state}>
            <div className="mth2"><b>{m.type}{m.title && m.title !== m.type ? ` · ${m.title}` : ""}</b><span className="spacer" />
              ...
              <button className="btn sm" type="button" disabled={pending} data-act="drop" onClick={() => run(() => dropMaterialAct(m.id, "04 에서 삭제"), "삭제 ✓ · 복구할 수 있습니다")}>삭제</button></div>
      ...
      <div className="tblwrap"><table data-g="students"><thead><tr><th>학생</th><th>학교</th><th>학교 진도 · 어디까지 나갔나</th><th>오늘 낼 것</th><th>남은 것</th></tr></thead><tbody>
        {rows.map((r) => <tr key={r.id} className={r.known ? "" : "hi"} data-g="student-row" data-student={r.id} data-known={r.known ? "1" : "0"}>
          ...
          <td data-g="today-give">{r.today.length ? r.today.map((t) => <span key={t} className="tag on">{t}</span>) : <span className="mute">없음</span>}</td>
      ...
      {/* (어94) 「+ 자료」 양식은 05 와 **같은 한 벌**이다(app/_shell/materialmodal.js · 원칙-1) — 여기는 시험이 이미 정해져 있어 고르개가 없다 */}
      {add && <MaterialModal exam={e} types={b.types ?? []} takers={b.takers ?? []} save={addMaterialAct} onClose={() => setAdd(false)}
