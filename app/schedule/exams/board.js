...
export default function Board({ d }) {
  ...
  return <>
    <div className="wv" style={{ marginBottom: 8 }} data-g="head">
      ...
      <Sibs here="/schedule/exams" /><Link prefetch={false} className="btn sm goto" href="/schedule">📅 일정</Link><Link prefetch={false} className="btn sm goto" href="/schedule/import">📡 학사일정</Link>
    </div>
...
function ExamCard({ e, b, today, pending, run, stName, pk }) {
  ...
  return <div className="exr" style={{ borderColor: e.english_on ? undefined : "var(--miss)" }} data-g="exam-card" data-exam={e.id}>
    <div className="exh"><PickBox pick={pk} id={e.id} label={`${examHead(e)} 고르기`} /><span className="ai">🗓️</span><b data-g="exam-head">{examHead(e)}</b>
      ...
      <span className="tag" data-g="takers">{takers.length}명</span><Link prefetch={false} className="btn sm goto" href={`/schedule/exams/prep?e=${e.id}`} data-act="prep">📄 자료</Link>
