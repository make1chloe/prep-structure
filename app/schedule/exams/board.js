...
export default function Board({ d }) {
  ...
  return <>
    <div className="wv" style={{ marginBottom: 8 }} data-g="head">
      ...
      <Sibs here="/schedule/exams" /><Link prefetch={false} className="btn sm" href="/schedule">📅 일정 ↗</Link><Link prefetch={false} className="btn sm" href="/schedule/import">📡 받아오기 · + 시험 ↗</Link>
    </div>
...
function ExamCard({ e, b, today, pending, run, stName, pk }) {
  ...
  return <div className="exr" style={{ borderColor: e.english_on ? undefined : "var(--miss)" }} data-g="exam-card" data-exam={e.id}>
    ...
    {!e.english_on && <div className="lf over"><span className="ln">!</span><div><b>영어 시험일 없음</b><small>교재 멈춤이 안 섭니다</small></div>
...
function StopCard({ b, today, pending, run }) {
  ...
  return <div className="exr" style={{ marginTop: 8 }} data-g="stop-card">
    <div className="exh"><span className="ai">⏸</span><b>교재 멈춤</b><span className="spacer" /></div>
    <div className="left">
