...
import { doneAct, undoAct, dueAct, dropAct, unitTestAct, unitTestMadeAct, noteAct, repeatAct, repeatActiveAct, printAllAct, dropMaterialAct } from "./actions.js";
import { cardsOf, filterSchool, sortCards, columnsOf, hiddenOf, counts, behindOf, printAllOf, dueLine, isOverdue, kindName, schoolTag, flowOf, repeatText, monthDay } from "@/lib/todo-plan";
...
export default function Board({ d }) {
  ...
  const [ut, setUt] = useState({ studentId: "", topicId: "", qCount: "25" }); const [note, setNote] = useState({ title: "", dueOn: today, dueTime: "", studentId: "" }); const [rp, setRp] = useState({ name: "", every: "month", day: "25", weekday: "1", lead: String(b.rules?.["todo.repeat_lead"] ?? 3) });
  ...
  return <>
    ...
    {nw === "repeat" && <div className="card" data-g="new-repeat" style={{ marginTop: 8 }}><div className="ctitle"><span className="cemo">⏰</span>되풀이 — 때가 되면 늘 하는 일(규칙만 적어 두면 할 일은 저절로)</div>
      <div className="wv"><input type="text" value={rp.name} placeholder="수납 안내 보내기" aria-label="되풀이 이름" onChange={(x) => setRp({ ...rp, name: x.target.value })} style={{ flex: "1 1 200px" }} />
        <div className="seg sm" data-g="every"><button type="button" aria-pressed={rp.every === "month"} onClick={() => setRp({ ...rp, every: "month" })}>매달</button><button type="button" aria-pressed={rp.every === "week"} onClick={() => setRp({ ...rp, every: "week" })}>매주</button></div>
        {rp.every === "month" ? <input type="text" inputMode="numeric" className="scr" value={rp.day} aria-label="며칠" onChange={(x) => setRp({ ...rp, day: x.target.value.replace(/\D/g, "") })} /> : <select value={rp.weekday} aria-label="요일" onChange={(x) => setRp({ ...rp, weekday: x.target.value })} style={{ width: "auto" }}>{WD.map((w, i) => <option key={i} value={i}>{w}</option>)}</select>}
        <label className="fl" style={{ margin: 0 }}>며칠 전부터</label><input type="text" inputMode="numeric" className="scr" value={rp.lead} aria-label="며칠 전부터" onChange={(x) => setRp({ ...rp, lead: x.target.value.replace(/\D/g, "") })} />
