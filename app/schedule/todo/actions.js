...
export async function noteAct(f) { return wrap(async () => { const { sb } = await staff(); return { id: await addNote(sb, { title: f?.title, dueOn: f?.dueOn || null, dueTime: f?.dueTime || null, studentId: f?.studentId || null, note: f?.note || null, startOn: f?.startOn || null }) }; }); }   // (어78) 마감·시작일은 없어도 된다 — 📌 퀵 메모와 05 가 같은 손을 쓴다
export async function editNoteAct(todoId, f) { return wrap(async () => { const { sb } = await staff(); await editTodo(sb, todoId, { title: f?.title, dueOn: f?.dueOn || null, dueTime: f?.dueTime || null, studentId: f?.studentId || null, startOn: f?.startOn || null }); return {}; }); }   // (어80) ✎ 한 줄 고치기 — 퀵 메모와 **같은 양식**이 넣기·고치기를 다 한다(원칙-1)
