...
export default function Board({ d, canFlag = true }) {   // canFlag — 「권한 설정」의 me.flags(「표시」). 끄면 ❗ 를 못 단다(자꾸 잘못 누르는 아이 · 목업 08 의 진도 체크 끄기와 같은 결)
  ...
  /* (어92) 부품이 아니라 함수로(위와 같은 까닭) */
  const Sub = ({ s, c, k }) => <div key={k} className={"rsub" + (s.own ? " own" : "")} data-g="sub" data-unit={s.id} data-status={s.status} data-pending={s.pending ? "1" : "0"}>
  ...
  /* (어92) 부품이 아니라 함수로(위와 같은 까닭) */
  const Chapter = ({ c, k }) => { const isOpen = c.now || Boolean(open[c.chapter]); const pct = c.total ? Math.round((c.done + c.skip) / c.total * 100) : 0;
    return <div key={k} className={"rd" + (c.now ? " now" : "")} data-g="chapter" data-chapter={c.chapter} data-status={c.status} data-now={c.now ? "1" : "0"} data-open={isOpen ? "1" : "0"}>
      <div className="rdh">
      ...
      {isOpen && <div className="rsubs">{c.subs.map((s) => Sub({ s, c, k: s.id }))}</div>}
    </div>; };
  return <>
    ...
    <div className="rlist" data-g="road">{road.chapters.map((c) => Chapter({ c, k: c.chapter }))}{!road.chapters.length && <p className="note" style={{ margin: 0 }}>소단원이 없어요</p>}</div>
