...
export default function Board({ d }) {
  ...
  if (!b.book) return <div className="task" data-card="road"><div className="h"><b><span className="cemo">🛤️</span>내 교재 로드맵</b></div><p className="note" style={{ margin: "8px 0 0" }}>배정된 교재가 없어요</p><a className="btn sm" href="/me">나 ↗</a></div>;
  ...
  return <>
    <div className="wv" style={{ margin: "0 0 8px" }}><a className="btn sm" href="/me">← 나</a><b style={{ fontSize: "var(--fs-6)" }} data-g="book-name">{b.book.name}</b><span className="spacer" /><span className="pill">{md(today)}</span></div>
    ...
    <div className="tags" style={{ marginBottom: 12 }} data-g="book-tags">{books.map((x) => <a key={x.book_id} className={"tag" + (x.book_id === b.book.id ? " on" : "")} href={`/me/book?b=${x.book_id}`} style={x.state === "book_off" ? { color: "var(--mute)" } : undefined}>{x.text}</a>)}</div>
