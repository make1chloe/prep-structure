...
/* ══ 1. 아이가 손댈 수 있는 자리 ══════════════════════════════════════
 * ⚠️ **지어낸 목록이 아니다.** `supabase/migrations/0084_child_class_said.sql` 의
 *    `child_said` 정책이 `slot in ('home','next','class')` 를 연다.
 ...
 *
 * ⚠️⚠️ 2026-09-03 — **학원 줄(class)이 여기 들어왔다.** 전에는 없어서
 *    「학원에서 할 것」이 둘째 줄부터 영영 안 열렸다(실측: class 32줄 중 said_done_at 0줄).
 *    원장님 답: 「**o면 숙제인데 한 숙제를 왜 할것으로 열어??**」 —
 *    ○/△/✕ 는 **집에서 해온 숙제를 검사한 결과**이고, 「지금 학원에서 할 것」의 차례와
 *    아무 상관이 없다. 그러므로 **차례는 아이가 연다.**
 * ⚠️ 그래도 **원장님 ○ 은 원장님 것이다** — 아이가 쓰는 칸은 `said_done_at` 하나뿐이고,
 *    그것은 「검사해 주세요」라는 신호다(0082 문지기가 나머지 칸을 막는다). */
export const 아이가_찍는_칸 = Object.freeze(["class", "home", "next"]);

/**
 * 학원에서 하는 것 — **차례대로 하나씩**. 아이가 「다 했어요」를 눌러 다음을 연다.
 * ⚠️ 이 칸은 `아이가_찍는_칸` 에도 들어 있다(0084). 화면이 이 묶음을 **두 번 그리지 않도록**
 *    가르는 자리는 `오늘줄나누기()` 한 곳이다 — 화면에서 slot 글자를 다시 적지 않는다.
 */
...
/**
 * 그 줄을 **아이가 「다 했어요」로 눌렀나** (0082).
 *
 * ⚠️⚠️ 예전에는 `status === "done"` 으로 봤다. 그 칸은 **원장님의 검사 결과**라,
 *    원장님이 ○ 을 주면 아이 화면에도 「다 했어요」가 눌린 것처럼 보였고
 *    아이가 누르면 원장님 검사 목록에서 사라졌다 — **한 칸에 두 사실**이었다.
 * ⚠️ 이제 아이 화면은 **아이가 누른 것**만 본다. 원장님 검사 결과는 `검사됨()` 이 따로 준다.
 */
export const 끝난줄 = (r = {}) => r.said_done_at != null;

/** 원장님이 검사하셨나 — **아이가 누른 것과 다른 사실**이다 */
export const 검사됨 = (r = {}) => r.status != null && r.status !== "none";

/**
 * 오늘 줄들을 화면이 그릴 세 묶음으로 나눈다 — **가르는 자리는 여기 한 곳뿐이다**(원칙-1).
 *
 * ⚠️ `학원_칸` 은 `아이가_찍는_칸` 에도 들어 있다(0084). 화면이 두 목록을 각각 훑으면
 *    학원 줄이 **두 번 그려진다** — 그래서 여기서 한 번만 빼낸다.
 * ⚠️ 학원 줄만 `차례대로()` 를 지난다. 집·미리볼 것은 차례가 없다(아무 것이나 먼저 해도 된다).
 *
 * @returns { 학원것, 내가찍는것: [{칸, 줄들}], 검사것 }
 */
export function 오늘줄나누기(줄들 = []) {
  return {
    학원것: 차례대로(줄들.filter((r) => r.slot === 학원_칸)),
    내가찍는것: 아이가_찍는_칸
      .filter((칸) => 칸 !== 학원_칸)
      .map((칸) => ({ 칸, 줄들: 줄들.filter((r) => r.slot === 칸) })),
    검사것: 줄들.filter((r) => r.slot === "check"),
  };
}
 ...
 *    「정리 중」이 되는 날은 **두 가지**다 —
 *      ① 판은 내려왔는데 **아직 마감을 안 한 날** (0084 뒤로 아이는 제 판을 마감 전에도 읽는다)
 *      ② 판이 아예 안 선 날인데 **수업 요일이었던 날** (요일 이력이 근거다 — 지어낸 것이 아니다)
 * ⚠️⚠️ 0084 전에는 아이에게 **마감한 판만** 내려와서 ①이 원리적으로 없었다.
 *    그때 코드는 「판이 있으면 무조건 closed」였다 — 그대로 두면 마감 안 한 오늘이
 *    달력에 **「왔음 · 숙제 2」로 굳어** 보인다. 그래서 `closed_at` 을 여기서 다시 본다.
 *
...
export function 달력칸({ 오늘, first, last, 판들 = [], 수업이력 = [], 시험들 = [], 재원시작 = null,
  ...
  for (const 날 of daysBetween(first, last)) {
    ...
    let 상태 = "none";
    if (너무이름) 상태 = "before";
    // ⚠️ 판이 있어도 **마감했는지를 따로 본다**(0084). 마감 전이면 「수업함 · 정리 중」이다
    else if (s) 상태 = s.closed_at ? "closed" : "open";
