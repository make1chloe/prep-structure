 ...
 *
 * ⚠️ 되돌릴 수 없는 것은 **서버 답을 기다린다**(대전제 8). 숙제 ○ 이 그렇다 —
 *    접근 규칙(`child_done`)이 `status='done'` 만 허용해서 **아이는 도로 못 내린다.**
 */
...
/* ── ① 숙제 「다 했어요」 ─────────────────────────────────────────────
 * ⚠️ **되돌릴 수 없다.** 접근 규칙이 `status='done'` 만 허용한다 —
 *    아이가 잘못 눌러도 아이 손으로는 못 내린다. 화면은 그 사실을 눌리기 전에 적는다. */
export async function 다했어요(itemId) {
  ...
  if (r.error) return 안됨(옮겨적기(r.error));
  if (!r.data?.length)
    // ⚠️ 0줄인데 「됐다」고 말하지 않는다 — 마감 전이거나 학원에서 하는 줄이면 여기로 온다
    return 안됨("이 줄은 지금 찍을 수 없습니다 — 학원에서 하는 것이거나 아직 정리 중입니다.");
  return { ok: true };
}
