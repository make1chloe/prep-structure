 ...
 *    실패하면 **그 단추만** 되돌리고 알린다.
 *    ⚠️ 단 **되돌릴 수 없는 것은 서버 답을 기다린다**(속도-5) — 숙제 「다 했어요」가 그렇다
 *    (0082 문지기가 아이 손으로 내리는 것을 막는다).
 *    ⚠️⚠️ 그리고 덮개는 **화면이 보는 그 칸(`said_done_at`)에 서버가 준 값을 그대로** 넣는다.
 *    전에는 `status:"done"` 을 넣어서 — 화면은 `said_done_at` 을 보는데 — 눌러도
 *    단추도 숫자도 안 움직였다(규칙-어긋난곳 ⑰). **칸 이름이 어긋나면 아무 일도 안 일어난다.**
 *
...
export default function Screen(props) {
  ...
  /* ── 오늘 할 것 ─────────────────────────────────────────────────
   * ⚠️ 덮개는 **화면이 보는 칸 이름 그대로** 얹는다 — `said_done_at` 이다(0082 · ⑰).
   * ⚠️ 묶음을 가르는 자리는 `오늘줄나누기()` **한 곳**이다. 여기서 slot 글자를 직접 적으면
   *    그 목록이 바뀌는 날 화면만 옛 목록으로 남고, 학원 줄이 두 번 그려진다. */
  const 입힌줄 = 오늘줄.map((r) =>
    덮개숙제[r.id] ? { ...r, said_done_at: 덮개숙제[r.id] } : r);
  const { 학원것, 내가찍는것, 검사것 } = 오늘줄나누기(입힌줄);

  const 숙제찍기 = (item) => {
    // ⚠️ 되돌릴 수 없다 → **서버 답을 기다린다.** 낙관 갱신을 하지 않는다(속도-5)
    시작(async () => {
      const r = await 다했어요(item.id);
      // ⚠️ 서버가 준 시각을 그대로 얹는다 — 여기서 `new Date()` 로 지어내지 않는다(표-10)
      if (r?.ok && r.said_done_at) set덮개숙제((m) => ({ ...m, [item.id]: r.said_done_at }));
    ...
    <>
      ...
      {어떻게 !== "show" ? (
        ...
        <>
          ...
          {/* ⚠️⚠️ 학원 줄도 **아이가 찍는다**(0084). 전에는 찍기를 안 줘서
              둘째 줄부터 영영 안 열렸다(⑱ — 실측 class 32줄 중 said_done_at 0줄).
              ⚠️ 원장님 ○△✕ 는 이것과 **다른 사실**이라 아래 「쌤이 검사한 것」에 따로 선다 */}
          <할일묶음
            제목={칸이름[학원_칸]} 줄들={학원것} 차례있음
            도움="위에서부터 하나씩 해요. 하나를 끝내고 ○ 을 누르면 다음 것이 열립니다. ⚠️ 한 번 누르면 되돌릴 수 없어요."
            찍기={찍기} 저장중={저장중}
          />
          {내가찍는것.map(({ 칸, 줄들 }) => (
            <할일묶음
...
function 할일묶음({ 제목, 줄들, 도움, 찍기, 저장중, 차례있음 = false }) {
  ...
  return (
    <section className="me-group">
      ...
      <ul className="me-list">
        {남은.map((r) => (
          <할일줄 key={r.id} r={r} 차례있음={차례있음} 찍기={찍기} 저장중={저장중} />
        ))}
      </ul>
      {끝난.length > 0 && (
        <details className="me-fold">
    ...
    <li className={"me-item" + (끝 ? " is-done" : 잠김 ? " is-later" : "")}>
      ...
      <span className="me-act">
        {끝 ? (
          ...
          // ⚠️ 0084 뒤로 아이가 못 찍는 묶음은 없다 — 접힌 「다 한 것」 목록에서만 여기로 온다
          <span className="pill pilloff">아직</span>
        )}
      </span>
    </li>
  );
