...
export async function 학생화면읽기() {
  ...
  // ── 등원·하원 (0083) — **날짜별 기록**(원장님 2026-09-03 ⑩)
  // ⚠️ 걸음 1~3 은 등원, 4 는 하원이다. 어느 것이 도착 시각인지는 화면이 안 고른다 —
  //    `등하원날짜별()` 한 벌이 정한다(원칙-4).
  const 등하원 = await 물어본다("내 등원·하원", () =>
    q().from("arrival").select("date,step,at")
      .eq("student_id", sid).gte("date", first).lte("date", last).order("date"));
  ...
  return {
    ...
    달력: {
      ...
      못싣는것: [
        ...
        "등원·하원 시각은 아이가 **찍은 날에만** 뜹니다 — 원장님이 손으로 출결을 찍어 주신 날은 비어 있어요.",
      ],
    },
  };
}
