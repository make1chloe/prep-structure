...
export default function Board({ d }) {
  ...
  return <>
    <div className="wv" style={{ marginBottom: 8 }} data-g="head">
      ...
      <Link prefetch={false} className="btn sm goto" href="/schedule/exams">🗓️ 학교 시험</Link>
    </div>
    {err && <p className="note" role="alert" style={{ margin: "0 0 8px", color: "var(--miss)" }}>{err}</p>}
    {msg && <p className="note" data-g="msg" style={{ margin: "0 0 8px", color: "var(--on-ok)" }}>{msg}</p>}
    {!e && <p className="note" data-g="empty">본 시험이 없습니다. 🗓️ 학교 시험에서 시험과 영어 시험일을 넣으면 여기 섭니다.</p>}
    {e && <>
