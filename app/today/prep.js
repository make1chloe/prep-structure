...
export default function PrepCard({ student, prep = [], date, closed = false }) {
  ...
  return (<div className="card" data-card="prep">
    ...
    {list.map(({ exam: e, stopped, rows, notGiven, scopes, dday }) => <div key={e.id} data-g="prep-exam" data-exam={e.id} style={{ marginBottom: 8 }}>
      <div className="wv" style={{ margin: "0 0 8px" }}><span className="pill">🏫 {e.school ?? "전국"} · {e.name}</span>{e.english_on ? <span className="pill warn">영어 {mdDot(e.english_on)} · {dday}</span> : <span className="pill">영어일 없음</span>}<span className="spacer" />
        <span className="tag" data-g="prep-stopped">{stopped.map((b) => b.name).join(" · ")} 멈춤{stopped[0]?.stop_until ? ` · ${mdDot(stopped[0].stop_until)} 에 풀림` : ""}</span></div>
