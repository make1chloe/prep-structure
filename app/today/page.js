...
export default async function Today() {
  ...
  let date, r, band;
  try { date = await today(sb); [r, band] = await Promise.all([roster(sb, date), warnBand(sb, date)]); }   // 반·아이 → 판(없으면 세운다 — 검사 줄이 따라온다) ∥ 월초 정리 띠
  ...
  return frame(<>
    ...
    {r.classes.map((c) => (
      <section key={c.id ?? "makeup"} aria-label={c.nickname || c.start}>
        ...
        {c.students.map((s, i) => <Row key={s.id} student={s} sheet={s.sheet} classId={c.id} date={date} minutes={minutesOf(c.start, c.end)} defaultOpen={i === 0 && !s.sheet?.closed && !s.plan?.absent} />)}
      </section>
    ))}
  </>);
}
