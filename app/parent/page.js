...
export default async function Parent({ searchParams }) {
  ...
  const cards = orderCards([
    ...
    { id: 'recent', name: '수업', node: can(PARENT.recent) && d.last && <Card emo="📋" title={d.last.date === date ? "오늘 수업" : `${md(d.last.date)} 수업`} id="recent" {...fold("recent")}>
    ...
    { id: 'cal', name: '달력', node: can(PARENT.recent) && <Link prefetch={false} className="task" href={keepAs(`/parent/cal?s=${d.student.id}`, me, q)} data-card="cal" style={{ display: "block", textDecoration: "none", color: "inherit" }}><div className="h"><b><span className="cemo">📅</span>달력</b><span className="spacer" /><span className="pill">달력 👉</span></div></Link> },
