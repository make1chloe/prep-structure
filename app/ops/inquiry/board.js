      ...
      {cols.columns.map((col) => <div className={"col" + (col.urgent ? " urg" : "")} key={col.key} data-g="col" data-stage={col.key}><div className="colh">{col.label} <span className="n" data-g="col-count">{col.cards.length}</span></div>
        {col.cards.map((q) => <div className={"kc" + (q.hot ? " hot" : "")} key={q.id} data-g="card" data-id={q.id} data-hot={q.hot ? "1" : "0"}><b>{q.name}{q.stage === "new" ? " 어머니" : ""}</b>
          ...
          {q.stage === "new" && <>{!q.answered_at && <button className="btn pri sm" type="button" style={{ marginTop: 8, width: "100%" }} disabled={pending} data-act="answer" onClick={() => run(() => answerAct(q.id), "안내 보냄으로 적었습니다(문자는 아직 — 전화·카톡으로)")}>📨 안내 보내기</button>}
            <div className="wv" style={{ marginTop: 6, gap: 4 }}><input type="datetime-local" value={v(q.id, "visit", toLocal(q.visit_at))} aria-label={`${q.name} 방문`} onChange={(x) => setV(q.id, "visit", x.target.value)} style={{ width: "auto" }} /><button className="btn sm" type="button" disabled={pending || !v(q.id, "visit")} data-act="visit-save" onClick={() => run(() => setAct(q.id, { visitAt: fromLocal(v(q.id, "visit")) }), "방문을 잡았습니다 — 상담 잡힘")}>방문 잡기</button><button className="btn sm" type="button" disabled={pending} data-act="drop" onClick={() => run(() => stageAct(q.id, "dropped", v(q.id, "why", "")), "안 옴으로")}>안 옴</button></div></>}
          {q.stage === "visit" && <div className="wv" style={{ marginTop: 6, gap: 4 }}><input type="datetime-local" value={v(q.id, "test", toLocal(q.test_at))} aria-label={`${q.name} 레벨`} onChange={(x) => setV(q.id, "test", x.target.value)} style={{ width: "auto" }} /><button className="btn sm" type="button" disabled={pending || !v(q.id, "test")} data-act="test-save" onClick={() => run(() => setAct(q.id, { testAt: fromLocal(v(q.id, "test")) }), "레벨테스트를 잡았습니다")}>레벨 잡기</button>
            <input type="text" value={v(q.id, "level", q.level_note ?? "")} placeholder="점수 — 단어 32/40 · 문법 18/25" aria-label={`${q.name} 점수`} onChange={(x) => setV(q.id, "level", x.target.value)} style={{ flex: "1 1 160px" }} /><button className="btn sm pri" type="button" disabled={pending || !v(q.id, "level")} data-act="level-save" onClick={() => run(() => setAct(q.id, { levelNote: v(q.id, "level") }), "점수를 적었습니다 — 레벨 봄")}>점수</button><button className="btn sm" type="button" disabled={pending} data-act="drop" onClick={() => run(() => stageAct(q.id, "dropped", v(q.id, "why", "")), "안 옴으로")}>안 옴</button></div>}
          {q.stage === "test" && <div className="wv" style={{ marginTop: 6, gap: 4 }}><input type="text" value={v(q.id, "suggest", q.suggest ?? "")} placeholder="제안 — 중등3800제2 · 수능딥독1" aria-label={`${q.name} 제안`} onChange={(x) => setV(q.id, "suggest", x.target.value)} style={{ flex: "1 1 160px" }} /><button className="btn sm" type="button" disabled={pending || !v(q.id, "suggest")} data-act="suggest-save" onClick={() => run(() => setAct(q.id, { suggest: v(q.id, "suggest") }), "제안을 적었습니다")}>제안</button>
          ...
          {q.stage === "dropped" && <div className="wv" style={{ marginTop: 6, gap: 4 }}><input type="text" value={v(q.id, "why", q.why ?? "")} placeholder="사유 — 다른 학원 · 시간이 안 맞음" aria-label={`${q.name} 사유`} onChange={(x) => setV(q.id, "why", x.target.value)} style={{ flex: "1 1 160px" }} /><button className="btn sm" type="button" disabled={pending} data-act="why-save" onClick={() => run(() => setAct(q.id, { why: v(q.id, "why") }), "사유를 적었습니다")}>사유</button></div>}
        </div>)}
