    ...
    <div className="kb" data-g="kb">
      {cols.columns.map((col) => <div className={"col" + (col.urgent ? " urg" : "")} key={col.key} data-g="col" data-stage={col.key}><div className="colh">{col.label} <span className="n" data-g="col-count">{col.cards.length}</span></div>
        {col.cards.map((q) => <div className={"kc" + (q.hot ? " hot" : "")} key={q.id} data-g="card" data-id={q.id} data-hot={q.hot ? "1" : "0"}><PickBox pick={pk} id={q.id} label={`${q.name} 고르기`} disabled={q.stage === "joined"} /><b>{q.name}{q.stage === "new" ? " 어머니" : ""}</b>
          ...
          {q.stage === "new" && <>{!q.answered_at && <button className="btn pri sm" type="button" style={{ marginTop: 8, width: "100%" }} disabled={pending} data-act="answer" onClick={() => run(() => answerAct(q.id), (r) => r.sms ? (r.sms.sent ? `안내 문자를 보냈습니다 ${r.sms.to}` : r.sms.sink === "off" ? `안내 문자 · 리허설(NOTIFY_SINK=off)이라 발송 이력만 ${r.sms.to}` : `안내 문자를 못 보냈습니다. ${r.sms.why}`) : "안내 보냄으로 적었습니다(문자 길이 없어 · 연동 solapi 줄 · 전화·카톡으로)")}>📨 안내 문자</button>}
            <div className="wv" style={{ marginTop: 6, gap: 4 }}><DateBox type="datetime-local" value={v(q.id, "visit", toLocal(q.visit_at))} aria-label={`${q.name} 방문`} onChange={(x) => setV(q.id, "visit", x.target.value)} style={{ width: "auto" }} /><button className="btn sm" type="button" disabled={pending || !v(q.id, "visit")} data-act="visit-save" onClick={() => run(() => setAct(q.id, { visitAt: fromLocal(v(q.id, "visit")) }, q.updated_at ?? null), "방문을 잡았습니다. 상담 잡힘")}>방문 잡기</button><button className="btn sm" type="button" disabled={pending} data-act="drop" onClick={() => run(() => stageAct(q.id, "dropped", v(q.id, "why", "")), "안 옴으로")}>안 옴</button></div></>}
          {q.stage === "visit" && <div className="wv" style={{ marginTop: 6, gap: 4 }}><DateBox type="datetime-local" value={v(q.id, "test", toLocal(q.test_at))} aria-label={`${q.name} 레벨`} onChange={(x) => setV(q.id, "test", x.target.value)} style={{ width: "auto" }} /><button className="btn sm" type="button" disabled={pending || !v(q.id, "test")} data-act="test-save" onClick={() => run(() => setAct(q.id, { testAt: fromLocal(v(q.id, "test")) }, q.updated_at ?? null), "레벨테스트를 잡았습니다")}>레벨 잡기</button>
          ...
          {q.stage === "test" && <div className="wv" style={{ marginTop: 6, gap: 4 }}><input type="text" value={v(q.id, "suggest", q.suggest ?? "")} placeholder="제안 · 중등3800제2 · 수능딥독1" aria-label={`${q.name} 제안`} onChange={(x) => setV(q.id, "suggest", x.target.value)} style={{ flex: "1 1 160px" }} /><button className="btn sm" type="button" disabled={pending || !v(q.id, "suggest")} data-act="suggest-save" onClick={() => run(() => setAct(q.id, { suggest: v(q.id, "suggest") }, q.updated_at ?? null), "제안을 적었습니다")}>제안</button>
            <button className="btn sm pri" type="button" style={{ width: "100%" }} data-act="convert-open" onClick={() => { setConv(q.id); setSteps(null); setCv({ classId: "", bookIds: [], loginId: suggestLoginId({ phone: q.student_phone, parent_phone: q.phone }, today), joinedOn: today, extra: "" }); }}>등록 전환</button><button className="btn sm" type="button" disabled={pending} data-act="drop" onClick={() => run(() => stageAct(q.id, "dropped", v(q.id, "why", "")), "안 옴으로")}>안 옴</button></div>}
          {q.stage === "joined" && q.student_id && <Link prefetch={false} className="btn sm goto" style={{ marginTop: 8, width: "100%", display: "block", textAlign: "center" }} href={`/ops/students?s=${q.student_id}`} data-act="open-student">학생 화면</Link>}
      ...
      <div className="mdlb">
        {!steps && <>
          <div className="wv"><label className="fl" style={{ margin: 0 }}>반</label><select value={cv.classId} aria-label="반" data-g="conv-class" onChange={(x) => setCv({ ...cv, classId: x.target.value })} style={{ width: "auto" }}><option value="">반 고르기</option>{(b.classes ?? []).map((c) => <option key={c.id} value={c.id}>{classText({ nickname: c.nickname, weekdays: c.weekdays ?? [], start_time: c.start_time })}</option>)}</select>
            <label className="fl" style={{ margin: 0 }}>들어온 날</label><DateBox type="date" className="dt" value={cv.joinedOn} aria-label="들어온 날" onChange={(x) => setCv({ ...cv, joinedOn: x.target.value })} style={{ width: "auto" }} /></div>
