...
export async function setAct(id, f) { return wrap(async () => { const { sb } = await staff(); return setInquiry(sb, id, f ?? {}); }); }
export async function answerAct(id) { return wrap(async () => { const { sb } = await staff(); await answerInquiry(sb, id); return {}; }); }
