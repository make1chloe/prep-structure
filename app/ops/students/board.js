...
import { usePick, PickAll, PickBox, PickBar } from "../../_shell/pick.js";
import AssignModal from "../../_shell/assignmodal.js";   // (어41) 교재 배정 모달 한 벌(대시보드 · 01 과 같은 부품)
import { academyAct } from "../../settings/progress/actions.js";   // (어41) 진도 체크 학원 열기·닫기는 그 자리에서   /* 고르기 한 벌((어28) · 대전제-20 · 원장님 9/15 「모든 목록은 전체, 일부선택 ->선택후 일괄액션」) */
import { addAct, accountsManyAct, setAct, stateManyAct, classManyAct, classAct, feeAct, consultAct, consultAttachAct, siblingAct, showAct, studentAccountAct, parentAccountAct, parentNameAct, resetAct, ccLinkAct, editModeAct, confirmMarkAct, revertMarkAct, confirmAllAct, flagAct } from "./actions.js";
import { FIRST_PW, kpis, bookLines, scoreRows, weakText, attendRow, attendSummary, unitChips, historyLines, listRows, siblingText, tenureText, gradeText2, classLine, suggestLoginId, canReset, STATE, stateName } from "@/lib/student-plan";
...
export default function Board({ d }) {
  ...
  return <><div className="stsplit"><div className="stlist">
    <div className="lf" style={{ margin: "0 0 8px" }} data-g="list-head"><span className="ln">🧑‍🎓</span><div><b data-g="counts">재원생 {list.active} · 퇴원생 {list.left}{list.paused ? ` · 쉼 ${list.paused}` : ""}</b></div>
      ...
      <button className="btn sm pri" type="button" data-act="add-open" onClick={() => setAddOpen(!addOpen)}>+ 학생</button><Link prefetch={false} className="btn sm goto" href="/ops/inquiry">☎️ 신규 상담</Link></div>
    ...
    {addOpen && <div className="card" data-g="add-form"><div className="ctitle"><span className="cemo">＋</span>학생</div>
      <div className="wv"><input type="text" value={nf.name} placeholder="이름" aria-label="이름" onChange={(x) => setNf({ ...nf, name: x.target.value })} style={{ maxWidth: 140 }} /><input type="text" inputMode="numeric" className="scr" value={nf.grade} placeholder="학년" aria-label="학년" onChange={(x) => setNf({ ...nf, grade: x.target.value.replace(/\D/g, "") })} />
        ...
        <input type="text" inputMode="numeric" value={nf.parentPhone} placeholder="학부모 전화" aria-label="학부모 전화" onChange={(x) => setNf({ ...nf, parentPhone: x.target.value })} style={{ maxWidth: 150 }} /><DateBox type="date" className="dt" value={nf.joinedOn} aria-label="들어온 날" onChange={(x) => setNf({ ...nf, joinedOn: x.target.value })} style={{ width: "auto" }} />
        <button className="btn pri sm" type="button" disabled={pending || !nf.name.trim()} data-act="add-save" onClick={() => run(() => addAct(nf), (r) => { pick(r.id); return r.account ? `✓ 아이디 ${r.account.login_id} · 첫 비밀번호 ${r.account.password ?? "그대로"}${r.account.bumped ? ` · ${r.account.bumped} 는 이미 있어 뒤 숫자` : ""}` : `✓ 넣음 · 계정은 아직 · ${r.why ?? ""}`; })}>저장</button></div></div>}
    ...
    <PickBar pick={pk} unit="명">{/* 고른 아이들에 한 번에 · 퇴원(나간 날 하나 · 줄은 남는다 · 복귀 가능) · 복귀 · 반 옮기기(이 날부터) */}
      <DateBox type="date" className="dt" value={leave} aria-label="나간 날" onChange={(x) => setLeave(x.target.value)} style={{ width: "auto" }} />
      ...
      <button type="button" className="btn sm" disabled={pending} data-act="accounts-picked" onClick={() => run(() => accountsManyAct(pk.ids), (r) => [`${r.n}명 발급`, r.made.join(" · "), r.skipped.length ? `건너뜀 ${r.skipped.join(" · ")}` : "", r.failed.length ? `막힘 ${r.failed.join(" · ")}` : ""].filter(Boolean).join(" / "), pk.clear)}>🔑 계정 발급</button>{/* (어54) 이미 들어와 있는 아이들에게 한 번에 · 전화가 있는 아이만 */}
      ...
      <DateBox type="date" className="dt" value={mv.from} aria-label="반 옮기는 날" onChange={(x) => setMv({ ...mv, from: x.target.value })} style={{ width: "auto" }} />
    ...
    {st && <div data-g="student" data-student={st.id} style={{ marginTop: 12 }}>
      ...
      {edit && <div className="card" data-g="edit"><div className="ctitle"><span className="cemo">✎</span>수정 · {st.name}</div>
        <div className="wv"><input type="text" value={form.name} aria-label="이름" onChange={(x) => setForm("name", x.target.value)} style={{ maxWidth: 140 }} /><input type="text" inputMode="numeric" className="scr" value={form.grade} placeholder="학년" aria-label="학년" onChange={(x) => setForm("grade", x.target.value.replace(/\D/g, ""))} />
          ...
          <DateBox type="date" className="dt" value={form.joinedOn} aria-label="들어온 날" onChange={(x) => setForm("joinedOn", x.target.value)} style={{ width: "auto" }} />
          <DateBox type="date" className="dt" value={form.birth} aria-label="생년월일" data-g="birth" onChange={(x) => setForm("birth", x.target.value)} style={{ width: "auto" }} />
        ...
        <div className="left" style={{ marginTop: 8 }}>
          <div className="lf" data-g="account"><span className="ln">🔑</span><div><b>계정 · 학생</b><small>{st.login_id ? `아이디 ${st.login_id}${st.must_change_pw ? ` · 첫 비밀번호(${FIRST_PW}) 아직 안 바꿈` : ""}${st.issued_by_app ? " · 앱이 발급" : " · 이어진 계정(비밀번호는 그대로)"}` : "아직 없음 · chloe + 폰 뒤 4자리로 채워 두었습니다 · 「발급」만 누르면 됩니다(겹치면 다섯째 숫자 2·3 … · 첫 비밀번호 ${FIRST_PW} · 첫 로그인에 바꿉니다)"}</small></div>
            {!st.login_id && <><input type="text" value={acc.loginId || suggestLoginId(st, today)} aria-label="학생 아이디" onChange={(x) => setAcc({ ...acc, loginId: x.target.value })} style={{ maxWidth: 160 }} /><button className="btn sm pri" type="button" disabled={pending} data-act="student-account" onClick={() => run(() => studentAccountAct(st.id, acc.loginId || suggestLoginId(st, today)), (r) => (r.bumped ? `${r.bumped} 는 이미 있어 ${r.login_id} 로 · ` : "") + (r.adopted ? `이미 있던 계정 ${r.login_id} 을 이었습니다 · ${r.password ? `첫 비밀번호 ${r.password}` : "비밀번호는 그대로"}` : `학생 계정 ${r.login_id} · 첫 비밀번호 ${r.password}`))}>{acc.loginId || suggestLoginId(st, today)} 발급</button></>}
            {st.login_id && canReset({ issued_by_app: st.issued_by_app }) && <button className="btn sm" type="button" disabled={pending} data-act="reset-student" onClick={() => sure.ask("reset-student")}>비밀번호 {FIRST_PW}</button>}
            <Sure on={sure.is("reset-student")} text={`${st.name} 의 비밀번호를 ${FIRST_PW} 으로 되돌릴까요?. 쓰던 비밀번호는 사라지고, 아이에게 ${FIRST_PW} 을 알려 주셔야 들어옵니다`} yes={`${FIRST_PW} 으로`} pending={pending} onNo={sure.off} onYes={() => { sure.off(); run(() => resetAct(st.profile_id), (r) => `${r.login_id} · ${FIRST_PW} 으로 초기화(첫 로그인에 바꿉니다)`); }} /></div>
          <div className="lf" data-g="parent-account"><span className="ln">👨‍👩‍👧</span><div><b>계정 · 학부모</b><small>{st.parents?.length ? st.parents.map((p) => `${p.name ?? p.login_id}${p.rel ? `(${p.rel})` : ""} · ${p.login_id ?? "아이디 없음"}${p.issued_by_app ? " · 앱이 발급" : ""}`).join(" / ") : "아직 없음 · 전화번호가 아이디입니다(같은 번호가 있으면 형제로 잇습니다)"}</small></div>
            <input type="text" inputMode="numeric" value={acc.parentPhone || st.parent_phone || ""} placeholder="학부모 전화" aria-label="학부모 계정 전화" onChange={(x) => setAcc({ ...acc, parentPhone: x.target.value })} style={{ maxWidth: 150 }} /><button className="btn sm pri" type="button" disabled={pending} data-act="parent-account" onClick={() => run(() => parentAccountAct(st.id, acc.parentPhone || st.parent_phone), (r) => r.created ? `학부모 계정 ${r.login_id} · 첫 비밀번호 ${r.password}` : r.adopted ? `이미 있던 계정 ${r.login_id} 을 이었습니다 · ${r.password ? `첫 비밀번호 ${r.password}` : "비밀번호는 그대로"}` : `있던 학부모 계정 ${r.login_id}에 이었습니다(형제)`)}>발급·연결</button>
            {st.parents?.filter((p) => canReset(p)).map((p) => <span key={p.profile_id}><button className="btn sm" type="button" disabled={pending} data-act="reset-parent" onClick={() => sure.ask(`reset-${p.profile_id}`)}>{p.login_id} {FIRST_PW}</button>
              <Sure on={sure.is(`reset-${p.profile_id}`)} text={`${p.login_id} 의 비밀번호를 ${FIRST_PW} 으로 되돌릴까요?. 쓰던 비밀번호는 사라지고, 학부모님께 ${FIRST_PW} 을 알려 주셔야 들어옵니다`} yes={`${FIRST_PW} 으로`} pending={pending} onNo={sure.off} onYes={() => { sure.off(); run(() => resetAct(p.profile_id), (r) => `${r.login_id} · ${FIRST_PW} 으로 초기화`); }} /></span>)}
            {(st.parents ?? []).map((p) => <span key={`nm-${p.profile_id}`} className="wv" style={{ gap: 4, marginBottom: 0 }} data-g="parent-name" data-profile={p.profile_id}><input type="text" value={pn[p.profile_id] ?? p.name ?? ""} placeholder="학부모 계정 이름" aria-label={`학부모 계정 이름 ${p.login_id ?? ""}`} onChange={(x) => setPn({ ...pn, [p.profile_id]: x.target.value })} style={{ maxWidth: 150 }} /><button className="btn sm" type="button" disabled={pending || (pn[p.profile_id] ?? p.name ?? "") === (p.name ?? "")} data-act="parent-name" onClick={() => run(() => parentNameAct(st.id, p.profile_id, pn[p.profile_id] ?? p.name ?? ""), (r) => `학부모 계정 이름 · ${r.name}`, () => setPn({}))}>이름 저장</button></span>)}</div>
          <div className="lf" data-g="sibling"><span className="ln">👥</span><div><b>형제 묶기</b><small>같은 집</small></div>
          ...
          <div className="lf" data-g="class-edit"><span className="ln">🏫</span><div><b>반 · {classLine(st.class)}</b><small>이 날부터</small></div>
            <select value={cls.classId} aria-label="반" onChange={(x) => setCls({ ...cls, classId: x.target.value })} style={{ width: "auto" }}><option value="">반 고르기</option>{(b.classes ?? []).map((c) => <option key={c.id} value={c.id}>{classLine(c)}</option>)}</select><Link prefetch={false} className="btn sm gho goto" href="/schedule/classes" data-act="classes">반 화면</Link><DateBox type="date" className="dt" value={cls.from} aria-label="반 시작일" onChange={(x) => setCls({ ...cls, from: x.target.value })} style={{ width: "auto" }} /><button className="btn sm" type="button" disabled={pending || !cls.classId} data-act="class-save" onClick={() => run(() => classAct(st.id, cls.classId, cls.from), "반을 옮겼습니다. 그날부터", () => setCls({ classId: "", from: today }))}>옮기기</button></div>
          <div className="lf" data-g="fee-edit"><span className="ln">💳</span><div><b>학생별 금액</b><small>언제부터 얼마</small></div>
            <input type="text" inputMode="numeric" className="fee" value={fee.amount} placeholder="금액" aria-label="학생별 금액" onChange={(x) => setFee({ ...fee, amount: x.target.value.replace(/\D/g, "") })} /><DateBox type="date" className="dt" value={fee.from} aria-label="금액 시작일" onChange={(x) => setFee({ ...fee, from: x.target.value })} style={{ width: "auto" }} /><button className="btn sm" type="button" disabled={pending || !fee.amount} data-act="fee-save" onClick={() => run(() => feeAct(st.id, fee.amount, fee.from), (r) => `${r.amount.toLocaleString()}원 · ${fee.from} 부터`, () => setFee({ amount: "", from: today }))}>저장</button></div>
      ...
      <Cols screen="student" cards={orderCards([   /* 카드 차례 — 기본은 목업 14 차례 · 바꾸면 그 사람 것((어15) screen_pref student · 확정-⑮) · 앞 절반은 왼쪽 열, 뒤 절반은 오른쪽 */
          { id: "books", name: "교재 진도", node: <div className="card" style={{ margin: "0 0 8px" }} data-g="books"><div className="ctitle"><span className="cemo">📚</span>교재 진도<span className="spacer" /><button type="button" className="btn sm" data-act="add-book" onClick={() => setAssign({ id: st.id, name: st.name })}>+ 교재 배정</button></div>{/* (어41) 교재 배정 모달 한 벌(17 · 01 · 14) · 원장님 9/15 「숙제를 최초에 어디서 배정을 하냐」 */}
          ...
          { id: "cc-link", name: "클래스카드 아이디", node: <div className="card" style={{ margin: "0 0 8px" }} data-g="cc-link"><div className="ctitle"><span className="cemo">🃏</span>클래스카드 아이디</div>
              ...
              {d.cc && !st.login_id && /^chloe[0-9]/i.test(String(d.cc.cc_login_id || d.cc.cc_user_idx)) && <div className="lf warn" data-g="cc-app-id"><span className="ln">🔑</span>{/* (어54) 원장님이 앱 아이디를 이 칸에 적어 두신 채로 아이가 못 들어오던 것(9/16 사진) · 한 번에 앱 계정으로 옮긴다 */}
                <div><b>{d.cc.cc_login_id || d.cc.cc_user_idx} 는 앱 로그인 아이디</b><small>여기는 클래스카드 칸 · 앱 계정으로 내야 아이가 들어옴</small></div>
                <button className="btn sm pri" type="button" disabled={pending} data-act="cc-to-app" onClick={() => run(() => studentAccountAct(st.id, String(d.cc.cc_login_id || d.cc.cc_user_idx)), (r) => `✓ 아이디 ${r.login_id} · 첫 비밀번호 ${r.password ?? "쓰던 것 그대로"}`)}>🔑 앱 계정으로</button></div>}
              <div className="wv"><input type="text" value={cf2.idx} placeholder="클래스카드 user_idx(숫자)" aria-label="클래스카드 아이디" onChange={(x) => setCf2({ ...cf2, idx: x.target.value })} style={{ flex: "1 1 160px" }} />
                ...
                <button className="btn pri sm" type="button" disabled={pending || !cf2.idx.trim()} data-act="cc-link" onClick={() => run(() => ccLinkAct(st.id, cf2.idx, cf2.login), "이었습니다", () => setCf2({ idx: "", login: "" }))}>연결</button></div>
            </div> },
          { id: "progress-edit", name: "진도 체크", node: <div className="card" style={{ margin: "0 0 8px" }} data-g="progress-edit"><div className="ctitle"><span className="cemo">✎</span>진도 체크</div>
              <div className="wv" style={{ marginBottom: 6 }}><span className="pill" data-g="pe-open">{pe.academy_open ? `학원 열림 · ${daysOpenText(pe.opened_on, today)}` : "학원 닫힘"}</span>
                ...
                <button type="button" className="btn sm" data-act="pe-academy" disabled={pending} onClick={() => run(() => academyAct(!pe.academy_open), pe.academy_open ? "학원 닫힘" : "학원 열림")}>{pe.academy_open ? "✎ 학원 닫기" : "✎ 학원 열기"}</button><Link prefetch={false} className="btn sm gho goto" href="/settings/progress">진도 체크 화면</Link></div>
          ...
          { id: "scores", name: "성적", node: <div className="card" style={{ margin: "0 0 8px" }} data-g="scores"><div className="ctitle"><span className="cemo">📈</span>성적</div>
              <div className="tblwrap"><table><thead><tr><th>시험</th><th>원점수</th><th>등급</th><th>틀린 영역</th></tr></thead><tbody>
                {scores.map((s) => <tr key={s.id} data-g="score-row"><td className="sch">{s.title}{s.pending ? <span className="tag act" style={{ marginLeft: 4 }}>확인 전</span> : null}</td><td className="num">{s.raw}{s.full !== 100 ? `/${s.full}` : ""}{s.deltaText && <span className={"tag" + (s.delta > 0 ? " on" : "")} style={{ marginLeft: 4 }} data-g="score-delta">{s.deltaText}</span>}</td><td className="num">{s.grade}</td><td>{s.sum.slice(0, 3).map((x, i) => <span key={x.kind} className="tag" style={i === 0 ? MISS : undefined}>{x.kind} {x.n}</span>)}{!s.sum.length && <span className="mute"></span>}</td></tr>)}
                {!scores.length && <tr><td colSpan={4} className="note">아직 없습니다</td></tr>}
              </tbody></table></div>
              {weak && <div className="weak" data-g="weak"><b>⚠️ {weak.text}</b><button type="button" className="tag act" data-act="weak-book" onClick={() => setAssign({ id: st.id, name: st.name })}>교재 배정</button></div>}
              {assign && <AssignModal studentId={assign.id} name={assign.name} date={today} onClose={() => setAssign(null)} />}</div> },
          { id: "attend", name: "출결", node: <div className="card" style={{ margin: "0 0 8px" }} data-g="attend"><div className="ctitle"><span className="cemo">🕘</span>출결
          ...
          { id: "files", name: "자료", node: <div className="card" style={{ margin: "0 0 8px" }} data-g="files"><div className="ctitle"><span className="cemo">📎</span>자료 <span className="tag">{(b.files ?? []).length}</span></div>
              ...
              {!(b.files ?? []).length && <p className="note" style={{ margin: 0 }}>이 아이의 자료가 없습니다(자료실 20)</p>}</div> },
          { id: "unit-tests", name: "단원평가", node: <div className="card" style={{ margin: "0 0 8px" }} data-g="unit-tests"><div className="ctitle"><span className="cemo">✍️</span>단원평가</div>
          ...
          { id: "history", name: "지나온 것", node: <div className="card" style={{ margin: "0 0 8px" }} data-g="history"><div className="ctitle"><span className="cemo">🧾</span>지나온 것</div>
