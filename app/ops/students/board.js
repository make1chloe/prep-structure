 ...
 *  ✎ 고치기 모달: 이름·학년·학교·전화·들어온 날·메모 · 계정(학생 아이디 발급 · 학부모 전화 발급/연결 · 학부모 계정 이름 · 비밀번호 0000 초기화 · 앱이 발급한 것만) · 형제 묶기 · 반(날짜부터) · 학생별 금액 · 성적 공개 · 퇴원 처리/복귀. 세는 것은 lib/student-plan 한 벌 */
    ...
    {st && <div data-g="student" data-student={st.id} style={{ marginTop: 12 }}>
      ...
      {edit && <div className="card" data-g="edit"><div className="ctitle"><span className="cemo">✎</span>고치기 · {st.name}</div>
        ...
        <div className="left" style={{ marginTop: 8 }}>
          <div className="lf" data-g="account"><span className="ln">🔑</span><div><b>계정 · 학생</b><small>{st.login_id ? `아이디 ${st.login_id}${st.must_change_pw ? " · 첫 비밀번호(0000) 아직 안 바꿈" : ""}${st.issued_by_app ? " · 앱이 발급" : " · 이어진 계정(비밀번호는 그대로)"}` : "아직 없음 · chloe + 폰 뒤 4자리로 채워 두었습니다 · 「발급」만 누르면 됩니다(겹치면 다섯째 숫자 2·3 … · 첫 비밀번호 0000 · 첫 로그인에 바꿉니다)"}</small></div>
