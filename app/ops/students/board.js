    ...
    {st && <div data-g="student" data-student={st.id} style={{ marginTop: 12 }}>
      ...
      {edit && <div className="card" data-g="edit"><div className="ctitle"><span className="cemo">✎</span>고치기 · {st.name}</div>
        ...
        <div className="left" style={{ marginTop: 8 }}>
          <div className="lf" data-g="account"><span className="ln">🔑</span><div><b>계정 · 학생</b><small>{st.login_id ? `아이디 ${st.login_id}${st.must_change_pw ? " · 첫 비밀번호(0000) 아직 안 바꿈" : ""}${st.issued_by_app ? " · 앱이 발급" : " · 이어진 계정(비밀번호는 그대로)"}` : "아직 없음 · 아이디(chloe + 숫자 넷)를 정해 발급합니다(첫 비밀번호 0000 · 첫 로그인에 바꿉니다)"}</small></div>
            {!st.login_id && <><input type="text" value={acc.loginId || suggestLoginId(st, today)} aria-label="학생 아이디" onChange={(x) => setAcc({ ...acc, loginId: x.target.value })} style={{ maxWidth: 160 }} /><button className="btn sm pri" type="button" disabled={pending} data-act="student-account" onClick={() => run(() => studentAccountAct(st.id, acc.loginId || suggestLoginId(st, today)), (r) => r.adopted ? `이미 있던 계정 ${r.login_id} 을 이었습니다 · ${r.password ? `첫 비밀번호 ${r.password}` : "비밀번호는 그대로"}` : `학생 계정 ${r.login_id} · 첫 비밀번호 ${r.password}`)}>발급</button></>}
