 ...
 *  층: 로그인 확인 → 주소 인자 → 오늘 → 판 한 벌(학생 student_board 또는 💰 수강료 fee_board · 갈래가 달라도 파도는 하나) = 4단. 학원 사람만(상담 카드는 ops.consult 로 가린다) */
...
import Fee from "../fee.js";   // (어28) 수강료 13 = 학생 안 갈래(?v=fee) · 판·손은 그대로(lib/fee)
...
/** 머리 세그 · 학생 · 💰 수강료((어28) 운영 탭을 학생에 합침). 수강료는 열쇠(ops.fee)가 있는 사람에게만 보인다(답 ⑮ 「강사는 수강료 못 보게」) */
const ViewSeg = ({ view, fee, ym }) => <div className="seg sm" data-g="view" style={{ marginBottom: 8 }}>
  <Link prefetch={false} href="/ops/students" aria-pressed={view !== "fee"}>학생</Link>{fee && <Link prefetch={false} href={`/ops/students?v=fee${ym ? `&m=${ym}` : ""}`} aria-pressed={view === "fee"}>💰 수강료</Link>}</div>;
export default async function Students({ searchParams }) {
  ...
  const fee = String(sp?.v ?? "") === "fee";   /* (어28) 💰 수강료 갈래 = 옛 /ops 화면 그대로 · 갈래가 달라도 층은 넷(파도 하나에 태운다 · 속도-1 · check-fast) */
  let d;
