...
export default async function Students({ searchParams }) {
  ...
  let d;
  try { const date = await today(sb); const sel = /^[0-9a-f-]{36}$/.test(String(sp?.s ?? "")) ? String(sp.s) : null; const board = await studentBoard(sb, sel, date); d = { date, sel, board, consult: decide(me.role, board.access ?? [], OPS.consult) === true, principal: me.role === "principal" }; }
