...
export default function Fee({ d }) {
  ...
  return <>
    <div className="wv" style={{ marginBottom: 8 }} data-g="head">
      ...
      {prevUnpaidText(d.prev_unpaid) && <Link prefetch={false} className="pill warn goto" data-g="prev-unpaid" href={`/ops/students?v=fee&m=${nextYm(d.ym, -1)}`} title={(d.prev_unpaid?.names ?? []).join(", ")}>{prevUnpaidText(d.prev_unpaid)}</Link>}
