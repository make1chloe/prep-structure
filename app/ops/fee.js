...
export default function Fee({ d }) {
  ...
  return <>
    <div className="wv" style={{ marginBottom: 8 }} data-g="head">
      ...
      <button type="button" className="btn sm" disabled={pending || !t.unpaidCount} data-act="remind-fees" onClick={() => run(() => remindAct(d.ym), (r) => `${r.n}집에 수강료 안내 — ${r.sink === "off" ? "🧪 리허설(off): 자취만 남고 실제로는 안 나갔습니다" : `보냄 ${r.sent} · 못 보냄 ${r.failed}`}`)}>💰 안 받은 집에 안내</button>
      <button type="button" className="btn sm" disabled={pending || !t.unpaidCount} data-act="schedule-fees" onClick={() => run(() => scheduleFeesAct(d.ym, when, customOf(when, cDate, cTime)), (r) => `예약했습니다 — ${r.n}집 · ${whenLabel(r.at, d.date)}`)}>⏰ 안 받은 집 예약</button>
    ...
    <div className="savebar" style={{ marginTop: 8 }} data-g="fee-bar">
      ...
      <button className="btn" type="button" disabled={pending || !t.unpaidCount} data-act="pay-all" onClick={() => sure.ask("pay-all")}>✔ 안 받음 {t.unpaidCount}줄 다 받음</button>
