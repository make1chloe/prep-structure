...
export function usePick(ids = [], initial = []) {   // initial: 처음부터 고른 채 시작하는 것(10 발송의 마감한 판 · (어28)-⑤)
  ...
  return { ...st, has: (id) => sel.has(id), toggle: (id) => setSel((s) => togglePick(s, id)), setAll: (on) => setSel(pickAll(ids, on)), only: (id) => setSel(new Set([id])), clear: () => setSel(new Set()),
    setMany: (list, on) => setSel((s) => pickMany(s, list, on)), allOf: (list = []) => list.length > 0 && list.every((id) => sel.has(id)), someOf: (list = []) => list.some((id) => sel.has(id)) };   // 묶음(반 등) 하나만 넣고 빼기((어31))
}
...
/** 묶음 머리의 네모(반 등 · (어31) · 원장님 「반별 선택도 가능하게」) · 그 묶음의 줄(ids)만 넣고 뺀다 · 다른 묶음에서 고른 것은 그대로 · 띠는 같은 띠 */
