...
export default function AskCard({ asks = [], send, title = "선생님께 남기실 말", placeholder = "예: 다음 주 수요일 병원이라 늦어요", note = null }) {
  ...
  return (
    <div className="task" data-card="ask">
      ...
      <div className="wv" style={{ marginTop: 4, marginBottom: 0 }}><button type="button" className="btn sm pri" data-act="ask" disabled={pending || !text.trim()} onClick={go}>보내기</button></div>
