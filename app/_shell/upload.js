...
export default function Upload({ rules = {}, studentId = null, kids = null, itemId = null, todoId = null, paste = false, sendRef = null, onDone = null, label = "📷 사진 · 📄 파일 보내기", hint = "원장님만 봅니다", compact = false }) {
  ...
  return (
    <div data-g="upload">
      ...
      {files.length > 0 && <div className="tags" style={{ marginTop: 8 }} data-g="picks">{pickRows(files).map((r) => <span key={r.i} className="tag" data-g="pick-row" style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>{r.photo && urls[r.i] ? <img src={urls[r.i]} alt={r.name} style={{ width: 28, height: 28, objectFit: "cover", borderRadius: 4 }} /> : "📄"} {r.name} <small>{r.size}</small><button type="button" className="btn sm gho" data-act="pick-remove" {...icon(`${r.name} 제외`)} onClick={() => setFiles(withoutPick(files, r.i))} style={{ padding: "0 6px" }}>✕</button></span>)}</div>}
