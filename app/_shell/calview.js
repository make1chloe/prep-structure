...
import { legendFor } from "@/lib/cal-plan";
export default function CalView({ d, base, extra = "", backHref, backLabel, kids = [] }) {
  ...
  return (<div className="calpage">
    ...
    <div className="clegend">{legendFor(d.who).map(([cls, ch, name]) => <span key={cls}><i className={"cm " + cls}>{ch}</i>{name}</span>)}</div>
    <div className="card" style={{ padding: 8 }} data-g="day">
