/** 📅 달력 그리기(목업 09b) — 아이·학부모가 같은 그림. 데이터는 lib/cal.calendar(), 링크만으로 움직인다(?m 달 · ?d 날 · 형제는 ?s). 앞날은 다음 달까지 · 지난 것은 재원 기간만(확정-⑯) */
...
export default function CalView({ d, base, extra = "", backHref, backLabel }) {
  ...
  return (<>
    <div className="calhead">
      {d.canPrev ? <a className="btn sm" href={link(d.prev)} aria-label="지난 달">◂</a> : <span className="btn sm dim" aria-disabled="true">◂</span>}
      <b data-g="month">{d.label}</b>
      {d.canNext ? <a className="btn sm" href={link(d.next)} aria-label="다음 달">▸</a> : <span className="btn sm dim" aria-disabled="true">▸</span>}
      <span className="spacer" /><span className="pill">{d.student.name}</span><a className="btn sm gho" href={backHref}>{backLabel}</a></div>
