...
const wrap = (fn) => act(fn, "발송 10");   // 손 한 벌은 lib/act.js · 삼키지 않고 서버 기록에 까닭을 남긴다(원칙-1)
...
export async function saveTemplateAct(kind, body) { return wrap(async () => { const { sb } = await staff(); return saveTemplate(sb, kind, body); }); }   // (커) ✉️ 문자 문구 수정
/** (어39) 줄을 펼쳐 그 자리에서 · 나갈 글 수정 · 하원 지연 안내 보내기(사유가 비었으면 적어서). 다른 화면으로 안 보낸다(대전제-22) */
...
/** (뎌-2) 문자로도 보낼 유형 켜고 끄기 · 판단·쓰기는 lib/send.js 한 벌 */
