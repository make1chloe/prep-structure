...
export default function When({ rules = {}, date, when, setWhen, cDate, setCDate, cTime, setCTime }) {
  return (<>
    ...
    {when === "custom" && <><DateBox type="date" value={cDate} min={date} onChange={(e) => setCDate(e.target.value)} style={{ width: "auto" }} aria-label="예약 날짜" /><DateBox type="time" value={cTime} onChange={(e) => setCTime(e.target.value)} style={{ width: "auto" }} aria-label="예약 시각" /></>}
  </>);
}
