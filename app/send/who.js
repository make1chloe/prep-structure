 ...
 *    그러니 `/send` 는 **스스로** 봐야 한다. 안 보면 학생 폰에서 발송 화면이 그대로 열린다 —
 *    여기는 **밖으로 나가는 단추**가 있는 화면이라 다른 어느 화면보다 나쁘다.
 *
 * ⚠️ 「원장·강사인가」를 여기에 **글자로 다시 적지 않는다.**
 *    `lib/supabase-server.js` 의 `HOME` 표가 이미 그 답을 갖고 있다 —
 *    첫 화면이 `/` 인 역할이 곧 원장·강사다. 목록을 두 벌로 두면 역할이 하나 느는 날
 *    한쪽만 고쳐진다(원칙 1). `app/page.js`·`app/today/who.js` 는 아직 목록을 따로 적고 있다 —
 *    보고의 `notes` 에 적었다.
 *
...
import { serverClientFromStore, roleOf, keys, homeFor, SCHEMA } from "../../lib/supabase-server.js";
...
/** 첫 화면이 `/` 인 역할 = 원장·강사·조교. **표는 lib 에 한 벌뿐이다**
 * ⚠️ 2026-09-03 — `HOME` 에 조교가 들어가면서 이 줄이 **저절로 조교까지 받는다.**
 *    그것이 이 꼴을 고른 까닭이다: 역할이 하나 느는 날 이 파일을 안 고쳐도 된다(원칙-1). */
const isStaff = (role) => homeFor(role) === "/";
...
export async function staffOnly() {
  ...
  const 문 = blockedBy(who.role, "page.send", 권한.rows, 권한.why);
  if (!문.ok)
