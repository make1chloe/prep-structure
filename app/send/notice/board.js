...
export default function Board({ d }) {
  ...
  return <>
    <div className="wv" style={{ marginBottom: 8 }} data-g="head"><b style={{ fontSize: "var(--fs-5)" }}>📢 공지</b><span className={"pill" + (c.unsent ? " warn" : "")} data-g="unsent">안 보냄 {c.unsent}</span><span className="pill" data-g="sent">보냄 {c.sent}</span><PickAll pick={pk} disabled={!(b.notices ?? []).some((n) => sendable(n))} /><span className="spacer" /><Link prefetch={false} className="btn sm" href="/send">📨 발송 ↗</Link><button type="button" className="btn pri sm" data-act="add-open" aria-pressed={open} onClick={() => setOpen(!open)}>+ 공지</button></div>
