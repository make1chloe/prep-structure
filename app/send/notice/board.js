...
import { addAct, attachAct, sendAct, sendManyAct } from "./actions.js";
...
export default function Board({ d }) {
  const router = useRouter(); const [pending, start] = useTransition(); const [err, setErr] = useState(""); const [msg, setMsg] = useState(""); const sure = useSure();
  ...
  const pk = usePick((b.notices ?? []).filter((n) => sendable(n)).map((n) => n.id));   /* 안 보낸 공지만 고른다((어28)-④ · 대전제-20) · 보낸 것은 손이 없어 네모가 잠긴다 · 고른 것은 화면 안에만 */
  ...
  return <>
    <div className="wv" style={{ marginBottom: 8 }} data-g="head"><b style={{ fontSize: "var(--fs-5)" }}>📢 공지</b><span className={"pill" + (c.unsent ? " warn" : "")} data-g="unsent">안 보냄 {c.unsent}</span><span className="pill" data-g="sent">보냄 {c.sent}</span><PickAll pick={pk} disabled={!(b.notices ?? []).some((n) => sendable(n))} /><span className="spacer" /><Link prefetch={false} className="btn sm goto" href="/send">📨 발송</Link><button type="button" className="btn pri sm" data-act="add-open" aria-pressed={open} onClick={() => setOpen(!open)}>+ 공지</button></div>
    ...
    {open && <div className="card" data-g="add-form" style={{ marginBottom: 8 }}><div className="ctitle"><span className="cemo">＋</span>공지</div>
      ...
      <textarea value={f.body} aria-label="본문" placeholder="본문(선택)" rows={3} onChange={(x) => setF({ ...f, body: x.target.value })} style={{ width: "100%" }} />
    ...
    {(b.notices ?? []).map((n) => <div className="card" key={n.id} data-g="notice" data-notice={n.id} data-state={n.sent_at ? "sent" : "draft"} style={{ marginBottom: 8 }}>
      <div className="ctitle"><PickBox pick={pk} id={n.id} label={`${n.title} 고르기`} disabled={!sendable(n)} /><span className="cemo">📢</span><span data-g="title">{n.title}</span><span className="tag" data-g="target">{targetText(n)}</span><span className={"pill" + (n.sent_at ? " hw" : " warn")} data-g="status">{statusText(n)}</span></div>
