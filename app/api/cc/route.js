 ...
 *  키: 헤더 `Authorization: Bearer <확장 키>` · 설정 🔌 연동 설정의 🃏 줄(v2.integration 'classcard' token)과 견준다(확정-72).
 *  키가 없거나 다르면 401 · 짐이 크면 413 · 꼴이 아니면 400. 어떤 경우에도 **키는 답에 안 싣는다**(대전제-9). */
...
export async function POST(req) {
  ...
  if (!want) return Response.json({ error: "확장 키가 아직 없습니다. 설정 🔌 연동 설정에서 넣어 주세요" }, { status: 401 });
  const auth = req.headers.get("authorization") ?? "";
  if (!sameToken(auth.replace(/^Bearer\s+/i, "").trim(), want)) return Response.json({ error: "키가 다릅니다" }, { status: 401 });
