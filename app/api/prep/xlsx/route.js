/** ⬇ 내신 자료 엑셀(04) · 회차 하나: 학생 × 자료 줄(학교 · 학교 진도 · 자료 종류 · 유형 · 항목 · 받음 · 단계). 학원 사람만 · 판은 prep_board 한 벌 */
...
export async function GET(req) {
  ...
  for (const t of b.takers ?? []) {
    ...
    if (!mine.length) rows.push({ 학생: t.name, 학교: t.school ?? "", 학년: t.grade ?? "", "학교 진도": t.school_prog ?? "", 출처: "", 자료종류: "", 유형: "", 항목: "", 받음: "", 단계: "" });
    for (const m of mine) { const g = m.gives.find((x) => x.student_id === t.id); rows.push({ 학생: t.name, 학교: t.school ?? "", 학년: t.grade ?? "", "학교 진도": t.school_prog ?? "", 출처: m.source, 자료종류: m.type, 유형: m.title, 항목: (m.items ?? []).map((i) => i.name).join(", "), 받음: g?.handed_at ? "줌" : "", 단계: stage(g?.stage ?? "none") }); }
  }
  const ws = XLSX.utils.json_to_sheet(rows, { header: ["학생", "학교", "학년", "학교 진도", "출처", "자료종류", "유형", "항목", "받음", "단계"] });
