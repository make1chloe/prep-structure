...
export default function Board({ d }) {
  ...
  return <>
    <div className="wv" style={{ marginBottom: 8 }} data-g="head"><span className="pill" style={{ fontWeight: 700 }} data-g="open-pill">{edit.is_open ? `✎ 열림 · ${daysOpenText(edit.opened_on, today)}` : "✎ 닫힘"}</span><span className={"pill" + (rows.length ? " warn" : "")} data-g="pending-count">아이가 찍은 것 {rows.length}</span><span className={"pill" + (flags.length ? " warn" : "")} data-g="flag-count">❗ {flags.length}</span><span className="spacer" /><Link prefetch={false} className="btn sm goto" href="/settings">⚙️ 설정</Link><Link prefetch={false} className="btn sm goto" href="/today">📋 오늘 수업</Link></div>
    ...
    <div className="exr" style={{ marginTop: 8 }} data-g="edit">
      ...
      <div className="left">
        <div className="lf ok" data-g="academy"><span className="ln">🏢</span><div><b>학원 전체</b><small>기본값 · 아래에서 안 고친 아이는 이걸 따라갑니다 · 날짜로 저절로 안 닫힙니다. 「N일째」가 알려 줍니다</small></div>
