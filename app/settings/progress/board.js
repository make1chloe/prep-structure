...
export default function Board({ d }) {
  ...
  return <>
    <div className="wv" style={{ marginBottom: 8 }} data-g="head"><span className="pill" style={{ fontWeight: 700 }} data-g="open-pill">{edit.is_open ? `✎ 열림 · ${daysOpenText(edit.opened_on, today)}` : "✎ 닫힘"}</span><span className={"pill" + (rows.length ? " warn" : "")} data-g="pending-count">아이가 찍은 것 {rows.length}</span><span className={"pill" + (flags.length ? " warn" : "")} data-g="flag-count">❗ {flags.length}</span><span className="spacer" /><Link prefetch={false} className="btn sm" href="/settings">⚙️ 설정 ↗</Link><Link prefetch={false} className="btn sm" href="/today">📋 오늘 수업에서 진도 체크 ↗</Link></div>
