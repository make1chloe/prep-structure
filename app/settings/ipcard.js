...
export default function IpCard() {
  ...
  return (<div className="wv" style={{ marginTop: 8 }}>
    <button type="button" className="btn sm pri" data-act="allow-ip" onClick={() => start(async () => { const r = await allowThisIp(); setMsg(r.ok ? (r.added ? `더했습니다 — ${r.ip}` : `이미 있습니다 — ${r.ip}`) : r.msg); })}>이 주소 더하기</button>
