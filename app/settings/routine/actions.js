...
import { addItem, editItem, retireItem, setLine, setLineMany, moveLine, customizeStudent, resetStudent, reviveStudentLine, setBook, assignBook, customizeBook, resetBook, endBook, nextRound } from "@/lib/routine";
import { parseChecks } from "@/lib/routine-plan";
const wrap = (fn) => act(fn, "루틴 11");   // 손 한 벌은 lib/act.js · 삼키지 않고 서버 기록에 까닭을 남긴다(원칙-1)
export async function addItemAct(f) { return wrap(async () => { const { sb } = await staff(); return addItem(sb, { area: f.area, name: f.name, method: f.method, checks: parseChecks(f.checks), place: f.place, required: Boolean(f.required), byApp: f.byApp || null }); }); }
export async function editItemAct(id, f) { return wrap(async () => { const { sb } = await staff(); await editItem(sb, id, { name: f.name, method: f.method, checks: parseChecks(f.checks), byApp: f.byApp || null }); return {}; }); }
export async function retireItemAct(id) { return wrap(async () => { const { sb } = await staff(); return retireItem(sb, id); }); }   // 항목 자체 삭제((가)-⑨)
...
export async function nextRoundAct(id) { return wrap(async () => { const { sb } = await staff(); return nextRound(sb, { studentBookId: String(id) }); }); }   // (어50) 다음 회독 시작(원장님 9/16 「교재에 진도 2회독은 체크가안돼」)
