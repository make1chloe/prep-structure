/** 목업 00 로그인 — 역할마다 아이디가 다르다. 학생 chloe+폰 뒤 4자리 · 학부모 전화번호 · 원장 이메일 · 선생님·조교 아이디((어64)). 꼬리 도메인은 화면에 없다 */
import { redirect } from "next/navigation";
import { whoami } from "@/lib/session";
import { signIn } from "./actions.js";
import { FIRST_PW } from "@/lib/student-plan";   // 첫 비밀번호는 한 곳((어65) · 인증이 여섯 자 미만을 안 받는다)
export const dynamic = "force-dynamic";
const CARDS = [
  { kind: "student", emo: "🧑‍🎓", title: "학생", label: "아이디", placeholder: "chloe0515", mode: "text", note: <><b>chloe + 내 폰 뒤 4자리</b>입니다. 이메일이 아닙니다.</> },
  { kind: "parent", emo: "👨‍👩‍👧", title: "학부모", label: "전화번호", placeholder: "01012345678", mode: "numeric", note: <><b>전화번호가 곧 아이디</b>입니다. 처음 비밀번호는 <b>{FIRST_PW}</b>이고, 들어오시면 바로 바꾸는 화면이 뜹니다.</> },
  { kind: "staff", emo: "🧑‍🏫", title: "원장 · 선생님 · 조교", label: "이메일 · 아이디", placeholder: "name@example.com", mode: "email", note: <>선생님·조교는 <b>설정 → 「누가 무엇을 보나」</b>에서 원장님이 켠 만큼만 보입니다. 안 정했으면 메뉴가 0칸입니다</> },
];
