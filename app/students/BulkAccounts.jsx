...
export default function BulkAccounts({ who = "student" }) {
  ...
  // **비밀번호는 계정마다 다르다.** 예전에는 다 0000 이라 한 줄로 적어뒀는데,
  // 아이디가 규칙으로 만들어지는 만큼 비번은 달라야 한다 (남이 짐작하게 두면 안 된다).
      ...
      <p className="hint" style={{ margin: "6px 0 0" }}>
        {parent ? (
          <>
            ...
            <br />
            첫 비밀번호는 <b>계정마다 다른 네 자리</b>이고, 어머니가 처음 들어가시면 새로 정합니다.
            <b> 형제자매는 한 계정</b>으로 묶여서 한 번 로그인하면 두 아이가 다 보입니다.
          </>
        ) : (
          <>
            아이디는 <b>chloe0001</b> 부터 차례로, 첫 비밀번호는{" "}
            <b>계정마다 다른 네 자리</b>입니다. 학생이 처음 들어가면 새로 정합니다. 이미 계정이 있는 학생은 그대로 둡니다.
          </>
        )}
      </p>
