...
export default function BulkAccounts({ who = "student" }) {
  ...
  // **비밀번호는 모두 0000 이다** (원장님, 2026-08-06). 그래도 줄마다 적어둔다 —
  // 나눠주는 종이에 아이디만 있으면 「비번은 뭐였죠」 를 다시 물어보시게 된다.
  const lines = (res?.made || []).map(
    (m) => `${m.name}${parent && m.kids > 1 ? ` (${m.kids}명)` : ""}  ${m.loginId}  ${m.password || ""}`
  );
      ...
      <p className="hint" style={{ margin: "6px 0 0" }}>
        {parent ? (
          <>
            ...
            <br />
            첫 비밀번호는 <b>0000</b> 이고, 어머니가 처음 들어가시면 <b>바꾸는 화면부터</b> 뜹니다
            (바꾸기 전에는 아무것도 못 보십니다).
            <b> 형제자매는 한 계정</b>으로 묶여서 한 번 로그인하면 두 아이가 다 보입니다.
          </>
        ) : (
          <>
            아이디는 <b>chloe0001</b> 부터 차례로, 첫 비밀번호는 <b>0000</b> 입니다.
            학생이 처음 들어가면 <b>바꾸는 화면부터</b> 뜹니다. 이미 계정이 있는 학생은 그대로 둡니다.
          </>
        )}
      </p>
