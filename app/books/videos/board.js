...
export default function Board({ d }) {
  ...
  const Video = ({ v, k }) => { const c = counts(v.assigns, cut), rows = (v.assigns ?? []).filter((a) => a.state !== "retired"); return (
    <div key={k} className="card" style={{ margin: "0 0 8px" }} data-g="video" data-video={v.id} data-state={v.state}>
      ...
      {rows.map((a) => { const s = statusOf(a, cut); return <div className="vrow" key={a.id} data-g="vrow" data-student={a.student_id} data-status={s.key}><span className="vn">{a.student_name}</span><div className="bar"><div className="fill" style={{ width: `${s.key === "done" ? 100 : a.pct ?? 0}%`, background: fillColor(s) }} /></div><span className="bkv" style={{ color: s.key === "none" ? "var(--miss)" : undefined }} data-g="bkv">{s.text}</span>
        <small className="note" style={{ margin: 0 }}>{a.due_on ? dueText(a.due_on, today) : "마감 없음"}{a.done_at ? ` · ${md(seoulDate(a.done_at))} 다 봄` : ""}{opensText(a.opens) ? ` · ${opensText(a.opens)}` : ""}</small><button type="button" className="btn sm gho" disabled={pending} data-act="retire" {...icon(`${a.student_name} 배정 삭제`)} onClick={() => run(() => retireAct(a.id), "배정을 삭제했습니다")}>✕</button></div>; })}
      ...
      {assignFor === v.id && <div className="exr" style={{ marginTop: 8 }} data-g="assign-form"><div className="exh"><span className="ai">🧑‍🎓</span><b>누구에게 · 마감</b><span className="spacer" /><DateBox type="date" value={due} aria-label="마감" min={today} onChange={(e) => setDue(e.target.value)} style={{ width: "auto" }} /></div>
  ...
  return <>
    ...
    <PickBar pick={pk} unit="개">{/* (어28)-③ 고른 영상에 한 번에 · 폴더 옮기기 · 삭제(지우지 않는다) · 복구 · 📨 안 본 아이에게 알림 */}
      ...
      <button type="button" className="btn sm gho" disabled={pending || !toDown.length} data-act="hide-picked" onClick={() => run(() => setManyAct(toDown.map((v) => v.id), { state: "hidden" }), (r) => `${r.n}개 삭제 ✓ · 복구할 수 있습니다`, pk.clear)}>삭제 {toDown.length}</button>
