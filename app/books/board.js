...
export default function Board({ d }) {
  ...
  const pk = usePick((book?.units ?? []).map((u) => u.id));   /* 단원 줄 고르기((어28)-④ · 대전제-20) · 고른 것은 화면 안에만 */
  return <>
    ...
    <div className="bkw">
      ...
      {book ? <div className="bkdet" data-g="detail" data-book={book.id}>
        ...
        <div className="tune" style={{ marginBottom: 8 }} data-g="tune">
          ...
          <div><label className="fl">배정 겹 · 한 번에 나가는 분량</label><div className="seg sm" data-g="chunk">{CHUNK.map(([k, nm]) => <button key={k} type="button" aria-pressed={book.chunk_depth === k} disabled={pending} onClick={() => run(() => setBookAct(book.id, { chunk_depth: k }), `배정 겹 ${nm}`)}>{nm}</button>)}</div></div>
        ...
        <div className="tblwrap"><table data-g="unit-table"><thead><tr><th><PickAll pick={pk} disabled={!(book.units ?? []).length} /></th><th>대단원</th><th>중단원</th><th>소단원</th><th>활동명</th><th>학습유형</th><th>쪽</th><th>문항</th><th>문법 분류</th><th>고치기</th></tr></thead><tbody>
          {(book.units ?? []).map((u) => <tr key={u.id} data-g="unit-row" data-unit={u.id} data-state={u.state} style={u.state === "hidden" ? { color: "var(--mute)" } : undefined}><td><PickBox pick={pk} id={u.id} label={`${u.sub ?? u.chapter} 고르기`} /></td><td className="sch">{u.chapter}</td><td>{u.mid ?? ""}</td><td>{u.sub ?? ""}</td><td>{u.activity}</td><td><span className={"tag" + (u.is_workbook ? "" : " type")}>{u.is_workbook ? "워크북" : "본책"}</span></td>
            ...
            <td><span className="wv" style={{ gap: 4 }}>{topicsOf(u.id).map((t) => <span key={t.topic_id} className="um hit"><b>{t.name}</b> <button type="button" className="btn sm gho" disabled={pending} data-act="topic-remove" onClick={() => run(() => topicsAct(u.id, topicsOf(u.id).filter((x) => x.topic_id !== t.topic_id).map((x) => x.topic_id)), "분류를 뗐습니다")}>✕</button></span>)}
              <select value="" aria-label={`${u.sub ?? u.chapter} 문법 분류`} disabled={pending} data-g="topic-pick" onChange={(x) => x.target.value && run(() => topicsAct(u.id, [...topicsOf(u.id).map((t) => t.topic_id), x.target.value]), "분류를 이었습니다")} style={{ width: "auto" }}><option value="">+ 배정</option>{(b.topics_all ?? []).filter((t) => !topicsOf(u.id).some((x) => x.topic_id === t.id)).map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></span></td>
            <td><span className="wv" style={{ gap: 4 }}>{uedit[u.id] && <button className="btn sm pri" type="button" disabled={pending} data-act="unit-save" onClick={() => run(() => unitAct(u.id, { pages: ue(u, "pages", (pagesText(u) ?? "").replace(/^p\./, "")), qCount: ue(u, "qCount", u.q_count ?? ""), gist: u.gist ?? "" }), "단원을 고쳤습니다(쪽·문항 · 조절·회차가 새 값으로 셉니다)", () => setUedit((st) => ({ ...st, [u.id]: undefined })))}>저장</button>}
              <button className="btn sm gho" type="button" disabled={pending} data-act="unit-state" onClick={() => run(() => unitStateAct(u.id, u.state === "hidden" ? "active" : "hidden"), u.state === "hidden" ? "복구했습니다" : "숨겼습니다. 자동 배정·회차·범위에서 빠집니다(지우지 않았습니다)")}>{u.state === "hidden" ? "복구" : "숨김"}</button></span></td></tr>)}
          {!(book.units ?? []).length && <tr><td colSpan={10} className="note">단원 없음 · ⬆ 올리기</td></tr>}
        </tbody></table></div>
        <PickBar pick={pk} unit="단원">{/* (어28)-④ 고른 단원에 한 번에 · 줄의 「숨김/복구」 손과 같다(지우지 않는다) */}
          <button className="btn sm gho" type="button" disabled={pending} data-act="hide-picked" onClick={() => run(() => unitStateManyAct(pk.ids, "hidden"), (r) => `${r.n}단원을 숨겼습니다. 자동 배정·회차·범위에서 빠집니다(지우지 않았습니다)`, () => pk.clear())}>숨기기 {pk.count}</button>
          <button className="btn sm" type="button" disabled={pending} data-act="unhide-picked" onClick={() => run(() => unitStateManyAct(pk.ids, "active"), (r) => `${r.n}단원을 복구했습니다`, () => pk.clear())}>복구 {pk.count}</button>
        </PickBar>
...
/** 📦 올린 기록(0057 · 0142) · 최근 8 · 되돌릴 수 있는 것(같은 표의 마지막 살아 있는 올린 기록 · SQL 이 정한다)만 「되돌리기」 · 두 번 눌러야 한다(되돌릴 수 없는 낙관 갱신은 안 한다 · 속도-5) */
function Runs({ runs, run, pending }) {
  const [arm, setArm] = useState(null);
  if (!runs.length) return null;
  return <div className="card" style={{ marginTop: 8 }} data-g="runs">
    <div className="ctitle"><span className="cemo">📦</span>올린 기록<span className="spacer" /><span className="note k" style={{ margin: 0 }}>잘못 올렸으면 한 번에 되돌립니다. 같은 표는 마지막 것부터 · 진도가 걸린 줄은 지우지 않고 숨깁니다</span></div>
    {runs.map((r) => { const l = runLine(r); return <div key={r.id} className="wv" style={{ padding: "4px 0", color: l.state === "undone" ? "var(--mute)" : undefined }} data-g="run" data-run={r.id} data-state={l.state}>
...
function Upload({ close, run, pending }) {
  ...
  return <div className="mdlov" data-g="upload"><div className="mdl" style={{ width: "min(640px, 100%)" }}>
    ...
    <div className="mdlf">{plan && plan.kind === "books" && <button className="btn pri" type="button" disabled={pending || !plan.rows.length} data-act="upload-save" onClick={() => run(() => applyBooksAct(plan.rows, plan.name), (r) => `저장했습니다. ${r.note}${r.held ? ` · 보류 ${r.held}` : ""}${r.failed.length ? ` · 실패: ${r.failed.join(" / ")}` : ""} · 올린 기록 #${r.run}(아래 「올린 기록」에서 되돌릴 수 있습니다)`, close)}>{plan.bad.length ? `고칠 줄 ${plan.bad.length}을 뺀 나머지 저장` : "저장"}</button>}
      {plan && plan.kind !== "books" && <button className="btn pri" type="button" disabled={pending} data-act="upload-save" onClick={() => run(() => applyAct(plan.rows, { modes, holds }, plan.name), (r) => `저장했습니다. 교재 ${r.books}권 · 줄 ${r.put + r.replaced}${r.skipped ? ` · 건너뜀 ${r.skipped}권` : ""}${r.holds ? ` · 보류 ${r.holds}권` : ""}${r.failed.length ? ` · 실패: ${r.failed.join(" / ")}` : ""} · 올린 기록 #${r.run}(아래 「올린 기록」에서 되돌릴 수 있습니다)`, close)}>{holdsLeft ? `보류 ${holdsLeft}줄을 뺀 나머지 저장` : "저장"}</button>}
      <button className="btn gho" type="button" onClick={close}>닫기</button><span className="spacer" /><span className="pill">지우지 않습니다. ②만 지우고, 그것도 진도가 걸리면 막힙니다 · 올리기 번호로 되돌릴 수 있습니다</span></div>
  </div></div>;
}
