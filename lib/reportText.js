...
export function reportParts(r, date, rule = {}) {
  ...
  const behind = (r?.checks || [])
    ...
    .map((c) => `${c.name} ${c.status === "missing" ? "미제출" : "미흡"}`)
    .join(", ");
...
export function buildReportText(r, date, academy = "클로이영어", msg = {}) {
  ...
  // 오늘 남아서 한 것 — **결과만**. 넘어간 것은 아이 숙제 안내에 적힌다
  ...
  if (stayDone.length || stayMoved.length) {
    ...
    L.push(
      `· ${STAY_LABEL}: ${[
        stayDone.length ? `${stayDone.length}건 마치고 하원` : "",
        stayMoved.length ? `${stayMoved.length}건은 숙제로 넘김` : "",
      ].filter(Boolean).join(" · ")}`
    );
  }
  ...
  // **숙제는 여기서 늘어놓지 않는다.** 아이 앱에 그대로 있고, 어머니 화면에도
  // 「지금 나간 숙제」 칸이 따로 있다. 같은 것을 두 번 읽으시면 위쪽 점수를 놓치신다
  if (r.next?.length) {
    L.push("");
    L.push(`숙제 ${r.next.length}가지를 앱에 올려두었습니다.`);
  }
...
export function buildHomeworkText(r, date, academy = "클로이영어", msg = {}) {
  const L = [];
  L.push(`[${academy}] ${r.student.name} ${dateLabel(date)} 숙제`);
  L.push("");
