...
export async function calendar(sb, st, ym, date, today, role = "student") {
  ...
  const shaped = rows(sheets, "판").map((s) => { const home = (s.day_item ?? []).filter((i) => i.slot === "home" && !i.off); return { ...s, home_count: home.length, home_names: home.map(itemTitle), late: (s.late_stay ?? [])[0] ?? null }; });
