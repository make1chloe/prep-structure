...
export async function calendar(sb, st, ym, date, today, role = "student") {
  ...
  const [days, abs, lates, arrival, sheets, quizzes, exams, holidays, access, dues, member] = await Promise.all([
    ...
    liveArrival(() => db(sb).from("arrival").select("date,step,at").eq("student_id", id).gte("date", from).lte("date", to)),
    db(sb).from("day_sheet").select("id,date,attend,attend_reason,closed_at,comment,day_item(id,slot,off,range_note,learn_items(name)),late_stay(until_at,reason)").eq("student_id", id).gte("date", from).lte("date", to).order("date"),
  ...
  const shaped = rows(sheets, "수업 일지").map((s) => { const home = (s.day_item ?? []).filter((i) => i.slot === "home" && !i.off); return { ...s, home_count: home.length, home_names: home.map(itemTitle), late: (s.late_stay ?? [])[0] ?? null }; });
  const ctx = { days: rows(days, "수업일"), absences: rows(abs, "결석"), lates: rows(lates, "지각 예정"), arrival: rows(arrival, "등원"), quizzes: rows(quizzes, "시험"), exams: rows(exams, "학교 시험"), holidays: rows(holidays, "휴강"), dues: rows(dues, "내가 정한 마감"), today, who: role };   // (어75) 아이 달력은 수업 일정만 — 가르는 곳은 lib/cal-plan 한 곳
  const sheetOf = (d) => shaped.find((s) => s.date === d) ?? null;
  const minYm = ymOf(rows(member, "재원")[0]?.from_date ?? today), maxYm = nextYm(ymOf(today), 1);
  return {
    who: role, student: st, ym, label: monthLabel(ym), prev: nextYm(ym, -1), next: nextYm(ym, 1), canPrev: ym > minYm, canNext: ym < maxYm, date, access: rows(access, "권한"),
