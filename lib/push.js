 ...
 *  키(VAPID)는 v2.integration 의 push 줄(옛 앱 설정에서 0072 가 옮겼다 · 주소·키가 같아야 이미 켠 기기가 산다) */
...
export function reason(code, body = "") {
  ...
  if (code === 401 || code === 403) return `알림 키가 거절됐습니다 (${code}). 알림 키를 다시 만들고 기기마다 알림을 다시 켜야 합니다${tail}`;
...
/** 기기마다 제 짐(payloadFor(기기) · r 은 그 사람의 발송 이력 번호)을 싣는다. 실패를 삼키지 않는다(옛 앱 2026-08-07 사고: 404·410 만 챙기고 나머지를 버려 「보냈어요」가 한 통도 안 갔을 때도 나왔다).
...
export async function pushToAll(keys, subs, payloadFor) {
  if (!keys?.publicKey || !keys?.privateKey) throw new Error("알림 키가 없습니다. 연동(v2.integration) push 줄에 publicKey·privateKey 가 있어야 합니다(0072 가 옛 앱에서 옮겼다)");
  configure(keys);
