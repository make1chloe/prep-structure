 ...
 * ✅ **찾았다 (2026-09-02)** — 옛 앱은 열쇠를 **환경변수가 아니라 DB 에** 넣는다.
 *    `main:lib/push.js` — 「보내는 데 필요한 키(VAPID)는 설정 화면에서 한 번 만들어
 *    integrations 에 저장한다」. 그래서 Vercel 환경변수에는 있을 수가 없었다.
 *    0072 가 그 값을 `v2.integration(id='push')` 로 옮겼다 — **환경변수보다 그쪽을 먼저 본다.**
 *    (구독하는 쪽 화면은 `NEXT_PUBLIC_…` 이 있어야 브라우저에서 읽는다 — 환경변수도 계속 받는다)
 * · `VAPID_SUBJECT` 는 푸시 서버가 문제 생겼을 때 연락할 자리다. 배달에는 영향이 없지만
 *   **없으면 web-push 가 던진다.** 없을 때는 지어내지 않고 배포 주소(`VERCEL_URL`)를 쓴다.
 */
export function vapidFrom(env = process.env, saved = null) {
  const host = str(env.VERCEL_URL);
  const c = saved || {};
  return {
    // ⚠️ **DB 가 먼저다.** 옛 앱이 거기 넣어 뒀고, 그 열쇠라야 폰에 박힌 구독이 산다.
    subject: str(c.contact) || str(env.VAPID_SUBJECT) || (host ? "https://" + host : ""),
    publicKey: str(c.publicKey) || str(env.VAPID_PUBLIC_KEY) || str(env.NEXT_PUBLIC_VAPID_PUBLIC_KEY),
    privateKey: str(c.privateKey) || str(env.VAPID_PRIVATE_KEY),
  };
}

/** DB 에 저장된 열쇠를 읽는다 — 옛 앱이 넣어 둔 자리(0072 로 옮겨 왔다) */
export async function vapidSaved(db) {
  try {
    const r = await db.query("select config from v2.integration where id = 'push'");
    return r?.rows?.[0]?.config ?? null;
  } catch { return null; }   // ⚠️ 못 읽으면 환경변수로 간다 — 여기서 던지면 발송이 통째로 멈춘다
}
 ...
 */
export function pushReady(env = process.env, saved = null) {
  const v = vapidFrom(env, saved);
  const miss = [];
  ...
  if (miss.length)
    return { ok: false, why: "no_key",
      msg: "⚠️ 웹푸시 열쇠가 없어 **한 대도 못 쏩니다** — " + miss.join(" · ") +
           " 가 비어 있습니다. ⚠️ 새로 만들지 마세요 — 옛 앱 열쇠는 **DB 의 " +
           "v2.integration(id='push')** 에 있습니다(0072 로 옮겨 왔다). 새로 만들면 " +
           "폰에 박힌 구독이 그날로 전부 죽습니다" };
  try {
...
export function makePush(db, opts = {}) {
  ...
  let ready = null, saved = opts.saved ?? null, looked = opts.saved !== undefined;

  return async function push(sub, payload) {
    // ⚠️ ① 열쇠가 없으면 **여기서 던진다.** 「없으니 그냥 통과」하면
    //    자취에는 「보냄」이 남고 폰에는 아무것도 안 간다 — 그게 제일 나쁘다
    // ⚠️ **DB 를 먼저 본다** — 옛 앱이 열쇠를 거기 넣어 뒀고(0072), 그 열쇠라야
    //    폰에 박힌 구독이 산다. 한 통에 기기가 여럿이라 **한 번만** 읽는다.
    if (!looked) { saved = await vapidSaved(db); looked = true; }
    if (ready === null) ready = pushReady(env, saved);
    ...
    try {
      await send(
        ...
        { vapidDetails: vapidFrom(env, saved), TTL: ttl }
      );
    } catch (e) {
