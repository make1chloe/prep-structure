...
const ATTEND_NAME = Object.freeze({ present: "출석", late: "지각", absent: "결석", early: "조퇴", online: "온라인", makeup: "보강으로 옴", off: "휴강" });
...
export function dayDetail(date, { sheet = null, days = [], absences = [], lates = [], arrival = [], quizzes = [], exams = [], holidays = [], dues = [], today }) {
  ...
  for (const g of dues.filter((g) => g.due_on === date && g.stage !== "done")) rows.push({ n: "마", b: `🚩 내가 정한 마감 · ${g.material?.title ?? ""}`.trim(), small: "" });
