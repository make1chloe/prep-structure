 ...
 * 파기 목록 표를 읽는 **한 곳**이다.
 *
 * ⚠️⚠️ **`after_days` 를 반드시 같이 읽는다.** 2026-09-03 까지 이 select 에 그 칸이 없어서
 *    `how='expire'` 두 줄(`excel_row.before` · `excel_run.note`, 둘 다 90일)이
 *    `planFor()` 에 **기한이 null 인 채로** 도착했고, 그러면 「날짜로 지우는데 기한이 없다」로
 *    **막힌다(blocked).** 크론의 「기한파기」 걸음은 한 줄이라도 막히면 던지므로
 *    **밤 8시 크론이 날마다 울었다.** 늘 빨간 줄이 하나 서 있으면
 *    **진짜 고장이 났을 때 거기 묻혀 못 알아본다** (규칙-어긋난곳 ⑲).
 * ⚠️ 안 하면 무엇이 터지나 — 기한 파기가 **영영 한 줄도 안 돈다.**
 *    `excel_row.before` 에는 **바꾸기 전 DB 줄 전체**(이름·전화·상담 글)가 들어 있어
 *    90일이 몇 해가 되어도 그대로 남는다 (처음-3).
 * ⚠️ 칸을 더할 때도 **여기 한 곳만** 고친다. 다른 파일이 `v2.purge_map` 을 직접 읽으면
 *    `scripts/check-purge.mjs` 의 「파기 목록 표를 읽는 곳도 lib/purge.js 뿐이다」가 깨진다.
 */
export async function purgeMap(db) {
  const r = await db.query(
    `select schema_name, tbl, col, how, after_days, note from v2.purge_map order by tbl, col`, []);
  return r.rows;
}
...
/**
 * 기한 파기를 **지금 돌려도 되나** — 판단은 여기 한 곳이다 (대전제-4).
 *
 * ⚠️⚠️ 원장님 답(2026-09-03)은 **조건부 승낙**이었다: 「백업 따로 만들면 지워」.
 *    백업을 못 지었으므로(안 되는 까닭은 `app/api/cron/route.js` 의 `stepPurgeExpired` 머리주석)
 *    **조건이 안 채워진 것**이고, 그러면 안 지운다. 「지어낸 백업」보다 「안 지움」이 맞다.
 * ⚠️ 이것은 **막힘이 아니다.** 막힘은 터뜨려야 하고(규칙-어긋난곳 ⑲), 이것은 원장님이
 *    아직 안 켜신 것이다 — 크론 보고에 다르게 떠야 원장님이 「고장」과 안 헷갈리신다.
 * ⚠️ 값이 없으면 **꺼진 것으로 본다.** 지어내서 켜지 않는다.
 *
 * @param config `v2.integration` 의 'purge' 줄 config (없으면 null)
 * @returns { on, why }
 */
export function expireGate(config) {
  const on = config?.expire_on === true;
  return {
    on,
    why: on ? null
            : "원장님이 아직 안 켜셨습니다 — 「백업 따로 만들면 지워」의 **백업을 아직 못 지었습니다**"
              + " (v2.integration 의 purge 줄 config.expire_on 을 true 로 하면 켜집니다)",
  };
}

/** 그 스위치 한 줄만 읽는다. ⚠️ `v2.integration` 은 평문 열쇠가 든 표라 **이 한 줄 말고 안 읽는다** */
export async function readExpireGate(db) {
  const r = await db.query(`select config from v2.integration where id = 'purge'`, []);
  return expireGate(r.rows?.[0]?.config ?? null);
}
