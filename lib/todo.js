...
export async function handOut(sb, materialId, studentIds = null) {
  ...
  if (!left.length) {
    ...
    changed(await db(sb).from("todo").update({ state: "done", done_at: now() }).eq("material_id", materialId).in("kind", wait ? ["make", "print"] : ["make", "print", "hand"]).in("state", ["todo", "doing"]).select("id"), "배부 업무를 못 끝냄", { zero: "ok" });
