...
export async function reuseMaterial(sb, { examId, fromId, studentIds = [], takers = [], types = [], revised = false }) {
  ...
  if (p.studentIds.length) row(await db(sb).from("material_give").insert(p.studentIds.map((sid) => ({ material_id: ins.id, student_id: sid }))).select("material_id"), "배정을 못 넣음");
  if (revised) changed(await db(sb).from("material").update({ reuse_of: null }).eq("id", ins.id).select("id"), "자료를 못 고침");   // 개정판 — 지난번 것을 가리키지 않는다(만들기 체크 없이)
  const todos = await rpc(sb, "sync_material_todos", { p_material: ins.id }, "할 일을 못 세움");
...
async function stepMaterial(sb, materialId, step, back = false) {
  ...
  if (step === "hand") changed(await db(sb).from("material_give").update({ handed_at: back ? null : now() }).eq("material_id", materialId).select("material_id"), "배부를 못 적음", { zero: "ok" });
  if (step === "hand" && !back) changed(await db(sb).from("material_give").update({ handed_at: now() }).eq("material_id", materialId).is("handed_at", null).select("material_id"), "배부를 못 적음", { zero: "ok" });
}
...
async function closeEarlier(sb, materialId, kind) {
  ...
  if (earlier.length) changed(await db(sb).from("todo").update({ state: "done", done_at: now() }).eq("material_id", materialId).in("kind", earlier).in("state", ["todo", "doing"]).select("id"), "앞 단계 할 일을 못 끝냄", { zero: "ok" });
}
...
export async function finishTodo(sb, todoId) {
  ...
  changed(await db(sb).from("todo").update({ state: "done", done_at: now() }).eq("id", todoId).select("id"), "할 일을 못 끝냄");
...
export async function undoTodo(sb, todoId) {
  ...
  changed(await db(sb).from("todo").update({ state: "todo", done_at: null }).eq("id", todoId).select("id"), "할 일을 못 되돌림");
...
export async function dropTodo(sb, todoId, why = null) {
  ...
  changed(await db(sb).from("todo").update({ state: "dropped", why: String(why ?? "").trim() || t.why || "뺐습니다" }).eq("id", todoId).select("id"), "할 일을 못 뺌");
...
export async function dropMaterial(sb, materialId, why = null) {
  changed(await db(sb).from("material").update({ state: "dropped" }).eq("id", materialId).select("id"), "자료를 못 뺌");
  changed(await db(sb).from("todo").update({ state: "dropped", why: String(why ?? "").trim() || "못 따라가서 줄임(05 🔥)" }).eq("material_id", materialId).in("state", ["todo", "doing"]).select("id"), "할 일을 못 뺌", { zero: "ok" });
}
...
export async function printAll(sb, materialIds = []) {
  ...
  changed(await db(sb).from("todo").update({ state: "done", done_at: now() }).in("material_id", ids).in("kind", ["make", "print"]).in("state", ["todo", "doing"]).select("id"), "인쇄 할 일을 못 끝냄", { zero: "ok" });   // 뽑았으면 만든 것이다
...
export async function handOut(sb, materialId, studentIds = null) {
  ...
  if (!left.length) { await stepMaterial(sb, materialId, "hand"); changed(await db(sb).from("todo").update({ state: "done", done_at: now() }).eq("material_id", materialId).in("kind", ["make", "print", "hand"]).in("state", ["todo", "doing"]).select("id"), "배부 할 일을 못 끝냄", { zero: "ok" }); }
...
export async function unitTestMade(sb, id, date) {
  const r = changed(await db(sb).from("unit_test").update({ state: "made", assigned_on: date }).eq("id", id).eq("state", "todo").select("id"), "단원평가를 못 고침"); if (!r?.length) throw new Error("이미 출제한 단원평가입니다");
}
/** (버) 저절로 선 단원평가 카드(todo_board unit_test_due — 루틴 11 의 「단원평가 본다」가 켜진 교재의 끝난 묶음)의 「출제함」 — 묶음(교재·회독·대단원|묶음 번호·덮는 것)을 새겨 made 로 세운다(01 카드가 그때부터 선다). 같은 묶음 두 번은 0149 색인이 막는다 */
...
export async function setRepeatActive(sb, id, active) { const r = changed(await db(sb).from("auto_rule").update({ active: Boolean(active) }).eq("id", id).select("id"), "되풀이 규칙을 못 고침"); if (!r?.length) throw new Error("고쳐진 줄이 없습니다"); }
...
export async function todoMany(sb, ids = [], op, value = null) {
  const list = [...new Set((ids ?? []).filter(Boolean))]; if (!list.length) throw new Error("고른 할 일이 없습니다");
  const one = { done: (id) => finishTodo(sb, id), undo: (id) => undoTodo(sb, id), due: (id) => setTodoDue(sb, id, value), drop: (id) => dropTodo(sb, id, null) }[op];
