...
import { parseMaterial, parseRepeat, KINDS, SHOW_ON, kindList } from "./todo-plan.js";
...
async function closeEarlier(sb, materialId, kind) {
  ...
  if (earlier.length) changed(await db(sb).from("todo").update({ state: "done", done_at: now() }).eq("material_id", materialId).in("kind", earlier).in("state", ["todo", "doing"]).select("id"), "앞 단계 업무를 못 끝냄", { zero: "ok" });
}
...
export async function finishTodo(sb, todoId) {
  ...
  changed(await db(sb).from("todo").update({ state: "done", done_at: now() }).eq("id", todoId).select("id"), "업무를 못 끝냄");
...
export async function restoreTodo(sb, todoId) {
  ...
  if (r.state !== "dropped") throw new Error("뺀 업무가 아닙니다");
...
export async function dropTodo(sb, todoId, why = null) {
  ...
  changed(await db(sb).from("todo").update({ state: "dropped", why: String(why ?? "").trim() || t.why || "뺐습니다" }).eq("id", todoId).select("id"), "업무를 못 뺌");
  if (t.material_id && t.kind === "make") await dropMaterial(sb, t.material_id, why);
}
/** 자료 빼기(줄이기 🔥) · 자료 dropped + 하는 중인 업무 dropped */
export async function dropMaterial(sb, materialId, why = null) {
  ...
  changed(await db(sb).from("todo").update({ state: "dropped", why: String(why ?? "").trim() || "못 따라가서 줄임(05 🔥)" }).eq("material_id", materialId).in("state", ["todo", "doing"]).select("id"), "업무를 못 뺌", { zero: "ok" });
}
...
export async function printAll(sb, materialIds = []) {
  ...
  changed(await db(sb).from("todo").update({ state: "done", done_at: now() }).in("material_id", ids).in("kind", ["make", "print"]).in("state", ["todo", "doing"]).select("id"), "인쇄 업무를 못 끝냄", { zero: "ok" });   // 뽑았으면 만든 것이다
...
export async function handOut(sb, materialId, studentIds = null) {
  ...
  if (!left.length) { await stepMaterial(sb, materialId, "hand"); changed(await db(sb).from("todo").update({ state: "done", done_at: now() }).eq("material_id", materialId).in("kind", ["make", "print", "hand"]).in("state", ["todo", "doing"]).select("id"), "배부 업무를 못 끝냄", { zero: "ok" }); }
...
export const addNote = (sb, { title, dueOn = null, dueTime = null, studentId = null, note = null, startOn = null }) => addTodo(sb, { title, dueOn, dueTime, studentId, note, startOn });   // (어78) 마감·시작일은 없어도 된다 — 퀵 메모가 같은 손을 쓴다(원칙-1)
...
export async function listKinds(sb) {
  const { data, error } = await db(sb).from("todo_kind").select("kind,name,cls,ord,app,show_in,state").order("ord");
...
export async function addKind(sb, { name, cls = "", showOn = "todo" }) {
...
export async function editKind(sb, kind, { name, cls, showOn, ord } = {}) {
  ...
  if (!Object.keys(patch).length) throw new Error("고칠 것이 없습니다");
  changed(await db(sb).from("todo_kind").update(patch).eq("kind", kind).select("kind"), "분류를 수정 못 함");
}
...
export async function dropKind(sb, kind) {
  ...
  changed(await db(sb).from("todo_kind").update({ state: "off" }).eq("kind", kind).select("kind"), "분류를 못 내림");
}
