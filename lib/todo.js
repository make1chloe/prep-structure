...
export async function setRepeatActive(sb, id, active) { const r = changed(await db(sb).from("auto_rule").update({ active: Boolean(active) }).eq("id", id).select("id"), "반복 규칙을 못 고침"); if (!r?.length) throw new Error("고쳐진 줄이 없습니다"); }
