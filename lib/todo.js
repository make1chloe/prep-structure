...
export async function prepBoard(sb, examId, date) {
  const b = await rpc(sb, "prep_board", { p_exam: examId ?? null, p_on: date }, "내신 자료를 못 읽음");
...
export async function addRepeat(sb, { name, every, day, weekday, lead }, date) {
  const nm = String(name ?? "").trim(); if (!nm) throw new Error("무엇을 되풀이하는지 적으세요");
  const th = parseRepeat({ every, day, weekday, lead });
