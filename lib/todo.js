...
const SQL_ADD = `
  insert into v2.todo(kind, title, note, due_on, why, rule_id, student_id, exam_id, material_id, private)
  values ($1, $2, $3, $4::date, $5, $6::uuid, $7::uuid, $8::uuid, $9::uuid, coalesce($10::boolean, false))
  returning id, kind, title, due_on, state, why, rule_id, private`;
...
export async function addTodo(db, one = {}) {
  ...
  // ⚠️ **0줄이면 실패다**(검사-⑪). 접근 규칙이 막았는데 화면이 「저장됨」이라 말하면 안 된다
  if (!rows.length) throw new Error("addTodo: 한 줄도 안 들어갔다 (접근 규칙이 막았을 수 있다)");
  return rows[0];
}
