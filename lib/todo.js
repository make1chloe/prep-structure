...
/** 자료 걸음에서 카드로 세우는 걸음 셋. ⚠️ `solve`·`score` 는 안 세운다 — 걸음의 `score`(채점)를
 *  할 일 종류의 `score`(성적 받기)와 섞으면 **성적 받기 카드가 화면에서 사라진다**(위의 ⚠️⚠️) */
const MATERIAL_CARD_STEPS = Object.freeze(["make", "print", "hand"]);
...
export async function countedCards(db, { today, from = null, sheets = null, todos = [] } = {}) {
  ...
  for (const m of await loadMaterials(db, { on: today })) {
    const at = m.step?.at;
    if (!MATERIAL_CARD_STEPS.includes(at) || already.has(m.id)) continue;
    out.push({ id: `material:${m.id}:${at}`, kind: at, title: m.title,
               material_id: m.id, exam_id: m.exam_id ?? null,
               due_on: null, state: "todo",
...
export async function myTodos(db, { today, filter = "all", from = null, to = null,
  ...
  const classDays = await academyDays(db, addDays(a, -7), b);
