...
export function studentAmount(live, baseCount, unit, student = {}) {
  ...
  // 수업일이 기준보다 많은 달에도 정가를 넘겨 받지 않는다 (한 달 정액이므로)
  const ratio = Math.min(1, mine.length / base);
  const amount = Math.round((unit * ratio) / 10) * 10; // 10원 단위
  return {
    sessions: mine.length,
    base,
    amount,
    full: mine.length >= live.length,
    over: mine.length > base,
  };
}
