...
async function settleParts(sb, { studentId, unitId, round, note, date }) {
  ...
  changed(await db(sb).from("progress_part").upsert({ student_id: studentId, unit_id: unitId, round, ...part, note: String(note), done_on: date }, { onConflict: "student_id,unit_id,round,q_from,q_to,page_from,page_to", ignoreDuplicates: true, count: "exact" }), "조각을 못 남김", { zero: "ok" });   /* 0줄 허용 — 같은 조각 두 번(0065 progress_part_one_span)은 조용히 넘어간다 */
  ...
  if (parts.error) throw new Error(`조각을 못 읽음: ${parts.error.message}`);
