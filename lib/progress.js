...
export async function tree(sb, sheetId, bookId) {
  ...
  const [units, prog, items, mark, book, streak, memoRule, parts] = await Promise.all([
    ...
    db(sb).from("progress_part").select("unit_id,q_from,q_to,page_from,page_to").eq("student_id", sheet.student_id).eq("round", round),   // (러) 조각 — 줄 밑 「낸 것 · 남은 것」(확정-⑳)
  ]);
  ...
  return { book: book.data, round, closed: Boolean(sheet.closed_at), memo: mark.data?.class_memo ?? "", today, partsOf, ...summary, marks: Object.fromEntries((prog.data ?? []).map((p) => [p.unit_id, p])), memo_streak: Number(streak.data ?? 0), memo_rule: Number(memoRule.data?.value ?? 3) };
}
...
export async function setUnits(sb, sheetId, unitIds, status) {
  ...
  const rounds = new Map(); for (const bk of new Set(units.map((u) => u.book_id))) rounds.set(bk, await roundOf(sb, sheet.student_id, bk, sheet.date));
  const rs = await Promise.all(units.map((u) => write(sb, { studentId: sheet.student_id, unitId: u.id, round: rounds.get(u.book_id), status, by: "staff", date: sheet.date, force: true })));
...
export async function doneUpTo(sb, sheetId, bookId, unitId) {
  const sheet = await sheetRow(sb, sheetId);
  const round = await roundOf(sb, sheet.student_id, bookId, sheet.date);
  ...
  const r = await setUnits(sb, sheetId, todo, "done");
