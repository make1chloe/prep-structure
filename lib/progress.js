...
export function mergeRanges(notes = []) {
  ...
  if (rs.some((r) => r.kind === "unknown")) return { kind: "unknown", spans: [] };
  const kinds = new Set(rs.map((r) => r.kind));
  // 「p.35-40」과 「1-30번」이 같은 단원에 같이 왔다 — 어느 자로 재야 할지 모른다
  if (kinds.has("page") && kinds.has("q")) return { kind: "unknown", spans: [] };
...
export function winner(marks = []) {
  ...
  for (const m of marks) {
    ...
    if (m.mark === "done") g.notes.push(m.range ?? null);   // 범위를 안 적은 ○ 은 「통째」로 셈한다
    const better = !g.had
...
export async function checkProgress(db, input = {}, opts = {}) {
  ...
  try {
    for (const p of plan) {
      ...
      if (p.d.act === "set" && p.m.mark === "done" && p.range.spans.length) {
        // 자를 못 정했으면 **메모가 스스로 밝힌 축**으로 남긴다 (쪽으로 지어내지 않는다)
        const ax = p.d.axis ?? (p.range.kind === "q" ? "q" : "page");
        const note = (p.m.notes ?? [p.m.range]).filter(Boolean).join(" · ") || null;
        for (const [a, b] of p.range.spans) {
