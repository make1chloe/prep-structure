...
export async function editTodo(sb, todoId, { title, kind = undefined, dueOn = null, dueTime = null, studentId = null, note, startOn = null } = {}) {
  ...
  const patch = { title: t, due_on: dueOn || null, due_time: dueTime || null, student_id: studentId || null };
...
async function okKind(sb, kind) {
  ...
  const r = await db(sb).from("todo_kind").select("kind,show_in,state").eq("kind", k).maybeSingle();
  const v = r?.data; if (!v) throw new Error(`없는 업무 종류입니다: ${k}`);
  if (v.state !== "active") throw new Error("삭제한 업무 종류입니다");
  if (v.show_in === "schedule") throw new Error("일정에만 쓰는 종류입니다");
  return k;
}
export async function addTodo(sb, { title, kind = null, dueOn = null, dueTime = null, studentId = null, note = null, startOn = null }) {
  const t = String(title ?? "").trim(); if (!t) throw new Error("업무를 적으세요");
  if (dueOn && !isDate(dueOn)) throw new Error(`날짜가 아닙니다: ${dueOn}`);
  if (startOn && !isDate(startOn)) throw new Error(`시작일이 날짜가 아닙니다: ${startOn}`);
  if (startOn && dueOn && startOn > dueOn) throw new Error("시작일이 마감보다 늦습니다");
  const payload = { kind: await okKind(sb, kind), title: t, note: String(note ?? "").trim() || null, due_on: dueOn || null, due_time: dueTime || null, student_id: studentId || null };
