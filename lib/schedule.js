...
/** + 업무 · 한 줄(제목 · 날짜 · 시각 · 아이). 05 업무 화면이 서면 그쪽이 주인 */
export async function addTodo(sb, { title, dueOn, dueTime = null, studentId = null, note = null }) {
  ...
  const ins = row(await db(sb).from("todo").insert({ kind: "note", title: t, note: String(note ?? "").trim() || null, due_on: dueOn, due_time: dueTime || null, student_id: studentId || null }).select("id").single(), "업무를 못 넣음");
  return ins.id;
}
