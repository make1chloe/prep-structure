import { addDays as addDaysISO, dowOf } from "./day.js";
...
export function sessionNumbers(klass = {}, ym, offDates = new Set(), makeupDays = []) {
  ...
  const inTerm = (d) =>
    (!klass.starts_on || d >= klass.starts_on) && (!klass.ends_on || d <= klass.ends_on);
...
export function reviewClass(
   ...
   * 실제보다 많게 잡혔다.
   *
   * 날짜가 안 적혀 있으면(정규반) 아무것도 안 자른다 — 무기한이라는 뜻이다.
   */
  const inTerm = (d) => {
    if (klass.starts_on && d < klass.starts_on) return false;
    if (klass.ends_on && d > klass.ends_on) return false;
    return true;
  };
