...
/** 읽기 — 01·14 처럼 판 RPC 에 안 얹힌 화면이 제 파도(Promise.all)에 태운다(속도-1 · 층이 안 는다). 없으면 null(기본 차례) */
export async function prefOf(sb, profileId, screen) {
  if (!SCREENS[screen]) throw new Error(`화면이 아닙니다: ${screen}`);
  return row(await db(sb).from("screen_pref").select("layout").eq("profile_id", profileId).eq("screen", screen).maybeSingle(), "카드 차례를 못 읽음")?.layout ?? null;
}
