 ...
 *  키는 v2.integration 의 anthropic 줄(옛 앱 설정에서 0072 가 옮겼다) · 서버에서만 읽고 화면엔 글만 돌려준다. 코드에도 대화에도 키는 없다 */
...
/** 규칙 값 한 조회(뼈대-5) · 유형마다 길이 · 늦은밤 시작 · 다시 시키는 횟수. 지금 서울 시각도 같이(「늦은밤」이 저절로 골라지게). 줄이 없으면 던진다 */
...
const explain = (e) =>
  e instanceof Anthropic.AuthenticationError ? "AI 키가 맞지 않습니다. 연동(anthropic)의 key 를 확인하세요"
...
export async function draftComment(sb, sheetId, { kind, cap, keys } = {}, cfg) {
  ...
  if (!KIND.some(([k]) => k === kind)) throw new Error(`유형이 아닙니다: ${kind}`);
  ...
  if (sheetQ.error) throw new Error(`수업 일지를 못 읽음: ${sheetQ.error.message}`);
  const key = String(ai?.key ?? "").trim();
  if (!key) throw new Error("AI 키가 없습니다. 연동(v2.integration)의 anthropic 줄에 key 가 있어야 합니다. 옛 앱 설정에 넣은 키는 0072 가 옮겼습니다");
  ...
  const [quizzes, warns, mineQ] = await Promise.all([quizzesOf(sb, [raw.student_id], raw.date), warnStates(sb, [raw.student_id], raw.date),
    db(sb).from("day_sheet").select("date,comment").eq("student_id", raw.student_id).neq("id", sheetId).not("closed_at", "is", null).not("comment", "is", null).order("date", { ascending: false }).limit(5)]);   // 시험 + 오늘 경고·반성문 처분(2차-7 남긴 것) + (서) 이 아이에게 지난번에 보낸 글 다섯(말투 본보기 맨 앞) — 같은 층
  if (mineQ.error) throw new Error(`지난 글을 못 읽음: ${mineQ.error.message}`);
  const today = quizzes.filter((q) => q.taken_on === raw.date), next = quizzes.filter((q) => q.state === "planned" && q.taken_on == null);
  const lines = attached({ next, late: sheet.late, warn: warns[0] ?? null });
  const mine = (mineQ.data ?? []).filter((s) => String(s.comment ?? "").trim()).map((s) => `[이 아이 · ${String(s.date).slice(5).replace("-", "/")}] ${String(s.comment).trim()}`);   // (서) 아이별 지난 글 — 같은 아이의 호칭·톤
  const samples = [...mine, ...(samplesQ.data ?? []).map((s) => (s.tag ? `[${s.tag}] ${s.body}` : s.body))];
  ...
  changed(await db(sb).from("day_sheet").update(patch, { count: "exact" }).eq("id", sheetId), "초안을 못 저장함");
