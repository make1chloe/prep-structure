...
import { KIND, isOver, countChars, cutAtSentence, sameAsDraft, facts, promptFor, attached } from "./comment-plan.js";
...
export async function commentRules(sb) {
  const { data, error } = await db(sb).from("rule").select("key,value").like("key", "comment.%");
  if (error) throw new Error(`글 규칙을 못 읽음: ${error.message}`);
  const v = Object.fromEntries((data ?? []).map((r) => [r.key, r.value]));
  const caps = Object.fromEntries(KIND.map(([k]) => [k, parseInt(v[`comment.cap.${k}`], 10)]));
  for (const [k, n] of Object.entries(caps)) if (!n) throw new Error(`규칙 줄이 없다: comment.cap.${k} — 0111 의 씨앗을 본다`);
  if (!v["comment.late_from"] || !v["comment.retry"]) throw new Error("규칙 줄이 없다: comment.late_from · comment.retry — 0111 의 씨앗을 본다");
  return { caps, lateFrom: parseInt(v["comment.late_from"], 10), retry: parseInt(v["comment.retry"], 10), hour: seoulHour(new Date()) };
}
...
export async function draftComment(sb, sheetId, { kind, cap, keys } = {}, cfg) {
  ...
  const [sheetQ, ai, aiRules, samplesQ] = await Promise.all([
    db(sb).from("day_sheet").select("*,students(name,grade),day_item(*,units(label,short,book_id),learn_items(name)),late_stay(*)").eq("id", sheetId).single(),
  ...
  const sheet = { ...raw, check: items.filter((i) => i.slot === "check"), class: items.filter((i) => i.slot === "class"), home: items.filter((i) => i.slot === "home"), late: (raw.late_stay ?? [])[0] ?? null };
