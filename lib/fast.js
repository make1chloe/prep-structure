 ...
 *    커서를 한 판에 주는 일은 지금 **여기 SQL 이 `v2.cursor_of` 를 lateral 로 부르며** 한다.
 ...
 * ⚠️ 교재 고르는 조건은 `booksOf` 와 같고, 커서는 옛 길과 같이 **두 칸짜리**(오늘 기준)를 부른다.
 *    세 칸짜리(날짜 받는 것)를 부르면 지난 날짜 판에서 **답이 달라진다** — 그건 따로 정할 일이다.
 */
const Q_CURSORS = `select sb.book_id, c.round, c.chapter, c.is_workbook, c.left_in_chapter
   from v2.student_book sb
   cross join lateral v2.cursor_of($1::uuid, sb.book_id) c
