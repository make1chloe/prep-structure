/** 나이스 학사일정 판단 한 벌(순수, 목업 12b · 확정-㊲) · 주소 만들기 · 답 읽기 · 줄 → 일 · 이름 펴기 · 유형(전국은 원장님의 낱말 목록 v2.exam_word 로) · 여러 날 연결 · 회차 후보(학교 기간 · 전국 하루) · 출처 키 · (저) 받아온 것과 있던 줄 견주기(기간이 옮겨진 회차는 옛 줄에 · 0152).
...
/** 유형 · 쉬는 날이 먼저(「대수능시험 휴업일」은 시험이 아니다) › 전국(낱말 목록) › 평가(수행·성취도 · 회차 아님) › 내신 지필 › 행사 */
...
/** 학교 회차 후보 · 내신 지필 기간마다 한 줄(학년이 하나만이면 학년을 적는다). 출처 키 = 학교코드:이름:시작날 */
...
/** 전국 회차 후보 · 모의고사는 학년마다 하루씩(학년마다 시험지가 다르다) · 수능·나머지는 하루 한 줄. 학교가 아홉 곳이어도 키가 같아 한 줄 */
...
/** (저) 받아온 후보와 있던 나이스 줄 견주기(0152 · 확정-69) · 같은 회차(유형·학교·학년·이름)인데 기간이 다르면 「날짜 바뀜」. 키에 시작날이 들어 있어(examPeriods · nationalRows)
 *  시작이 옮겨지면 새 줄이 될 뻔한 것을 옛 줄에 잇는다(id 그대로 · 범위·안 봄·교재 보류가 붙어 있다). 옛 줄 하나는 한 번만 잇는다 · 그대로인 것과 새 것은 fresh(키로 upsert) */
