...
export function flagLines(flags = []) {
  return flags.map((f) => ({ id: f.id, title: `${f.chapter} › ${f.short}`, said: f.kind === "other" ? (f.said || "그 밖에") : flagKindName(f.kind), when: `${md(String(f.raised_at).slice(0, 10))}에 달았어요`, state: f.seen_at ? (f.outcome === "changed" ? "바꿨어요" : "그대로 두기로 했어요") : "기다리는 중", waiting: !f.seen_at }));
}
...
export function staffFlagLine(f) {
  ...
  return { id: f.id, title: `${f.student} · ${f.book} › ${f.chapter} › ${f.short}`, small: `「${f.kind === "other" ? (f.said || "그 밖에") : flagKindName(f.kind)}」 · ${md(String(f.raised_at).slice(0, 10))} 달았음 · 지금 ${cur}으로 되어 있음`, action, said: f.said ?? "" };
}
