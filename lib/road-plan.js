...
import { chapterSummary, TRI, coverage, partsText } from "./progress-plan.js";
import { markCh, markText } from "./mark.js";
import { projectEnd, stopOn, STOP } from "./routine-plan.js";
import { md, seoulDate } from "./dash-plan.js";
export { TRI };
export const FLAG_KIND = Object.freeze([["not_done", "이거 아직 안 했어요"], ["already_done", "이미 했어요"], ["other", "기타"]]);
...
/** 줄 하나만 봐서 — 원장·검사가 찍은 줄이 아닌가(없거나 · 내가 찍은 것 · 아직인 것). RLS 0052 ② 와 같은 판단 · 손(lib/road.js 통째 찍기)도 이것을 쓴다 */
export const rowMarkable = (row) => !row || row.last_by === "student" || row.status === "none";
/** 이 아이가 이 줄을 찍을 수 있나 — 열려 있고(can_edit) 위가 참일 때 */
export const canMark = (edit, row) => Boolean(edit?.can_edit) && rowMarkable(row);
...
export function roadOf(b) {
  ...
  const partsOf = new Map();   // (허) 조각 — 02b 와 같은 셈(progress-plan coverage·partsText 한 벌 · 확정-⑳)
  for (const pp of b.parts ?? []) { const list = partsOf.get(pp.unit_id) ?? []; list.push(pp); partsOf.set(pp.unit_id, list); }
  const sum = chapterSummary(units, prog.map((p) => ({ unit_id: p.unit_id, status: p.status })));
  const sub = (u) => { const r = rows.get(u.id) ?? null; return { id: u.id, short: u.short, activity: u.activity, is_workbook: u.is_workbook, status: r?.status ?? "none", by: r ? (r.last_by === "student" ? "내가" : r.last_by === "check" ? "검사" : "쌤") : null, own: r?.last_by === "student", pending: Boolean(r && r.confirmed === false), doneOn: r?.done_on ?? null, can: canMark(edit, r), parts: partsOf.has(u.id) ? partsText(coverage(u, partsOf.get(u.id))) : "" }; };
  const chapters = sum.chapters.map((c) => { const subs = c.units.map(sub); const pending = subs.filter((s) => s.pending).length; const finished = c.total > 0 && c.done + c.skip === c.total; const started = c.done + c.skip + c.doing > 0; return { chapter: c.chapter, total: c.total, done: c.done, skip: c.skip, doing: c.doing, subs, pending, finished, started, markable: subs.filter((x) => x.can).map((x) => x.id), now: c.chapter === sum.now }; });
  for (const c of chapters) c.status = c.finished ? "done" : c.started || c.now ? "doing" : "none";   // (어43) 대단원 상태 하나 · 08 이 말(lib/mark kid)과 색으로 그린다
...
/** 머리 꼬리표 · 「3회독」 · 「소단원씩」 · 「다 한 단원 4/18」 · 「이대로면 12월 초」 */
export function headTags(b, road) {
  ...
  return { round: `${sb.round ?? 1}회독`, basis: basis === "chapter" ? "대단원씩" : "소단원씩", finished: `다 한 단원 ${road.finished}/${road.total}`, end, sessions: pj.sessions, endDate: pj.endDate };
}
/** 내 교재들 · 「중등3800제3 · 진행중」 · 「수능딥독1 · 숙제 보류 (수업만)」 · 「능률보카 · 교재 보류 · 10.17에 풀림」 */
...
export function flagLines(flags = []) {
  return flags.map((f) => ({ id: f.id, title: `${f.chapter} › ${f.short}`, said: f.kind === "other" ? (f.said || "기타") : flagKindName(f.kind), when: `${md(seoulDate(f.raised_at))}에 달았어요`, state: f.seen_at ? (f.outcome === "changed" ? "바꿨어요" : "그대로 두기로 했어요") : "기다리는 중", waiting: !f.seen_at }));
}
/** 진도 체크 열림 띠 — 「열려 있어요」 · 「닫혀 있어요」 · 이 아이만 끔 */
export function editBand(edit = {}, student = {}) {
  if (edit.can_edit) return { open: true, title: "내가 찍을 수 있어요", small: "다 한 것은 ✓ · 잘못된 것은 ❗", pill: "열림" };   // (어43) 초5 말
  if (student.progress_edit === "off") return { open: false, title: "지금은 보기만 해요", small: "내 것만 닫혀 있어요 · 잘못된 것은 ❗", pill: "닫힘" };
  return { open: false, title: "지금은 보기만 해요", small: "지금은 보기만 · 쌤이 열어 주면 찍을 수 있어요", pill: "닫힘" };
}
...
export function staffFlagLine(f) {
  const cur = `${markText(f.status ?? "none")} ${markCh(f.status ?? "none")}`;   // (어43) 말·부호는 lib/mark.js 한 곳
  const action = f.kind === "not_done" ? { status: "none", label: "아직 안 함으로" } : f.kind === "already_done" ? { status: "done", label: "완료로" } : null;
  return { id: f.id, title: `${f.student} · ${f.book} › ${f.chapter} › ${f.short}`, small: `「${f.kind === "other" ? (f.said || "기타") : flagKindName(f.kind)}」 · ${md(seoulDate(f.raised_at))} 달았음 · 지금 ${cur}으로 되어 있음`, action, said: f.said ?? "" };
}
