...
/** ⑥ 다음 영상 — 지금 것 뒤로 안 본 것(차례대로) · 없으면 앞에서 · 다 봤으면 null. ④ 「N번 열어봄」 글 */
/** 폴더로 묶기(4단계-6) — 폴더 이름 차례 · 폴더 없는 것은 「폴더 없음」으로 맨 뒤 · 안에서는 제목 차례 */
export function groupByFolder(videos = []) {
  const m = new Map(); for (const v of videos) { const k = String(v.folder ?? "").trim(); if (!m.has(k)) m.set(k, []); m.get(k).push(v); }
  return [...m.entries()].sort((a, b) => (a[0] === "" ? 1 : b[0] === "" ? -1 : a[0].localeCompare(b[0], "ko"))).map(([folder, list]) => ({ folder: folder || "폴더 없음", empty: !folder, videos: [...list].sort((a, b) => String(a.title ?? "").localeCompare(String(b.title ?? ""), "ko")) }));
}
export const folders = (videos = []) => [...new Set(videos.map((v) => String(v.folder ?? "").trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b, "ko"));
export function nextOf(rows = [], currentId = null) {
  const i = (rows ?? []).findIndex((r) => r.video_id === currentId);
  const after = (rows ?? []).slice(i + 1).find((r) => r.status.key !== "done"), before = (rows ?? []).slice(0, Math.max(0, i)).find((r) => r.status.key !== "done");
  return after ?? before ?? null;
}
export const opensText = (n) => (Number(n) > 0 ? `${Number(n)}번 열어봄` : "");
