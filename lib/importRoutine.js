...
export const RT_HEADERS = ["교재명", "순서", "단계이름", "등원(·로 구분)", "숙제(·로 구분)", "회독(빈칸=전부)", "예습숙제-다음단원(·로 구분)"];
...
const splitNames = (v) =>
  String(v ?? "")
    ...
    .map((x) => x.trim())
    .filter(Boolean)
    // 항목 이름 뒤 대괄호 = 그 항목의 주의사항 (0139) —
    //   클카 스크램블[6000점 이상] → 이름과 메모로 가른다.
    //   괄호()는 항목 이름에 쓰인다(단어(온라인)) — 대괄호만 메모다.
    .map((x) => {
      const m = x.match(/^(.*?)\s*\[([^\]]+)\]\s*$/);
      return m ? { name: m[1].trim(), note: m[2].trim() } : { name: x, note: "" };
    });
...
export function parseRoutineAoA(aoa = []) {
  ...
  body.forEach((r, i) => {
    ...
    lastBook = book;
    if (inclass.length === 0 && home.length === 0 && splitNames(r?.[6]).length === 0) {
    ...
    // 예습 숙제 (0136) — 다음 단원이 붙는 선행 숙제
    const homeNext = splitNames(r?.[6]);
    rows.push({ book, sort, label, inclass, home, round, homeNext });
  });
