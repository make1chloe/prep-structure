...
export const HW_HEADERS = ["이름", "분류", "순서", "준비물", "어디서(집/등원)"];

const KIND = { "집": "home", "등원": "inclass", "학원": "inclass" };
...
export function parseHomeworkAoA(aoa = []) {
  ...
  body.forEach((r, i) => {
    ...
    const whereRaw = String(r?.[4] ?? "").trim();
    const kind = whereRaw ? KIND[whereRaw] : "home";
    if (whereRaw && !kind) {
      problems.push(`${line}줄 — 어디서는 「집」 또는 「등원」 으로 적어주세요 (지금: ${whereRaw})`);
      return;
    }
    rows.push({ name, category, sort, tool, kind });
  });
