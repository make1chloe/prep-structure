/** 신규 상담 18 의 손 · 판 읽기(한 벌 inquiry_board) · + 전화 문의 · 수정(방문 · 레벨 · 점수 · 제안 · 사유) · 안내 보냄(답한 때) · 단계 · 등록 전환 일곱(학생 등록 · 계정 둘 · 반 · 교재 · 루틴(저절로) · 결제선생 업무 · 첫 등원 안내).
...
/** + 전화 문의 받기 · 전화 끊고 바로. 이름 · 학부모 전화 · 학교 · 학년 · 유형 · 물음 */
...
/** 수정 — 방문 · 레벨 때 · 점수 · 제안 · 사유 · 물음. 방문을 잡으면 「상담 잡힘」, 점수를 적으면 「레벨 봄」으로 단계가 따라간다(원장님이 따로 안 옮겨도) */
...
export async function setInquiry(sb, id, f = {}, seenAt = null) {
  ...
  const r = row(await q.select("id"), "문의를 수정 못 함"); if (!r?.length) throw new Error(seenAt ? STALE : "고쳐진 줄이 없습니다");
...
export async function answerInquiry(sb, id) {
  ...
  if (await smsReady(svc) && phoneDigits(q.phone)) {
    ...
    if (f.missing.length) throw new Error(`문구(sms_guide)의 치환 칸이 비었습니다: ${f.missing.join(", ")}`);
...
export async function convertInquiry(sb, id, f, { date, by = null, schools = [] } = {}) {
  ...
  const { id: studentId } = await addStudent(sb, { name: q.name, grade: q.grade, schoolId: school?.id ?? null, phone: q.student_phone, parentPhone: q.phone, memo: [q.way ? `유입: ${q.way}` : null, q.body ? `물음: ${q.body}` : null, q.level_note ? `레벨: ${q.level_note}` : null, q.suggest ? `제안: ${q.suggest}` : null].filter(Boolean).join("\n") || null, joinedOn: p.joinedOn ?? date }, date);
  ...
  if (!steps.some((s) => s.key === "books")) steps.push({ key: "books", ok: true, text: books ? `교재 ${books}권 · 기준·회차는 루틴 11 에서` : "교재는 나중에(루틴 11 에서 배정)" });
  steps.push({ key: "routine", ok: true, text: "영역 루틴이 저절로 · 짤 것이 없습니다" });
  try { await addTodo(sb, { title: `결제선생 등록 · ${q.name}`, dueOn: p.joinedOn ?? date, studentIds: studentId ? [studentId] : [] }); steps.push({ key: "fee", ok: true, text: "업무에 카드로 섰습니다" }); } catch (e) { steps.push({ key: "fee", ok: false, text: String(e.message) }); }
  try {   // (커) 첫 등원 안내 = 문자(솔라피 · 확정-71) — 앱 주소·아이디·처음 비밀번호·수업·교재·규정은 문구(msg_template sms_welcome · 원장님이 10 에서 고친다) · {{덧붙임}}은 이 아이만
    if (await smsReady(svc) && acc && par && phoneDigits(q.phone)) {
      ...
      if (f.missing.length) throw new Error(`문구(첫 등원 안내)의 치환 칸이 비었습니다: ${f.missing.join(", ")} · 발송 10 「✉️ 문자 문구」에서 고치거나 값을 채워 주세요`);
      ...
      steps.push({ key: "welcome", ok: r.failed === 0, text: r.sent ? `문자 보냄 ${r.to} · ${lenText(f.text).text}` : r.sink === "off" ? `문자 · 리허설(NOTIFY_SINK=off)이라 발송 이력만 ${r.to}` : `문자 못 보냄 · ${r.why}` });
    } else {
  ...
  changed(await db(sb).from("inquiry").update({ stage: "joined", student_id: studentId }).eq("id", id).select("id"), "문의를 수정 못 함");
  return { studentId, steps, done: steps.filter((s) => s.ok).length };
}
/** 고른 문의 여럿을 한 번에((어28)-④ · 대전제-20) · 한 장의 손(단계 · 📨 안내)을 차례로 · 막히면 그 장부터 멈춘다 */
export async function stageMany(sb, ids = [], stage, why = null) { const list = [...new Set((ids ?? []).filter(Boolean))]; if (!list.length) throw new Error("고른 문의이 없습니다"); let n = 0; for (const id of list) { await setStage(sb, id, stage, why); n++; } return { n }; }
export async function answerMany(sb, ids = []) { const list = [...new Set((ids ?? []).filter(Boolean))]; if (!list.length) throw new Error("고른 문의이 없습니다"); let n = 0, sms = 0; for (const id of list) { const r = await answerInquiry(sb, id); n++; if (r.sms?.sent) sms++; } return { n, sms }; }
