...
export async function setInquiry(sb, id, f = {}) {
  const patch = {};
  ...
  row(await db(sb).from("inquiry").update(patch).eq("id", id).select("id"), "문의를 못 고침");
  return { stage: patch.stage ?? cur.stage };
}
/** 📨 안내 보냄 — 답한 때를 찍는다(문자는 아직 — 안내 글은 화면이 복사한다) */
export async function answerInquiry(sb, id) { row(await db(sb).from("inquiry").update({ answered_at: new Date().toISOString() }).eq("id", id).select("id"), "답한 때를 못 적음"); }
export async function setStage(sb, id, stage, why = null) {
  ...
  row(await db(sb).from("inquiry").update(patch).eq("id", id).select("id"), "단계를 못 바꿈");
}
...
export async function convertInquiry(sb, id, f, { date, by = null, schools = [] } = {}) {
  ...
  row(await db(sb).from("inquiry").update({ stage: "joined", student_id: studentId }).eq("id", id).select("id"), "문의를 못 고침");
