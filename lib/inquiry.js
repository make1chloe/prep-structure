...
export async function convertInquiry(sb, id, f, { date, by = null, schools = [] } = {}) {
  ...
  try { await addTodo(sb, { title: `결제선생 등록 · ${q.name}`, dueOn: p.joinedOn ?? date, studentId }); steps.push({ key: "fee", ok: true, text: "업무에 카드로 섰습니다" }); } catch (e) { steps.push({ key: "fee", ok: false, text: String(e.message) }); }
