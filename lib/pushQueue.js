 ...
 * 담는다 — 다음 :00/:30 에 나가고, 그 전엔 발송 「보낼 것」 예약 목록에서
 * 보이고 취소할 수 있다. (「같은 집 알림 한 통 합치기」는 만든 적이 없고,
 * 원장님 확정(2026-08-26)으로 필요 없음이 정해졌다 — 학부모는 학생별로
 * 한 화면에서 보면 되고, 알림을 합쳐 보낼 필요는 없다.)
 */

/**
 * 다음 발송 시각 — 30분 단위 :00/:30 (원장님 2026-08-21 「30분 단위로
 * 해줘」). 확정 직후 최소 1분의 무르기 여유는 보장한다.
 */
function nextHourSeoul(now = new Date()) {
  const d = new Date(now.getTime());
  d.setMinutes(d.getMinutes() < 30 ? 30 : 60, 0, 0);
  if (d.getTime() - now.getTime() < 60 * 1000) d.setMinutes(d.getMinutes() + 30);
