...
export async function loadReportRows(supabase, date, academy = "클로이영어", msg = {}) {
  ...
  let { data: reports, error: repErr } = await supabase
    .from("daily_reports")
    .select(`${BASE}, sent_at, report_text, homework_text, homework_sent_at, late_until, late_reason, late_text, late_sent_at`)
    .eq("date", date);
  ...
  // 일괄 공지 — 대상이 다르면 나가는 문자도 다르다
  //   kind='notice'  학부모용 → 데일리리포트에
  //   kind='deliver' 학생용   → 숙제 문자에
  const { data: noticeRows } = await supabase
    .from("notices")
    .select("id, kind, body")
    .eq("date", date)
    .in("kind", ["notice", "deliver"]);
  ...
  (receipts || []).forEach((r) => {
    ...
    if (!n?.body) return;
    const bucket = n.kind === "deliver" ? studentNoticesOf : noticesOf;
  ...
  const rows = reports
    .map((rep) => {
      ...
      return {
        ...
        lateText: rep.late_text || buildLateText(data, date, academy, msg, rule),
        lateAuto: buildLateText(data, date, academy, msg, rule),
      };
    })
    .filter(Boolean)
