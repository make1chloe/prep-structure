...
export async function loadReportRows(supabase, date, academy = "클로이영어") {
  const BASE =
  ...
  const rows = reports
    .map((rep) => {
      ...
      return {
        ...
        text: rep.report_text || buildReportText(data, date, academy),
        auto: buildReportText(data, date, academy),
        hwEdited: !!rep.homework_text,
        hwSentAt: rep.homework_sent_at || null,
        hwText: rep.homework_text || buildHomeworkText(data, date, academy),
        hwAuto: buildHomeworkText(data, date, academy),
