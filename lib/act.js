 ...
 *
 *  ⚠️ **삼키지 않는다.** 실패는 화면에 우리 말로 뜨고(msg), 서버 기록에는 까닭이 통째로 남는다.
...
const trace = (where, e) => { try { console.error(`[손 실패] ${where || "?"}:`, e instanceof Error ? e.stack ?? e.message : e); } catch {} };
...
export async function wrap(fn, where = "") {
  ...
  catch (e) { trace(where, e); return { ok: false, msg: String(e?.message ?? e) }; }
}
...
export const done = (path, where = "") => (fn) => async (...a) => {
  ...
  catch (e) { trace(where || path, e); return { ok: false, msg: String(e?.message ?? e) }; }
};
