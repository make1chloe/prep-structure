...
/** (어42) 항목 나무 · 영역 › 📕 교재 › 대단원 › 단원 › 활동(원장님 9/15 「영역을 봐야 책을 보고 책을 봐야 단원을 보고 단원을 펼쳐봐야 항목검사를 할 거 아냐」) · unitTree(교재·대단원 › 단원 › 활동) 위에 영역·교재 마디를 얹는다 · 교재 없는 줄은 「그 밖에」 마디(맨 끝) · 01 검사·학습·숙제 · 07 · 09 가 같은 나무(app/_shell/tree.js) */
 ...
 *  open: 아직 안 본 줄 + 이 자리에서 다시 연 줄(opened · id) · done: 검사한 줄을 교재별로(itemForest 차례 · 칩 한 줄) · stopped: 교재 보류(stopped · book_id) 교재의 줄 전부(맨 밑 · 접힌 채 · open·done 에 안 든다) · 원본은 안 건드린다 */
export function splitChecks(rows = [], { stopped = new Set(), opened = new Set() } = {}) {
  const isStopped = (r) => stopped.has(r.units?.book_id);
  const live = rows.filter((r) => !isStopped(r));
  const open = live.filter((r) => isUnchecked(r) || opened.has(r.id));
  const doneRows = live.filter((r) => !isUnchecked(r) && !opened.has(r.id));
  const done = itemForest(doneRows, rows).map((b) => ({ key: b.key, book: b.book, area: b.area, rows: b.chapters.flatMap((ch) => ch.units.flatMap((g) => g.rows)) }));
  return { open, done, stopped: rows.filter(isStopped) };
}
