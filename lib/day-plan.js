...
export function workText(sheet, books = [], date) { const stopped = books.filter((b) => stopOn(b, date) === "book_off").length; return [`학원 ${sheet?.class?.length ?? 0} · 숙제 ${sheet?.home?.length ?? 0}`, stopped ? `멈춤 ${stopped}` : null].filter(Boolean).join(" · "); }
...
export const ATTEND = Object.freeze([["present", "왔음"], ["late", "지각"], ["absent", "결석"], ["early", "조퇴"], ["online", "온라인"]]);
/** 검사 줄 차례((어30) · 원장님 2026-09-15 「숙제검사화면에서, 교재별로 묶어서, 루틴순서대로 나열할 것. 확인하기 편하게」).
 *  교재별로 묶고(교재 이름 차례 · 교재 없는 줄은 맨 끝) · 묶음 안은 루틴 차례(그 판에서 그 교재의 숙제 줄에 항목이 처음 나온 차례 = layRoutine 이 깐 줄 차례 · 손으로 낸 줄은 그 교재 끝) · 같은 줄이면 소단원 차례 → 오래된 날짜부터.
 *  all: 차례를 셀 때 보는 줄 전부(이미 검사한 줄도 · 첫 줄이 검사됐어도 차례가 안 흔들린다). 원장 검사 줄(lib/day.js notYetChecked · shape)과 아이 화면 07 이 같은 것을 쓴다(원칙-1) */
export function checkOrder(rows = [], all = rows) {
  ...
  const book = (r) => r.units?.books?.name ?? null;
  return [...rows].sort((a, b) => { const ba = book(a), bb = book(b); if ((ba == null) !== (bb == null)) return ba == null ? 1 : -1;
    return String(ba ?? "").localeCompare(String(bb ?? ""), "ko") || lineRank(a) - lineRank(b) || Number(a.units?.sort ?? 0) - Number(b.units?.sort ?? 0) || String(a.day_sheet?.date ?? "").localeCompare(String(b.day_sheet?.date ?? "")) || Number(a.sort ?? 0) - Number(b.sort ?? 0); });
}
/** 검사 카드가 그리는 묶음 [{ book: 교재 이름 | null, rows }] · checkOrder 로 선 줄을 교재가 바뀌는 자리에서 자른다 */
export function bookGroups(rows = []) { const out = []; for (const r of rows) { const b = r.units?.books?.name ?? null; const last = out[out.length - 1]; if (last && last.book === b) last.rows.push(r); else out.push({ book: b, rows: [r] }); } return out; }
