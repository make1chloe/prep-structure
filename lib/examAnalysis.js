...
export function analyze(questions = [], scores = [], items = [], students = []) {
  ...
  /**
   * **배점 셈은 전부 적혔을 때만** (원장님 확정 설계, 2026-08-21 코드로).
   * 전에는 「배점이 하나라도 적힌 묶음」 만 배점 비율을 써서, 반만 적으면
   * 배점 셈과 문항 수 셈이 한 표에 섞였다 — 그 비교는 거짓말이다.
   * 빈 문항 수(pointsMissing)를 같이 돌려줘 화면이 말해주게 한다.
   */
  const pointsMissing = qs.filter((q) => !(Number(q.points) > 0)).length;
  const allPointed = qs.length > 0 && pointsMissing === 0;
  const share = (m) =>
    [...m.entries()]
      .map(([key, list]) => {
        ...
        return {
          ...
          count: list.length,
          points: pts || null,
          pct: allPointed ? pctOf(pts, totalPoints) : pctOf(list.length, qs.length),
          byPoints: allPointed,
          nos: list.map((q) => q.no),
        };
      })
  ...
  return {
    ...
    hasSpec: qs.length > 0,
    // 배점이 일부만 적혔으면 배점 셈을 안 한다 — 몇 문항이 비었는지 알려준다
    pointsMissing: pointsMissing > 0 && pointsMissing < qs.length ? pointsMissing : 0,
  ...
  if (a.shared.length > 0) {
    ...
    out.push({
      ...
      body:
        ...
        `${s.unit ? ` (${s.unit})` : ""}` +
        // **유형이 번호와 같으면 안 붙인다.** 「13번을 틀렸습니다 (7과) · 13번」
        // 처럼 같은 말이 두 번 나온다 (한 달 살아보기에서 나온 것)
        `${s.detail && s.detail !== `${s.no}번` ? ` · ${s.detail}` : ""}. ` +
