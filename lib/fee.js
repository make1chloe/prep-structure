...
import { rowsOf, ruleChanges, parseWon } from "./fee-plan.js";
...
export async function savePayments(sb, ym, edits = [], board) {
  ...
  for (const e of edits) {
    ...
    row(await db(sb).from("payment").upsert({ student_id: st.id, ym, amount, paid_on: paidOn, source: "manual" }, { onConflict: "student_id,ym" }).select("id"), "수납을 못 적음"); saved++;
