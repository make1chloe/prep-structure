...
export async function savePayments(sb, ym, edits = [], board) {
  ...
  for (const e of edits) {
    ...
    row(await db(sb).from("payment").upsert({ student_id: st.id, ym, amount, paid_on: paidOn, source: "manual", ...(method === undefined ? {} : { method }) }, { onConflict: "student_id,ym" }).select("id"), "수납을 못 적음"); saved++;
    const ch = ruleChanges(st, board.rules ?? [], board.by_grade ?? {}, ym, amount);
    if (ch.close) row(await db(sb).from("fee_rule").update({ to_date: ch.close.to_date }).eq("id", ch.close.id).select("id"), "단가 줄을 못 닫음");
    if (ch.update) row(await db(sb).from("fee_rule").update({ amount: ch.update.amount }).eq("id", ch.update.id).select("id"), "단가 줄을 못 고침");
...
export async function importPayments(sb, ym, parsed = [], board) {
  ...
  for (const r of parsed) {
    ...
    row(await db(sb).from("payment").upsert({ student_id: hits[0].id, ym, amount: r.amount, paid_on: r.paidOn, method: r.method, source: "excel" }, { onConflict: "student_id,ym" }).select("id"), "수납을 못 적음"); put++;
  }
...
export async function setByGrade(sb, f) {
  ...
  const { error } = await db(sb).from("integration").upsert({ id: "tuition", config: { ...(cur?.config ?? {}), byGrade } }, { onConflict: "id" });
  if (error) throw new Error(`학년별 기준을 못 적음: ${error.message}`);
