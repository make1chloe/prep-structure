...
import { feeLine } from "./sms-plan.js";   // (뎌-2) 문자 「한 줄」
...
export async function feeBoard(sb, ym) {
  ...
  if (error) throw new Error(`수납 화면을 못 읽음: ${error.message}`);
...
export async function savePayments(sb, ym, edits = [], board) {
  ...
  for (const e of edits) {
    ...
    if (ch.update) changed(await db(sb).from("fee_rule").update({ amount: ch.update.amount }).eq("id", ch.update.id).select("id"), "단가 줄을 수정 못 함");
...
/** 💰 수납 안내(13 · 4단계-2a) · 이 달 수납 줄이 있고(원장님이 금액을 적어 저장한 것) 아직 안 받은 집의 학부모에게. 적지 않은 금액(단가·학년 기준으로 세어 나온 것)은 학부모 화면에 없어 안 보낸다 · 알림 길 하나 · 발송 이력 why 는 수납 줄 */
export async function remindFees(svc, sb, ym, board, ids = null) {   // ids: 고른 집만((어28) · 대전제-20) · null 이면 안 받은 집 전부
  const want = ids ? new Set(ids) : null;
  const rows = rowsOf(board, ym).filter((r) => r.state === "unpaid" && r.payment_id && (!want || want.has(r.student_id)));
  let sent = 0, failed = 0, sink = null;
  for (const r of rows) { const x = await notify(svc, { kind: "fee", studentId: r.student_id, url: "/parent#fee", tag: `fee-${ym}-${r.student_id}`, smsLine: feeLine(ym, r.amount), why: { table: "payment", id: r.payment_id } }); sink = x.sink; sent += x.sent; failed += x.failed; }
  return { n: rows.length, sent, failed, sink };
}

/** (어) 예약 때가 되어 크론이 보낸다 — 그 달 수납 줄이 있고(금액 적힘) 아직 안 받았으면 안내 · 받았거나 줄이 없으면 건너뛴다(그때 상태로) */
export async function remindFeeDue(svc, studentId, ym) {
  const q = await db(svc).from("payment").select("id,amount,paid_on").eq("student_id", studentId).eq("ym", ym).maybeSingle();
  if (q.error) throw new Error(`수납 줄을 못 읽음: ${q.error.message}`);
  if (!q.data || q.data.amount == null) return { skipped: "금액 없음", sent: 0, failed: 0, sink: null };
  if (q.data.paid_on) return { skipped: "이미 받음", sent: 0, failed: 0, sink: null };
  return notify(svc, { kind: "fee", studentId, url: "/parent#fee", tag: `fee-${ym}-${studentId}`, smsLine: feeLine(ym, q.data.amount), why: { table: "payment", id: q.data.id } });
}
