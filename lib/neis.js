/** 나이스 받아오기의 손 — 열쇠(연동 neis, 원장만 읽힌다) · 학교 찾기 · 코드 붙이기 · 받아서 회차로(덮지 않는다: 출처+열쇠로 upsert, 영어 시험일은 안 건드린다 · (저) 기간이 옮겨진 회차는 옛 줄에 잇고 이전 기간을 남긴다 — 0152 · 확정-69). 판단은 lib/neis-plan.js(순수) */
...
export async function importExams(sb, today, fetchImpl = fetch) {
  ...
  if (diff.fresh.length) {
    ...
    const r = changed(await db(sb).from("exams").upsert(rows, { onConflict: "source,source_key" }).select("id"), "회차를 못 넣음");
    put = (r ?? []).length;
  }
  const now = new Date().toISOString();
  for (const c of diff.changed) changed(await db(sb).from("exams").update({ source_key: c.source_key, term_from: c.term_from, term_to: c.term_to, prev_term_from: c.prev_term_from, prev_term_to: c.prev_term_to, changed_at: now, changed_seen_at: null }).eq("id", c.id).select("id"), "옮겨진 회차를 못 적음");   // 옛 줄 그대로(범위·안 봄·멈춤이 붙어 있다) · 영어일은 안 건드린다 · 「봤음」은 다시 비운다
