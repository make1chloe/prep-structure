...
/**
 * **시험 이름의 갈래는 lib/examKind 한 곳에서만 정한다** (2026-08-09 전면
 * 재검토 — 원장님: 「단편적으로 반영하다 보니 예외 규칙이 너무 많아진 것
 * 같아. 일정 관련 코드를 전면 재검토해서 바로잡아」).
 *
 * 예전에는 이 파일이 제 정규식(EXAM · NOT_EXAM · NATIONWIDE)을 따로 들고
 * 있었고, lib/examList.js 도 저만의 정규식을 들고 있었다. 전수 대조를
 * 돌려보니 「6월 모의평가」 를 한쪽은 전국, 한쪽은 내신이라고 했다 —
 * **두 벌은 반드시 어긋난다.** 이제 여기서는 갈래를 가져다 쓰기만 한다.
 */
const OFF = /(방학|휴업|휴교|휴일|재량|공휴|개교기념)/;
...
export function isNationwide(eventName = "") {
  const k = classifyExam(eventName || "");
  // 모의고사·대수능은 교육청·평가원이 정하는 날 — 전국이 같은 날이다.
  // 공휴일도 학교가 정하지 않는다. 내신(school)·수행평가류(assess)는 학교 것이다.
  return k === "mock" || k === "suneung" || HOLIDAY.test(eventName || "");
}
...
export function examName(eventName = "", date = "") {
  const raw = (eventName || "").trim();
  if (!raw) return raw;
  // 내신 지필만 손본다 — 모의고사·수능·수행평가는 여기 것이 아니다.
  // 「2차시험」 처럼 학교마다 다른 표기가 school 로 들어오는 것은
  // lib/examKind 의 넓은 낱말(고사|시험|평가|지필)이 책임진다 (2026-08-08 옥련여고)
  if (classifyExam(raw) !== "school") return raw;
...
export function kindOf(eventName = "", sbtr = "") {
  const n = eventName || "";
  // **쉬는 날이 먼저다** — 「대수능시험 휴업일」 은 이름에 「시험」 이 있어도
  // 아이들이 쉬는 날이지 시험 보는 날이 아니다 (2026-08-09 에 겪었다)
  if (/휴업/.test(sbtr) || OFF.test(n)) return "off";
  // 「exam」 은 **내신 지필 기간**이라는 뜻이다 — 이 기간에만 정규수업이
  // 흔들리고 시험 회차가 만들어진다. 모의고사·수능은 하루짜리 전국 일정이라
  // event 로 남고, 회차는 mockPeriods 가 따로 만든다
  if (classifyExam(n) === "school") return "exam";
  return "event";
}
 ...
 * 정해두면 그런 해에 어긋난다.
 */
function mockName(date = "", level = "고", grade = 1) {
 ...
 * **저장은 한 줄로, 보기는 날마다.**
 *
 * @param keyOf 무엇이 같으면 「같은 일정」 인가. 기본은 제목이다 —
 *   받아오기는 학교 이름이 제목에 이미 들어 있어서 그것으로 충분하다.
 *   「나이스 원본」 화면은 학교와 원래 이름을 따로 들고 있어서 열쇠를 준다.
 *   **규칙은 한 벌이어야 한다** — 두 벌이면 화면과 실제 받아오기가 다르게
 *   이어붙여서, 진단 도구가 거짓말을 하게 된다.
 */
export function mergeRuns(tasks = [], keyOf = (t) => t.title || "") {
  const sorted = [...tasks].sort(
    (a, b) => keyOf(a).localeCompare(keyOf(b), "ko") || a.due_on.localeCompare(b.due_on)
  );
  const out = [];
  sorted.forEach((t) => {
    const last = out[out.length - 1];
    if (last && keyOf(last) === keyOf(t) && bridged(last.end_on || last.due_on, t.due_on)) {
  ...
  return out
    ...
    .sort((a, b) => a.due_on.localeCompare(b.due_on) || keyOf(a).localeCompare(keyOf(b), "ko"));
}
...
/**
 * **나이스가 준 줄 하나를, 우리가 어떻게 보는가** (원장님, 2026-08-09 —
 * 「순수하게 나이스에 입력된 일정을 전수 볼 수 있게 해줘. 지금 오류가 난
 * 건지 입력이 안 된 건지 알 수가 없네」).
 *
 * 「나이스 원본」 화면이 한 줄에 나란히 적어주는 값들이다. 판단은 이미 위에
 * 다 있다 (isNoise · examName · kindOf · isNationwide) — 여기서는 **그
 * 판단들이 이 줄에 대해 뭐라고 하는지를 모아 놓기만** 한다. 화면이 제
 * 나름대로 다시 재면 언젠가 실제 받아오기와 다른 말을 하게 되고, 그러면
 * 이 화면은 진단 도구로서 쓸모가 없어진다.
 *
 *   버림   토요휴업일처럼 알려주는 것이 없어 안 받는 줄
 *   전국   수능 · 모의고사 · 공휴일 (학교에 안 매달고 한 줄로 넣는다)
 *   시험   내신 지필 기간 (시험 회차가 만들어진다)
 *   쉼     방학 · 재량휴업일
 *   행사   나머지 (달력에만 들어간다)
 */
export function explainRow(row = {}, school = {}) {
  const date = toDate(row.AA_YMD);
  const raw = (row.EVENT_NM || "").trim();
  const sbtr = (row.SBTR_DD_SC_NM || "").trim();
  const event = examName(raw, date || "");
  const noise = isNoise(raw);
  const kind = noise ? "" : kindOf(event, sbtr);
  const nationwide = noise ? false : isNationwide(event);
  const how = noise ? "버림"
    : nationwide ? "전국"
    : kind === "exam" ? "시험"
    : kind === "off" ? "쉼"
  ...
  return {
    date,
    raw,
    // 우리가 이름을 폈으면 편 것도 같이 (같으면 null — 같은 말을 두 번 안 적는다)
    event: event !== raw ? event : null,
    sbtr: sbtr || null,
    grades: gradesOf(row),
    note: (row.EVENT_CNTNT || "").trim() || null,
    how,
    isExam: how === "시험",
    // 전국 줄은 학교 열쇠가 아니다 (학교코드가 안 들어간다)
    sourceId: nationwide || noise ? null : `${school.schul_code}:${row.AA_YMD}:${event}`,
  };
}
