/** 나이스 가져오기의 손 · 키(연동 neis, 원장만 읽힌다) · 학교 찾기 · 코드 붙이기 · 받아서 회차로(덮지 않는다: 출처+키로 upsert, 영어 시험일은 안 건드린다 · (저) 기간이 옮겨진 회차는 옛 줄에 잇고 이전 기간을 남긴다 · 0152 · 확정-69). 판단은 lib/neis-plan.js(순수) */
import { db } from "./supabase.js";
import { schoolUrl, scheduleUrl, readNeis, whyFailed, toSchool, parseCode, makeCode, schoolYear, planImport, diffExams } from "./neis-plan.js";
...
export async function searchSchools(sb, name, fetchImpl = fetch) {
  ...
  const key = await neisKey(sb); if (!key) throw new Error("나이스 키가 없습니다. 연동(v2.integration) neis 줄의 key");
...
export async function setSchoolCode(sb, schoolId, atpt, schul) {
  ...
  const r = changed(await db(sb).from("schools").update({ neis_code: code }).eq("id", schoolId).select("id"), "코드를 못 붙임");
  if (!r?.length) throw new Error("고쳐진 줄이 없습니다(검사-⑪)");
}
/** 🔄 다시 받기 · 코드 있는 학교 전부, 이 학년도. 넣는 것은 회차(학교 기간 · 전국 하루)뿐 · 출처+키로 upsert(같은 줄이 하나) · 영어 시험일·손으로 넣은 줄은 안 건드린다 */
export async function importExams(sb, today, fetchImpl = fetch) {
  const key = await neisKey(sb); if (!key) throw new Error("나이스 키가 없습니다. 연동(v2.integration) neis 줄의 key 를 넣어 주세요(0072 가 옛 앱 설정에서 옮겼습니다)");
  const { from, to } = schoolYear(today);
  const [schools, words, had] = await Promise.all([row(await db(sb).from("schools").select("id,name,level,neis_code").eq("state", "active"), "학교를 못 읽음"), row(await db(sb).from("exam_word").select("word,scope"), "낱말을 못 읽음"), row(await db(sb).from("exams").select("id,scope,school_id,grade,name,term_from,term_to,english_on,source_key,state").eq("source", "neis").gte("term_from", from).lte("term_from", to), "있던 회차를 못 읽음")]);   // had: 이 학년도의 나이스 줄 — 기간이 옮겨졌나 견준다((저) diffExams)
  ...
  const diff = diffExams(had ?? [], plan.exams);   // (저) 같은 회차의 기간이 옮겨졌나 — 옛 줄에 잇고 이전 기간을 남긴다(0152 · 확정-69)
  let put = 0;
  if (diff.fresh.length) {
    const rows = diff.fresh.map((e) => ({ scope: e.scope, school_id: e.school_id, grade: e.grade, name: e.name, term_from: e.term_from, term_to: e.term_to, source: "neis", source_key: e.source_key, state: "active", ...(e.english_on ? { english_on: e.english_on } : {}) }));
    const r = changed(await db(sb).from("exams").upsert(rows, { onConflict: "source,source_key" }).select("id"), "시험을 못 넣음");
    put = (r ?? []).length;
  }
  const now = new Date().toISOString();
  for (const c of diff.changed) changed(await db(sb).from("exams").update({ source_key: c.source_key, term_from: c.term_from, term_to: c.term_to, prev_term_from: c.prev_term_from, prev_term_to: c.prev_term_to, changed_at: now, changed_seen_at: null }).eq("id", c.id).select("id"), "옮겨진 회차를 못 적음");   // 옛 줄 그대로(범위·안 봄·보류가 붙어 있다) · 영어 시험일은 안 건드린다 · 「봤음」은 다시 비운다
  await syncAllStops(sb, today);   // 기간이 바뀌었을 수 있다 · 영어 시험일 있는 회차의 교재 보류 창을 다시 맞춘다(06b)
  return { schools: bySchool.length, put, changed: diff.changed, skipped: plan.skipped, failed, notes, from, to };
}
