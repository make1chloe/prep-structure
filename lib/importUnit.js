...
/**
 * **엑셀이 문항범위를 날짜로 바꿔놨나** (2026-08-06).
 *
 * 「1-25」 를 엑셀에서 열면 **1월 25일**로 알아듣고 고쳐 쓴다. 그러면 파일에는
 * `2026-01-25` 나 `45678`(날짜 일련번호)이 들어가고, 화면에는 그것이 그대로
 * 보인다 — **오류는 안 난다.** 그냥 그 단원의 분량이 영영 틀린 채로 남는다.
 *
 * AI 가 만들어준 표를 원장님이 엑셀에서 한 번 열어보고 저장하시면 바로 이렇게
 * 된다. 그래서 **미리보기에서 짚어드린다** — 고치는 것은 사람이 해야 한다
 * (「1-25」 였는지 「1월 25일」 이었는지는 우리가 알 수 없다).
 */
export function rangeMangled(v) {
  const s = (v ?? "").toString().trim();
  if (!s) return false;
  // 2026-01-25 · 2026/1/25 · 1/25/2026 처럼 연도가 끼어든 것
  if (/\d{4}[-./]\d{1,2}[-./]\d{1,2}/.test(s)) return true;
  if (/\d{1,2}[-./]\d{1,2}[-./]\d{4}/.test(s)) return true;
  // 날짜 일련번호 — 문항 번호가 만 단위일 수는 없다
