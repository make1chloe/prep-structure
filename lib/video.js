/** 영상 19 의 손 · 판 읽기(한 벌 video_board) · + 영상 · 수정·삭제 · 배정(영상 × 아이 + 마감) · 마감 미루기 · 📨 안 본 아이에게 알림(알림 길 하나 lib/notify) · 아이의 구간 찍기(video_mark · 겹침은 Postgres 가 합친다) · 길이 채우기.
...
export async function setVideo(sb, id, patch = {}) {
  ...
  changed(await db(sb).from("video").update(p, { count: "exact" }).eq("id", id), "영상을 수정 못 함");
}
...
export async function retireAssign(sb, assignId) { changed(await db(sb).from("video_assign").update({ state: "retired" }, { count: "exact" }).eq("id", assignId), "배정을 못 삭제함"); }
/** 📨 안 본 아이에게 알림 · 아직·보다 만 아이에게 앱 알림(아이 기기 + 학부모 기기, who=all). 길은 lib/notify 하나. 스위치가 off 면 발송 이력만 남는다(대전제-0 · 그렇게 말한다) */
...
/** 여럿 한 번에((어28)-③ · 대전제-20) · 폴더·삭제·복구(setVideo 그대로) · 📨 알림(remindVideo 그대로 · 셈은 합친다) */
export async function videoMany(sb, ids = [], patch = {}) { const list = [...new Set((ids ?? []).filter(Boolean))]; if (!list.length) throw new Error("고른 영상이 없습니다"); let n = 0; for (const id of list) { await setVideo(sb, String(id), patch); n++; } return { n }; }
export async function remindMany(svc, sb, ids = [], date) { const list = [...new Set((ids ?? []).filter(Boolean))]; if (!list.length) throw new Error("고른 영상이 없습니다"); let n = 0, sent = 0, failed = 0, sink = null; for (const id of list) { const r = await remindVideo(svc, sb, String(id), date); n += r.n ?? 0; sent += r.sent ?? 0; failed += r.failed ?? 0; sink = r.sink ?? sink; } return { n, sent, failed, sink }; }
