...
/** (어76) 아이가 낸 것과 반려는 **원래 숙제 줄(slot='home')** 에 산다 — 검사 줄(slot='check')은 carry_of 로 그것을 가리킨다.
 *  두 곳에 적으면 반드시 어긋나므로(원칙-1) 읽는 자리도 쓰는 자리도 이 셋을 거친다.
 *  손으로 더한 검사 줄은 carry_of 가 없다 — 그때는 제 줄이 곧 원본이다((어60)). */
export const srcOf = (it) => it?.carry ?? it;                       // 읽을 줄(붙은 파일 · 반려)
export const srcId = (it) => it?.carry_of ?? it?.id ?? null;        // 쓸 줄 번호
/** 아이가 **낸** 것 — 원장이 **준** 자료(같은 줄에 붙는다)와 갈라야 한다. 가르는 잣대는 올린 사람의 자격이다.
 *  내린 파일(state hidden)은 빼고, 붙은 차례대로. byProfile 을 주면 그 사람 것만(아이 제 화면) */
export function submitted(it, { byProfile = null } = {}) {
  const links = srcOf(it)?.file_link ?? [];
  return links.map((l) => l.file ?? null)
    .filter((f) => f && f.state === "active" && (f.profiles?.role ?? "student") === "student" && (!byProfile || f.by_profile === byProfile));
}
/** 반려 한 줄 — 없으면 null */
export function rejectOf(it) {
  const s = srcOf(it); if (!s?.reject_reason) return null;
  return { reason: s.reject_reason, note: s.reject_note ?? "", at: s.rejected_at ?? null };
}
export const rejectText = (r) => (r ? `반려 · ${r.reason}${r.reason === "기타" && r.note ? ` · ${r.note}` : ""}` : "");
/** (어77) **완료 뒤 다음 걸음** — 원장님 2026-09-17 「다했어요 누르면 클래스카드 숙제는 클래스카드 라고 버튼
 *  누르고 넘어가규 교재숙제는 사진으로 제출 가능하게해줘」.
 *  가르는 잣대는 **이름이 아니라 칸**이다(learn_items.by_app · 0177) — 이름으로 짐작하면 이름을 고치는 날 어긋난다.
 *  완료(said_done_at)를 누르기 전에는 아무것도 안 뜬다 — 다음 걸음은 「다 한 뒤에 할 일」이라서다. */
export const APP_CC = "cc";
export const byApp = (it) => srcOf(it)?.learn_items?.by_app ?? it?.learn_items?.by_app ?? null;
export function nextStep(it) {
  if (!it?.said_done_at) return null;
  return byApp(it) === APP_CC ? "cc" : "photo";
}
