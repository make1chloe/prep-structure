...
import { CHECK_DOT as UNIT_MARK, markCh } from "./mark.js";   // (어43) 진도 점 부호는 lib/mark.js 한 곳
...
export function todayUnits(sheet, books = []) {
  ...
  for (const it of sheet?.class ?? []) if (it?.units && !out.has(it.units.id)) out.set(it.units.id, { id: it.units.id, label: `${name(it.units.book_id)} ${it.units.label ?? it.units.short ?? ""}`.trim(), mark: markCh("doing"), on: false });
  return [...out.values()];
}
