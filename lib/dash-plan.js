...
export const gapText = (g) => g.kind === "no_routine" ? `${g.area} 영역 루틴이 아직 안 만들어졌습니다` : g.kind === "no_round" ? "배정할 때 한 번에 나갈 소단원 수(한 수업에 몇 덩어리)를 안 정하고 넘어갔습니다" : g.kind === "cursor_stuck" ? `${Math.floor(g.days / 7)}주째 진도가 안 움직였습니다. 소단원 하나가 ○을 못 받아 다음이 안 열립니다` : "안 한 소단원이 없습니다. 다음 회독을 열거나 새 교재를 배정해야 합니다";
...
export function examLines({ exams = [], schools = [], today, horizon = 60 }) {
  const live = exams.filter((e) => (e.state ?? "active") === "active" && !e.hidden);   // 숨긴 회차는 대비·재촉·멈춤에서 빠진다(06b) — 임박에서도
