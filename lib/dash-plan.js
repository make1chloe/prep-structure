...
export function bookGaps({ books = [], routines = [], todo = [], date }) {
  const mine = new Set(routines.filter((r) => r.student_id).map((r) => `${r.student_id}|${r.area}`));   // 학생 루틴이 있는 (아이, 영역)
  ...
  for (const b of books) {
    ...
    const kind = !(mine.has(`${b.student_id}|${area}`) || areas.has(area)) ? "no_routine" : !(b.per_session > 0) ? "no_round" : (left.get(`${b.student_id}|${b.book_id}`) ?? 0) === 0 ? "no_units" : null;
