...
export const GAP = Object.freeze({ no_routine: "루틴 없음", no_round: "한 번에 몇 개인지 없음", no_units: "남은 소단원 없음", cursor_stuck: "커서 잠김" });   // cursor_stuck: 5단계-①(목업 17 「3주째 CHAPTER 5에 서 있습니다」 · 규칙 dash.cursor_stuck_days)
...
/** 반 한 줄 · 「매일 5:00 리허설 17:00–18:30」 · 별명이 없으면 유형 이름. 오늘 수업(01)의 알약과 같은 글 */
...
export const gapText = (g) => g.kind === "no_routine" ? `${g.area} 영역 루틴이 아직 안 만들어졌습니다` : g.kind === "no_round" ? "배정할 때 한 번에 나갈 소단원 수(한 수업에 몇 분량)를 안 정하고 넘어갔습니다" : g.kind === "cursor_stuck" ? `${Math.floor(g.days / 7)}주째 진도가 안 움직였습니다. 소단원 하나가 ○을 못 받아 다음이 안 열립니다` : "안 한 소단원이 없습니다. 다음 회독을 열거나 새 교재를 배정해야 합니다";
...
/** (저) 나이스가 기간을 옮긴 회차 — 「봤음」 전까지(0152 changed_at · changed_seen_at · 확정-69). 06b 꼬리표 · 대시보드 📅 · 12b 결과가 같은 글을 쓴다 */
export const dateChanged = (e) => Boolean(e?.changed_at) && !e?.changed_seen_at;
const span = (a, b) => `${md(a)}${b && b !== a ? `~${md(b)}` : ""}`;
export const changeText = (e) => `${span(e.prev_term_from, e.prev_term_to)} → ${span(e.term_from, e.term_to)}`;
/** 시험 줄 · 학교 시험의 영어 시험일(없으면 기간 시작)이 오늘부터 horizon 일 안이면 「신정중 2학기 중간 D-45」 가까운 차례. 아이들 학교 중 그런 시험이 없는 학교는 「영어 시험일 없음」(06 에 넣어야 루틴이 선다) */
export function examLines({ exams = [], schools = [], today, horizon = 60 }) {
  const live = exams.filter((e) => (e.state ?? "active") === "active" && !e.hidden);   // 숨긴 회차는 대비·알림·보류에서 빠진다(06b) · 임박에서도
  const soon = live.filter((e) => e.scope !== "national").map((e) => ({ ...e, on: e.english_on ?? e.term_from ?? null })).filter((e) => e.on)
  ...
  const changed = live.filter(dateChanged).map((e) => ({ id: e.id, text: `${e.schools?.name ?? (e.scope === "national" ? "전국" : "")} ${e.name} ${changeText(e)}`.trim(), english_on: e.english_on ?? null }));   // (저) 나이스가 기간을 옮긴 회차 — 「봤음」 전만(0152)
  return { soon: soon.map((e) => ({ id: e.id, school_id: e.school_id, text: `${e.schools?.name ?? ""} ${e.name} ${dday(today, e.on)}`.trim() })), missing, changed };
}
...
/** (어84) ⏰ 마감 필요 — 원장님 2026-09-18 「당일 처리할 업무(일지작성후발송, 숙제검사발송 출결처리등)이 안된 것을
 *  모달로 바로바로 처리할 수 있게 목록화해놔」 · 「b로 하되 서로 구별되게 두파트로 나눠서 제시 페이지여백없게」.
 *  재료는 dash_ops.due 한 조회가 준다(0181 — 대시보드는 조회 20이 상한이라 새 조회를 안 만든다 · 속도-1).
 *  **무엇이 남았나**는 여기 한 곳에서 정한다 — 화면이 제각기 세면 01 과 대시보드가 어긋난다(원칙-1) */
export const DUE_STEPS = Object.freeze([["attend", "출결"], ["check", "숙제 검사"], ["close", "마감"], ["send", "발송"]]);
export const dueStepName = (k) => DUE_STEPS.find(([x]) => x === k)?.[1] ?? k;
/** 그 아이에게 남은 걸음(차례대로).
 *  ⚠️ **마감이 그날의 끝이다.** 마감한 판은 출결·검사를 더 못 고치므로(검사-⑤ · 무르는 길이 없다)
 *     남은 걸음으로 세지 않는다 — 못 하는 것을 목록에 띄우면 눌러도 안 되고 목록이 영영 안 빈다.
 *     출결을 안 찍고 마감한 날은 **학생 14 의 출결 셈에 「아직」으로 남는다**(원장님 2026-09-18 「그대로 두고」).
 *  발송은 **마감한 뒤에만** 남은 걸음이 된다(마감한 것만 나간다). */
export function dueOf(r = {}) {
  if (r.closed) return r.sent ? [] : ["send"];
  const out = [];
  if ((r.attend ?? "none") === "none") out.push("attend");
  if ((r.left ?? 0) > 0) out.push("check");
  out.push("close");
  return out;
}
export const dueRows = (due) => ((due?.students ?? []).map((r) => ({ ...r, steps: dueOf(r) })).filter((r) => r.steps.length));
export const dueTodos = (due) => (due?.todos ?? []);
export const dueCount = (due) => dueRows(due).length + dueTodos(due).length;
