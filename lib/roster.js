...
import { dowOf, addDays } from "./day.js";
...
/**
 * **한 학생이 이 기간에 오는 날들** — ["2026-09-01", …].
 *
 * rosterOn 의 짝이다. rosterOn 은 「그날 누가 오나」, 이쪽은 「그 아이가
 * 언제 오나」 — 같은 잣대(meetsOn + toTermShape + 특강 쉬는 날)를 쓴다.
 * 날마다 rosterOn 을 부르면 하루에 왕복 세 번이라, 반·특강을 한 번만
 * 읽고 날짜만 훑는다.
 *
 * 왜 필요했나 (감사 ⑥-5, 2026-08-29): 학부모 결석 요청을 수락할 때
 * 「수업 있는 날」 을 **정규반 요일로만** 셌다. 정규반이 없고 특강만
 * 다니는 아이는 수락해도 출결이 **한 줄도 안 생겼다** — 원장님은 결석
 * 처리한 줄 아시는데 대시보드·수강료·보강 목록 어디에도 안 남는다.
 */
export async function comingDates(supabase, studentId, from, to) {
  if (!studentId || !from) return [];
  const last = to || from;
  if (last < from) return [];
  ...
  const [memQ, exQ] = await Promise.all([
    ...
    supabase
      .from("student_extra_schedules")
      .select("label, days, from_date, to_date, off_dates")
      .eq("student_id", studentId)
      .lte("from_date", last)
      .gte("to_date", from),
  ]);
  const ids = (memQ.data || []).map((m) => m.class_id).filter(Boolean);
  // 기간 칸까지 읽는다 — 요일만 보면 종강한 반에도 결석이 깔린다
  const classes = ids.length ? await loadClassesWithTerm(supabase, "id, days", ids) : [];
  const extras = ((exQ.error ? [] : exQ.data) || []).map((x) => ({
    shape: toTermShape(x),
    off: new Set(x.off_dates || []),
  }));                                                // 0164 전 DB 면 조용히 정규만
  ...
  for (let d = from; d <= last; d = addDays(d, 1)) {
    const dow = dowOf(d);
    const comes =
      classes.some((c) => meetsOn(c, d, dow)) ||
      extras.some((x) => meetsOn(x.shape, d, dow) && !x.off.has(d));
    if (comes) out.push(d);
  }
  return out;
}
