...
import { FLAG_KIND } from "./road-plan.js";
...
export async function studentMark(sb, { studentId, unitId, round, status, date }) {
  ...
  if (prev) row(await db(sb).from("progress").update(patch).eq("student_id", studentId).eq("unit_id", unitId).eq("round", round).select("unit_id"), "진도를 못 찍음");
