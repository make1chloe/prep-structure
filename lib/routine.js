...
export function lumpLabel(units = []) {
  ...
  const many = units.length > 2 ? ` (${units.length}줄)` : "";
 ...
 * ⚠️ 남는 구멍 — 가운데 줄이 이미 ○ 라 걸러진 탓에 **다른 중단원의 같은 이름이 맞닿으면**
 *    아직 한 덩어리가 된다. `mid` 를 열쇠에 넣으면 막히지만, 그러면 ㉙(소단원 기준에서
 *    본책+워크북 나란히)이 깨진다 — 실측으로 워크북 있는 11권이 전부 `chapter` 기준이라
 *    지금은 안 부딪히지만, 원장님이 한 권을 소단원 기준으로 바꾸시면 그날 골라야 한다.
 */
export function lumpsOf(units = [], opt = {}) {
  // 줄 차례(sort)대로 훑으며 **이름이 끊길 때마다** 새 덩어리 번호를 준다
  const seq = [...units].sort((a, b) => (a.sort ?? 0) - (b.sort ?? 0));
  const keyOf = new Map();
  const nth = new Map();
  let prev = null;
  for (const u of seq) {
    ...
    if (base !== prev) nth.set(base, (nth.get(base) ?? 0) + 1);
...
/** 그 대단원에서 아직 안 한 줄. ⚠️ `doing`(◐)은 **남는다** — 조각이 다 덮이면 그때 ○ 가 된다(절 ⑳) */
export async function chapterUnits(db, { studentId, bookId, chapter, round }) {
  ...
  return rows.filter((r) => r.prog !== "done" && r.prog !== "skip");
}
 ...
 *    ⚠️ 이 줄들은 **아직 안 한 것**만 온다 — `cursor_of` 의 `todo` 와 같은 조건이다.
 */
export async function afterUnits(db, { studentId, bookId, round, chapter, isWorkbook, orderBasis = "sub" }) {
  ...
  const { rows } = await db.query(
    `with u as (
     ...
     select u.id, u.chapter, u.mid, u.sub, u.activity, u.is_workbook, u.sort,
            ...
            v2.unit_label(u.id, true) as label
       from u
      where u.id not in (select p.unit_id from v2.progress p
                          where p.student_id = $1::uuid and p.round = $3::smallint
  ...
  const there = at(rest[0]);
  return rest.filter((r) => at(r) === there);
}
...
export async function routineOf(db, studentId, areas = []) {
  ...
  const { rows } = await db.query(
    `select 'area' as src, r.area, r.item_id, li.name, li.method, li.tool, li.checks,
      ...
      where r.area = any($1::text[]) and li.state = 'active'
      union all
     select 'student', r.area, r.item_id, li.name, li.method, li.tool, li.checks,
       ...
       join v2.learn_items li on li.id = r.item_id
      where r.student_id = $2::uuid and r.area = any($1::text[]) and li.state = 'active'
      order by 2, 9, 4`,
    [list, studentId]);
...
export async function routineNext(db, opt = {}) {
  ...
  for (const b of books) {
    ...
    const all = await chapterUnits(db, { studentId, bookId: b.bookId, chapter: cur.chapter, round });
    // ⚠️ 대단원 기준이면 **지금 갈래만** 본다 — 안 가르면 본책과 워크북이 한 덩어리로 뭉쳐
    //    「대단원 기준」이 켜져 있어도 화면만 그렇고 실제로는 소단원 기준으로 나간다(㊻ 118번).
    const mine = b.orderBasis === "chapter" ? all.filter((u) => (u.is_workbook === true) === cur.isWorkbook) : all;
    const lumps = lumpsOf(mine, { orderBasis: b.orderBasis, chunkDepth: b.chunkDepth });
    ...
    if (again) {
      ...
      Object.assign(card, l, { again: true, empty: isEmpty(l) });
      if (card.empty) card.why = emptyWhy(b, items);
      out.push(card);
      continue;
    }
    ...
    let allParts = parts;
    if (!nextLumps.length) {
      const rows = await afterUnits(db, {
        studentId, bookId: b.bookId, round,
        chapter: cur.chapter, isWorkbook: cur.isWorkbook, orderBasis: b.orderBasis,
      });
      nextLumps = lumpsOf(rows, { orderBasis: b.orderBasis, chunkDepth: b.chunkDepth }).slice(0, count);
    ...
    Object.assign(card, l, { empty, leftLumps: Math.max(0, lumps.length - count) });
    if (empty) {
