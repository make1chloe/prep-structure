...
export async function givePool(sb, sheetId, bookId = null) {
  ...
  const [units, todo, lines, have] = await Promise.all([
  ...
  return {
    ...
    units: (units.data ?? []).map((u) => ({ id: u.id, chapter: u.chapter, short: u.short ?? u.label ?? u.sub ?? "?", pages: u.page_start ? `${u.page_start}${u.page_end && u.page_end !== u.page_start ? `~${u.page_end}` : ""}` : null, qs: u.q_count ?? null, left: left.has(u.id) })),
