...
export async function givePool(sb, sheetId, bookId = null) {
  ...
  const list = books.map((b) => ({ book_id: b.book_id, name: b.books?.name ?? "?", area: b.books?.area ?? null }));
  ...
  const [units, todo, lines, have] = await Promise.all([
    ...
    db(sb).from("day_item").select("id,slot,item_id,unit_id,off,carry_of,units!inner(book_id)").eq("sheet_id", sheetId).eq("units.book_id", bookId).in("slot", ["class", "home"]),
  ]);
  ...
  return {
    ...
    lines: { class: linesFor(ls, "class").map((l) => ({ item_id: l.item_id, name: l.name, required: Boolean(l.required) })), home: linesFor(ls, "home").map((l) => ({ item_id: l.item_id, name: l.name, required: Boolean(l.required) })) },
...
export async function applyGive(sb, sheetId, bookId, slot, { unitIds = [], itemIds = [], range = null } = {}) {
  ...
  const [lines, have, all] = await Promise.all([
    ...
    db(sb).from("day_item").select("sort").eq("sheet_id", sheetId).in("slot", ["class", "home"]),
  ]);
  if (have.error) throw new Error(`수업 일지 줄을 못 읽음: ${have.error.message}`);
  if (all.error) throw new Error(`차례를 못 읽음: ${all.error.message}`);
  const ls = lines.of(b).filter((l) => its.includes(l.item_id));
  const key = (i, u) => `${i}|${u}`;
  const mine = (have.data ?? []).filter((r) => isAuto(r));   // 루틴 모양 줄만 — 넘어온 줄·글 줄은 이 모달이 안 건드린다
  const had = new Map(mine.map((r) => [key(r.item_id, r.unit_id), r]));
  ...
  for (const u of us) for (const l of ls) {
    ...
    if (!ex) { rows.push({ sheet_id: sheetId, slot, item_id: l.item_id, unit_id: u, range_note: range || null, sort: ++sort, required: Boolean(l.required), gate_prev: Boolean(l.gate_prev) }); continue; }
