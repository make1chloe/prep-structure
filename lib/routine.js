...
import { planBook, stopOn, offFor, waves, wavePlan, redoUnits, nextCarry, moveSort, parseChecks, choiceOrder, PLACE_OF, PLACE, AREAS, STOP, MODE } from "./routine-plan.js";
...
export async function layRoutine(sb, sheetId) {
  ...
  const [books, laid, missed] = await Promise.all([
  ...
  const [prevNext, have] = await Promise.all([
    ...
    db(sb).from("day_item").select("id,slot,item_id,unit_id,carry_of").eq("sheet_id", sheetId).in("slot", ["class", "home"]),
  ]);
...
export async function pickWave(sb, sheetId, bookId, slot, unitIds) {
  ...
  const res = await Promise.all([
    ...ups.map((u) => db(sb).from("day_item").update({ ...(u.unit_id ? { unit_id: u.unit_id, range_note: null, said_done_at: null } : {}), off: u.off, ...(u.sort != null ? { sort: u.sort } : {}) }, { count: "exact" }).eq("id", u.id).then((r) => (changed(r, "단원을 못 바꿈"), r))),
