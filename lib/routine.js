...
import { planBook, stopOn, offFor, waves, redoUnits, nextCarry, moveSort, parseChecks, PLACE_OF, PLACE, AREAS, STOP, MODE } from "./routine-plan.js";
...
export async function layRoutine(sb, sheetId) {
  ...
  todoBooks.forEach((b, i) => {
    ...
    const w = plan.units.length ? waves({ units, todo: todoHere, done: redo.length ? redo : prev ? name(prev) : null }) : { class: [], home: [] };
...
export async function pickWave(sb, sheetId, bookId, slot, unitIds) {
  ...
  const res = await Promise.all([
    ...ups.map((u) => db(sb).from("day_item").update(u.unit_id ? { unit_id: u.unit_id, off: u.off } : { off: u.off }, { count: "exact" }).eq("id", u.id).then((r) => (changed(r, "단원을 못 바꿈"), r))),
