...
export async function addItem(sb, { area, name, method = null, checks = null, place = "both", required = false }) {
  ...
  if (found && found.state !== "active" && dup) { if (dup.state !== "active") row(await db(sb).from("area_routine").update({ state: "active" }).eq("id", dup.id).select("id"), "줄을 못 되살림"); return { itemId, lineId: dup.id, revived: true }; }   // 항목째 내렸던 것을 같은 이름으로 되살리면 줄도 그대로 산다(자리가 달라도 새 줄을 안 만든다)
...
export async function retireItem(sb, itemId) {
  const r = row(await db(sb).from("learn_items").update({ state: "retired" }).eq("id", itemId).select("id,name"), "항목을 못 내림");
...
export async function resetStudent(sb, studentId, area) {
  const r = row(await db(sb).from("student_routine").update({ state: "retired" }).eq("student_id", studentId).eq("area", area).eq("state", "active").is("book_id", null).select("id"), "줄을 못 내림");
  return (r ?? []).length;
}
...
export async function customizeBook(sb, studentId, bookId) {
  ...
  for (const l of base) {
    ...
    if (ex) { if (ex.state !== "active") row(await db(sb).from("student_routine").update({ state: "active", sort: l.sort }).eq("id", ex.id).select("id"), "줄을 못 되살림"); }
...
export async function resetBook(sb, studentId, bookId) {
  const r = row(await db(sb).from("student_routine").update({ state: "retired" }).eq("student_id", studentId).eq("book_id", bookId).eq("state", "active").select("id"), "줄을 못 내림");
  return (r ?? []).length;
}
...
export async function endBook(sb, studentBookId, on) {
  ...
  const r = row(await db(sb).from("student_book").update({ to_date: to }).eq("id", studentBookId).select("id"), "교재를 못 끝냄");
  if (!r?.length) throw new Error("고쳐진 줄이 없습니다(검사-⑪)");
  return { to };
}
