 ...
 *  통과선·방식은 학생 × 교재 × 회독마다 한 곳(v2.style_for: 아이 것 → 교재 것 → 학원 기본값). 미통과면 재시험 줄(retry_of · 더 어렵게)이 서고 하원 지연 사유에 붙는다(목업 01 노트).
 *  「오늘은 재시험 건너뜀」은 재시험 줄만 skipped 로 · 점수는 그대로, 오늘만 빠진다(9/5 ②). 교재 보류이면 시험도 못 낸다(DB quiz_guard). */
import { db } from "./supabase.js";
import { assertOpen } from "./day.js";
import { tagReason } from "./late.js";
import { KIND, SOURCE, splitQuizzes, scopeText, retestTag, parseStyle } from "./quiz-plan.js";
import { changed } from "./sqlError.js";
export { KIND, SOURCE, splitQuizzes, scopeText };
const SEL = "id,student_id,kind,source,assigned_sheet_id,assigned_on,book_id,unit_from,free_note,scope_id,total,cut_pct,taken_sheet_id,taken_on,wrong,retry_of,state,style_id,harder,passed,pct,books(name),units!unit_from(id,chapter,short,label),quiz_style(id,round,cut_pct,text,student_id,book_id,kind,mc_meaning,sa_meaning,mc_word,sa_word,first_hint,units_per,s_way),prep_scope(id,free_note,exam_id,books(name),units(chapter,short),exams(name))";
...
async function scopeRow(sb, scopeId) {
  const { data, error } = await db(sb).from("prep_scope").select("id,exam_id,book_id,unit_id,free_note,removed_on").eq("id", scopeId).maybeSingle();
  if (error) throw new Error(`내신 범위를 못 읽음: ${error.message}`); if (!data || data.removed_on) throw new Error("내신 범위가 없거나 학교가 제외한 것입니다"); return data;
}
export async function addQuiz(sb, sheetId, { kind, bookId = null, unitId = null, freeNote = null, round = 1, scopeId = null }) {
  if (!KIND.some(([k]) => k === kind)) throw new Error(`시험 유형이 아닙니다: ${kind}`);
  const sheet = await sheetRow(sb, sheetId);
  const sc = scopeId ? await scopeRow(sb, scopeId) : null;   // 내신 범위(prep_scope — 04·06b 가 넣은 것 · 5단계-③)
  if (sc) { bookId = sc.book_id; unitId = sc.unit_id; }
  const source = sc ? "prep" : bookId ? "book" : "manual";
  if (source === "manual" && !String(freeNote ?? "").trim()) throw new Error("범위를 적으세요. 교재를 고르거나 내신 범위를 고르거나 직접 적습니다");
  const style = await styleFor(sb, sheet.student_id, bookId, round, kind);
  const row = { student_id: sheet.student_id, kind, source, book_id: bookId, unit_from: unitId, scope_id: sc?.id ?? null, free_note: source === "manual" ? String(freeNote).trim() : null,
...
/** 낸 시험 수정 — 전체 개수 · 통과선 · 범위(직접). 낸 것(planned)만 */
export async function setQuiz(sb, sheetId, quizId, { total, cutPct, source, freeNote, scopeId }) {
  await assertOpen(sb, sheetId);
  const patch = {};
  if (scopeId !== undefined) { const sc = await scopeRow(sb, scopeId); Object.assign(patch, { source: "prep", scope_id: sc.id, book_id: sc.book_id, unit_from: sc.unit_id, free_note: null }); source = undefined; }   // 내신 범위로(5단계-③)
  ...
  if (source !== undefined) { if (source === "prep") throw new Error("내신 범위는 범위를 골라서(scopeId) · 유형만 바꿀 수 없습니다"); if (!SOURCE.some(([k]) => k === source)) throw new Error(`범위 유형이 아닙니다: ${source}`); patch.source = source; if (source === "manual") { patch.book_id = null; patch.unit_from = null; patch.free_note = String(freeNote ?? "").trim() || "(범위)"; } }
  ...
  if (error) throw new Error(`시험을 수정 못 함: ${error.message}`);
...
/** 시험 보기 · 틀린 개수(·전체)를 적으면 통과·맞은 개수·%는 세어 나온다. 미통과면 재시험 줄 + 하원 지연 사유, 통과면 둘 다 걷는다 */
export async function takeQuiz(sb, sheetId, quizId, { wrong, total }) {
  ...
  changed(await db(sb).from("quiz").update(patch, { count: "exact" }).eq("id", quizId), "시험 결과를 못 씀");
  ...
  if (state !== q.state) { changed(await db(sb).from("quiz").update({ state }, { count: "exact" }).eq("id", quizId), "시험 상태를 못 씀"); }
...
async function dropRetest(sb, sheet, q) {
  ...
  await Promise.all((have ?? []).map((r) => db(sb).from("quiz").update({ state: "skipped" }, { count: "exact" }).eq("id", r.id).then((x) => changed(x, "재시험을 건너뜀으로 못 적음"))));
  await tagReason(sb, sheet.id, tagOf(q), false);
}
/** 「📄 재시험지 만들기」 · 재시험 줄을 세운다(있으면 그대로). 시험지(종이)는 업무 05 를 지을 때 */
...
/** 「⏭ 오늘은 재시험 건너뜀」 · 재시험 줄만 skipped/planned. 점수는 그대로, 하원 지연 사유에서 오늘만 빠진다 */
...
/** 방식 수정(5단계-③ · 목업 01 「방식 수정 · 이 아이만 다르게도 됩니다」) · 학생 × 교재 × 회독 × 유형 줄을 quiz_style 에 세우거나 고치고(style_for 가 아이 줄을 먼저 본다), 아직 안 본 같은 짝의 시험은 새 방식·통과선을 따른다 */
export async function setStyle(sb, sheetId, quizId, form) {
  const sheet = await sheetRow(sb, sheetId);
  const { data: q, error } = await db(sb).from("quiz").select(SEL).eq("id", quizId).single();
  if (error || !q) throw new Error(`시험을 못 읽음: ${error?.message ?? "없음"}`);
  if (q.student_id !== sheet.student_id) throw new Error("이 아이의 시험이 아닙니다");
  const round = q.quiz_style?.round ?? 1, fields = parseStyle(q.kind, form);
  const row = { student_id: sheet.student_id, book_id: q.book_id ?? null, round, kind: q.kind, ...fields };
  const { data: st, error: e2 } = await db(sb).from("quiz_style").upsert(row, { onConflict: "student_id,book_id,round,kind" }).select("id,cut_pct").single();
  if (e2) throw new Error(`방식을 못 적음: ${e2.message}`);
  let q2 = db(sb).from("quiz").select("id,quiz_style(round)").eq("student_id", sheet.student_id).eq("kind", q.kind).eq("state", "planned").is("retry_of", null);
  q2 = q.book_id ? q2.eq("book_id", q.book_id) : q2.is("book_id", null);
  const { data: same, error: e3 } = await q2; if (e3) throw new Error(`낸 시험을 못 읽음: ${e3.message}`);
  const ids = (same ?? []).filter((x) => (x.quiz_style?.round ?? 1) === round).map((x) => x.id);
  if (ids.length) { changed(await db(sb).from("quiz").update({ style_id: st.id, cut_pct: st.cut_pct }, { count: "exact" }).in("id", ids), "낸 시험에 못 옮김", { zero: "ok" }); }
  return { id: st.id, applied: ids.length };
}
