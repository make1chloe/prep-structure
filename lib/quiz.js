...
import { KIND, SOURCE, splitQuizzes, scopeText, retestTag, parseStyle, QUIZ_POS, quizPosName } from "./quiz-plan.js";
...
export async function setQuiz(sb, sheetId, quizId, { total, cutPct, source, freeNote, scopeId }) {
  ...
  if (source !== undefined) { if (source === "prep") throw new Error("내신 범위는 범위를 골라서(scopeId) — 갈래만 바꿀 수 없습니다"); if (!SOURCE.some(([k]) => k === source)) throw new Error(`범위 갈래가 아닙니다: ${source}`); patch.source = source; if (source === "manual") { patch.book_id = null; patch.unit_from = null; patch.free_note = String(freeNote ?? "").trim() || "(범위)"; } }
...
/** 방식 고치기(5단계-③ · 목업 01 「방식 고치기 — 이 아이만 다르게도 됩니다」) — 학생 × 교재 × 회독 × 갈래 줄을 quiz_style 에 세우거나 고치고(style_for 가 아이 줄을 먼저 본다), 아직 안 본 같은 짝의 시험은 새 방식·통과선을 따른다 */
...
/** 🔤 시험 카드 자리(0143 students.quiz_pos) — 아이마다 「시작하자마자 / 다 끝내고」. 루틴 11 아이 머리가 부르고 오늘 01 줄이 읽는다. 값 둘은 quiz-plan QUIZ_POS 한 곳(DB 도 students_quiz_pos_choice 로 막는다) */
export async function setQuizPos(sb, studentId, at) {
  if (!QUIZ_POS.some(([k]) => k === at)) throw new Error(`시험 카드 자리가 아닙니다: ${at}`);
  const { data, error } = await db(sb).from("students").update({ quiz_pos: at }).eq("id", studentId).select("id");
  if (error) throw new Error(`시험 카드 자리를 못 바꿈: ${error.message}`);
  if (!data?.length) throw new Error("고쳐진 아이가 없습니다");
  return { at, name: quizPosName(at) };
}
