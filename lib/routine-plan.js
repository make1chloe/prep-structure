...
export function wavePlan(rows = [], ids = []) {
  ...
  const pool = rows.map((r) => r.sort).filter((s) => s != null).sort((a, b) => a - b);   // 이 묶음이 쓰던 차례. 목표 단원 차례대로 다시 나눈다(돌려 쓴 줄에 옛 차례가 남으면 대비문제가 1-4 앞에 서고, 「+ 시험 더하기」 기본 단원이 첫 줄을 봐서 어긋난다) · 모자라면 fresh(소비처가 맨 뒤 번호를 pos 차례대로 준다)
