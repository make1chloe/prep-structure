...
/** 아이의 한 영역 — 학생 루틴 줄이 있으면 그것(이 아이만 수정), 없으면 영역 루틴 그대로(확정-㉒). 뺀 것 = 영역 줄 중 아이 줄에 없는 항목 */
 ...
 *  자리(학원·숙제)마다 따로 · ① 시작한 줄(started_at · 시작 차례 · pin) ② 지난 시간에서 넘어온 줄(옛 차례) ③ 교재 줄(rank 등수 → 옛 차례 · 검사 나머지 조각은 아님) ④ 기타(교재 없음 · 조각 · 옛 차례)
