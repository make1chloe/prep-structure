...
import { parseCuts, parseQuestions, parseQuestionSheet, parseWrong, SHOW, rowsOf, parsePercentile } from "./score-plan.js";
...
export async function scoreBoard(sb, examId, date) {
  ...
  if (error) throw new Error(`성적 화면을 못 읽음: ${error.message}`);
...
export async function setCuts(sb, examId, text) {
  ...
  const r = changed(await db(sb).from("exams").update({ cuts: cuts.length ? cuts : null }).eq("id", examId).select("id"), "등급컷을 못 적음") ?? [];
...
/** 회차의 문항표를 엑셀로(확정-⑤ · 5단계-⑤) — 열 「번호 · 영역」 · 통째로 바꾼다(한 곳 set_exam_questions) · 고칠 줄은 빼고 적는다 */
export async function setQuestionSheet(sb, examId, sheetRows = []) {
  const { rows, bad } = parseQuestionSheet(sheetRows);
  if (!rows.length) throw new Error(`읽을 줄이 없습니다. 첫 줄이 「번호 · 영역」이어야 합니다${bad.length ? ` (고칠 줄 ${bad.length}: ${bad.slice(0, 3).map((b) => `${b.line}행 ${b.why}`).join(" / ")})` : ""}`);
  const n = row(await db(sb).rpc("set_exam_questions", { p_exam: examId, p_rows: rows }), "문항표를 못 적음");
  return { rows: rows.length, saved: Number(n ?? 0), bad };
}
async function examOf(sb, examId) {
  ...
  if (!e || e.state !== "active") throw new Error("시험이 없거나 물렸습니다");
  return e;
}
...
export async function saveScore(sb, { examId, studentId, raw, full = 100, wrongs = "", note = null, byWho = "staff", showTo = null, percentile }, date) {
  ...
  let id = ex?.id ?? null;
  if (id) changed(await db(sb).from("score").update(patch).eq("id", id).select("id"), "성적을 수정 못 함");
...
export async function confirmScore(sb, scoreId, showTo = null) {
  ...
  changed(await db(sb).from("score").update({ confirmed: true, show_to: show }).eq("id", scoreId).select("id"), "확인을 못 적음");
  return { show };
}
...
export async function unconfirmScore(sb, scoreId) {
  const r = changed(await db(sb).from("score").update({ confirmed: false, show_to: "staff" }).eq("id", scoreId).eq("confirmed", true).select("id"), "확인을 못 풂");
...
export async function setShow(sb, scoreId, showTo) {
  ...
  const r = changed(await db(sb).from("score").update({ show_to: showTo }).eq("id", scoreId).eq("confirmed", true).select("id"), "공개를 못 바꿈") ?? [];
...
export async function studentSubmit(sb, user, { examId, raw, full, wrongs }, date) {
  ...
  const e = row(await db(sb).from("exams").select("id,scope,school_id,grade,name,term_from,term_to,english_on,state,hidden,exam_skip(student_id,skipped)").eq("id", examId).maybeSingle(), "시험을 못 읽음");
  if (!e || !takes({ ...st, level: st.schools?.level }, e)) throw new Error("내 학교 시험이 아닙니다");
...
export async function importScores(sb, examId, parsed = [], board, date) {
  ...
  for (const r of parsed) {
    ...
    if (r.grade != null) changed(await db(sb).from("score").update({ grade: r.grade }).eq("id", id).select("id"), "등급을 못 적음");   // 학교가 발표한 등급은 컷보다 이긴다
...
/** 📨 안 낸 아이에게 알림(16 · 4단계-2a) · 보는 아이 중 성적 줄이 없는 아이에게. 아이가 넣는 것이라 아이 기기도(who=all) · 알림 길은 lib/notify 하나 · 발송 이력 why 는 회차 */
export async function remindScores(svc, sb, examId, date) {
  const b = await scoreBoard(sb, examId, date); if (!b?.exam) throw new Error("시험이 없습니다");
...
/** 고른 성적 줄 여럿을 한 번에((어28)-④ · 대전제-20) · 한 줄의 손(확인 · 확인 풀기)을 차례로 · 막히면 그 줄부터 멈춘다 */
export async function confirmMany(sb, ids = []) { const list = [...new Set((ids ?? []).filter(Boolean))]; if (!list.length) throw new Error("고른 성적이 없습니다"); let n = 0; for (const id of list) { await confirmScore(sb, id); n++; } return { n }; }
export async function unconfirmMany(sb, ids = []) { const list = [...new Set((ids ?? []).filter(Boolean))]; if (!list.length) throw new Error("고른 성적이 없습니다"); let n = 0; for (const id of list) { await unconfirmScore(sb, id); n++; } return { n }; }
