...
export async function saveAreaMemo(sb, sheetId, area, memo) {
  ...
  changed(await db(sb).from("day_area_memo").upsert({ sheet_id: sheetId, area, memo: text, updated_at: new Date().toISOString() }, { onConflict: "sheet_id,area" , count: "exact" }), "영역 메모를 못 저장함");
}
