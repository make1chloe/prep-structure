import { sameSchool, sameGrade } from "./who.js";
...
/**
 * **여기 있던 자기만의 규칙을 지웠다** (2026-08-09).
 *
 * 「신송중학교」 를 이 파일은 「신송」 으로, lib/schoolName 은 「신송중」 으로
 * 다듬고 있었다. 같은 뜻이라고 적어두고 **서로 다른 답을 냈다** — 그래서
 * 일정 대상이 화면마다 달랐다. 규칙은 lib/who 한 곳에만 둔다.
 */
 ...
 */
function taskForStudent(task = {}, me = {}) {
  // 비공개(= 나만 보기)면 무엇이 적혀 있든 아무에게도 안 간다
  if (task.private) return false;
  ...
  const schoolOk =
    schoolId
      ...
      : school
      ? sameSchool(me.school, school)
      : true;                          // 학교는 안 적고 학년만 적은 것
  const gradeOk = !grade || sameGrade(me.grade, grade);
...
export function audienceLabel(task = {}) {
  // **비공개가 곧 「나만 보기」다** (원장님, 2026-08-06 — 「나만보기가 비공개네,
  // 비공개는 따로 만들지마」). 그래서 따로 만들지 않고 **누가 보나의 한 가지**로
  // 둔다. 켜져 있으면 다른 무엇이 적혀 있어도 아무에게도 안 간다.
  if (task.private) return { text: "비공개", tone: "tag-muted" };
  ...
  return { text: "비공개", tone: "tag-muted" };
}
