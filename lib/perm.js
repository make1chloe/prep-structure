/** 「누가 무엇을 보나」 판단 한 벌(대전제-4). 키 목록의 주인은 여기다 · DB 는 값만 든다.
 *  기본값은 코드에 없다: 줄이 없으면 「안 정함」이고 막힌다(fail closed). 원장은 묻지 않는다.
 *  ⚠️ 키 이름은 옛 앱과 같다 · 원장님이 2026-09-03 에 정하신 32칸이 v2.role_access 에 그대로 있다. 이름을 바꾸면 그 답이 사라진다. */
...
export const KEYS = Object.freeze([
  ...
  { key: "page.ops",      name: "학생",       group: "page",   roles: 학원사람,       href: "/ops" },   // (어28) 운영 탭 = 학생 14(수강료 13 은 그 안 유형) · 키 이름은 그대로(page.ops)
  ...
  { key: "me.grid",       name: "학교별 표",  group: "me",     roles: [ROLES.STUDENT] },   // (너) 2026-09-09 원장님 「학생학부모는 따로 권한두기」 — 무엇을 보이나는 표마다 공개 스위치(grid.share)
  ...
  { key: "parent.word",     name: "단어",       group: "parent", roles: [ROLES.PARENT], unused: "아직 이 권한을 보는 화면이 없습니다. 켜도 달라지는 것이 없습니다" },   // (서2) 2026-09-10: 소비처 0. 칸은 원장님이 정하신 34 라 안 뺀다(이름을 빼면 그 답이 사라진다) · 대신 화면이 스스로 말한다(대전제-0). 쓸 자리가 생기면 이 줄의 unused 를 지운다(check-perm 이 지킨다)
  { key: "parent.reports",  name: "리포트",     group: "parent", roles: [ROLES.PARENT] },
  { key: "parent.sent",     name: "보낸 것",    group: "parent", roles: [ROLES.PARENT] },
  { key: "parent.grid",     name: "학교별 표",  group: "parent", roles: [ROLES.PARENT] },   // (너) 같은 뜻 — 학부모 쪽
].map(Object.freeze));
export const GROUP_NAME = Object.freeze({ page: "대메뉴", ops: "운영 카드", me: "학생 화면 카드", parent: "학부모 화면 카드" });

/** 아이 화면 카드 키 · KEYS 에서 갈라낸다(글자를 다시 적지 않는다, check-perm). ME.arrival = "me.arrival" … */
export const ME = Object.freeze(Object.fromEntries(KEYS.filter((k) => k.group === "me").map((k) => [k.key.split(".")[1], k.key])));
/** 학부모 화면 카드 키 · 같은 결. PARENT.recent = "parent.recent" … */
export const PARENT = Object.freeze(Object.fromEntries(KEYS.filter((k) => k.group === "parent").map((k) => [k.key.split(".")[1], k.key])));
/** 운영 카드 키 · 같은 결. OPS.fee = "ops.fee" … */
export const OPS = Object.freeze(Object.fromEntries(KEYS.filter((k) => k.group === "ops").map((k) => [k.key.split(".")[1], k.key])));
...
/** 아직 안 정한 칸 · 역할마다 그 역할에 걸리는 키만 센다 */
