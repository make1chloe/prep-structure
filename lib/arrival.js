...
export async function staffStamp(sb, { studentId, date, step, hhmm = null }) {
  ...
  const r = one(await db(sb).from("arrival").upsert({ student_id: studentId, date, step: n, at, stamped_by: "staff", undone_at: null }, { onConflict: "student_id,date,step" }).select("at").single(), "시각을 못 적음");
...
export async function clearStamp(sb, { studentId, date, step }) {
  ...
  if (!gone?.length) throw new Error("아이 앱이 찍은 것은 취소할 수 없습니다. 시각 고치기로 바꾸세요");
