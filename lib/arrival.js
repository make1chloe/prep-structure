...
export async function staffStamp(sb, { studentId, date, step, hhmm = null }) {
  ...
  const r = one(await db(sb).from("arrival").upsert({ student_id: studentId, date, step: n, at, stamped_by: "staff" }, { onConflict: "student_id,date,step" }).select("at").single(), "시각을 못 적음");
