 ...
 * ⚠️ 반이 둘이면(정규+특강 같은 날) **가장 이른 시작 시각의 반**에 찍고 나머지를 `others` 로 밝힌다.
 *    아이는 한 번 왔다 — 두 줄에 나눠 찍으면 특강 출결이 정규로 샌다.
 ...
 * }
 */
export async function arrivalView(db, { studentId, date = null, graceMin = 0 } = {}) {
  if (!studentId) throw new Error("학생이 없다");
  ...
  const late = arrivedAt
    ? lateOf({ startTime: cls.pick?.startTime ?? null, atHm: arrivedAt, graceMin })
    : null;
...
export async function markArrival(db, one = {}, opts = {}) {
  ...
  const view = await arrivalView(db, { studentId: one.studentId, date: one.date ?? null, graceMin });
  ...
  // ③ 열 시간 넘게 늦은 것은 판을 안 세운다 — 원장님께 넘긴다
