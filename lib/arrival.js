...
export async function allowIp(sb, ip, note = "") {
  ...
  const { error } = await db(sb).from("integration").update({ config: { ips, graceMin: cfg.graceMin, note }, updated_at: new Date().toISOString() }).eq("id", "arrival");
  if (error) throw new Error(`학원 회선을 못 적음: ${error.message}`);
