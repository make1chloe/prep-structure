...
export async function myStudent(sb, profileId) {
  const { data, error } = await db(sb).from("students").select("id,name,profile_id,grade,school_id,phone,parent_phone,birth,progress_edit,schools(name,level)").eq("profile_id", profileId).eq("state", "active").limit(1).maybeSingle();
...
/** (어48) 학원 사람이 찍는다 · 01 출결 곁의 「하원」과 시각 수정(원장님 2026-09-16 「학생들이 어플에서 하원처리를 안했을 경우를 대비해서 출석체크 근처에 하원도 넣고 … 시간 정정가능하게」).
 ...
 *  학원 사람이 찍은 것만 내릴 수 있다 — 아이 앱이 찍은 것은 1차 기준이라 그대로 둔다(그때는 시각 수정 ✎ 로) */
export async function clearStamp(sb, { studentId, date, step }) {
  ...
  if (!gone?.length) throw new Error("아이 앱이 찍은 것은 취소할 수 없습니다. 시각 수정로 바꾸세요");   // 칸이 없으면 위 changed() 가 「등원을 못 적음: column … undone_at」 로 말한다(0170 을 넣으시면 됩니다)
