 ...
 *   arrivedAt: 'HH:MM' | null,                   // 걸음 1~3 중 **가장 이른 것**
 *   leftAt: 'HH:MM' | null,                      // 하원 (걸음 4 · 0083)
 *   canLeave: boolean,                           // 「집에 가요」를 보여 줄까
...
export async function arrivalView(db, { studentId, date = null, graceMin = 0, classId = null } = {}) {
  ...
  // ⚠️⚠️ **도착 시각은 걸음 1~3 에서만 고른다.** 하원(4)도 같은 표에 있어서(0083)
  //    `rows[0]` 을 그냥 쓰면, 등원이 없고 하원만 있는 날(원장님이 손으로 적은 날)
  //    **하원 시각이 도착 시각으로 읽혀 지각 셈이 통째로 틀어진다.**
  const arrivedAt = rows.find((r) => Number(r.step) <= 3)?.hm ?? null;   // SQL 이 `order by at`
  const leftAt = at.get(LEAVE.step) ?? null;               // 하원 — 저장된 사실 그대로
  ...
  return {
    ...
    leftAt,
    // ⚠️ 등원을 찍은 날에만 찍을 수 있다. 이미 찍었으면 다시 안 묻는다(대전제-3)
    canLeave: !!arrivedAt && !leftAt,
...
// ── ⑥ 하원 — **등원과 같은 자리에 날짜별로** (원장님 2026-09-03 ⑩) ────────────

/**
 * 하원. **걸음이 아니다** — 그래서 `STEPS` 에 안 든다(`stepOf(4)` 는 던진다).
 *
 * ⚠️ 옛 앱은 `public.arrival_checks.leave_at` **네 번째 칸**이었다. v2 는 (학생, 날짜, 걸음)
 *    한 줄이라 **걸음 4** 하나면 「등원과 같은 자리에 날짜별로」가 그대로 된다 —
 *    새 표를 세우면 「그날 학원에 있었던 자취」가 두 곳으로 갈린다(원칙-1).
 * ⚠️ 하루에 **한 번**이다 — 열쇠가 (학생, 날짜, 4) 라 DB 가 지킨다.
 */
export const LEAVE = Object.freeze({ step: 4, key: "leave", label: "집에 가요" });

/**
 * 아이가 하원을 찍는다.
 * ⚠️ `on conflict … do nothing` — 두 번 눌러도 한 줄이고 **먼저 찍은 시각을 지킨다**(등원과 같다).
 * ⚠️ **시각을 안 보낸다.** DB 의 `default now()` 와 `arrival_stamp` 문지기가 정한다(표-10, 0083).
 */
const SQL_LEAVE = `/* arrival:leave */
insert into v2.arrival (student_id, date, step, ip)
values ($1, coalesce($2::date, v2.today()), 4, $3::inet)
on conflict (student_id, date, step) do nothing
returning to_char(at at time zone 'Asia/Seoul', 'HH24:MI') as hm`;

/**
 * 원장님이 하원 시각을 **손으로 적는다** (아이가 못 찍고 간 날).
 * ⚠️ 아이는 이 길로 못 온다 — 문지기(`arrival_stamp`)가 아이의 update 를 거절하고
 *    insert 의 `at` 을 now() 로 덮는다. **원장·강사만 시각을 고른다.**
 * ⚠️ `$3` 은 'HH:MM' 글자다. 그날 서울 시각으로 읽는다.
 */
const SQL_LEAVE_SET = `/* arrival:leaveset */
insert into v2.arrival (student_id, date, step, at)
values ($1, coalesce($2::date, v2.today()), 4,
        ((coalesce($2::date, v2.today()) + $3::time) at time zone 'Asia/Seoul'))
on conflict (student_id, date, step) do update set at = excluded.at
returning to_char(at at time zone 'Asia/Seoul', 'HH24:MI') as hm`;

/**
 * 아이가 하원을 찍는다 — **판은 안 세운다.** 등원에서 이미 섰다.
 *
 * ⚠️ **등원을 안 찍은 날은 하원도 못 찍는다.** 안 막으면 「온 적 없는 날 집에 갔다」가 남고,
 *    그 줄이 그날 가장 이른 줄이 되어 **도착 시각으로 읽힐 뻔했다**(그래서 `arrivalView` 는
 *    도착 시각을 걸음 1~3 에서만 고른다).
 * ⚠️ 관문은 등원과 **같다** — 학원에서 나가며 찍는 것이라 학원 회선이어야 한다.
 *    원장·강사는 관문을 안 지난다(아이가 못 찍고 간 날 손으로 남겨 준다).
 *
 * @returns { ok, why, msg, first, view }
 */
export async function markLeave(db, one = {}) {
  const gate = one.gate;
  if (!gate || typeof gate !== "object") {
    throw new Error("관문(netGate)을 안 지났다 — 학원 회선 조건을 건너뛸 길은 없다");
  }
  if (gate.ok !== true) {
    return { ok: false, why: gate.why, msg: gate.msg, seenIp: gate.seenIp ?? null, first: false, view: null };
  }
  if (!one.studentId) throw new Error("학생이 없다");
  const graceMin = Number.isFinite(Number(one.graceMin)) ? Number(one.graceMin) : 0;
  const seen = { studentId: one.studentId, date: one.date ?? null, graceMin, classId: one.classId ?? null };

  const before = await arrivalView(db, seen);
  if (!before.arrivedAt) {
    // ⚠️ **조용히 안 찍는다.** 등원이 없는 하원은 그날 자취를 거꾸로 만든다
    return { ok: false, why: "not-arrived", first: false, view: before,
      msg: "등원을 아직 안 찍어서 하원을 못 찍습니다 — 등원부터 찍어 주세요" };
  }

  const put = await db.query(SQL_LEAVE, [one.studentId, one.date ?? null, normIp(one.ip) || null]);
  const first = (put.rows ?? []).length > 0;
  const view = await arrivalView(db, seen);

  if (!view.leftAt) {
    // 방금 넣었는데 안 읽힌다 — 접근 규칙이 막았다. **찍혔다고 말하지 않는다**
    return { ok: false, why: "blocked", first, view,
      msg: "한 줄도 안 남았습니다 — 접근 규칙이 막았습니다" };
  }
  return {
    ok: true, why: first ? "left" : "already", first, view,
    msg: first ? `${view.leftAt} 에 하원으로 남았습니다`
               : `이미 하원을 찍었습니다 (${view.leftAt})`,
  };
}

/**
 * 원장님이 하원 시각을 손으로 적는다 (또는 잘못 찍힌 것을 고친다).
 *
 * ⚠️ **지우는 길은 없다**(대전제-6). `v2.arrival` 에는 `authenticated` 에게 DELETE 권한이
 *    아예 없다(실측 2026-09-03) — 잘못 찍혔으면 **시각을 고친다.**
 * ⚠️ 등원이 없는 날에도 원장님은 적을 수 있다 — 아이가 폰 없이 온 날 손으로 다 남기는 자리다.
 *    (아이 손으로는 못 한다 — `markLeave` 가 등원을 먼저 본다.)
 */
export async function setLeaveTime(db, { studentId, date = null, hm = null, graceMin = 0 } = {}) {
  if (!studentId) throw new Error("학생이 없다");
  const t = String(hm ?? "").trim();
  if (!t) {
    return { ok: false, why: "no-time", changed: 0,
      msg: "지울 수는 없습니다 — 하원 시각을 고쳐 적어 주세요(대전제-6)" };
  }
  hmOf(t);                                   // 모양이 틀리면 여기서 던진다 (0 으로 눕히지 않는다)
  const r = await db.query(SQL_LEAVE_SET, [studentId, date, t.length === 5 ? `${t}:00` : t]);
  if (!(r.rows ?? []).length) {
    // ⚠️ 0줄이면 **성공이라 말하지 않는다** (검사 ⑪)
    return { ok: false, why: "blocked", changed: 0,
      msg: "한 줄도 안 바뀌었습니다 — 접근 규칙이 막았습니다(원장·강사만 적습니다)" };
  }
  const view = await arrivalView(db, { studentId, date, graceMin });
  return { ok: true, changed: 1, why: "set", leftAt: view.leftAt, view,
    msg: `하원 ${view.leftAt} 로 적었습니다` };
}

/**
 * 약속(예상 귀가)과 실제 하원의 **차이를 센다** — 저장하지 않는다(원칙-5).
 * ⚠️ 계획 ⑭: 「예상 귀가는 **약속이 된다.** 실제 하원을 찍어 차이를 같이 남기지 않으면
 *    「10시라더니 10시 40분」이 어디에도 안 남는다.」
 * @returns { minutes, late, sure } — `sure:false` 는 **못 셌다**는 뜻이다
 */
export function lateStayGap({ untilAt, leftAt }) {
  const u = untilAt ? hmOf(String(untilAt).slice(0, 5)) : null;
  const l = leftAt ? hmOf(String(leftAt).slice(0, 5)) : null;
  if (u === null || l === null) {
    return { minutes: null, late: null, sure: false,
      why: u === null ? "예상 귀가를 안 적었다" : "아직 하원을 안 찍었다" };
  }
  return { minutes: l - u, late: l > u, sure: true, why: null };
}
