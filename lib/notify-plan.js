/** 알림 판단 한 벌(순수) · 밖으로 나가는 길은 lib/notify.js 한 곳(대전제-7), 여기는 그 길이 쓰는 셈만: 유형 이름 · 잠금화면 문구 · 스위치(NOTIFY_SINK) · 받는 기기 고르기 · 방해금지.
 *  화면(클라이언트)과 서버가 같은 것을 본다 */
import { seoulTime, plusDays, seoulStamp } from "./day-plan.js";
/** 유형 → 안내 이름(옛 앱 lib/notify.js LABEL 의 말 그대로 · 어머니가 보시던 말). 등원·하원은 원장님 2026-08-23 */
export const LABEL = Object.freeze({ daily: "수업 안내", late: "하원 지연 안내", arrival: "등원 안내", leave: "하원 안내", plan_absent: "결석 예정 안내", plan_late: "지각 예정 안내", monthly: "월간 리포트", notice: "공지사항", welcome: "첫 등원 안내", video: "영상 안내", score: "성적 입력 안내", fee: "수강료 안내", schedule: "수업 일정 안내", guide: "상담 안내", reject: "숙제 다시 내기 안내", give: "학습지 배부 안내" });   // score·fee 는 4단계-2a(16 알림 · 13 안내) · schedule 은 4단계-3b(12 그 달 일정 확정 → 학부모 · ㉚ 9/7)
export const KIND_NAMES = Object.freeze(Object.keys(LABEL));
export const labelOf = (kind) => LABEL[kind] ?? null;
/** 잠금화면에 적을 말 · 내용은 앱을 열어야 보인다(원장님 2026-08-07 「미리보기에서 내용 알 수 없게 해줘 … 눌러서 어플 들어와야 알 수 있게」). 발송 이력(notify_log)에 본문 칸이 없는 까닭 */
...
export function titleFor(kind, academy = "클로이영어") {
  const t = labelOf(kind); if (!t) throw new Error(`알림 유형이 아닙니다: ${kind}`);
  const a = String(academy ?? "").trim();
  return !a || t.startsWith(`[${a}]`) ? t : `[${a}] ${t}`;
}
/** 한 통의 짐 · 다섯 칸(public/sw.js 계약: title · body · tag · url · r). r 은 발송 이력 번호 · 폰이 받은 때·누른 때를 이 번호로 회신한다 */
...
/** 스위치 · off(기본: 발송 이력만 남기고 안 보낸다 · 미리보기·리허설이 학부모 폰에 뜨는 일이 구조적으로 없다) · self(학원 사람 기기에만 · 원장님 폰 시험) · live(진짜) */
...
/** (려) **화면용** — 스위치가 이상해도 화면은 열려야 한다(원장님 2026-09-10 밤: 값을 `1` 로 넣으셨더니 발송 10 이 통째로 안 열렸다).
 *  보내는 자리(notify·sms)는 그대로 sinkOf 가 **던져서 막는다** — 짐작으로 켜지 않는다(대전제-9 와 같은 뜻).
 *  화면은 **안 나가는 쪽(off)으로 잠근 채** 열고, 무엇이 이상한지 그 자리에서 말한다(대전제-0). */
export function sinkSafe(env = {}) {
  try { return { sink: sinkOf(env), bad: null }; }
  catch (e) { return { sink: "off", bad: String(e?.message ?? e) }; }
}
