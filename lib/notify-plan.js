...
export const LABEL = Object.freeze({ daily: "수업 안내", late: "늦은 귀가 안내", arrival: "등원 안내", leave: "하원 안내", plan_absent: "결석 예정 안내", plan_late: "지각 예정 안내", monthly: "월간 리포트", notice: "공지사항", welcome: "첫 등원 안내", video: "영상 안내", score: "성적 입력 안내", fee: "수강료 안내" });   // score·fee 는 4단계-2a(16 재촉 · 13 안내)
...
export function pickDevices({ subs = [], parents = [], studentId = null, who = "parent" } = {}) {
  ...
  for (const s of subs) {
    ...
    const isParent = parents.includes(s.profile_id), isChild = who === "all" && studentId && s.student_id === studentId;
