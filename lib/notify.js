...
export async function notify(svc, { kind, studentId, url = "/parent", tag = null, why = null, sheetId = null, jobId = null, who = "parent", smsLine = null }, env = process.env) {
  ...
  const also = await smsAlong(svc, { kind, studentId, url, smsLine, why }, env);   // (뎌-2) 켠 갈래는 문자로도 — 앱을 안 깐 집에도 닿는다
  return { sink, logged: logs.length, sent, failed, ...(also ? { sms: also } : {}) };
}
/** (뎌-2) 앱 알림과 **함께** 문자 한 통 — 규칙(send.sms_kinds)이 켠 갈래만.
 *  문구는 원장님이 고치는 v2.msg_template(sms_<갈래>) · {{한 줄}} 은 그 갈래를 아는 손이 준 것(비면 그 줄이 사라진다).
 *  문자가 안 돼도 **앱 알림은 이미 나갔다** — 여기서 던지지 않고 까닭만 돌려준다(자취는 sms() 가 남긴다) */
export async function smsAlong(svc, { kind, studentId, url = "/parent", smsLine = null, why = null }, env = process.env) {
  try {
    ...
    if (!(await smsReady(svc))) return { skipped: "솔라피 열쇠가 아직 없습니다" };
    ...
    if (f.missing.length) return { skipped: `치환 자리가 빕니다: ${f.missing.join(" · ")}` };
