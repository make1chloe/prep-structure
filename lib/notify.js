/** 밖으로 나가는 길 · 이 한 곳(대전제-7). 발송 이력(v2.notify_log)를 먼저 남기고, 스위치(NOTIFY_SINK)가 허락한 기기에만 보낸다.
 *  off(기본)면 발송 이력에 sink=off 로 남고 sent_at 은 빈다 · 「안 보냈다」가 사실(0069). 미리보기·리허설이 학부모 폰에 진짜로 뜨는 일이 구조적으로 없다.
 *  self 면 학원 사람 기기에만(원장님 폰으로 시험) · live 면 진짜. 서버 자신(service role)만 부른다 — 사람은 notify_log 에 못 쓴다(0017).
 *  (커) 문자(솔라피)도 이 길로 · sms(): 발송 이력(channel sms · 가린 번호)를 먼저 남기고 live 일 때만 나간다(확정-71). 부르는 자리는 lib/solapi.js 하나.
 *  한 통 = 받는 사람(학부모 계정) 하나의 발송 이력 한 줄. 기기가 여럿이면 짐마다 그 줄 번호(r)를 싣는다 · 폰이 받은 때·누른 때를 그 번호로 회신한다(mark_notify_seen) */
...
import { phoneDigits, maskPhone, fill, smsAlso } from "./sms-plan.js";
...
export async function notify(svc, { kind, studentId, url = "/parent", tag = null, why = null, sheetId = null, jobId = null, who = "parent", smsLine = null }, env = process.env) {
  if (!labelOf(kind)) throw new Error(`알림 유형이 아닙니다: ${kind}`);
  ...
  // 받는 사람 · 학부모(기본) · who=all 이면 학부모 + 아이 기기 · who=student 면 아이 계정만(공지 「학생」 · 4단계-6: 발송 이력도 아이 줄에)
  ...
  // 발송 이력 · 받는 사람마다 한 줄. 같은 일을 다시 시도하면 이미 있는 줄을 채운다(발송 이력이 늘지 않게)
  const old = rows(oldQ, "발송 이력");
  if (!parents.length) {
    if (!old.length) { const { error } = await db(svc).from("notify_log").insert({ ...base, profile_id: null, failed_at: now, fail_why: who === "student" ? "아이 계정이 없습니다" : "학부모 계정이 이 아이와 이어져 있지 않습니다" }); if (error) throw new Error(`발송 이력을 못 남김: ${saidBy(error.message)}`); }
  ...
  const logs = [...old, ...rows(ins, "발송 이력")];
  const logOf = new Map(logs.map((l) => [l.profile_id, l.id]));
  // 기기 · 학부모 기기(+ who=all 이면 아이 기기). 스위치가 off 면 여기서 끝(발송 이력만)
  ...
  catch (e) {   // 키가 없는 것 같은 통째 실패 · 발송 이력에 남기고 큐 일도 실패로(원장님께 뜬다)
  ...
  for (const [profileId, logId] of logOf) {
    ...
    changed(await db(svc).from("notify_log").update(patch, { count: "exact" }).eq("id", logId), "발송 이력을 못 적음");
  }
  const also = await smsAlong(svc, { kind, studentId, url, smsLine, why }, env);   // (뎌-2) 켠 유형은 문자로도 · 앱을 안 깐 집에도 닿는다
  return { sink, logged: logs.length, sent, failed, ...(also ? { sms: also } : {}) };
}
/** (뎌-2) 앱 알림과 **함께** 문자 한 통 · 규칙(send.sms_kinds)이 켠 유형만.
 *  문구는 원장님이 고치는 v2.msg_template(sms_<유형>) · {{한 줄}} 은 그 유형을 아는 손이 준 것(비면 그 줄이 사라진다).
 *  문자가 안 돼도 **앱 알림은 이미 나갔다** · 여기서 던지지 않고 까닭만 돌려준다(발송 이력은 sms() 가 남긴다) */
export async function smsAlong(svc, { kind, studentId, url = "/parent", smsLine = null, why = null }, env = process.env) {
  try {
    const [kinds, st] = await Promise.all([
      ruleList(svc, "send.sms_kinds").catch(() => []),
      db(svc).from("students").select("name,parent_phone").eq("id", studentId).maybeSingle(),
    ]);
    if (!smsAlso(kinds, kind)) return null;
    if (st?.error) throw new Error(`아이를 못 읽음: ${saidBy(st.error.message)}`);
    const to = st?.data?.parent_phone;
    if (!phoneDigits(to)) return { skipped: "학부모 전화가 없습니다" };
    if (!(await smsReady(svc))) return { skipped: "솔라피 키가 아직 없습니다" };
    const [tpl, academy] = await Promise.all([smsTemplate(svc, `sms_${kind}`), academyName(svc)]);
    const f = fill(tpl, { 학원명: academy, 이름: st?.data?.name ?? "", "한 줄": smsLine ?? "" });
    if (f.missing.length) return { skipped: `치환 칸이 빕니다: ${f.missing.join(" · ")}` };
    return await sms(svc, { kind, to, text: f.text, studentId, url, why }, env);
  } catch (e) { return { skipped: String(e?.message ?? e).slice(0, 200) }; }
}
...
/** (커) 문자 한 통(솔라피 · 확정-71) · 발송 이력을 먼저 남기고(channel sms · 번호는 가려서), 스위치가 live 일 때만 나간다.
 *  off 면 발송 이력만(리허설이 진짜 문자가 되는 일이 구조적으로 없다 · 0069 와 같은 뜻) · self 는 문자에 「학원 사람 기기」가 없어 안 보낸다.
 *  치환 칸이 남아 있으면 못 나간다(뼈대-11). 서버 자신(service role)만 부른다(0017) */
export async function sms(svc, { kind, to, text, studentId = null, profileId = null, why = null, url = "/parent", subject = null }, env = process.env) {
  ...
  if (/\{\{/.test(String(text ?? ""))) throw new Error("치환 칸이 남아 있습니다. 못 나갑니다(뼈대-11)");
  ...
  if (ins.error) throw new Error(`발송 이력을 못 남김: ${saidBy(ins.error.message)}`);
  const logId = ins.data.id;
  const fail = async (w) => { changed(await db(svc).from("notify_log").update({ failed_at: now, fail_why: String(w).slice(0, 300) }, { count: "exact" }).eq("id", logId), "발송 이력을 못 적음"); return { sink, logged: 1, sent: 0, failed: 1, why: w, to: masked }; };
  ...
  if (!cfg?.key || !cfg?.secret || !cfg?.from) return fail("솔라피 키가 없습니다. 연동(v2.integration) solapi 줄(key · secret · from)");
  const r = await sendOne(cfg, { to: digits, text, subject: subject ?? titleFor(kind, academy?.name) });
  if (!r.ok) return fail(r.why);
  changed(await db(svc).from("notify_log").update({ sent_at: now, failed_at: null, fail_why: null }, { count: "exact" }).eq("id", logId), "발송 이력을 못 적음");
