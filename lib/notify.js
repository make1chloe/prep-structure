...
export async function notify(svc, { kind, studentId, url = "/parent", tag = null, why = null, sheetId = null, jobId = null, who = "parent" }, env = process.env) {
  ...
  const [linksQ, academy, oldQ] = await Promise.all([
  ...
  const parents = [...new Set(rows(linksQ, "학부모").map((l) => l.parent_profile_id).filter(Boolean))];
  ...
  if (!parents.length) {
    if (!old.length) { const { error } = await db(svc).from("notify_log").insert({ ...base, profile_id: null, failed_at: now, fail_why: "학부모 계정이 이 아이와 이어져 있지 않습니다" }); if (error) throw new Error(`자취를 못 남김: ${error.message}`); }
    return { sink, logged: 1, sent: 0, failed: 1, why: "학부모 계정 없음" };
  }
  ...
  const orParts = [`profile_id.in.(${parents.join(",")})`, ...(who === "all" ? [`student_id.eq.${studentId}`] : [])];
