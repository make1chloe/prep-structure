...
import { examShort, wrongSummary, gradeByCuts, cutsFor, gradeText } from "./score-plan.js";
...
/** 성적 줄 — 짧은 이름 · 원점수 · 등급(학교 것 › 컷으로) · 틀린 영역(많은 순 · 첫째는 miss) */
export function scoreRows(scores = []) {
  return scores.filter((s) => s.exam).map((s) => { const e = { ...s.exam, questions: s.exam.questions ?? [] }; const g = s.grade ?? gradeByCuts(s.raw, cutsFor(e)); const sum = wrongSummary(s.wrongs ?? [], e.questions); return { id: s.id, title: examShort(e), raw: s.raw, full: s.full_score ?? 100, grade: g == null ? "—" : gradeText(e.level, g), sum, pending: s.confirmed === false, on: s.exam.english_on ?? s.exam.term_from }; });
}
...
export function historyLines(h = {}, st = {}) {
  ...
  for (const c of h.consults ?? []) out.push({ on: String(c.at).slice(0, 10), title: "상담", small: `${c.way ?? ""}${c.body ? ` · ${String(c.body).slice(0, 40)}` : ""}`, tag: null });
...
export function listRows(list = [], { q = "", show = "active" } = {}) {
  ...
  const rows = list.map((s) => ({ ...s, classText: classLine(s.class), gradeText: gradeText2(s.level, s.grade), booksText: (s.books ?? []).join(" · ") || "교재 없음", lastConsult: s.last_consult ? md(String(s.last_consult).slice(0, 10)) : "—", stateName: stateName(s.state) }));
...
/** 학생 아이디 제안 — 「chloe」 + 전화 뒤 4자리(없으면 오늘 MMDD). 꼴은 DB 한 곳(0033·0034 profiles_login_id_shape: chloe + 숫자 넷 — 「폰 뒤 4자리와 맞나」를 셀 수 있게) */
...
export const parseLoginId = (raw) => { let id = String(raw ?? "").trim().toLowerCase().replace(/\s+/g, ""); if (/^[0-9]{4}$/.test(id)) id = `chloe${id}`; if (!/^chloe[0-9]{4}$/.test(id)) throw new Error("학생 아이디는 chloe + 숫자 넷(예 chloe0515) — 폰 뒤 4자리로"); return id; };
