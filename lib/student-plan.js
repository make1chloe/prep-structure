/** 학생 14 판단 한 벌(순수, DB 없음 — 목업 14 · 답 ⑩(하원 날짜별) · 원장님 9/3 「퇴원해도 줄은 남는다」 · 대전제-12). 한 벌(student_board)의 재료로 KPI 여섯 · 교재 막대 · 성적 줄·약한 영역 · 이 달 출결 · 단원평가 알약 · 지나온 것 · 재원 기간 · 목록 줄 · 수정 양식 읽기를 센다 */
...
import { attendPicked } from "./day-plan.js";   // (어83) 「아직 안 찍음」(none)은 출결 셈에 안 든다 — 판이 서도 출결은 아니다(0180)
/** 첫 비밀번호 한 곳((어65) · 원장님 2026-09-16 「비번짧아서 계정 못만듬」 — 화면 사진 「계정을 못 만듦: Password should be at least 6 characters.」).
 *  인증(Supabase Auth)이 **여섯 자 미만을 아예 안 받는다.** 0000 이던 동안은 학생·학부모·직원 **어느 계정도 못 났다**
 *  (원장님이 (어54) 때 「계속안돼」 하신 것이 이것이었다). 규칙 password.min_len 도 처음부터 6 이었다.
 *  ⚠️ 화면이 이 글자를 제 손으로 또 적지 않는다(원칙-1) — 적는 곳마다 이것을 가져다 쓴다. 바꾸면 화면·문자가 함께 바뀐다.
 *  여전히 「같은 숫자만」이라 첫 로그인에 반드시 바꾼다(app/password 가 막는다 · must_change_pw). */
export const FIRST_PW = "000000";
...
/** KPI 여섯 · 숙제 % · 단어 통과율 · 단원평가 n/m · 출석 n/m(결석 k) · 3주 안 하원 지연 · 이달 경고(반성문). 재료가 0이면 「·」 */
export function kpis(k = {}, rules = {}) {
  ...
  return [
    ...
    { key: "ut", emo: "✍️", big: k.ut_total ? `${k.ut_pass}/${k.ut_total}` : "", small: "단원평가 통과", bad: Boolean(k.ut_total) && k.ut_pass < k.ut_total / 2 },
    { key: "att", emo: "🕘", big: k.att_total ? `${k.att_present}/${k.att_total}` : "", small: `출석${k.att_absent ? ` · 결석 ${k.att_absent}` : ""}`, bad: Number(k.att_absent ?? 0) >= 2 },
    { key: "late", emo: "⏰", big: `${k.late21 ?? 0}회`, small: `하원 지연 · ${days}일 안`, bad: Number(k.late21 ?? 0) >= repeat },
...
/** 성적 줄 · 짧은 이름 · 원점수 · 등급(학교 것 › 컷으로) · 틀린 영역(많은 순 · 첫째는 miss · 문항표는 적힌 것 › 모의고사 표준) · 추이(같은 유형 지난 회차 대비 ▲▼ · 07·09 와 같은 한 벌 withTrend) */
...
export function attendRow(list = []) { return list.filter((a) => attendPicked(a.attend)).map((a) => { const [icon, cls] = ATT_ICON[a.attend] ?? ["?", "i-cls"]; return { date: a.date, attend: a.attend, icon, cls, title: `${md(a.date)} ${a.attend}${a.arrived_at ? ` · 등원 ${hhmm(a.arrived_at)}` : ""}${a.left_at ? ` · 하원 ${hhmm(a.left_at)}` : ""}`, arrived: hhmm(a.arrived_at), left: hhmm(a.left_at) }; }); }
/** (뎌) 그 달 출결 요약 · 「출석 6 · 지각 1 · 결석 1 · 보강 1」. 목록 줄도 카드 머리도 이 한 벌을 쓴다(원칙-1).
 *  줄(att)은 판이 세어 준 것(0156) · 카드는 날짜 배열에서 셀 수도 있어 둘 다 받는다 */
export function attendSummary(att) {
  const a = Array.isArray(att)
    ? { came: att.filter((x) => x.attend === "present" || x.attend === "online").length, late: att.filter((x) => x.attend === "late").length,
        absent: att.filter((x) => x.attend === "absent").length, makeup: att.filter((x) => x.attend === "makeup").length,
        days: att.filter((x) => attendPicked(x.attend)).length }
    : { came: Number(att?.came ?? 0), late: Number(att?.late ?? 0), absent: Number(att?.absent ?? 0), makeup: Number(att?.makeup ?? 0), days: Number(att?.days ?? 0) };
  const bits = [];
  if (a.came) bits.push(`출석 ${a.came}`);
  if (a.late) bits.push(`지각 ${a.late}`);
  if (a.absent) bits.push(`결석 ${a.absent}`);
  if (a.makeup) bits.push(`보강 ${a.makeup}`);
  return { ...a, text: bits.join(" · ") || (a.days ? `수업 ${a.days}일` : "수업 기록 없음"), bad: a.absent > 0 };
}
...
/** 지나온 것 · 등원 · 반 이동 · 교재 배정·회독 올림 · 상담을 한 줄로 합쳐 새것부터 */
export function historyLines(h = {}, st = {}) {
  ...
  for (const b of h.books ?? []) out.push({ on: b.from_date, title: b.round > 1 ? "회독 올림" : "교재 배정", small: `${b.name} ${b.round}회독${b.to_date ? ` ~ ${md(b.to_date)}` : ""}`, tag: b.round > 1 ? `${b.round - 1}회독 진도도 남음` : null });
...
export function listRows(list = [], { q = "", show = "active" } = {}) {
  ...
  const rows = list.map((s) => ({ ...s, att: attendSummary(s.att), classText: classLine(s.class), gradeText: gradeText2(s.level, s.grade), booksText: (s.books ?? []).join(" · ") || "교재 없음", lastConsult: s.last_consult ? md(seoulDate(s.last_consult)) : "", stateName: stateName(s.state) }));
...
/** 날짜 꼴 — 비었으면 지나가고, 적혔으면 2015-03-21 꼴이라야 한다(들어온 날 · 생년월일) */
const ymd = (what, v) => { if (v && !/^\d{4}-\d{2}-\d{2}$/.test(String(v))) throw new Error(`${what} 꼴은 2015-03-21`); };
/** 수정 양식 읽기 — 이름 필수 · 학년 1~6(초)·1~3 · 전화는 숫자만(10~11자리) · 들어온 날은 날짜 */
export function parseStudent({ name, grade, schoolId, phone, parentPhone, memo, joinedOn, birth }) {
  ...
  ymd("들어온 날", joinedOn); ymd("생년월일", birth);   // (어72) 생년월일 — 아이도 채우고(07) 원장님도 고치신다(14) · 꼴 검사는 한 줄로(원칙-1)
  return { name: nm, grade: g, school_id: schoolId || null, phone: ph || null, parent_phone: pp || null, memo: String(memo ?? "").trim() || null, joined_on: joinedOn || null, birth: birth || null };
}
/** 학부모 계정 이름 읽기((가)-⑩ · 남긴 것 14) — 발급 때 「OO 학부모」로 박힌 것을 14 에서 고친다 · 앞뒤 공백 버리고 사이는 하나 · 빈 것 막음 · 20자까지 */
export const parseParentName = (raw) => { const nm = String(raw ?? "").trim().replace(/\s+/g, " "); if (!nm) throw new Error("학부모 계정 이름을 적으세요"); if (nm.length > 20) throw new Error("학부모 계정 이름은 20자까지"); return nm; };
/** 학생 아이디 제안 · 「chloe」 + 전화 뒤 4자리(없으면 오늘 MMDD). 꼴은 DB 한 곳(0034·0100·0168 profiles_login_id_shape: chloe + 숫자 넷 + 다섯째 숫자 하나((어46) 겹칠 때 2·3 … nextLoginId) · 옛 앱이 낸 -2·-3 도 그대로 받는다(실 DB chloe8729-2 · 2026-09-07)) */
...
export const parseLoginId = (raw) => { let id = String(raw ?? "").trim().toLowerCase().replace(/\s+/g, ""); if (/^[0-9]{4}$/.test(id)) id = `chloe${id}`; if (!/^chloe[0-9]{4}([0-9]|-[0-9]{1,2})?$/.test(id)) throw new Error("학생 아이디는 chloe + 숫자 넷(예 chloe0515 · 폰 뒤 4자리가 겹치면 다섯째 숫자 chloe05152)"); return id; };
/** (어46) 겹치는 아이디의 다음 것(순수 · 원장님 9/16 「같은 아이디 이미있으면 5번째에 2붙여서」) · chloe9837 → chloe98372 → chloe98373 … 9 까지 · 옛 -2 꼴은 -3 으로 */
export function nextLoginId(id) {
  const m = /^(chloe[0-9]{4})([0-9])?(?:-([0-9]{1,2}))?$/.exec(String(id ?? "")); if (!m) throw new Error(`아이디 꼴 아님: ${id}`);
  if (m[3] != null) return `${m[1]}-${Number(m[3]) + 1}`;
  const n = m[2] == null ? 2 : Number(m[2]) + 1; if (n > 9) throw new Error(`${m[1]} 아홉 개 다 참 · 다른 아이디로`);
  return `${m[1]}${n}`;
}
/** 비밀번호 초기화를 해도 되나 — 앱이 발급한 계정만(대전제-12: 이관된 재원생·학부모 계정은 전환일까지 안 건드린다) */
export const canReset = (p) => Boolean(p?.issued_by_app);
/** **어느 화면이 누구를 되돌릴 수 있나** — (어74) 원장님 2026-09-17 「강사조교 비밀번호 초기화 가능하게 해줘 이게 제일급해」.
 *  손은 한 벌(lib/student.js resetPassword)이고, 부르는 화면이 제 몫만 말한다 — 14 는 아이·학부모, 설정 👤 는 선생님·조교.
 *  ⚠️ **원장 줄은 어느 쪽에도 없다** — 원장님 비밀번호를 앱 안에서 되돌리는 길은 두지 않는다(잠기면 들어올 문이 없다). */
export const RESET_KID = Object.freeze(["student", "parent"]);
export const RESET_STAFF = Object.freeze(["instructor", "assistant"]);
export { md };
