...
/**
 * **지각인가 · 몇 분 늦었나 — 세어 나온다** (원칙 5. 아무 데도 저장하지 않는다).
 *
 * 원장님 2026-09-02: 「지각은 시간이 필요없을 듯」 —
 * 아이가 등원을 찍은 **그 시각이 곧 도착 시각**이다. 물어볼 것도, 담을 칸도 없다.
 * 그래서 앞서 여기 있던 `lateMinutes()`(손으로 10·20·30·60분을 고르게 하던 것)를 **없앴다.**
 *
 * ⚠️ 이 셈은 **`lib/arrival.js` 로 옮긴다** (지금 다른 사람이 짓는 중이다).
 *    그쪽이 서면 여기 것을 지우고 **거기 것을 부른다** — 두 벌로 두면 그날부터 어긋난다(원칙 1).
 * ⚠️ 「몇 분부터 지각인가」(봐주는 시간)는 원장님께 **못 여쭸다.** 지어내지 않고
 *    **1분이라도 늦으면 늦은 것**으로 센다. 봐주는 시간이 정해지면 여기 한 줄이다.
 * ⚠️ 「지각으로 찍을까」는 여기서 안 정한다 — 찍는 것은 사람이고(`attend:'late'`),
 *    이 함수는 **찍힌 두 시각을 견주어 말해 줄 뿐**이다.
 *
 * @param startTime 그 반 수업 시작 시각 — `v2.class_schedule.start_time` ('HH:MM' · 'HH:MM:SS')
 * @param stampedAt 아이가 등원을 찍은 시각 — `v2.arrival.at` 을 **학원 시각**으로 바꾼 'HH:MM'
 *        ⚠️ timestamptz → 학원 시각은 **SQL 에서** 한다. 시간대 규칙을 여기서 다시 세지 않는다.
 * @returns { late, minutes, arriveAt, why } — 모르면 `late:null` 이고 `why` 에 「확인 안 됨」이 적힌다
 */
export function lateFromStamp({ startTime = null, stampedAt = null } = {}) {
  const s = hmOf(startTime), a = hmOf(stampedAt);
  if (s === null || a === null) {
    // ⚠️ 확인 안 됨 — 없는 시각을 0 으로 치면 안 늦은 아이가 지각이 된다. 모른다고 말한다
    return {
      late: null, minutes: null, arriveAt: a === null ? null : toHm(a),
      why: "⚠️ 확인 안 됨 — " +
        (s === null ? "그 반 수업 시작 시각" : "아이가 등원을 찍은 시각") + "을 몰라 몇 분 늦었는지 못 셌다",
    };
  }
  const minutes = a - s;
  return { late: minutes > 0, minutes, arriveAt: toHm(a), why: null };
}
...
export async function attendanceWrite(db, one, opts = {}) {
  ...
  //    아이가 등원을 찍은 그 시각이 곧 도착 시각이고, 몇 분 늦었는지는 `lateFromStamp()` 로 **센다.**
  ...
  if (one.late !== undefined && one.late !== null) {
    throw new Error("지각에 「얼마나」는 없다 (원장님 2026-09-02) — 아이가 등원을 찍은 그 시각이 곧 도착 시각이다. " +
      "몇 분 늦었는지는 lateFromStamp() 로 **세어 나온다.** 담을 칸도 없다");
  }
