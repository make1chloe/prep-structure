 ...
 *  처분 셋: homework 다음 시간 숙제(숙제 줄로 선다) · stay 오늘 남아서 쓰기(하원 지연 사유에 붙는다) · defer 유예(미룬 것 · 다음 경고에 다시 묻는다). 면제는 없다.
...
/** 반성문 처분 · 오늘 수업 일지에서 원장님이 고른다. 남아서면 하원 지연 사유에, 숙제면 숙제 줄에 선다. 같은 날 다시 고르면 바꾼다 */
export async function reflect(sb, sheetId, disposal, by) {
  ...
  changed(await db(sb).from("reflection").upsert(row, { onConflict: "student_id,asked_on" , count: "exact" }), "반성문 처분을 못 씀");
  ...
  if (disposal === "homework") {
    ...
    else if (had.off) changed(await db(sb).from("day_item").update({ off: false }, { count: "exact" }).eq("id", had.id), "반성문 숙제 줄을 못 켬");
  } else if (had && !had.off) changed(await db(sb).from("day_item").update({ off: true }, { count: "exact" }).eq("id", had.id), "반성문 숙제 줄을 못 뺌");
...
export async function resetWarnings(sb, month, action, by) {
  ...
  changed(await db(sb).from("warn_reset").upsert({ student_id: null, month: m, action, by_who: by ?? null }, { onConflict: "student_id,month" , count: "exact" }), "정리를 못 적음");
}
