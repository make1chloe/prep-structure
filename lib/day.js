 ...
 *         `on conflict … do update … where day_item.status = 'none'` 이 그 자리다.
 ...
 * ⚠️ `where day_item.status = 'none'` — **이미 찍힌 줄은 안 건드린다.**
...
/** 빈 줄은 **지우는 것**이다 — 대전제 6은 「사실」에 걸리고, 안 적은 메모는 사실이 아니다 */
const 빈줄 = (v) => v == null || String(v).trim() === "";

const SQL_AREA_MEMO_READ = `
select area, memo, updated_at
  from v2.day_area_memo where sheet_id = $1::uuid order by area`;
...
const SQL_AREA_MEMO_DEL = `
delete from v2.day_area_memo where sheet_id = $1::uuid and area = any($2::text[])
returning area`;
...
/**
 * 영역 메모를 적는다. `{ 문법: "간접의문문 어순", 독해: "" }` — **빈 줄은 지운다.**
 *
...
export async function putAreaMemos(db, sheetId, memos = {}) {
  ...
  const 지울것 = 들어온.filter(([, v]) => 빈줄(v)).map(([k]) => k);
  const 적을것 = 들어온.filter(([, v]) => !빈줄(v));

  let removed = 0, wrote = 0;
  if (지울것.length) removed = (await db.query(SQL_AREA_MEMO_DEL, [sheetId, 지울것])).rows.length;
  if (적을것.length) {
