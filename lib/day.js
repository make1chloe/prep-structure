...
export async function roster(sb, date, people = null) {
  ...
  const [{ data: sheets, error }, books, quizzes, warns, units] = await Promise.all([
  ...
  return { ...p, classes: p.classes.map((c) => ({ ...c, students: c.students.map((s) => ({ ...s, books: books.filter((b) => b.student_id === s.id), quizzes: splitQuizzes(quizzes.filter((q) => q.student_id === s.id), date), warn: warns.find((w) => w.student_id === s.id) ?? null, unitTests: units.filter((u) => u.student_id === s.id), sheet: shape(bySheet.get(`${s.id}|${c.id}`)) })) })) };
}
