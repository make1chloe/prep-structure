 ...
 *  층(속도-상한 오늘 4단): ① 오늘 도는 반과 그 아이들(한 조회) → ② 판(항목·늦귀가 붙여서) ∥ 배정 교재 ∥ 시험 — 한 파도. 판이 있으면 여기서 끝 — 4단 = 로그인 확인 · 오늘 · ① · ②.
...
export async function roster(sb, date, people = null) {
  ...
  const [{ data: sheets, error }, books, quizzes] = await Promise.all([
  ...
  return { ...p, classes: p.classes.map((c) => ({ ...c, students: c.students.map((s) => ({ ...s, books: books.filter((b) => b.student_id === s.id), quizzes: splitQuizzes(quizzes.filter((q) => q.student_id === s.id), date), sheet: shape(bySheet.get(`${s.id}|${c.id}`)) })) })) };
}
