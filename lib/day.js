...
export async function saveComment(sb, sheetId, comment, meta = null) { await assertOpen(sb, sheetId); const { error } = await db(sb).from("day_sheet").update(commentPatch(comment, meta)).eq("id", sheetId); if (error) throw new Error(`글을 못 저장함: ${error.message}`); }
...
export async function closeSheet(sb, sheetId, comment, by, meta = null) {
  ...
  const { error } = await db(sb).from("day_sheet").update({ ...commentPatch(comment, meta), closed_at: new Date().toISOString(), closed_by: by }).eq("id", sheetId);
