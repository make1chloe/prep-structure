 ...
 * **화면에 늘어놓는 차례는 월요일부터다.**
 *
 * 위 DOW 는 `getDay()` 가 돌려주는 숫자의 자리다 — 0 이 일요일이라 반드시
 * 일요일로 시작해야 한다. 그런데 반 요일을 고르거나 달력 머리를 그릴 때는
 * 월요일부터 늘어놓는 것이 우리가 읽는 차례다.
 *
 * **뜻이 다르니 이름도 다르게 둔다.** 이 둘을 섞으면 요일이 하루씩 밀리는데,
 * 오류는 안 나고 화면만 조용히 틀린다. 지금까지는 이 목록에 이름이 없어서
 * 아홉 파일이 각자 적어두고 있었다 (2026-08-09 전수검사).
 */
export const WEEK_ORDER = ["월", "화", "수", "목", "금", "토", "일"];
