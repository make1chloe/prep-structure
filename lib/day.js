...
export function shape(sheet) {
  ...
  return { ...sheet, items, check: by("check"), class: by("class"), home: by("home"), off: items.filter((i) => i.off), books: sheet.sheet_book ?? [], late: (sheet.late_stay ?? [])[0] ?? null, memos: sheet.day_area_memo ?? [], tests: [], closed: Boolean(sheet.closed_at) };   // 단어시험(word_test)은 판이 아니라 학생·날짜에 붙는다(0044 뒤) — 시험 카드를 지을 때 따로 읽는다
}
