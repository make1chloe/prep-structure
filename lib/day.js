...
async function examsSoon(sb, date) {
  ...
  const { data, error } = await db(sb).from("exams").select("id,scope,school_id,grade,name,term_from,term_to,english_on,state,hidden,exam_skip(student_id,skipped)")
