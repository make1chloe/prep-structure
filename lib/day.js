...
export async function roster(sb, date, people = null) {
  ...
  const { data: sheets, error } = await db(sb).from("day_sheet").select("*,day_item(*),late_stay(*)").eq("date", date).in("student_id", p.studentIds);
  ...
  return { ...p, classes: p.classes.map((c) => ({ ...c, students: c.students.map((s) => ({ ...s, sheet: shape(bySheet.get(`${s.id}|${c.id}`)) })) })) };
}
...
async function openSheets(sb, pairs, date) {
  ...
  return (made.data ?? []).map((s) => ({ ...s, day_item: items.filter((i) => i.sheet_id === s.id), late_stay: [] }));
}
...
function shape(sheet) {
  ...
  return { ...sheet, items, check: by("check"), class: by("class"), home: by("home"), late: (sheet.late_stay ?? [])[0] ?? null, tests: [], closed: Boolean(sheet.closed_at) };   // 단어시험(word_test)은 판이 아니라 학생·날짜에 붙는다(0044 뒤) — 시험 카드를 지을 때 따로 읽는다
}
