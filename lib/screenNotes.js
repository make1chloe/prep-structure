...
export const NOTE_GROUPS = [
  ...
  },
  {
    key: "menu",
    label: "선생님 메뉴",
    hint: "묶음 화면(오늘 · 학생 · 교재 …)을 열었을 때 맨 위에 뜹니다. 조교·강사에게 하는 말을 적어두시면 됩니다.",
    spots: [
      { key: "menu.today", label: "오늘", where: "오늘 묶음 화면" },
      { key: "menu.students", label: "학생", where: "학생 묶음 화면" },
      { key: "menu.books", label: "교재", where: "교재 묶음 화면" },
      { key: "menu.calendar", label: "일정", where: "일정 묶음 화면" },
      { key: "menu.send", label: "발송", where: "발송 묶음 화면" },
      { key: "menu.manage", label: "운영", where: "운영 묶음 화면" },
      { key: "menu.settings", label: "설정", where: "설정 묶음 화면" },
    ],
  },
];
...
/**
 * 이 자리에 적어둔 것 — 없으면 빈 문자열.
 *
 * 예전에는 `fallback` 을 세 번째로 받아 「없으면 원래 문구」 를 돌려주게
 * 돼 있었다. 그런데 **부르는 23곳이 하나도 그 인자를 안 넘겼다** (전부
 * 기본값 ""). 문서에 적힌 동작이 한 번도 일어난 적이 없는 것이다 —
 * 그런 인자는 다음 사람에게 「원래 문구를 여기 넣으면 되는구나」 라는
 * 틀린 길을 알려준다. 화면은 빈 값이면 ScreenNote 를 아예 안 그린다.
 */
export function noteOr(notes, key) {
