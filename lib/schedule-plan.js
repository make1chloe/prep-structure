/** 일정 판단 한 벌(순수, 목업 12) — 달력 42칸의 일들 · 반 회차(8회 채우기 — 대시보드 「못 채웁니다」도 같은 셈 (처)) · 하루의 줄 · 범례. 정상 수업은 안 띄운다(당연한 것이니까). 세는 것은 세어 나온다(원칙-5) */
...
/** 일의 유형 · 칸 안 표시의 차례(휴강 › 결석 › 영어 시험일 › 시험 기간 › 보강 › 지각 › 업무) · 그림 · 범례 이름 */
export const EVENT = Object.freeze([["hol", "🚫", "휴강"], ["abs", "✕", "결석"], ["exam2", "🅰️", "영어 시험일"], ["exam", "📝", "시험 기간"], ["mk", "↻", "보강"], ["late", "⏰", "지각"], ["todo", "📋", "업무"]]);
export const LEGEND = Object.freeze([["abs", "✕", "결석"], ["late", "⏰", "지각"], ["mk", "↻", "보강"], ["exam", "📝", "시험 기간"], ["exam2", "🅰️", "영어 시험일"], ["todo", "📋", "업무"], ["hol", "🚫", "휴강"]]);
...
export function eventsOf(date, b, classId = null) {
  ...
  for (const e of (b.exams ?? []).filter((e) => e.english_on === date)) out.push({ kind: "exam2", text: `🅰️ 영어 시험일 · ${e.school ?? e.name}`, id: e.id });
...
/** 고른 날의 줄(목업 12 아래 판) · 결석(아이마다 사유 → 보강 잡힘/안 잡힘) · 지각 · 보강 · 시험(기간 · 영어 시험일 · 출처) · 업무 · 휴강 */
export function dayRows(date, b, classId = null) {
  ...
  for (const e of (b.exams ?? []).filter((e) => examOn(e, date) || e.english_on === date))
    rows.push({ kind: e.english_on === date ? "exam2" : "exam", icon: e.english_on === date ? "🅰️" : "📝", title: `${e.english_on === date ? "영어 시험일" : "시험"} · ${examTitle(e)}${e.grade ? " · " + e.grade : ""}`, small: `${e.term_from ? `${md(e.term_from)}~${md(e.term_to)}` : ""}${e.english_on ? ` · 영어 ${md(e.english_on)}` : " · 영어 시험일 없음"}`, tag: e.source === "neis" ? "나이스" : e.source === "site" ? "홈페이지" : "손으로", id: e.id });
...
/** (처) 「화·목반 7회 — 8회를 못 채웁니다」 — 대시보드 17 이 이 달의 정규반 가운데 기준(규칙 schedule.sessions_per_month)에 못 미치는 것을 먼저 말한다(남긴 것 17).
 *  셈은 dash_ops.classes(session_count · class_extra_days 한 곳 — 이 달에 살아 있는 시간표가 있는 정규반만 · 0153) · 판단은 12 반 회차 카드와 같은 sessionsOf 한 벌 */
export function shortClasses(classes) {
  const target = Number(classes?.target ?? 8) || 8;
  return (classes?.rows ?? []).map((c) => ({ c, s: sessionsOf(c, target) })).filter(({ c, s }) => c.kind !== "special" && s.short > 0)
    .map(({ c, s }) => ({ id: c.id, ym: classes?.ym ?? null, n: s.n, short: s.short, target, text: `${classText(c)} ${s.n}회 · ${target}회를 못 채웁니다` }));
}
/** ── 그 달 일정 확정 도장(4단계-3b · 원장님 9/7 ㉚ 「예상수업일정보내기 기능 필요없음 무조건확정후 알림」) — 도장은 확정 하나(month_confirm step 3) · 살아 있는 반마다 한 줄 · 휴강이 들어오면 풀린다(0075 방아쇠 · undone_at) · 닫힌 반의 도장은 안 센다 */
const whenShort = (ts) => (ts ? `${md(seoulDate(ts))} ${seoulTime(ts)}` : "");
export function confirmState(stamps = [], classes = []) {
  const ids = new Set((classes ?? []).map((c) => c.id)), all = ids.size;
  const mine = (stamps ?? []).filter((s) => ids.has(s.class_id));
  const live = mine.filter((s) => !s.undone_at), undone = mine.filter((s) => s.undone_at);
  const last = (xs, k) => (xs.length ? xs.map((s) => String(s[k])).sort().at(-1) : null);
  if (!all) return { state: "none", ok: 0, all: 0, at: null, undone_at: null };
  if (live.length === all) return { state: "ok", ok: all, all, at: last(live, "at"), undone_at: null };
  if (undone.length) return { state: "undone", ok: live.length, all, at: last(live, "at"), undone_at: last(undone, "undone_at") };
  if (live.length) return { state: "partial", ok: live.length, all, at: last(live, "at"), undone_at: null };
  return { state: "none", ok: 0, all, at: null, undone_at: null };
}
/** 지난 달은 확정하지 않는다 — 이 달·앞달만 */
export const canConfirm = (ym, today) => String(ym) >= String(today).slice(0, 7);
/** 머리 글과 단추 — 「10월 일정 아직 확정 안 함」+「✅ 10월 일정 확정 → 학부모 알림」 · ok 「✅ 10월 일정 확정 9/21 14:00 · 알림 3명」(단추 없음) · undone 「⚠️ 휴강이 들어와 10월 확정이 풀렸습니다(9/22 10:00) — 다시 확정」 · partial 「⚠️ 10월 — 반 3 중 1 아직 확정 안 함」 */
export function confirmText(st, ym, sent = 0) {
  const m = Number(String(ym).slice(5, 7));
  if (!st.all) return { state: st.state, text: `${m}월 · 반이 없습니다`, bad: false, button: null };
  if (st.state === "ok") return { state: st.state, text: `✅ ${m}월 일정 확정 ${whenShort(st.at)} · 알림 ${sent}명`, bad: false, button: null };
  if (st.state === "undone") return { state: st.state, text: `⚠️ 휴강이 들어와 ${m}월 확정이 풀렸습니다(${whenShort(st.undone_at)}) · 다시 확정`, bad: true, button: "다시 확정 → 학부모 알림" };
  if (st.state === "partial") return { state: st.state, text: `⚠️ ${m}월 · 반 ${st.all} 중 ${st.all - st.ok} 아직 확정 안 함`, bad: true, button: "다시 확정 → 학부모 알림" };
  return { state: st.state, text: `${m}월 일정 아직 확정 안 함`, bad: true, button: `✅ ${m}월 일정 확정 → 학부모 알림` };
}
