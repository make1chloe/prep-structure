...
export const EVENT = Object.freeze([["hol", "🚫", "휴강"], ["abs", "✕", "결석"], ["exam2", "📝", "영어 시험일"], ["exam", "📝", "시험 기간"], ["mk", "↻", "보강"], ["late", "⏰", "지각"], ["todo", "📋", "업무"]]);
export const LEGEND = Object.freeze([["abs", "✕", "결석"], ["late", "⏰", "지각"], ["mk", "↻", "보강"], ["exam", "📝", "시험 기간"], ["exam2", "📝", "영어 시험일"], ["todo", "📋", "업무"], ["hol", "🚫", "휴강"]]);
...
export function dayRows(date, b, classId = null) {
  ...
  for (const e of (b.exams ?? []).filter((e) => examOn(e, date) || e.english_on === date))
    rows.push({ kind: e.english_on === date ? "exam2" : "exam", icon: "📝", title: `${e.english_on === date ? "영어 시험일" : "시험"} · ${examTitle(e)}${e.grade ? " · " + e.grade : ""}`, small: `${e.term_from ? `${md(e.term_from)}~${md(e.term_to)}` : ""}${e.english_on ? ` · 영어 ${md(e.english_on)}` : " · 영어 시험일 없음"}`, tag: e.source === "neis" ? "나이스" : e.source === "site" ? "홈페이지" : "손으로", id: e.id });
