 ...
 * ── 조심한 것 ───────────────────────────────────────────────
 *
 * **학교와 학년이 서로 다른 줄이 있다.** 「대건고1 / 중3」처럼. 문의받을 때
 * 적은 학교와 나중에 고친 학년이 부딪힌 것이다. 어느 쪽이 맞는지 우리는 모른다
 * — 그래서 **학년 칸을 쓰고, 어긋난 줄은 화면에 표시한다.** 조용히 하나를
 * 고르면 나중에 「왜 중3이지」 를 혼자 찾으셔야 한다.
 *
 ...
 *   입학결정      → 등록
 *   방문취소      → 미등록
 *   방문상담완료  → 상담 완료 (레테 날짜가 있으면 레벨테스트)
 *   방문전        → 문의종료면 미등록, 방문 약속이 있으면 상담 예정, 아니면 신규
 */
export function statusOf(stage, closed, hasVisit, hasTest) {
  const t = s(stage);
  if (t === "입학결정") return "enrolled";
  if (t === "방문취소") return "declined";
  if (t === "방문상담완료") return hasTest ? "tested" : "consulted";
  if (closed) return "declined";
  return hasVisit ? "scheduled" : "new";
}
...
export function parseInquiryAoA(aoa) {
  ...
  aoa.slice(1).forEach((cells) => {
    ...
    const name = rawName.replace(/\s*(임시|가등록)\s*$/, "").trim();
    if (!name) return;
    ...
    const grade = gradeCol || sc.grade || null;
    ...
    out.push({
      ...
      school: sc.school || null,
      grade,
      // 학교에서 뽑은 학년과 학년 칸이 어긋난 줄 — 화면에 표시한다
