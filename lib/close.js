...
const Q_LATE = `
  select id, reason, until_at, left_at, sent_at from v2.late_stay where sheet_id = $1`;
...
// ── 판을 쓰는 문 ────────────────────────────────────────────────
// ⚠️ 되돌릴 수 없는 자리다. `on conflict` 로 두 번 눌러도 같은 결과가 되게 한다
const Q_PROG_UP = `
  insert into v2.progress(student_id, unit_id, round, status, done_on, last_by, confirmed)
  select $1, u, $3::smallint, 'done', $4::date, 'staff', true from unnest($2::uuid[]) u
  on conflict (student_id, unit_id, round)
  do update set status = 'done', done_on = excluded.done_on, last_by = 'staff', confirmed = true
  returning unit_id`;
...
export async function closeSheet(db, sheetId, { by = null, confirm = [], expect = null, now = null, tx = true } = {}) {
  ...
  try {
    ...
    for (const b of gate.preview.autoDone) {
      const ids = b.units.map((u) => u.unit_id);
      const r = (await db.query(Q_PROG_UP, [gate.sheet.student_id, ids, b.round, gate.sheet.date])).rows;
      if (r.length !== ids.length) {
        if (tx) await db.query("rollback");
        return { ok: false, why: "progress_short", book_id: b.book_id, want: ids.length, got: r.length, gate };
      }
