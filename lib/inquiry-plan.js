...
export function parseConvert({ classId, bookIds = [], loginId, joinedOn }) {
  if (!classId) throw new Error("반을 고르세요");
  const ids = [...new Set((bookIds ?? []).filter(Boolean))];
  let id = String(loginId ?? "").trim().toLowerCase(); if (/^[0-9]{4}$/.test(id)) id = `chloe${id}`; if (!/^chloe[0-9]{4}$/.test(id)) throw new Error("학생 아이디는 chloe + 숫자 넷(예 chloe0515)");
  if (joinedOn && !/^\d{4}-\d{2}-\d{2}$/.test(String(joinedOn))) throw new Error(`날짜가 아닙니다: ${joinedOn}`);
  return { classId, bookIds: ids, loginId: id, joinedOn: joinedOn || null };
}
