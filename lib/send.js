...
import { TEMPLATES, lenText, lateLine, SMS_ALSO } from "./sms-plan.js";   // (커) ✉️ 문자 문구 · 글자 수·유형은 화면과 같은 셈
...
async function sheetOf(svc, id) {
  ...
  if (error) throw new Error(`수업 일지를 못 읽음: ${error.message}`); if (!data) throw new Error(`수업 일지가 없습니다: ${id}`);
  return data;
}
// ── 손 · 크론·백스톱·지금 보내기가 runDue 로 부른다(뼈대-9). 손이 없는 유형은 큐가 실패로 남긴다
handlers[KINDS.daily] = family(async ({ sheet_id }, { sb, row }) => {
  ...
  const holes = unfilled(s.comment); if (holes.length) throw new Error(`안 채운 치환 칸 · {{${holes.join("}} {{")}}}`);   // 뼈대-11: 보내지 않고 원장님께 되돌린다
...
handlers[KINDS.late] = family(async ({ sheet_id, late_id }, { sb, row }) => { const s = await sheetOf(sb, sheet_id);
  const late = row0(await db(sb).from("late_stay").select("until_at,reason").eq("id", late_id).maybeSingle(), "하원 지연 줄");   // (뎌-2) 문자 「한 줄」 · 그 유형을 아는 자리가 채운다
...
/** (어96) 배부 안내 — 원장님 2026-09-19 「내가 완료처리하면 ① 학생에게 알림이가고」.
 *  **아이 제 계정으로** 간다(who: "student") · 문은 여느 알림과 같아 **방해금지에 함께 걸린다**(family). */
handlers[KINDS.give] = family(async ({ student_id, material_id }, { sb, row }) => notify(sb, { kind: "give", studentId: student_id, who: "student", url: "/me", tag: `give-${material_id}-${student_id}`, why: { table: "material", id: material_id }, jobId: row.id }));
/** 예약이 때가 되면 큐로 · 한 바퀴 앞에 돈다(크론·백스톱·지금 보내기 모두). sent_at 은 「큐에 넘긴 때」, 실제로 나간 때는 발송 이력(notify_log.sent_at) */
export async function promoteScheduled(svc) {
  ...
  for (const r of data ?? []) {
    ...
    else continue;   // 손이 없는 유형은 그대로 둔다(누가 봐도 안 나간 채로)
...
/** 발송 판 · 파도 2(속도-상한 발송 6: 껍질 1 · 로그인 1 · 오늘 1 · 오늘 수업 일지 1 · send_board 1 = 5): 오늘 수업 일지 ∥ 한 벌(큐 · 발송 이력 · 예약 · 닿는 길 · 규칙). 전부 학원 사람 자격 */
export async function sendBoard(sb, date, env = process.env) {
  const [sheetsQ, boardQ, phQ] = await Promise.all([
    ...
    db(sb).from("placeholder").select("key,note,example,sort").order("sort"),   // 뼈대-8 치환 칸 설명은 표에
  ]);
  if (boardQ.error) throw new Error(`발송 화면을 못 읽음: ${boardQ.error.message}`);
  const b = boardQ.data ?? {};
  const placeholders = phQ.error ? [] : (phQ.data ?? []);   // 표가 없는 DB(㉖ 전)면 빈 목록
  const now = new Date().toISOString();
  const sheets = rows(sheetsQ, "오늘 수업 일지").map((s) => ({ ...s, student_name: s.students?.name ?? "?", late: (s.late_stay ?? [])[0] ?? null }));
...
async function closedSheets(sb, sheetIds) {
  ...
  const bad = ids.map((id) => { const s = (data ?? []).find((x) => x.id === id); return !s ? "없는 수업 일지" : !s.closed_at ? "마감 안 한 수업 일지" : unfilled(s.comment).length ? "치환 칸 안 채운 수업 일지" : null; }).filter(Boolean);
  if (bad.length) throw new Error(`${[...new Set(bad)].join(" · ")} · 마감하고 치환 칸을 채운 뒤 보냅니다`);
  return ids;
}
/** 📨 선택한 것 지금 보내기 — 마감한 판만 → 큐 → 바로 한 바퀴(서버 답을 기다린다, 속도-5) */
/** (어39) 나가기 전 글 수정 · 발송 10 의 줄을 펼쳐 그 자리에서(원장님 9/15 「토글을 써서 필요시 확인하고 수정하는 것까지」). 마감된 판도 **글만** 고친다(마감 자체는 그대로 · 검사-⑤ 「마감된 판은 고칠 수 없다」의 유일한 예외 · 큐에 든 것은 나갈 때 그 글을 읽는다) */
export async function editDaily(sb, sheetId, comment) {
  const { data: s, error } = await db(sb).from("day_sheet").select("id").eq("id", sheetId).maybeSingle();
  if (error) throw new Error(`수업 일지를 못 읽음: ${error.message}`); if (!s) throw new Error("수업 일지가 없습니다");
  changed(await db(sb).from("day_sheet").update({ comment: String(comment ?? "") }, { count: "exact" }).eq("id", sheetId), "글을 못 저장함");
}
...
export async function schedule(sb, sheetIds, choice, custom, by, today) {
  ...
  if (error) throw new Error(error.code === "23505" ? "이미 예약된 수업 일지가 있습니다. 예약된 것에서 취소한 뒤 다시" : `예약을 못 넣음: ${error.message}`);
  return { n: (data ?? []).length, at };
}
/** (어) 월간 리포트·수강료 안내 예약(확정-㉕) · 아이·달로 맨다(body = 달). 살아 있는 예약은 아이·유형·달마다 하나(0151 색인). 때가 되면 promoteScheduled 가 그때 상태로 보낸다 */
export async function scheduleFor(sb, kind, ym, studentIds, choice, custom, by, today) {
  if (!["monthly", "fee"].includes(kind)) throw new Error(`예약할 수 있는 유형이 아닙니다: ${kind}`);
...
/** 다시 보내기 · 발송 이력의 why 그대로 같은 유형의 일을 큐에 다시 넣고 바로 한 바퀴 */
export async function resend(sb, logId) {
  ...
  if (error) throw new Error(`발송 이력을 못 읽음: ${error.message}`); if (!l) throw new Error("발송 이력이 없습니다");
  ...
  const job = l.kind === "daily" ? [KINDS.daily, { sheet_id: l.sheet_id }]
    ...
    : null;
  if (!job) throw new Error(`다시 보낼 수 없는 유형: ${l.kind}`);
  await enqueue(sb, job[0], job[1], l.why ?? null);
  const svc = serviceClient();
  return runDue(svc, await todayOf(svc));
}
/** (커) ✉️ 문자 문구 저장 · 원장님이 발송 10 에서 고친다(확정-71). 틀은 v2.msg_template 한 곳(0154) · 치환 칸은 그대로 둔다(보낼 때 채운다) */
/** (뎌-2) 앱 알림과 **함께 문자로도** 보낼 유형 · 발송 10 에서 켜고 끈다(규칙 send.sms_kinds 한 줄 · 코드에 안 박는다) */
export async function setSmsKinds(sb, kinds) {
  ...
  await setRule(sb, "send.sms_kinds", list.join(","));   // (어72) 규칙 쓰기는 lib/rule.js 한 곳(원칙-1)
  return { kinds: list };
}
