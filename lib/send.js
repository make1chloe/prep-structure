...
export async function sendBoard(sb, date, env = process.env) {
  const [sheetsQ, boardQ] = await Promise.all([
  ...
  return { date, now, rules, sink: sinkOf(env), reach: b.reach ?? null,
           late: lateRows(sheets, logs), daily: dailyRows(sheets, logs, sch, jobs), auto: autoRows(jobs, now), scheduled: scheduledRows(sch, jobs, sheets, now, date), sent: sentRows(logs) };
}
