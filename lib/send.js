...
import { TEMPLATES, lenText } from "./sms-plan.js";   // (커) ✉️ 문자 문구 — 글자 수·갈래는 화면과 같은 셈
...
export async function sendBoard(sb, date, env = process.env) {
  ...
  return { date, now, rules, sink: sinkOf(env), reach: b.reach ?? null,
           ...
           templates: b.templates ?? [], smsReady: Boolean(b.sms_ready) };   // (커) ✉️ 문자 문구 · 문자 길(솔라피) — 판에 얹었다(조회를 안 늘린다)
}
