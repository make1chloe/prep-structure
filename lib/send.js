...
import { outboundAllowed, BLOCKED_DETAIL } from "./notify.js";
...
function sayWhy(body, status) {
  ...
  // 서명이 안 맞는 것은 거의 언제나 **Key 와 Secret 의 짝이 어긋난 것**이다.
  // (키를 새로 발급받고 Key 만 바꿔 넣으면 이렇게 된다)
...
export async function listAlimtalkTemplates(cfg) {
  ...
  // **쿼리 파라미터를 붙이지 않는다.**
  //   limit 도 pfId 도 안 받는다고 거절당했다. 솔라피가 무엇을 받는지
  //   추측하다 세 번 틀렸으니, 아예 안 붙이고 **받은 것을 우리가 거른다.**
  //   목록이 길어야 몇십 건이라 다 받아도 된다.
  let res;
  let body = null;
  try {
    res = await fetch(`${SOLAPI}/kakao/v2/templates`, {
      headers: { Authorization: hmacHeader({ apiKey, apiSecret }) },
    });
  ...
  const rows = list
    ...
    // 다른 발신프로필 것이 섞여 오면 뺀다 (그 채널로는 안 나간다).
    // pfId 를 안 알려주는 답이면 거르지 않는다 — 없는 것을 없다고 단정하지 않는다
    .filter((t) => !t.pfId || t.pfId === pfId)
    .map((t) => {
 ...
 * 설정에 따라 실제로 보낸다.
 *
 * **밖으로 나가는 것은 신규 상담뿐이다** (원장님, 2026-08-06). 재원생과 그
 * 학부모께는 앱 안 알림으로 간다 (lib/notify). 부르는 쪽에서 이미 갈라놓지만,
 * 여기서 한 번 더 막는다 — 새 발송 자리를 만들면서 audience 를 안 적으면
 * **아무 말 없이 학부모 폰으로 나가버린다.** 모를 때는 안 보내는 쪽이 맞다.
 *
 * @param meta.audience "inquiry" 만 밖으로 나간다. 안 적으면 안 나간다.
 */
export async function deliver(settings, list, meta = {}) {
  if (!outboundAllowed(meta.audience)) {
    return {
      channel: "app",
      results: list.map((m) => ({ ref: m.ref, ok: false, detail: BLOCKED_DETAIL })),
    };
  }
