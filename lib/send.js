...
/** (어39) 나가기 전 글 고치기 · 발송 10 의 줄을 펼쳐 그 자리에서(원장님 9/15 「토글을 써서 필요시 확인하고 수정하는 것까지」). 마감된 판도 **글만** 고친다(마감 자체는 그대로 · 검사-⑤ 「마감된 판은 고칠 수 없다」의 유일한 예외 · 큐에 든 것은 나갈 때 그 글을 읽는다) */
...
export async function setSmsKinds(sb, kinds) {
  ...
  changed(await db(sb).from("rule").update({ value: list.join(",") }).eq("key", "send.sms_kinds").select("key"), "문자 유형을 못 적음");
  return { kinds: list };
}
