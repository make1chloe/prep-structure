 ...
 *  판단은 새로 없다(원칙-1): 꼴 옮기기는 lib/site-plan.js, 무엇이 시험인지·이름·기간·옛 줄에 연결은 lib/neis-plan.js 그대로.
 *  받는 길은 app/api/site/route.js 하나 · 키는 확장 키(v2.integration 'classcard' token · 확장 한 벌이 둘 다 쓴다). */
...
export async function receiveSite(svc, body, today) {
  ...
  await syncAllStops(svc, today);   // 기간이 바뀌었을 수 있다 · 교재 보류 창을 다시 맞춘다(나이스와 같은 걸음)
