import { itemTitle, itemSub, itemLine } from "./item-plan.js";   // 항목 줄 글 한 벌((어27))
/** 하원 지연 판단 한 벌(순수) · 예상 귀가(약속)와 실제 하원의 차이 · 반복 띠 · 마감 전에 한 번 묻는 것(확정-⑭ · 확정-64). 화면(브라우저)과 검사가 같이 쓴다.
...
/** 반복 띠 · SQL 이 센 것(stayed · window_days · repeat_at · ask)을 말로. 「3주 안 3번째 남습니다 · 숙제량을 볼까요?」. 묻지 않을 때 null */
...
/** 마감 전에 한 번 묻는 것 · AI 초안 그대로(확정-64) · 안 보낸 하원 지연(확정-⑭). 둘 다 걸리면 한 상자에 같이 묻는다(두 번 안 묻는다). 막지 않는다. 없으면 [] */
...
export function reasonChips({ checks = [], warn = null, reason = "" } = {}) {
  const out = [];
  for (const c of checks ?? []) if (c?.status === "missing") out.push({ key: `miss:${c.id}`, text: `${itemLine(c)} 미제출` });   // 검사 카드가 보이는 글 그대로(제목 · 손 글 · lib/item-plan 한 벌)
...
export function stayRows(items = []) {
  return (items ?? []).filter((i) => i.slot === "stay").sort((a, b) => (a.sort ?? 0) - (b.sort ?? 0)).map((i) => ({
    id: i.id, text: itemTitle(i), sub: itemSub(i), from: i.carry_of ? "숙제에서 옮겨옴" : null,
