...
export function mapRows(headerCells, dataRows) {
...
// 붙여넣기 텍스트(탭/콤마) -> { headers, fields, rows }
export function parseTable(text) {
  ...
  const dataRows = lines.slice(1).map((line) => line.split(delim));
  return mapRows(headerCells, dataRows);
}
