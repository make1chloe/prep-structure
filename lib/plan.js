...
export async function plansToday(sb, date) {
  const [abs, mk, late] = await Promise.all([
    ...
    db(sb).from("makeup").select("id,student_id,of_date,on_date,at_time,state,students!inner(id,name,grade,state,school_id,quiz_pos)").eq("on_date", date).eq("state", "set").eq("students.state", "active"),
...
/** 02c 가 여는 것 — 그 아이의 한 달(앞뒤 여유) 수업일·휴강·보강 · 결석 예정 · 지각 예정 · 반 요일·시각 글 · (저) 이 아이가 보는 회차의 시험 기간(0152 student_exams — 📝 표시만) */
export async function planOpen(sb, studentId, ym) {
  ...
  const [days, abs, lates, sched, exams] = await Promise.all([
    ...
    db(sb).rpc("student_exams", { p_student: studentId, p_from: from, p_to: to }),   // (저) 이 아이가 보는 회차의 시험 기간 — 📝 결석 예상(표시만 · 0152 · 보는 아이는 exam_takers 한 곳)
  ]);
  for (const r of [days, abs, lates, sched, exams]) if (r.error) throw new Error(`예정을 못 읽음: ${r.error.message}`);
  ...
  return { ym, days: (days.data ?? []).map((d) => ({ date: d.date, kind: d.kind, class_id: d.class_id, start_time: d.start_time })), absences: abs.data ?? [], lates: lates.data ?? [], exams: exams.data ?? [], label };
}
...
export async function planSave(sb, studentId, date, { kind, reason = null, makeupOn = null, makeupAt = null, waived = false, minutes = null }, by) {
  ...
  const cancelAbs = async () => { if (abs.data) { changed(await db(sb).from("makeup").update({ state: "cancelled" }, { count: "exact" }).eq("id", abs.data.id), "결석 예정을 못 물림"); } };
  const cancelLate = async () => { if (lt.data) { changed(await db(sb).from("late_plan").update({ cancelled_at: new Date().toISOString() }, { count: "exact" }).eq("id", lt.data.id), "지각 예정을 못 물림"); } };
  ...
  if (kind === "absent") {
    ...
    changed(abs.data ? await db(sb).from("makeup").update(row, { count: "exact" }).eq("id", abs.data.id) : await db(sb).from("makeup").insert(row, { count: "exact" }), "결석 예정을 못 씀");
    return { kind, state };
  }
  if (kind === "late") {
    await cancelAbs();
    ...
    changed(lt.data ? await db(sb).from("late_plan").update(row, { count: "exact" }).eq("id", lt.data.id) : await db(sb).from("late_plan").insert(row, { count: "exact" }), "지각 예정을 못 씀");
    return { kind };
  }
...
export async function planNotify(sb, studentId, date) {
  ...
  changed(await db(sb).from(table).update({ notified_at: new Date().toISOString() }, { count: "exact" }).eq("id", plan.id), "알림 때를 못 적음");
}
