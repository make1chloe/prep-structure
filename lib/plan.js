...
export async function planSave(sb, studentId, date, { kind, reason = null, makeupOn = null, makeupAt = null, waived = false, minutes = null }, by) {
  ...
  const cancelAbs = async () => { if (abs.data) { changed(await db(sb).from("makeup").update({ state: "cancelled" }, { count: "exact" }).eq("id", abs.data.id), "결석 예정을 못 취소함"); } };
  const cancelLate = async () => { if (lt.data) { changed(await db(sb).from("late_plan").update({ cancelled_at: new Date().toISOString() }, { count: "exact" }).eq("id", lt.data.id), "지각 예정을 못 취소함"); } };
