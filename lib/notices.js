...
export function kindOf(key) {
  return (
    NOTICE_KINDS.find((k) => k.key === key) ||
    // 옛 줄은 「수업 메모」 자리에 놓고 보여준다 (겸하고 있으므로)
    (key === LEGACY ? NOTICE_KINDS.find((k) => k.key === "memo") : null) ||
    NOTICE_KINDS[0]
  );
}

export function labelOf(key) {
  return key === LEGACY ? "전달사항 (옛 기록)" : kindOf(key).label;
}
