...
export async function sendLate(sb, sheetId) {
  ...
  const { error } = await db(sb).from("late_stay").update({ sent_at: new Date().toISOString() }).eq("id", cur.id);
  if (error) throw new Error(`보낸 때를 못 적음: ${error.message}`);
  return id;
}
...
export async function setLeft(sb, studentId, date, hhmm) {
  ...
  const { error } = await db(sb).from("arrival").upsert({ student_id: studentId, date, step: 4, at }, { onConflict: "student_id,date,step" });
  if (error) throw new Error(`실제 하원을 못 적음: ${error.message}`);
}
