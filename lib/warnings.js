...
export const DEFAULT_RULE = {
  ...
  wordPassPct: 90,      // 단어시험 통과선 — **오답 10% 이내(90%)가 통과** (원장님 2026-08-21 확정)
  weakAt: 2,            // 숙제 '미흡'은 몇 건부터 경고인가 (미제출은 1건부터)
  countLate: true,
  countHomework: true,
  countWordTest: true,
};
...
export function judge(rep = {}, items = [], rule = DEFAULT_RULE) {
  ...
  if (rule.countHomework) {
    ...
    // 안 해온 것은 한 건이라도 경고다.
    // 다만 **해오긴 했는데 부족한 것**까지 한 건에 경고를 주면 매일 경고가 뜬다.
    // 성실한 아이도 하루에 한 항목쯤은 아쉬운 날이 있다 — 그건 수업에서 잡는다.
    const at = Number(rule.weakAt) > 0 ? Number(rule.weakAt) : 2;
    if (missing > 0) reasons.push(`숙제 미제출 ${missing}건`);
    else if (weak >= at) reasons.push(`숙제 미흡 ${weak}건`);
  }
...
const SETTLE_KINDS = ["reflection", "defer", "reset"];
