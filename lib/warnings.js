...
//
import { passed } from "./wordTest.js";
...
export const DEFAULT_RULE = {
  ...
  wordWrongPct: 10,     // 단어시험 — **오답 10% 이내면 통과** (맞은 비율이 아니다)
  countLate: true,
  countHomework: true,
  countWordTest: true,
};
...
export function judge(rep = {}, items = [], rule = DEFAULT_RULE) {
  ...
  if (rule.countWordTest && rep.word_total > 0) {
    const ok = passed(rep.word_correct, rep.word_total, rule.wordWrongPct);
    if (ok === false) {
      const wrong = rep.word_total - (rep.word_correct ?? 0);
      reasons.push(`단어시험 ${wrong}개 틀림 (${rep.word_correct ?? 0}/${rep.word_total})`);
    }
  }
...
export function warningLines(state, rule = DEFAULT_RULE, forParent = true, date = null) {
  ...
  if (deferToday) {
    L.push(
      forParent
        ? `⚠ 경고 ${at}회가 되어 반성문 대상이었지만, 이번에는 유예했습니다.`
        : `⚠ 경고 ${at}회 — 이번엔 반성문을 유예해줬어요. 다음엔 꼭 지켜주세요.`
    );
    if (forParent) L.push("가정에서도 한 번 짚어주시면 좋겠습니다.");
    return L;
  }
  ...
  if (state.need) {
    ...
    const before = state.deferred ? "지난번에 한 번 유예해드렸습니다. " : "";
    L.push(
      forParent
        ? `${before}${left}회 더 쌓이면 반성문을 쓰게 됩니다.`
