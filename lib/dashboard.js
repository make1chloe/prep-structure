...
export async function loadDashboard(supabase, today = todaySeoul()) {
  ...
  const [
    ...
    // 최근 2주 리포트. 종전에는 attendance_kind 를 같이 읽고 실패 시
    // 칸 없이 한 번 더 묻는 사다리가 붙어 있었는데, 그 칸을 보던 것은
    // 등원 밀림 하나였다 (2026-08-29 그 카드를 없애면서 사다리도 걷었다)
    supabase.from("daily_reports").select("id, student_id, date")
    ...
    // 미래 판 보충 (§1-1 #15) — 오래 뜬 숙제·단어 셈의 원 조회는 상한이
    // 없어 미래 판(미리 내기)을 포함했다. warnRepQ 가 lte(today) 라 그
    // 미래분만 얇게 한 번 — 열·필터는 warnRepQ 와 같게 (archived_at is null).
    // attendance_kind 를 같이 읽던 사다리는 등원 밀림과 함께 걷었다 (8/29)
    supabase.from("daily_reports").select("id, student_id, date, word_correct, word_total")
  ...
  const [itemsQ, futureItemsQ, bq, uq, pq2] = await Promise.all([
  ...
  /**
   * ── **단원평가에 막힌 아이는 이제 할일이 말한다** (2026-08-29) ──
   *
   * 여기 카드가 있었다 (2026-08-06 ~ 08-29). 없앤 까닭은 **거기서 할 수
   * 있는 일이 없어서**다 — 원장님이 하실 일은 재시험지를 만드는 것 하나인데
   * 카드는 체크가 안 되니, 만들어 드린 뒤에도 다음 시험을 볼 때까지 그대로
   * 떠 있었다. 끌 수 없는 알림은 며칠 안에 배경이 된다.
   *
   * 이제 되풀이 할일의 사건 갈래가 대신 말한다 (0183 · repeat_kind='retest' ·
   * app/todo/routineActions 의 retestTasks) — 「○○ · 관계대명사 3번째 —
   * 재시험지 만들기」 가 할일 목록에 뜨고, 체크하면 사라지고 다시 안 생긴다.
   * 판정(세 번째부터)은 그대로 lib/unitStreak 한 곳에 산다.
   *
   * **여기서 다시 세면 안 된다** — 두 벌이 되는 순간 둘 다 못 믿게 된다
   * (원칙 1). scripts/check-badges.mjs 가 그것을 지킨다.
   */
   ...
   */
  /**
   * **등원 밀림 카드는 없앴다** (원장님 2026-08-29).
   *
   * 최근 2주 리포트에서 학생별 가장 최근 수업의 「다음 수업에 계속」
   * (carry_next) 이 2개 이상이면 딱지를 띄우던 자리다 (2026-08-20 ~ 08-29).
   *
   * 없애도 잃는 것이 없다 — **밀린 항목은 이미 원장님 눈앞에 있다.**
   * carry_next 가 켜진 항목은 다음 수업 판의 「오늘 학원」 목록으로 저절로
   * 올라온다 (app/today/page.jsx). 즉 그 아이 수업을 여는 순간 밀린 것이
   * 그대로 펴져 있다. 대시보드 딱지는 그 사실을 **한 번 더** 세던 것이라,
   * 두 숫자가 어긋나는 날 둘 다 못 믿게 될 자리였다 (원칙 1).
   *
   * 세던 재료도 같이 걷었다 — carry_next 조회 한 단, 그리고 그것 하나만
   * 보던 daily_reports.attendance_kind 사다리 둘.
   */
