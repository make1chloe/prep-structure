...
import { ccUserIdxOf, plannerRunningOut } from "./classcard";
import {
...
export async function loadDashboard(supabase, today = todaySeoul()) {
  ...
  const [
    ...
    supabase.from("attendance").select("student_id, date, status, planned, reason, note, makeup_waived").eq("status", "absent").gte("date", monthAgo).lte("date", weekEnd),
     ...
     */
    supabase.from("requests").select("id, student_id, kind, from_date, to_date, body, status, reply, thread, author_role, canceled_at, handled_at, created_at, photos").order("created_at", { ascending: false }).limit(30),
   ...
   */
  const classcard = { fetchedAt: null, runningOut: [], mismatch: [], shadow: null };
  {
    ...
    // 그림자 일치율 (0132) — 지난 30일, 자동 판정 vs 실제 판정
    const { data: sh } = await supabase
      .from("classcard_shadow")
      .select("auto_status, actual_status")
      .gte("date", addDays(today, -30));
    if (sh && sh.length) {
      const agree = sh.filter((x) => x.auto_status === x.actual_status).length;
      classcard.shadow = { total: sh.length, agree, pct: Math.round((agree / sh.length) * 100) };
    }
  }
