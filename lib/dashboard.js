...
export async function loadDashboard(supabase, today = todaySeoul()) {
  ...
  // ── 수강료 — 보강 필요 횟수 합계 ─────────────────────────────
  // 특강(0164)은 여기 안 넣는다 — 학생별 정액(fee)이라 휴강·결석이 금액을
  // 안 건드리고(원장 확정), 그래서 보강 필요·이월도 생기지 않는다.
  ...
  if (Number(today.slice(8, 10)) >= 25) {
    ...
    // rows 는 **실행된 결과**를 받는다 — 함수를 넘기면 조회가 실행조차 안
    // 되어 25일 이후 회차 확정 배지가 재원생 전원으로 오표시됐다 (8/27 적발)
    const mc = rows(
      await supabase.from("month_confirms").select("student_id, principal_at").eq("ym", nym)
    );
