...
export async function loadDashboard(supabase, today = todaySeoul()) {
  ...
  const [
    ...
    sendFailQ, prepExamQ, prepScopeQ, monthAttQ, settings, payQ,
  ] = await Promise.all([
    ...
    supabase.from("tasks").select("id, title, due_on, kind").lt("due_on", today).eq("status", "open").order("due_on", { ascending: true }).limit(10),
    ...
    supabase.from("requests").select("id, student_id, kind, from_date, to_date, body, status, created_at").eq("status", "new").order("created_at", { ascending: false }).limit(20),
    ...
    supabase.from("daily_reports").select("id, student_id, date").lte("date", today).gte("date", monthAgo).eq("report_written", true).is("sent_at", null).order("date", { ascending: true }),
    ...
    supabase.from("prep_exams").select("id, school, grade, term, exam_date").not("exam_date", "is", null).gte("exam_date", today).order("exam_date", { ascending: true }),
  ...
  // ── 발송 — 안 나간 것과 실패한 것 ────────────────────────────
  const unsentAll = rows(unsentQ);
  const unsentToday = unsentAll.filter((r) => r.date === today);
  const unsentPast = unsentAll
  ...
  const exams = rows(examQ);
  const scopeCount = new Map();
  rows(prepScopeQ).forEach((s) => scopeCount.set(s.exam_id, (scopeCount.get(s.exam_id) || 0) + 1));
  const prepExams = rows(prepExamQ);
  const examSoon = prepExams
    .filter((e) => e.exam_date <= addDays(today, 21))
    .map((e) => ({
      id: e.id,
      label: `${e.school} ${e.grade || ""} ${e.term || ""}`.replace(/\s+/g, " ").trim(),
      on: e.exam_date,
      dday: Math.round((new Date(`${e.exam_date}T00:00:00Z`) - new Date(`${today}T00:00:00Z`)) / 86400000),
  ...
  // ── 새 소식 ──────────────────────────────────────────────────
  const requests = rows(reqQ).map((r) => ({ ...r, studentName: nameOf.get(r.student_id) || "학생" }));
