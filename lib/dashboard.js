...
import { meetsOn } from "./classTerm.js";
import { reviewClass, monthsFrom, addDaysISO } from "./schedule.js";
...
import { ccUserIdxOf, plannerRunningOut, ccWordItem, ccStale, ccTodayGap, dayNum } from "./classcard";
import {
...
export async function loadDashboard(supabase, today = todaySeoul()) {
  ...
  // ── 1차: 서로 상관없는 것들을 한꺼번에 ──────────────────────────
  //
  // 사다리(실패 시에만 한 번 더)가 붙은 조회는 **미리 만든 promise** 로
  // 파도에 태운다. 배열을 만드는 도중 딴 조회가 동기로 던지면(검사의 가짜
  // DB) 닫힌 promise 가 고아 거부로 남아 프로세스째 죽으므로, 빈 손잡이를
  // 먼저 달아둔다 — 진짜 결과·거부는 그대로 Promise.all 이 받는다.
  //
  // 최근 2주 리포트 — attendance_kind 를 같이 읽는다 (§1-1 #9: 등원 밀림이
  // 이 조회를 재사용한다). 늦게 생긴 칸이라 실패하면 칸 없이 한 번 더
  // (사다리 신설, §3-2) — 두 번째가 종전 조회 그대로다. hasAttKind 는
  // 등원 밀림이 「재사용해도 되는가」 를 가르는 표식.
  const recentRepP = (async () => {
    const sel = (cols) => supabase.from("daily_reports").select(cols)
      .gte("date", twoWeeksAgo).lte("date", today).is("archived_at", null);
    const q = await sel("id, student_id, date, attendance_kind");
    if (!q.error) return { ...q, hasAttKind: true };
    return sel("id, student_id, date");
  })();
  recentRepP.catch(() => {});
  // 미래 판 보충 (§1-1 #9·#15 **공유 1조회**) — 등원 밀림·오래 뜬 숙제의
  // 원 조회는 상한이 없어 미래 판(미리 내기)을 포함했다. warnRepQ 가
  // lte(today) 라 그 미래분만 얇게 한 번 — 열·필터는 warnRepQ 와 같게
  // (archived_at is null 포함). 칸 실패면 열 없이 한 번 더 (사다리 신설).
  const futureRepP = (async () => {
    const sel = (cols) => supabase.from("daily_reports").select(cols)
      .gt("date", today).is("archived_at", null);
    const q = await sel("id, student_id, date, attendance_kind, word_correct, word_total");
    if (!q.error) return { ...q, hasAttKind: true };
    return sel("id, student_id, date, word_correct, word_total");
  })();
  futureRepP.catch(() => {});
  // 시험 — hidden 칸을 같이 읽는다 (§1-1 #2: 숨긴 시험 번호를 따로 묻던
  // hiddenExamIds 왕복 한 단 제거). 0060 전 DB 면 조회가 통째로 죽어
  // 시험 알림이 조용히 전멸하므로 칸 없이 한 번 더 (사다리 신설, §3-2 —
  // 그땐 「숨긴 것 없음」. lib/schedule 의 태도 그대로)
  const examP = (async () => {
    const sel = (cols) => supabase.from("exam_periods").select(cols)
      .gte("to_date", today).order("from_date", { ascending: true });
    const q = await sel("id, school, grade, name, from_date, to_date, english_on, hidden");
    if (!q.error) return q;
    return sel("id, school, grade, name, from_date, to_date, english_on");
  })();
  examP.catch(() => {});
  // 학생별 교재 배정 전수 (fetchAll) — 아무것에도 안 걸리는 조회라 1차
  // 파도에 태운다. 0154·0149 사다리는 그대로 실패 시에만 (§3-1 자리 유지)
  const inUseP = (async () => {
    const sel = (cols) => fetchAll(() =>
      supabase.from("student_textbooks").select(cols)
        .order("student_id").order("textbook_id"));
    let q = await sel("student_id, textbook_id, status, assigned_on, ended_on, pause, routine_set_at");
    if (q.error) {
      // 0154 전 — 루틴 도장 없이 (재촉이 조용히 빈다)
      q = await sel("student_id, textbook_id, status, assigned_on, ended_on, pause");
    }
    if (q.error) {
      q = await sel("student_id, textbook_id, status, assigned_on, ended_on");
    }
    return q;
  })();
  inUseP.catch(() => {});
  ...
  const [
    ...
    sendFailQ, prepScopeQ, monthAttQ, settings, payQ, extraQ,
    calQ, unitScoreQ, futureRepQ, inUse,
    linkQ0, rosterQ, planQ, dayQ,
  ] = await Promise.all([
    ...
    supabase.from("daily_reports").select("student_id, report_written").eq("date", today).is("archived_at", null),
    ...
    supabase.from("daily_reports").select("id, student_id, date, skip_kinds").lte("date", today).gte("date", monthAgo).eq("report_written", true).is("sent_at", null).is("archived_at", null).order("date", { ascending: true }),
    ...
    supabase.from("daily_reports").select("id, student_id, date, attendance_kind, word_correct, word_total").gte("date", sixtyAgo).lte("date", today).is("archived_at", null).order("date", { ascending: true }),
    ...
    // 특강 (0164 — 재원생 속성). 0164 전 DB 면 error → rows() 가 빈 배열로 본다
    supabase.from("student_extra_schedules").select("id, student_id, label, days, start_time, end_time, from_date, to_date, off_dates"),
    // 달력 tasks — 날짜만 있으면 되는 조회 (§1-1 #3 파도 합류)
    supabase
      .from("tasks")
      .select("id, title, kind, due_on, source, status")
      .gte("due_on", `${addMonths(ym, -1)}-01`)
      .lte("due_on", endOfMonth(addMonths(ym, 1)))
      .neq("status", "canceled")
      .order("due_on", { ascending: true }),
    // 단원평가 점수 120일 — 날짜만 있으면 된다 (§1-1 #5 파도 합류)
    supabase
      .from("scores")
    ...
    futureRepP,
    inUseP,
    // 클래스카드 재료 4벌 — 날짜만 있으면 된다 (§1-1 #11 파도 합류).
    // linkQ0 의 0131 전 폴백은 종전 자리(클카 블록 안, 실패 시에만) 그대로
  ...
  // 숨긴 시험은 알림에서 뺀다 — 번호는 examP 가 같이 받은 hidden 칸에서
  // 바로 모은다 (0060 전 사다리로 내려왔으면 칸이 없어 「숨긴 것 없음」)
  const hiddenExams = new Set(rows(examQ).filter((e) => e.hidden === true).map((e) => e.id));
  ...
  // 특강(0164 — 재원생 속성)도 오늘 수업이다 — label 이 가상 반으로 선다.
  // 특강 전용 학생(반 배정 0)이 여기 안 서면 「오늘 몇 명」 부터 틀린다 (T4).
  const extraScheds = rows(extraQ);
  {
    const todayExtras = extraScheds.filter(
      (x) => meetsOn(toTermShape(x), today, dow) && !(x.off_dates || []).includes(today)
    );
    const byLabel = new Map();
    todayExtras.forEach((x) => {
      if (!byLabel.has(x.label)) byLabel.set(x.label, []);
      byLabel.get(x.label).push(x);
      todayIds.add(x.student_id);
    });
    [...byLabel.entries()].forEach(([label, xs]) => {
      todayClasses.push({
        id: `extra:${label}`, name: `특강 · ${label}`, days: xs[0].days || [],
        start_time: xs[0].start_time || null, end_time: xs[0].end_time || null, room: null,
      });
    });
  ...
  // 조회는 1차 파도의 calQ — 날짜만 있으면 되는 것이라 따로 설 이유가 없었다
  ...
  // ── 2차 파도: 1차 결과의 id 에만 걸리는 것들을 한꺼번에 (§1-1) ────
  // 종전에는 이것들이 화면 아래쪽 블록마다 한 단씩 줄줄이 섰다 —
  // 전부 1차 파도의 id 값에만 걸리므로 여기서 나란히 던진다.
  const recentReports = rows(recentRepQ);
  const futureReps = futureRepQ?.error ? [] : rows(futureRepQ);

  /**
   * 등원 밀림(backlog)의 원천 — **recentRepQ 재사용 ∪ 미래 보충 + JS date
   * desc 재정렬** (§1-1 #9). 원 조회는 상한이 없어 미래 판(미리 내기 —
   * app/plan/actions)을 포함했다 — 같은 집합을 두 조회의 합으로 재현한다.
   * 재사용은 두 원천이 attendance_kind 째로 성공했을 때만이다. 아니면
   * (옛 DB) 종전 자기 폴백 조회 그대로 — attendance_kind 없이 · 필터
   * 전-행-통과 (보충 사다리로 대체 금지 — 검토자 시공 메모 ①).
   */
  let backlogReps = null;
  if (recentRepQ?.hasAttKind && futureRepQ?.hasAttKind) {
    backlogReps = [...futureReps, ...recentReports]
      .sort((a, b) => b.date.localeCompare(a.date));
  } else {
    // 출결 미러 칸이 없는 옛 DB — 필터 없이 종전대로
    ({ data: backlogReps } = await supabase
      .from("daily_reports")
  ...
  const backlogIds = [...latestRep.values()];

  const repIdsAll = warnReports.map((r) => r.id);
  const futureIds = futureReps.map((r) => r.id);
  const usedIds = [
    ...new Set(
      rows(inUse)
  ...
  const [itemsQ, carryQ, futureItemsQ, bq, uq, pq2] = await Promise.all([
    // 경고 재료 — warnRepQ 판들의 항목 전수 (#4. #16 재사용의 원천)
    repIdsAll.length
      ? fetchAll(() =>
          supabase.from("daily_report_items").select("daily_report_id, status")
            .in("daily_report_id", repIdsAll).order("daily_report_id"))
      : null,
    // 등원 밀림 — 가장 최근 수업 판의 「다음 수업에 계속」
    backlogIds.length
      ? supabase
          .from("daily_report_items")
          .select("daily_report_id, homework_item_id")
          .in("daily_report_id", backlogIds)
          .eq("status", "inclass")
          .eq("carry_next", true)
      : null,
    // 미래 판의 배정 항목 (§1-1 #16 — itemsQ 에 없는 몫만, assigned 만 얇게)
    futureIds.length
      ? fetchAll(() =>
          supabase.from("daily_report_items").select("daily_report_id, status")
            .in("daily_report_id", futureIds)
            .eq("status", "assigned")
            .order("daily_report_id"))
      : null,
    // 교재·단원·진도 — 종전 bq/uq 와 bq2/uq2 의 **합집합 열 한 벌** (§1-1
    // #7·#8 — 같은 두 표를 두 블록이 각각 읽던 왕복 2 제거). pq2 는 전수
    // 그대로 (행 축소는 판단 변경이라 금지 — §2-4)
    usedIds.length
    ...
    usedIds.length
      ? fetchAll(() =>
          supabase.from("textbook_units").select("id, textbook_id")
            .in("textbook_id", usedIds).order("id"))
      : null,
    usedIds.length
      ? fetchAll(() =>
          supabase.from("student_unit_progress")
  ...
  {
    // 조회는 1차 파도의 unitScoreQ — 날짜만 있으면 되는 것이라 파도 합류
    const byStudent = students.map((s) => ({
      student: s,
      scores: rows(unitScoreQ).filter((x) => x.student_id === s.id),
  ...
  // 특강 요일도 보강 후보다 — 출결 화면(lib/makeupTodo)과 같은 규칙
  extraScheds.forEach((x) => {
    if (x.to_date < today) return;              // 끝난 특강 요일은 아니다
  ...
  /**
   * **「보강 필요 N회 · 차액 얼마」 는 여기서 안 센다** (원장님, 2026-08-28).
   *
   * 대시보드가 반마다 classSessions·studentAmount 를 돌려 회차와 차액을
   * 합쳤고, 그 합계는 카드 한 장과 배지 한 줄에만 쓰였다. 그런데 남은 일
   * 목록(lib/menuBadges 의 plan)은 **같은 보강을 lib/makeupTodo 로** 세고
   * 있었다 — 하나는 회(回), 하나는 건(件). 같은 이름의 숫자 둘이 늘 다른
   * 말을 하는 상태였다.
   *
   * 이제 보강은 남은 일 한 곳에서만 말한다. 회차와 차액은 그것을 실제로
   * 쓰는 자리 — 수강료 화면 — 에 그대로 있다.
   */
  ...
  /**
   * **월간리포트 시점은 여기서 안 잰다** (원장님, 2026-08-28).
   * 남은 일 목록이 이미 월말 3일부터 「발송할 것 N건 (리포트 · 교재 안내 ·
   * 월간)」 으로 말한다 (lib/menuBadges 의 monthlyLeft — 같은 기준).
   */
  ...
  // (조회는 1차 파도의 inUse — 사다리째 파도에 탔다. usedIds 는 2차 파도 앞에서)
  let needUnits = [];
  if (usedIds.length) {
    // bq·uq 는 2차 파도 — 종전 bq2/uq2 와 합집합 열 한 벌 (§1-1 #7·#8).
    // 단원 합은 1000줄을 넘으면 멀쩡한 교재가 「단원 없음」 (B3) — fetchAll 유지
  ...
  let ccProg = [];
  if (usedIds.length) {
    // bq·uq·pq2 는 2차 파도에서 이미 받았다 (§1-1 #7·#8 — 종전 bq2/uq2 와
    // 같은 표를 두 번 읽던 왕복 2 제거). pq2 는 제일 큰 표 — 잘리면
    // 「곧 끝나는 교재」 가 앞 학생들 것만 본다 (A5) — fetchAll 그대로
    const bookById = new Map(rows(bq).map((b) => [b.id, b]));
    bookMeta = bookById;
    ccBookArea = new Map(rows(bq).map((b) => [b.id, b.area || ""]));
    ccProg = rows(pq2);
    const total = new Map();
    ...
    bookEnding = rows(inUse)
      ...
      .map((r) => {
        ...
        // studentId·textbookId 를 같이 실어 보낸다 — 대시보드가 그 자리에서
        // **다음 교재 배정** 팝오버를 열려면 누구의 어느 교재인지가 있어야 한다
        // (원장님 2026-08-28 — 「곧 끝나는 교재가 있으면 다음 교재 배정이
        //  필요한 상황. 그걸 위한 장치가 연결되어야 함」)
        return { id: `${r.student_id}|${r.textbook_id}`, studentId: r.student_id,
                 textbookId: r.textbook_id, name: who.name, book: b.name, area: b.area || "", left };
      })
      .filter(Boolean)
    ...
    progressIdle = rows(inUse)
      ...
      .map((r) => {
        ...
        if (!b || !who) return null;
        // 그 자리에서 진도를 찍으려면 누구의 어느 교재인지가 있어야 한다
        return { studentId: r.student_id, textbookId: r.textbook_id,
                 name: who.name, book: b.name, since: r.assigned_on };
      })
      .filter(Boolean)
  ...
  const backlog = [];
  {
    // 판의 수집(latestRep — recentRepQ ∪ 미래 보충, 옛 DB 면 자기 폴백)과
    // 항목 조회(carryQ)는 2차 파도 앞·안에서 끝났다 (§1-1 #9) — 여기서는 센다
    if (backlogIds.length) {
      const cnt = new Map();
      (carryQ?.data || []).forEach((x) => cnt.set(x.daily_report_id, (cnt.get(x.daily_report_id) || 0) + 1));
      latestRep.forEach((rid, sid) => {
        const n = cnt.get(rid) || 0;
        // id 를 같이 — 대시보드에서 누르면 그 아이가 펴진 채로 오늘 수업에 간다
        if (n >= 2) backlog.push({ id: sid, name: nameOf.get(sid) || "학생", count: n });
      });
  ...
  {
    // 재료 4벌(linkQ0·rosterQ·planQ·dayQ)은 1차 파도에서 이미 받았다 (§1-1 #11)
    ...
    if (!rosterQ.error && (rosterQ.data || []).length) {
      ...
      // 이름→숫자 규칙은 lib/classcard 의 dayNum 한 벌 — 「플래너에 맞추기」
      // 단추가 무엇을 완료로 찍을지 고를 때 **같은 자**를 써야 한다
      ...
      (linkRows || []).forEach((st) => {
        ...
        if (ccDays.length && appMax) {
          ...
          if (Math.abs(ccMax - appMax) >= 2) {
            // studentId 를 같이 — 「플래너에 맞추기」 팝오버가 누구인지 알아야 한다
            classcard.mismatch.push({ id: st.id, name, app: appMax, cc: ccMax });
          }
        }
      });
   ...
   */
  /**
   * **다음 달 회차 미확정은 여기서 안 센다** (원장님, 2026-08-28 — 「회차
   * 확정도 저렇게 나올 거면 그냥 남은 할일 목록에 저런 식의 것들을 다
   * 모아주는 게 나음」).
   *
   * 0123 때 대시보드에 카드로 붙였는데, 그 카드가 하는 일은 회차 화면으로
   * 가는 링크 한 줄이었다. 그런데 lib/menuBadges 가 **같은 month_confirms
   * 를 이미 읽어** schedule 키로 세고 있었다 — 두 벌.
   *
   * 여기 있던 조회는 파도 밖에서 홀로 기다리던 것이라 25일 이후에는 대시보드
   * 첫 로드에 왕복이 한 단 더 섰다. 지우면 그 한 단이 통째로 준다.
   */
   ...
   */
  const staleHomework = [];
  // 그 짝 — 다음 숙제가 안 나간 아이 (같은 재료로 잰다)
  const noAssign = [];
  {
    const STALE_DAYS = 7;
    const cut = addDays(today, -STALE_DAYS);
    /**
     * 판은 새로 안 읽는다 (§1-1 #15) — 원 조회(60일 + 상한 없음 = 미래 판
     * 포함)와 같은 집합을 **warnRepQ(60일, lte today) ∪ 미래 보충**의 합 +
     * **JS date desc 재정렬**로 재현한다 (:  같은 열·같은 필터,
     * `archived_at is null` 포함). pre-0168 옛 DB 에서는 warnRepQ·원 조회·
     * 보충이 **같이 실패**해 지금도 빈 결과 — 자연 보존이라 사다리 불요.
     */
    const reps = [...futureReps, ...warnReports].sort((a, b) => b.date.localeCompare(a.date));
    /**
     * 항목도 새로 안 읽는다 (§1-1 #16) — 과거 판의 배정 수는 itemsQ(#4 —
     * warnRepQ 판들의 status 전수)에서 걸러 세고, itemsQ 에 없는 **미래
     * 판의 배정만** 2차 파도의 보충(futureItemsQ — assigned 만, 얇게)이 준다.
     */
    const cnt = new Map();
    for (const it of rows(itemsQ)) {
      if (it.status !== "assigned") continue;
      cnt.set(it.daily_report_id, (cnt.get(it.daily_report_id) || 0) + 1);
    }
    for (const it of futureItemsQ?.error ? [] : futureItemsQ?.data || []) {
      cnt.set(it.daily_report_id, (cnt.get(it.daily_report_id) || 0) + 1);
    }
    ...
    for (const r of reps || []) {
      ...
      staleHomework.push({
        // 그 날짜 그 아이가 **펴진 채로** 열려야 한다 — 가서 뭘 할지 모르는
        // 것이 문제였다 (원장님 2026-08-28)
        id: r.student_id,
        name: who,
        date: r.date,
    ...
    /**
     * ── **다음 숙제가 안 나간 아이** (원장님 2026-08-28 — 「숙제검사 안 한
     * 아이와 숙제배정 안 된 아이 같이 보여줘」 · 「최초 숙제배정 어디서
     * 해야 해?」) ──────────────────────────────────────
     *
     * 뒷말이 진짜 요구다 — **배정하는 자리를 못 찾고 계신다.** 그래서 알리는
     * 데서 그치지 않고 칩에서 바로 배정 판이 열린다(app/DashFixBody).
     *
     * ── 무엇을 「배정 안 됨」으로 보나 (실측으로 정함) ──────
     *
     * 그 학생의 **가장 최근 판**(reps 는 날짜 내림차순)에 status='assigned'
     * 항목이 **하나도 없으면** 배정 안 됨으로 본다.
     *
     * 왜 「가장 최근 판」인가 — 숙제는 판마다 그날 나간다. 어제 판에 배정이
     * 있어도 오늘 판이 비어 있으면 오늘 아이 화면에는 아무것도 없다.
     * 반대로 오늘 판에 배정이 있으면 지난 판이 비었어도 문제가 아니다.
     * 미래 판도 같이 본다 — 미리 넣어두신 것을 「안 했다」고 재촉하면 안 된다.
     *
     * 판이 아예 없는 학생은 안 올린다 — 수업이 없었던 것이라 배정할 자리도
     * 없다 (오래 쉬는 아이·신입이 매일 재촉으로 뜨는 것을 막는다).
     *
     * ── **배정할 것이 없는 아이는 갈라 놓는다** ─────────────
     *
     * 쓰는 교재가 한 권도 없으면 배정할 것 자체가 없다. 그 아이를 같은 줄에
     * 세우면 「배정하라는데 배정할 게 없다」가 된다 — 눌러도 빈 판이 뜬다.
     * needBook 로 표시해서 화면이 **다른 줄로** 세우고, 그 칩은 배정 판이
     * 아니라 교재 배정으로 보낸다. (진도·루틴은 안 본다 — 루틴이 없어도
     * 항목을 손으로 골라 배정할 수 있으므로 막을 일이 아니다.
     * 그쪽은 「진도 시작 안 한 교재」·「루틴 안 정한 교재」 카드가 말한다.)
     */
    {
      const hasBook = new Set(
        rows(inUse).filter((r) => inUseOn(r, today)).map((r) => r.student_id)
      );
      const seen2 = new Set();
      for (const r of reps || []) {
        if (seen2.has(r.student_id)) continue;   // 가장 최근 판 하나만
        seen2.add(r.student_id);
        if (cnt.has(r.id)) continue;             // 배정이 있다 — 할 말 없음
        const who = nameOf.get(r.student_id);
        if (!who) continue;                      // 퇴원생은 뺀다
        noAssign.push({
          id: r.student_id,
          ...
          date: r.date,
          days: Math.round((Date.parse(today) - Date.parse(r.date)) / 86400000),
          needBook: !hasBook.has(r.student_id),
        });
      }
      noAssign.sort((a, b) => b.days - a.days || a.name.localeCompare(b.name, "ko"));
    }
  }
