...
export async function loadDashboard(supabase, today = todaySeoul()) {
  ...
  const [
    ...
    supabase.from("students").select("id, name, school, grade, status, enrolled_on, started_on, ended_on, birth_year").eq("status", "enrolled"),
  ...
  if (usedIds.length) {
    const [bq2, uq2, pq2] = await Promise.all([
      ...
      supabase.from("textbook_units").select("id, textbook_id").in("textbook_id", usedIds),
