...
export async function studentById(sb, id) {
  const st = row(await db(sb).from("students").select("id,name,profile_id,grade,school_id,phone,parent_phone,birth,progress_edit,schools(name,level)").eq("id", String(id)).maybeSingle(), "볼 아이");
