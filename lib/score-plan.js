 ...
 *  문항표 엑셀(읽기 · ⬇ 열) · 모의고사 표준 문항표(옛 앱 lib/examSpec 의 MOCK_SPEC 그대로 · 「모의고사는 앱이 알고, 내신은 시험지마다 적는다」) · 성적 양식 ⬇ · 성적 추이(같은 유형 지난 회차 대비 ▲▼)(5단계-⑤) */
...
/** 문항표 엑셀 줄(sheet_to_json) → 줄들 · 열 「번호」(문항·no) · 「영역」(유형·유형·kind). 번호는 「1-17」 도 된다 · 영역 낱말 다듬기(문법 → 어법 · 단어 → 어휘 · 서답형 → 서술형) · 못 읽는 줄은 「고칠 줄」 */
export function parseQuestionSheet(sheetRows = []) {
  const out = new Map(), bad = [];
  sheetRows.forEach((r, i) => {
    const no = pick(r, ["번호", "문항", "no", "q"]), kind0 = pick(r, ["영역", "유형", "유형", "kind", "분류"]);
    if (!no && !kind0) return;
...
// ── 성적 추이(07·09·14) · 같은 유형(내신/모의고사)의 지난 회차와 견준다 · 만점이 다르면 100점 기준으로 · 확인 전 줄은 견주지도 않고 견줌의 상대도 안 된다 · 첫 회차는 없음
