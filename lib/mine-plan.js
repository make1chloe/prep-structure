...
export const FILL = Object.freeze([
  ["parent_phone", "me.fill.parent_phone", "부모님 전화번호", "tel"],
  ["phone", "me.fill.phone", "내 전화번호", "tel"],
  ["grade", "me.fill.grade", "학년", "grade"],
  ["birth", "me.fill.birth", "생일", "date"],
...
export function parseFill(key, raw, today = "") {
  ...
  if (key === "birth") {
    const s = String(raw ?? "").trim(); if (!/^\d{4}-\d{2}-\d{2}$/.test(s)) throw new Error(`${fillName(key)} 꼴은 2015-03-21`);
    if (today && s > today) throw new Error(`${fillName(key)}이 앞날`);
    if (Number(s.slice(0, 4)) < 1980) throw new Error(`${fillName(key)} 해가 1980 앞`);
    return s;
  }
