/** 자료실 판단 한 벌(순수 · 목업 20 「주고받기」). 학교가 주는 종이가 여기로 모인다: 받은 것(유형만 고른다 · 학교·학년·학기는 아이에게서 저절로) · 유형별 올린 기록 · 보낸 것(붙인 숙제 · 아이가 처리했나) · 아이 쪽 1달(규칙 file.child_days).
...
import { itemTitle } from "./item-plan.js";
export const KINDS = Object.freeze(["수행평가", "시험 안내", "수업자료", "학사일정", "가정통신문", "기타"]);
...
export const ALLOWED_MIME = Object.freeze(["image/jpeg", "image/pjpeg", "image/png", "image/heic", "image/heif", "image/webp", "image/gif", "image/bmp", "image/tiff", "application/pdf",
  ...
  "audio/webm", "audio/mp4", "audio/mpeg", "audio/ogg", "audio/wav", "audio/x-m4a"]);   // (어76) 음성 숙제(원장님 2026-09-17 「숙제 음성녹음으로 제출도 가능하게 해줘」) — 폰마다 내는 꼴이 다르다(안드로이드 webm · 아이폰 mp4/m4a)
export const isImage = (mime) => /^image\//.test(String(mime ?? ""));
export const isAudio = (mime) => /^audio\//.test(String(mime ?? ""));   // (어76) 음성은 그림도 문서도 아니다 — 듣는 것이라 화면이 재생 막대를 낸다
export const icon = (mime) => (isImage(mime) ? "📷" : isAudio(mime) ? "🎧" : "📄");
...
export function extOf(name, mime) {
  ...
  return ({ "image/jpeg": "jpg", "image/png": "png", "image/webp": "webp", "image/gif": "gif", "image/heic": "heic", "application/pdf": "pdf", "text/plain": "txt", "text/csv": "csv", "audio/webm": "webm", "audio/mp4": "m4a", "audio/x-m4a": "m4a", "audio/mpeg": "mp3", "audio/ogg": "ogg", "audio/wav": "wav" })[mime] ?? "bin";
}
...
export function checkFile({ name, mime, bytes }, { maxMb = 4, audioMaxMb = 20 } = {}) {
  if (!ALLOWED_MIME.includes(String(mime ?? ""))) return `받지 않는 종류입니다. ${name ?? ""}(${mime || "종류 없음"}). 사진·음성·pdf·한글·워드·엑셀·파워포인트·글`;
  const audio = isAudio(mime); if (audio) maxMb = Number(audioMaxMb) || 20;   // (어76) 음성은 사진보다 크다 — 문턱도 제 것(규칙 file.audio_max_mb)
  const cap = (Number(maxMb) || 4) * 1048576;
  if (!(Number(bytes) > 0)) return `빈 파일입니다. ${name ?? ""}`;
  if (Number(bytes) > cap) return `${maxMb}MB 를 넘습니다. ${name ?? ""} ${sizeText(bytes)}. ${audio ? "짧게 다시 녹음해 줘요" : "사진이면 앱이 줄이니 문서만 걸립니다. 나눠 주세요"}`;
  return null;
}
...
export function whoText(f) {
  if (f.by_role === "parent") return `${f.student_name ?? "아이"} 부모님이 보냄`;
  if (f.by_role === "student") { const nm = f.student_name ?? f.by_name ?? "아이"; return `${nm}${ga(nm)} 보냄`; }
  const nm = f.by_name ?? "학원"; return `${nm}${ga(nm)} 보냄`;
}
/** 저절로 붙는 꼬리표 — 「🏫 옥련여고 1 · 26-2」. 학교가 없으면 그렇게 말한다(기타로 가라고) */
export const autoTag = (f) => (f.school_name ? `🏛️ ${f.school_name} ${f.student_grade ?? ""} · ${f.term ?? ""}`.replace(/\s+·/, " ·") : "학교 없음 · 「기타」로");
/** 방금 온 것 줄 · 새것부터. 유형은 원장님이 고른다 */
...
/** 유형별 칸 · 학교·학년마다 한 칸(올린 기록 카드: 「26-1 수행평가 · 사진 5 · 3.4MB · 마지막 9/2」) + 「🧑‍🎓 아이별」 칸(기타 · 학교와 상관없는 것, 아이마다). 빈 올린 기록은 안 그린다 */
export function columns(bins = []) {
  ...
  for (const b of bins ?? []) {
    ...
    if (!cols.has(key)) cols.set(key, { key, title: `🏛️ ${b.school ?? "학교 없음"} ${b.grade ?? ""}`.trim(), n: 0, cards: [] });
...
/** 아이 쪽 · 1달 안의 붙임만(규칙 file.child_days). 아직 처리 안 한 것은 숙제 줄에, 처리한 것은 「지난 것 보기」에 · 1달이 지나면 안 보인다(원장님 자료실엔 그대로) */
export function childLinks(links = [], today, days = 30) {
  const d = Number(days) || 30;
  const live = (links ?? []).filter((l) => daysBetween(seoulDate(l.created_at), today) < d);
  const line = (l) => ({ ...l, icon: icon(l.file?.mime), name: l.file?.orig_name ?? "(파일)", size: sizeText(l.file?.bytes), seen: seenText(l), item: l.day_item ? itemTitle(l.day_item) : "", on: l.day_item?.day_sheet?.date ?? null, until: md(plusDaysStr(seoulDate(l.created_at), d)) });
...
/** 숫자 셋 · 받은 것 · 보낸 것 · 안 본 것(유형 안 고른 것) */
...
export function sendTargets(students = [], studentId) {
  ...
  return { student: st, items, why: items.length ? "" : st.sheet ? `${md(st.sheet.date)} 수업 일지에 숙제 줄이 없습니다. 오늘 수업에서 숙제를 내고 붙이세요` : "이 아이의 오늘 숙제가 없습니다. 오늘 수업에서 숙제를 내고 붙이세요" };
}
/** ② 원장님 답 한 줄 · 유형을 고르면 「받았어요 · 「수행평가」로 넣었어요」(자동 글은 유형을 옮기면 따라 바뀐다 · 고쳐 쓴 글은 그대로 · SQL file_sort 와 같은 꼴) · 없으면 「아직 안 봤어요」 */
export const AUTO_REPLY = (kind) => `받았어요. 「${kind}」로 넣었어요`;
export const isAutoReply = (t) => /^받았어요[. —]+「.*」로 넣었어요$/.test(String(t ?? ""));   // 옛 줄(「받았어요 — 」)도 자동 글로 본다
...
/** ⑦ 보내기 전 고른 파일 줄 — 이름 · 크기 · 사진이면 미리보기 자리 · 지우면 빠진다(보내는 목록 = 이 줄들) */
export const pickRows = (files = []) => (files ?? []).map((f, i) => ({ i, name: f.name, size: sizeText(f.size), photo: isImage(f.type), type: f.type }));
export const withoutPick = (files = [], i) => (files ?? []).filter((_, k) => k !== i);
