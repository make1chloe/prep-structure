...
export const autoTag = (f) => (f.school_name ? `🏛️ ${f.school_name} ${f.student_grade ?? ""} · ${f.term ?? ""}`.replace(/\s+·/, " ·") : "학교 없음 · 「그 밖」으로");
...
/** 유형별 칸 · 학교·학년마다 한 칸(올린 기록 카드: 「26-1 수행평가 · 사진 5 · 3.4MB · 마지막 9/2」) + 「🧑‍🎓 아이별」 칸(그 밖 · 학교와 상관없는 것, 아이마다). 빈 올린 기록은 안 그린다 */
