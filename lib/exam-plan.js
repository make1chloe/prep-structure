/** 시험 회차 판단 한 벌(순수, 목업 06b) · 이 회차를 보는 아이(학교 · 학년 · 「안 봄」 · SQL exam_takers 와 같은 판단) · 교재 보류 창(영어 시험일 − N주 ~ 시험 끝나는 날, 확정-⑬·㊺b) ·
 *  부모님께 글의 시험전·시험후(2차-7 남긴 것) · 범위 올린 기록(교재 × 대단원) · 알약 셈 · 머리 글자. 세는 것은 세어 나온다(원칙-5) · 저장하지 않는다 */
import { plusDays } from "./day-plan.js";
import { daysBetween, md, dateChanged, changeText } from "./dash-plan.js";
export { dateChanged, changeText };   // (저) 「📡 날짜 바뀜」 — 06b 카드·전국 줄과 대시보드·12b 가 같은 글(0152)
...
/** 회차의 「그날」 · 영어 시험일, 없으면 기간 시작 · 끝나는 날 · 기간 끝·영어 시험일·시작 중 가장 늦은 것 */
...
/** 보류 창 · 영어 시험일 − N주 부터 시험 끝나는 날까지. 영어 시험일이 없으면 null(⚠️ 모르면 루틴을 안 세운다 · 기간 끝으로 잡으면 배부가 사흘 늦는다, 목업 06) */
...
/** 부모님께 글의 유형 · 「그날」이 앞으로 before 일 안이면 시험전 · 그날부터 after 일 안이면 시험후 · 아니면 null. 여러 회차면 가까운 것 */
...
/** 범위 올린 기록 · 교재 × 대단원마다 칩 하나(산 소단원 수 · 뺀 수) · 글로 적은 것은 한 줄씩. 처음 적은 날 뒤에 더한 것은 「더함」 · 다 빠졌으면 「학교가 뺌」 */
...
/** 알약 · 회차 N(숨긴 것 빼고) · 숨김 N · 범위 줄 N(산 것) · 영어 시험일 없음 N(학교 회차 · 아직 안 끝난 것) */
...
/** 보류 상태 글 · 창이 있으면 아직/중/지남 */
export function stopText(win, today) {
  if (!win) return null;
  if (today < win.from) return { state: "soon", text: `${md(win.from)}부터 보류 · ${md(win.until)}에 저절로 풀림` };
  if (today <= win.until) return { state: "on", text: `보류 중 · ${md(win.until)}에 저절로 풀림` };
  return { state: "past", text: `${md(win.until)}에 풀림` };
}
/** 「지금 보류」가 안 건드린 까닭 · 한 벌(lib/exam.js 가 세고, 화면이 이 말로 읽는다) */
export const SKIP_WHY = Object.freeze({ other: "다른 시험에 묶임", area: "내신 교재", rule: "규칙 줄 없음", hand: "손으로 보류", freed: "손으로 풀어 둠", none: "보는 아이 없음" });
const SKIP_TIP = Object.freeze({ [SKIP_WHY.other]: "그 시험에서 「풀기」를 먼저", [SKIP_WHY.rule]: "설정 → 교재 보류에서 주 수를 정하십시오", [SKIP_WHY.none]: "「안 봄」을 풀거나 아이의 학교·학년을 보십시오" });
/** 「지금 보류」의 끝말 · 0권이면 **까닭까지** 말한다(대전제-0. 2026-09-12 내신대비 한 판에서 「교재 0권」만 뜨고 까닭이 없어 막혔다) */
export function stopDone(r) {
  const kept = r?.kept ?? 0, skip = (r?.skip ?? []).filter((x) => x.n > 0);
  const head = `지금 멈췄습니다. 교재 ${kept}권`;
  if (r?.why) return `${head} · ${r.why}`;
  if (!skip.length) return head;
  const tip = kept === 0 ? SKIP_TIP[skip[0].why] : null;
  return `${head} · 안 건드린 것 · ${skip.map((x) => `${x.why} ${x.n}`).join(" · ")}${tip ? ` · ${tip}` : ""}`;
}
/** 「풀기」의 끝말 · 0권이면 까닭부터(검사-86 · 「지금 보류」과 짝이다) */
export const releaseDone = (r) => ((r?.released ?? 0) ? `풀었습니다. 교재 ${r.released}권` : `푼 교재가 없습니다. ${r?.why ?? "까닭 없음"}`);
/** 손으로 넣은 회차의 출처 키 · exams 는 (source, source_key) 가 nulls not distinct 유일이라 손으로 넣은 둘째 회차가 안 들어가던 사고(2026-09-06). 같은 학교·학년·이름·시작날이면 같은 회차다 */
