/** 교재 · 단원 15 · 엑셀 15b 의 손 · 판 읽기(한 벌 book_board) · + 교재 · 교재 고치기(교재ID · 영역 · 배정 겹 · 차례 기준 · 단원평가 · 상태) · 다른 이름 · 문법 분류 연결(한 곳 set_unit_topics) · 분류 더하기 ·
...
export async function setBook(sb, bookId, patch = {}) {
  ...
  const r = changed(await db(sb).from("books").update(p).eq("id", bookId).select("id"), "교재를 못 고침") ?? [];
  ...
  for (const p of plan.perRow) {
    try {
      if (p.act === "new") {
        ...
        changed(await db(sb).from("books").update(patch).eq("id", cur.id).select("id"), "교재를 못 고침");
...
export async function setUnit(sb, unitId, f) {
  ...
  const r = changed(await db(sb).from("units").update(p).eq("id", unitId).select("id"), "단원을 못 고침"); if (!r?.length) throw new Error("단원이 없습니다");
}
...
export async function setUnitState(sb, unitId, state) {
  ...
  const r = changed(await db(sb).from("units").update({ state }).eq("id", unitId).select("id"), "단원 상태를 못 고침"); if (!r?.length) throw new Error("단원이 없습니다");
}
