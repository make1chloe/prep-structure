/** 교재 · 단원 15 · 엑셀 15b 의 손 · 판 읽기(한 벌 book_board) · + 교재 · 교재 수정(교재ID · 영역 · 배정 겹 · 차례 기준 · 단원평가 · 상태) · 다른 이름 · 문법 분류 연결(한 곳 set_unit_topics) · 분류 더하기 ·
 ...
 *  **올린 기록**(0057 excel_run·excel_row · 5단계-④): 올릴 때마다 올린 기록을 열고 바꾼 줄(before = 줄 전체)을 적어 「올리기 번호로 되돌리기」(SQL 한 곳 undo_excel_run)가 된다. 지우지 않는다(대전제-6) · 판단은 lib/book-plan.js(순수) */
import { db } from "./supabase.js";
import { AREA_NAMES, MODE, parseUnitRows, matchBooks, planUpload, parseUnitEdit, sheetKind, parseBookRows, planBookUpload, bookPatch, splitUnitWrite, unitKey, runNote, bookRunNote, activityOrder, moveActivity, reorderByActivity } from "./book-plan.js";
...
export async function bookBoard(sb, date, bookId = null) {
  ...
  if (error) throw new Error(`교재 화면을 못 읽음: ${error.message}`);
...
export async function setBook(sb, bookId, patch = {}) {
  ...
  if ("mode" in patch) { if (!MODE.some(([k]) => k === patch.mode)) throw new Error(`진행 방식이 아닙니다: ${patch.mode}`); p.mode = patch.mode; }   // (가)-⑥ 0144
  ...
  const r = changed(await db(sb).from("books").update(p).eq("id", bookId).select("id"), "교재를 수정 못 함") ?? [];
...
/** 미리보기 · 첫 줄 열 이름으로 유형을 가른다: 교재 시트면 previewBookSheet · 단원 시트면 시트 줄 → 단원 줄 → 교재 맞추기 → 맞은 교재의 기존 단원과 견줘 「올리면 이렇게 됩니다」. 저장은 안 한다 */
...
/** 올린 기록 열기(excel_run) · 올리기가 무엇을 바꿨는지 excel_row 로 적어 통째로 되돌린다(0057 · 0142). 줄이 하나도 안 바뀌어도 올린 기록은 남는다(「이날 올렸다」) · 글은 끝나고 적는다 */
async function openRun(sb, { tbl, sheet = null, who = null }) { return row(await db(sb).from("excel_run").insert({ tbl, sheet: sheet || null, who: who || null }).select("id").single(), "올린 기록을 못 엶").id; }
async function noteRun(sb, id, note) { changed(await db(sb).from("excel_run").update({ note }).eq("id", id).select("id"), "올리기 글을 못 적음"); }
/** 저장 — 보류(못 맞춘 교재)는 해결(다른 이름 등록 · 새 교재 · 건너뛰기)한 것만. 덮어쓰기는 파일 줄만 upsert(파일에 없는 기존 줄은 그대로) · 지우고 새로는 한 곳(표가 막으면 그 교재는 통째로 실패).
 *  올린 기록: 먼저 올린 기록을 열고, 덮어쓰기는 새 줄·바뀐 줄(before = 기존 줄 전체)을 적고, 지우고 새로는 SQL 이 지운 줄·새 줄을 적는다 · 반쯤 돼도 그 올린 기록까지 되돌릴 수 있다 */
export async function applyUpload(sb, rows = [], date, { modes = {}, holds = {}, sheet = null, who = null } = {}) {
  ...
  let put = 0, replaced = 0, skipped = 0, added = 0, edited = 0, sortOnly = 0; const failed = [];   /* edited: 바뀐 줄 수 — changed 는 sqlError 의 문 */
  for (const p of plan.perBook) {
    ...
    try {
      if (p.mode === "replace") { replaced += Number(row(await db(sb).rpc("replace_book_units", { p_book: p.book_id, p_rows: body, p_batch: p.batch, p_run: run }), "단원을 못 새로 올림") ?? 0); }
        ...
        const r = changed(await db(sb).from("units").upsert(body, { onConflict: "book_id,chapter,mid,sub,activity" }).select("id,chapter,mid,sub,activity"), "단원을 못 올림") ?? []; put += r.length;
        ...
        if (recs.length) row(await db(sb).from("excel_row").insert(recs).select("id"), "올리기 줄을 못 적음");
        added += split.inserts.length; edited += split.updates.length - split.sortOnly; sortOnly += split.sortOnly;
      }
    } catch (e) { failed.push(`${p.name}: ${String(e?.message ?? e).replace(/^[^:]*: /, "")}`); }
  }
  const note = runNote({ books: plan.perBook.length - skipped, added, changed: edited, sortOnly, replaced, skipped });
...
/** 교재 시트 저장 · 줄마다 새로 만듦(excel 올린 기록) · 고침(시트 값이 있고 다른 칸만 · before = 교재 줄 전체) · 같음은 안 건드림 · 보류(후보 둘)·고칠 줄은 안 올린다. 올린 기록으로 되돌린다 */
  ...
  for (const p of plan.perRow) {
    try {
      if (p.act === "new") {
        ...
        row(await db(sb).from("excel_row").insert({ run_id: run, tbl: "books", row_id: ins.id, op: "insert" }).select("id"), "올리기 줄을 못 적음"); created++;
      } else if (p.act === "update") {
        const cur = list.find((b) => b.id === p.book.id); const { book_alias, aliases, ...before } = cur; const { patch } = bookPatch(p, cur);
        changed(await db(sb).from("books").update(patch).eq("id", cur.id).select("id"), "교재를 수정 못 함");
        row(await db(sb).from("excel_row").insert({ run_id: run, tbl: "books", row_id: cur.id, op: "update", before }).select("id"), "올리기 줄을 못 적음"); updated++;
...
/** 올린 기록 하나 되돌리기 · SQL 한 곳(undo_excel_run)이 줄을 거꾸로 밟는다 · 셈(restored · removed · hidden · revived)을 돌려준다 */
export async function undoRun(sb, runId) {
  const id = Number(runId); if (!Number.isInteger(id) || id <= 0) throw new Error("올리기 번호가 아닙니다");
  return row(await db(sb).rpc("undo_excel_run", { p_run: id }), "올린 기록을 못 되돌림");
}

/** 단원 한 줄 손으로 수정(쪽 · 문항 · 요지 — 엑셀 없이 · 4단계-4). 이름·차례는 엑셀로 */
export async function setUnit(sb, unitId, f) {
  const p = parseUnitEdit(f);
  const r = changed(await db(sb).from("units").update(p).eq("id", unitId).select("id"), "단원을 수정 못 함"); if (!r?.length) throw new Error("단원이 없습니다");
}
/** 활동 차례 ◀ ▶((가)-②) · 이 교재의 단원 줄을 새 활동 차례대로 다시 세운다(바뀐 줄만 · 한 번에 · 숨긴 줄도 같이). 칸이 아니라 줄 차례라 학습 자동 배정·진도가 저절로 따라온다. 되돌리기는 반대로 한 번 더 */
export async function moveActivityOrder(sb, bookId, activity, dir) {
  const units = row(await db(sb).from("units").select("id,book_id,chapter,activity,sort").eq("book_id", bookId), "단원을 못 읽음") ?? [];
  const order = moveActivity(units, activity, dir); if (!order) return { order: activityOrder(units), moved: 0 };
  const changes = reorderByActivity(units, order), byId = new Map(units.map((u) => [u.id, u]));
  if (changes.length) changed(await db(sb).from("units").upsert(changes.map((c) => ({ id: c.id, book_id: byId.get(c.id).book_id, chapter: byId.get(c.id).chapter, activity: byId.get(c.id).activity, sort: c.sort })), { onConflict: "id" }).select("id"), "단원 차례를 못 세움");   // 한 문장 — 비지 않는 칸(교재·대단원·활동명)은 그대로 보낸다
  return { order, moved: changes.length };
}
/** 단원 숨김·복구(state · 지우지 않는다 · 숨긴 단원은 자동 배정·회차·범위에서 빠진다) */
export async function setUnitState(sb, unitId, state) {
  if (!["active", "hidden"].includes(state)) throw new Error(`상태가 아닙니다: ${state}`);
  const r = changed(await db(sb).from("units").update({ state }).eq("id", unitId).select("id"), "단원 상태를 수정 못 함"); if (!r?.length) throw new Error("단원이 없습니다");
}
/** 고른 단원 여럿을 한 번에((어28)-④ · 대전제-20) · 한 줄의 손(setUnitState)을 차례로 · 막히면 그 줄부터 멈춘다 */
export async function setUnitStateMany(sb, ids = [], state) { const list = [...new Set((ids ?? []).filter(Boolean))]; if (!list.length) throw new Error("고른 단원이 없습니다"); let n = 0; for (const id of list) { await setUnitState(sb, String(id), state); n++; } return { n }; }
