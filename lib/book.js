...
async function noteRun(sb, id, note) { changed(await db(sb).from("excel_run").update({ note }).eq("id", id).select("id"), "묶음 글을 못 적음"); }
...
/** 활동 차례 ◀ ▶((가)-②) — 이 교재의 단원 줄을 새 활동 차례대로 다시 세운다(바뀐 줄만 · 한 번에 · 숨긴 줄도 같이). 칸이 아니라 줄 차례라 학습 깔기·진도가 저절로 따라온다. 되돌리기는 반대로 한 번 더 */
