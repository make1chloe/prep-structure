...
export async function moveActivityOrder(sb, bookId, activity, dir) {
  ...
  if (changes.length) row(await db(sb).from("units").upsert(changes.map((c) => ({ id: c.id, book_id: byId.get(c.id).book_id, chapter: byId.get(c.id).chapter, activity: byId.get(c.id).activity, sort: c.sort })), { onConflict: "id" }).select("id"), "단원 차례를 못 세움");   // 한 문장 — 비지 않는 칸(교재·대단원·활동명)은 그대로 보낸다
