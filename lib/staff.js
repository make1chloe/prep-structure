 ...
 *  볼 것을 정하는 자리는 이미 있다(설정 › 권한 설정 · v2.role_access) — 여기는 **계정을 내고 · 역할을 바꾸고 · 닫는** 손만.
...
import { ensureAuthUser, resetPassword } from "./student.js";   // (어66) 이미 있는 계정을 잇는 길은 아이 쪽과 한 벌((어36)) · (어74) 비밀번호 되돌리는 손도 한 벌
import { RESET_STAFF } from "./student-plan.js";
...
export async function listStaff(sb) {
  const all = rows(await db(sb).from("profiles").select("id,name,role,state,login_id,issued_by_app").in("role", [ROLES.PRINCIPAL, ROLES.INSTRUCTOR, ROLES.ASSISTANT]).order("name"), "직원");
...
export async function issueStaffAccount(svc, sb, { name, loginId, role }) {
  ...
  // (어74) **앱이 낸 계정 표시**를 같이 켠다 — 이 표시가 있어야 「비밀번호 되돌리기」가 듣는다(대전제-12).
  //         이어 쓰는 계정(issuedByApp=false)은 켜지 않는다 — 쓰던 비밀번호를 앱이 건드리지 않는다. 한 번 켜진 것은 내리지 않는다.
  const mine = u.issuedByApp ? { must_change_pw: true, issued_by_app: true } : {};
  if (had) changed(await db(svc).from("profiles").update({ role, name: nm, login_id: id, state: "active", ...mine }).eq("id", u.id).select("id"), "사람 줄을 수정 못 함");
  else changed(await db(svc).from("profiles").insert({ id: u.id, role, name: nm, login_id: id, state: "active", must_change_pw: u.issuedByApp, issued_by_app: u.issuedByApp }).select("id"), "사람 줄을 못 세움");
...
export async function setStaffRole(sb, profileId, role) {
  ...
  const r = changed(await db(sb).from("profiles").update({ role }).eq("id", profileId).in("role", [ROLES.INSTRUCTOR, ROLES.ASSISTANT]).select("id"), "역할을 수정 못 함");
...
export async function setStaffState(sb, profileId, state, meId = null) {
  ...
  const r = changed(await db(sb).from("profiles").update({ state }).eq("id", profileId).in("role", [ROLES.INSTRUCTOR, ROLES.ASSISTANT]).select("id"), "상태를 수정 못 함");
  if (!r?.length) throw new Error("선생님·조교가 아닙니다");
  return { state };
}

/** (어74) 직원 비밀번호 되돌리기 — 원장님 2026-09-17 「강사조교 비밀번호 초기화 가능하게 해줘 이게 제일급해」.
 *  손은 **아이 쪽과 한 벌**(lib/student.js resetPassword · 원칙-1) — 첫 비밀번호로 되돌리고 must_change_pw 를 켠다(첫 로그인에 반드시 바꾼다).
 *  RESET_STAFF 라 선생님·조교만 · **원장 줄은 못 한다**(잠기면 들어올 문이 없다) · 앱이 낸 계정만(대전제-12). */
export const resetStaffPassword = (svc, sb, profileId) => resetPassword(svc, sb, profileId, RESET_STAFF);
