...
/** 옛 설정 tuition.byGrade 의 키 · 「중2」 「고1」 */
...
/** 입금 확인 한 번에((가)-⑩ · 남긴 것 13 「도장」) — 그 달 「안 받음」 줄 전부를 받은 날 = 그날로 저장할 줄. 금액은 줄에 선 것(적은 것 › 세어 나온 제안 — 저장하면 payment 에 적힌다) · 수납 방법은 안 건드린다(칸을 안 보낸다) · 「받음」·「금액 없음」 줄은 그대로 · 손은 savePayments 그대로(도장 손을 따로 안 만든다) */
export const payAllEdits = (rows = [], date) => rows.filter((r) => r.state === "unpaid").map((r) => ({ student_id: r.student_id, amount: r.amount, paid_on: date }));
...
/** 학년별 기준(옛 설정 tuition.byGrade · 13 이 단가 줄 없는 아이의 제안에 쓴다) · 키 열둘 · 값은 원 · 비우면 없앤다(0원이 아니다) · 4단계-4 */
