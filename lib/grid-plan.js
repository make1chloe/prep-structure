...
import { monthGrid, monthLabel, nextYm } from "./plan-plan.js";
...
export const PICK_OF = Object.freeze([["book", "교재"], ["exam", "학교 시험"], ["unit", "단원"]]);
...
export const TEMPLATES = Object.freeze([
  { key: "scope", label: "학교별 시험범위", rows: "school", cols: [["학년", "select", GRADES], ["시험", "pick", { of: "exam" }], ["교재", "pick", { of: "book" }], ["범위", "text"], ["영어 시험일", "date"], ["비고", "text"]] },
...
const NOT_TYPE = (t) => `칸 종류가 아닙니다: ${t}`;   // 칸 읽기·값 읽기가 같은 글 한 벌(원칙-1)
/** 날짜 칸은 하루(「2026-03-04」) 또는 기간({from, to}) — 원장님 9/16 「날짜속성에 시작일 종료일 개념이 없어. 체크박스로 선택할수있게해」.
 *  종료일이 비면 하루로 되돌아간다(값 한 벌 · 옛 줄은 그대로 글자 하나) */
export const dateFrom = (v) => (v == null ? "" : typeof v === "object" && !Array.isArray(v) ? String(v.from ?? "") : String(v));
export const dateTo = (v) => (v != null && typeof v === "object" && !Array.isArray(v) ? String(v.to ?? "") : "");
...
export function parseCol({ label, type, options }) {
  ...
  if (!COL_TYPES.some(([k]) => k === type)) throw new Error(NOT_TYPE(type));
  let opts = [];
...
export function parseCell(col, raw) {
  ...
  switch (col.type) {
    ...
    case "date": { const f = dateFrom(raw), t = dateTo(raw);
      if (!isDate(f) || (t && !isDate(t))) throw new Error(`날짜가 아닙니다: ${isDate(f) ? t : f || raw}`);
      if (!t) return f;   // 종료일이 비면 하루로 되돌아간다
      if (t < f) throw new Error("종료일이 시작일보다 앞섭니다");
      return { from: f, to: t }; }
    ...
    default: throw new Error(NOT_TYPE(col.type));
  }
}
...
export function cellText(col, value, refs = {}) {
  ...
  switch (col.type) {
    ...
    case "date": { const f = dateFrom(value), t = dateTo(value); return isDate(f) ? (isDate(t) ? `${md(f)}~${md(t)}` : md(f)) : ""; }
...
export function convertCell(from, to, value, refs = {}) {
  ...
  const text = from.type === "pick" ? (() => { const t = cellText(from, value, refs); return t && !t.startsWith("(") ? t : null; })()
    ...
    : from.type === "date" ? (dateTo(value) ? `${dateFrom(value)}~${dateTo(value)}` : dateFrom(value) || null)
...
/** 「한 학교만 크게」 ▣((가)-⑦ — 목업 06c 「그 학교(학생) 하나만 크게 펴서 고칩니다」) — 표에 그 줄만 남긴다 · 없는 줄이면 전부 */
export const focusRows = (rows = [], focus = null) => (focus && (rows ?? []).some((r) => r.id === focus) ? rows.filter((r) => r.id === focus) : rows ?? []);
...
/** (너) 아이·학부모 「우리 학교」 카드(07·09) — grid_mine 한 벌을 줄로: 공개한 학교 줄 표마다 · 그 아이 학교 줄마다 · 살아 있는 칸의 값 글(cellText 한 벌 · 빈 값은 뺀다) · 「학년」 칸은 줄 이름으로 · 값 없는 줄·표는 뺀다 */
export function mineLines(board) {
  const refs = { books: board?.books ?? [], exams: board?.exams ?? [], units: board?.units ?? [] };
  return (board?.grids ?? []).map((g) => {
    const cols = alive(g.cols).slice().sort((a, b) => (a.sort ?? 0) - (b.sort ?? 0));
    const rows = (g.rows_ ?? []).map((r) => { const cells = cols.map((c) => ({ id: c.id, label: c.label, text: cellText(c, r.cells?.[c.id], refs) })).filter((x) => x.text); const grade = cells.find((x) => x.label === "학년")?.text; return { id: r.id, title: [r.school, grade].filter(Boolean).join(" · "), cells: cells.filter((x) => !(grade && x.label === "학년")) }; }).filter((r) => r.cells.length);
    return { id: g.id, label: g.label, rows };
  }).filter((g) => g.rows.length);
}
/** 정렬 — 「내가 정한 순서」(sort 그대로) 이거나 만든 칸 하나로(원장님 9/16 「내가 정한순서말고 무슨 순서가 더 없음. 만든 칸을 기준으로 정렬하게 하면됨」).
 *  견주는 열쇠는 종류마다 — 날짜는 시작일 · 예/아니오는 예 먼저 · 체크 목록은 덜 끝낸 것 먼저 · 나머지는 글(cellText 한 벌). 빈 칸은 방향과 상관없이 늘 뒤 */
export function sortKey(col, value, refs = {}) {
  if (value == null || !col) return "";
  switch (col.type) {
    case "date": return dateFrom(value);
    case "yn": return value === true ? "0" : value === false ? "1" : "";
    case "checklist": { const l = Array.isArray(value) ? value : []; if (!l.length) return ""; return String(Math.round((l.filter((x) => x.done).length / l.length) * 1000)).padStart(4, "0"); }
    default: return cellText(col, value, refs);
  }
}
export const SORT_DIRS = Object.freeze([["asc", "↑"], ["desc", "↓"]]);
export function sortRows(rows = [], cols = [], by = null, dir = "asc", refs = {}) {
  const list = (rows ?? []).slice();
  const col = (cols ?? []).find((c) => c.id === by); if (!col) return list;
  return list.map((r, i) => ({ r, i, key: sortKey(col, r.cells?.[col.id], refs) })).sort((a, b) => {
    if (!a.key && !b.key) return a.i - b.i;
    if (!a.key) return 1;
    if (!b.key) return -1;
    const c = a.key.localeCompare(b.key, "ko", { numeric: true });
...
/** 내 표 / 전체 표 — 원장은 전부, 선생님·조교는 제 것만(created_by) 보되 「전체 표(원장)」 는 원장만 */
...
export { md, monthLabel, nextYm };

/** 📅 달력 보기(원장님 9/16 「달력보기만 좀 만들어줘. 보드 끌어서옮기기는 없어도돼」) — 날짜 칸 하나를 축으로 그 달을 그린다.
 *  하루짜리는 그 날 한 칸 · 기간({from,to})은 시작일부터 종료일까지 칸마다. 날짜 칸이 없으면 axis 는 null(까닭은 화면이 말한다).
 *  달 칸은 일정 12 와 같은 한 벌(monthGrid) */
export function calendarOf(g, ym, colId = null, refs = {}) {
  const { cols, rows } = liveOf(g);
  const dates = cols.filter((c) => c.type === "date");
  const axis = dates.find((c) => c.id === colId) ?? dates[0] ?? null;
  const cells = monthGrid(ym).map((c) => ({ ...c, items: [] }));
  if (!axis) return { axis: null, dates, cells, none: rows, out: [] };
  const none = [], out = [];
  for (const r of rows) {
    const v = r.cells?.[axis.id], f = dateFrom(v);
    if (!isDate(f)) { none.push(r); continue; }
    const t = isDate(dateTo(v)) ? dateTo(v) : f;
    let hit = false;
    for (const c of cells) if (c.date >= f && c.date <= t) { c.items.push({ id: r.id, row: r, title: cardTitle(g, r, refs), head: c.date === f, span: t > f }); hit = true; }
    if (!hit) out.push(r);
  }
  return { axis, dates, cells, none, out };
}
