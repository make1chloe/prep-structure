 ...
 */
import { todaySeoul, addDays, addMonths, endOfMonth } from "./day.js";
// 내려간 반(옛 특강 — 0173)을 「배정됨」 으로 안 세려고. 판단은 classTerm 한 곳 (원칙 1)
import { loadClassesWithTerm, isArchived } from "./classTerm.js";
import { loadMakeupTodo } from "./makeupTodo.js";
import { needsScope, examTitle } from "./examList.js";
// **학교·학년을 여기서 견주지 않는다** — 판단은 lib/who 한 곳에 있다
import { takesExam } from "./who.js";
// 단원평가가 두 곳(판·아이가 낸 것)에서 온다는 판단은 한 곳에 (감사 ⑥-3)
import { madeKeys, unitScored } from "./unitScore.js";
 ...
 */
/**
 * **20초 안에는 다시 안 센다** (2026-08-14 — 「모든 페이지의 로딩 자체가
 * 느려」). 배지 세기는 열일곱 조회짜리라, 화면을 옮길 때마다 전부 다시
 * 돌면 그게 모든 화면의 세금이 된다. 남은 일 개수가 20초 늦게 맞춰지는
 * 것은 아무 문제가 아니다 — 어차피 다음 화면 이동에서 맞춰진다.
 *
 * 서버 메모리에 든다 — 서버가 새로 깨어나면 비어 있고(콜드스타트), 그때
 * 한 번 다시 센다. 같은 학원 선생님들끼리는 같은 숫자를 봐도 맞다.
 */
const _memo = { at: 0, day: "", value: null };
const MEMO_MS = 20 * 1000;
/**
 * **동시 호출 합치기** (성능수리 v3 §2-3-1). 대시보드는 TopBar 와 TodoBar 가
 * 형제로 **동시에** menuTodos 를 부른다 — 메모가 식은 순간이면 ~20조회짜리
 * 셈이 두 벌 나란히 나갔다. 진행 중인 promise 를 같이 기다리면 한 벌이 된다.
 *
 * 규칙 세 가지 (창 연장이 아니다 — 같은 순간의 중복 제거일 뿐):
 *   ① 메모와 같은 production 게이트 — 검사(가짜 DB 여러 벌)에는 안 끼어든다
 *   ② 키에 today 포함 (_memo.day 와 동일) — 날이 바뀌면 딴 셈이다
 *   ③ 실패한 promise 는 안 남긴다 — 거부를 물려주면 20초간 배지 전멸이 고착된다
 */
let _inflight = null; // { day, promise }

export async function menuTodos(supabase, today = todaySeoul()) {
  // 배포에서만 기억한다 — 검사는 가짜 DB 를 바꿔가며 여러 번 부르는데
  // 기억이 끼면 첫 답이 계속 나온다 (실제로 check-badges 가 그걸 잡았다)
  const memoOn = process.env.NODE_ENV === "production";
  if (memoOn && _memo.value && _memo.day === today && Date.now() - _memo.at < MEMO_MS) {
    return _memo.value;
  }
  if (memoOn && _inflight && _inflight.day === today) return _inflight.promise;
  const p = (async () => {
    const out = await _menuTodos(supabase, today);
    _memo.at = Date.now();
    _memo.day = today;
    _memo.value = out;
    return out;
  ...
  if (memoOn) {
    _inflight = { day: today, promise: p };
    // 끝나면 비운다 — 성공이면 메모가 이어받고, 실패면 다음 호출이 새로 센다
    p.then(
      () => { if (_inflight?.promise === p) _inflight = null; },
      () => { if (_inflight?.promise === p) _inflight = null; }
    );
  }
  return p;
...
async function _menuTodos(supabase, today) {
  if (!supabase) return {};
  ...
  const [
    ...
    unsent, students, monthlyDone, unitItems, unitScoresToday, schoolScores,
    inquiries, overdueTodos, exams, scopes,
    inClass, skipRows, makeup, confirms, bookWait,
    classRows, extraNow,
  ] = await Promise.all([
    ...
    n(() => supabase.from("daily_reports").select("id", { count: "exact", head: true })
      .eq("date", today).eq("report_written", true).is("archived_at", null)),
    /**
    ...
    rows(() => supabase.from("daily_reports")
      .select("id, student_id, sent_total, sent_unit").eq("date", today).is("archived_at", null)),
     ...
     */
    rows(() => supabase.from("daily_reports").select("id, student_id, skip_kinds")
      .lte("date", today).gte("date", monthAgo).eq("report_written", true).is("sent_at", null).is("archived_at", null)),
    ...
    /**
     * **아이가 오늘 직접 낸 단원평가** (0106 — 감사 ⑥-3).
     * 아이는 판을 못 쓰니 scores 에만 들어간다. 이것을 안 보면 아이가
     * 냈는데도 「점수 안 적힘」 배지가 안 꺼져서, 끌 방법이 없는 숫자가 된다.
     * source 칸이 없는 옛 DB 면 조회가 error → 빈 목록 (예전 그대로).
     */
    rows(() => supabase.from("scores").select("student_id, taken_on, kind, source")
      .eq("kind", "unit").eq("taken_on", today)),
     ...
     */
    /**
     * hidden 칸을 **같이** 읽는다 (성능수리 v3 §2-3-2) — 숨긴 시험 번호를
     * 따로 물으면(hiddenExamIds) 이 파도 **뒤에** 왕복이 한 단 더 선다.
     * 숨김 판정은 이 창 안의 시험에만 쓰이므로 같은 줄의 칸이면 충분하다.
     * 단 hidden 은 0060 에 늦게 생긴 칸 — 없는 DB 에서 조회가 통째로
     * 죽으면 시험 배지가 조용히 전멸하므로, 실패하면 **칸 없이 한 번 더**
     * (사다리 — 그땐 「숨긴 것 없음」 으로 본다. lib/schedule 과 같은 태도)
     */
    (async () => {
      const sel = (cols) => supabase.from("exam_periods").select(cols)
        .gte("english_on", addDays(today, -35)).lte("english_on", addDays(today, 60));
      try {
        const q = await sel("id, school, grade, name, neis_name, english_on, hidden");
        if (!q.error) return q.data || [];
        const q2 = await sel("id, school, grade, name, neis_name, english_on");
        return q2.error ? [] : q2.data || [];
      } catch { return []; }
    })(),
    rows(() => supabase.from("prep_scopes").select("exam_id")),

    // 반 배정 — 어느 반에도 안 든 재원생 (어느 반인지까지 — 내려간 반을 거르려고)
    rows(() => supabase.from("class_students").select("class_id, student_id")),
    ...
    // 다음 달 회차 확정 도장 (0123) — 25일부터 「회차」 에 미확정을 센다
    rows(() => supabase.from("month_confirms")
      .select("student_id, principal_at").eq("ym", addMonths(ym, 1))),

    // 안내 안 나간 사용 예정 교재 (0125) — 상담 등록·직접 추가로 배정만
    // 되고 문자가 안 나간 것. 발송 화면 안내 탭의 확인 목록과 같은 기준
    rows(() => supabase.from("student_textbooks")
      .select("student_id")
      .eq("status", "active").is("notified_on", null).gt("assigned_on", today)),

    // 반 목록 (기간 칸째로) — 내려간 반(옛 특강, 0173)의 배정 줄은
    // 「배정됨」 이 아니다. 기간 칸이 없는 옛 DB 면 전부 무기한으로 읽힌다
    loadClassesWithTerm(supabase, "id").catch(() => []),
    // 특강 보유자 (0164) — 반이 없어도 미배정이 아니다 (수강료 noClass 와 같은 기준)
    rows(() => supabase.from("student_extra_schedules")
      .select("student_id").gte("to_date", today)),
  ]);
   ...
   */
  // 판정(done·weak·missing)이 이미 찍힌 (판, 항목) 쌍 — 대기에서 빠진다
  const marked = new Set(
    items
      .filter((i) => i.status === "done" || i.status === "weak" || i.status === "missing")
      .map((i) => `${i.daily_report_id}|${i.homework_item_id}`)
  );
  const waiting = items.filter(
    (i) =>
      i.student_done_at &&
      i.status === "inclass" &&
      !unitIds.has(i.homework_item_id) &&
      !marked.has(`${i.daily_report_id}|${i.homework_item_id}`)
  ).length;
   ...
   * **아이가 직접 낸 것도 적힌 것으로 센다** (0106 — 감사 ⑥-3). 아이는 판을
   * 못 쓰니 scores 에만 들어가는데, 그것을 안 보면 냈는데도 배지가 안 꺼진다.
   * 두 곳을 견주는 판단은 lib/unitScore 한 곳이다.
   */
  const hasUnit = new Set(
    items.filter((i) => unitIds.has(i.homework_item_id)).map((i) => i.daily_report_id)
  );
  const madeToday = madeKeys(unitScoresToday);
  const unitNoScore = todayReports.filter(
    (r) => hasUnit.has(r.id) && !unitScored(r, madeToday, today)
  ).length;

  // **숨긴 시험은 뺀다** — 대시보드도 그렇게 센다. 안 빼면 치웠는데도
  // 배지가 안 꺼져서 「이건 왜 안 없어지지」 가 된다.
  // 번호는 위에서 같이 받은 hidden 칸에서 바로 — 왕복 한 단이 통째로 준다
  const hidden = new Set(exams.filter((e) => e.hidden === true).map((e) => e.id));
  ...
  /**
   * **내려간 반은 「배정됨」 이 아니다** (0173 — 옛 특강반 하강).
   * 배정 줄(class_students)은 기록으로 남지만, 그 반이 종강·보관으로
   * 내려갔으면 그 아이는 지금 반이 없는 것이다 — 안 거르면 옛 특강 줄이
   * 미배정 경고를 영영 가린다. 특강(0164) 보유자는 반이 없어도 미배정이
   * 아니다 (수강료 화면의 noClass 와 같은 기준 — 5단계 M7).
   */
  const gone = new Set(classRows.filter((c) => isArchived(c, today)).map((c) => c.id));
  const assigned = new Set([
    ...inClass.filter((c) => !gone.has(c.class_id)).map((c) => c.student_id),
    ...extraNow.map((x) => x.student_id),
  ]).size;
   ...
   */
  const monthlyLeft = daysToMonthEnd <= 3 ? Math.max(0, enrolled - monthlyDone) : 0;
  ...
  const out = {
    ...
    prep: noScope,
    scores: noScoreYet,
    // 기한 지난 할일은 **「할일」 칸**에 붙는다 (2026-08-28). 메뉴가 다섯으로
    // 바뀌면서 「달력」 과 「할일」 이 나란히 섰다 — 숫자는 고치러 가는 자리에
    // 붙어야 한다 (달력에 붙으면 눌러도 할일이 안 보인다)
    todo: overdueTodos,
    /**
     * 리포트 = 리포트 미발송 + 교재 안내 안 나간 학생 (0125) **+ 월간
     * 미작성**. 재원생만.
     *
     * **월간이 여기로 합쳐졌다** (원장님, 2026-08-28 — 「일일과 월간을
     * 합쳐서 리포트로 만들고」). 메뉴가 한 칸이 되었으니 셈도 한 칸이다 —
     * 없어진 키로 세면 그 숫자는 조용히 아무 데도 안 붙는다.
     *
     * 오히려 이쪽이 맞다. 이 화면의 첫 탭 「보낼 것」 은 **처음부터**
     * 리포트 · 교재 안내 · 월간을 한 숫자로 세고 있었는데(SendTodo 의
     * total), 메뉴만 둘로 갈라 세고 있었다 — 위의 규칙 3(화면 안의
     * 숫자와 같아야 한다)이 그동안 어긋나 있던 자리다.
     */
    report:
      // 재원생만 (2026-08-21) — 화면은 퇴원생 리포트를 빼는데 배지는 다 세서
      // 「발송 1건」 이 영영 꺼지지 않았다 (끌 수 없는 숫자 금지)
      unsent.filter((r) =>
        !(r.skip_kinds || []).includes("report") &&
        students.some((st) => st.id === r.student_id)
      ).length +
      new Set(
        bookWait
          .map((b) => b.student_id)
          .filter((sid) => students.some((st) => st.id === sid))
      ).size +
      monthlyLeft,
    consult: inquiries,
    /**
     * 다음 달 회차 미확정 (0123) — 25일부터 월말까지만. 0123 커밋이
     * 조회만 넣고 구조분해에 이름을 안 달아 결과가 버려지고 있었다
     * (2026-08-15 발견) — 배지가 한 번도 안 붙던 잠복 버그.
     */
    schedule:
      Number(today.slice(8, 10)) >= 25
        ? students.filter(
            (st) => !confirms.some((c) => c.student_id === st.id && c.principal_at)
          ).length
        : 0,
  };
...
export const TODO_LABEL = {
  ...
  scores: (v) => `성적 미입력 ${v}명`,
  todo: (v) => `기한 지난 할일 ${v}건`,
  // 월간이 이 칸으로 합쳐졌다 (2026-08-28) — 셋을 한 숫자로 말한다
  report: (v) => `발송할 것 ${v}건 (리포트 · 교재 안내 · 월간)`,
  consult: (v) => `미처리 문의 ${v}건`,
  schedule: (v) => `다음 달 회차 미확정 ${v}명`,
};
