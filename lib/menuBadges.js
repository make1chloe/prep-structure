...
export async function menuTodos(supabase, today = todaySeoul()) {
  ...
  const [
    cameToday, writtenToday, todayReportIds,
    unsent, enrolled, monthlyDone,
    ...
    // 숙제 검사 — 아이가 「다 했어요」 를 누른 것 중 아직 안 찍은 것.
    // 항목은 리포트에 매달려 있어서 오늘 리포트 번호를 먼저 알아야 한다
    rows(() => supabase.from("daily_reports").select("id").eq("date", today)),

    // 발송 — 써놓고 아직 안 보낸 것 (어제 것도 본다. 오늘 것만 보면 영영 안 보인다)
    n(() => supabase.from("daily_reports").select("id", { count: "exact", head: true })
      .lte("date", today).gte("date", monthAgo).eq("report_written", true).is("sent_at", null)),

    n(() => supabase.from("students").select("id", { count: "exact", head: true }).eq("status", "enrolled")),
    ...
    // 내신 대비 — 곧 보는 시험인데 범위를 아직 안 넣은 것
    rows(() => supabase.from("exam_periods").select("id, school, grade, name, neis_name, english_on")
      .gte("english_on", today).lte("english_on", addDays(today, 21))),
  ...
  // 오늘 리포트가 있어야 검사 대기를 셀 수 있다
  const ids = todayReportIds.map((r) => r.id);
  const waiting = ids.length
    ? await n(() => supabase.from("daily_report_items").select("id", { count: "exact", head: true })
        .in("daily_report_id", ids).not("student_done_at", "is", null).eq("status", "assigned"))
    : 0;
  ...
  const noScope = exams.filter((e) => !hidden.has(e.id) && needsScope(e) && !scoped.has(e.id)).length;
  ...
  const out = {
    today: Math.max(0, cameToday - writtenToday),
    check: waiting,
    plan: (makeup || []).length,
    classes: noClass,
    prep: noScope,
...
/**
 * 배지에 마우스를 올렸을 때 뜨는 말. **무엇이 남았는지 한 줄로.**
 * 숫자만 있으면 「3이 뭐지」 하고 눌러봐야 안다.
 */
export const TODO_LABEL = {
  today: (v) => `리포트를 아직 안 쓴 학생 ${v}명`,
  check: (v) => `아이가 다 했다고 누른 것 ${v}건 — 아직 안 찍었습니다`,
  plan: (v) => `보강을 아직 안 잡은 결석 ${v}건`,
  classes: (v) => `어느 반에도 안 든 재원생 ${v}명`,
  prep: (v) => `3주 안에 보는 시험 중 범위를 안 넣은 것 ${v}개`,
  tasks: (v) => `기한이 지났거나 오늘까지인 할일 ${v}건`,
  report: (v) => `써놓고 아직 안 보낸 리포트 ${v}건`,
  monthly: (v) => `이번 달 월간리포트를 아직 안 쓴 학생 ${v}명`,
  consult: (v) => `아직 등록도 거절도 안 한 문의 ${v}건`,
};
