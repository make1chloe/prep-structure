...
export async function menuTodos(supabase, today = todaySeoul()) {
   ...
   */
  const noScoreYet = missingScores({ exams, students, scores: schoolScores, hidden, today }).length;
 ...
 */
export function missingScores({ exams = [], students = [], scores = [], hidden = new Set(), today = todaySeoul() } = {}) {
        ...
        .forEach((st) => {
          ...
          if (!got) {
            out.push({
              ...
              examId: e.id, examName: e.name || "시험", on: e.english_on,
