...
export async function menuTodos(supabase, today = todaySeoul()) {
  ...
  const [
    ...
    rows(() => supabase.from("students").select("id, school, grade").eq("status", "enrolled")),
    ...
    rows(() => supabase.from("scores").select("student_id, taken_on")
   ...
   *
   * 시험과 성적은 표가 이어져 있지 않다(scores 에 시험 번호가 없다).
   * 그래서 **그 학교 · 그 학년 아이**의 성적이 시험 무렵에 있는지로 본다.
   * 모의고사는 빼고(내신만), 숨긴 시험도 뺀다.
   */
  const scoreDays = new Map();
  schoolScores.forEach((sc) => {
    if (!sc.taken_on) return;
    if (!scoreDays.has(sc.student_id)) scoreDays.set(sc.student_id, []);
    scoreDays.get(sc.student_id).push(sc.taken_on);
  });
  let noScoreYet = 0;
  exams
    ...
    .forEach((e) => {
      ...
      students
        ...
        .forEach((st) => {
          const got = (scoreDays.get(st.id) || []).some((d) => d >= from);
          if (!got) noScoreYet += 1;
        });
    });
...
export const TODO_LABEL = {
  today: (v) => `리포트를 아직 안 썼거나 단원평가 점수를 안 적은 것 ${v}건`,
  ...
  scores: (v) => `시험은 끝났는데 성적이 아직 안 들어온 학생 ${v}명`,
