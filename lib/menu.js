...
export const SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      // 돈은 원장님 것이다. 조교·강사 메뉴에서는 빠진다 (0079 로 표까지 막았다)
  ...
  },
  {
    ...
    items: [
      // 발송 열쇠 · service_role 키 · 이관은 전부 원장님 것이다
...
export function menuFor(profile) {
  ...
  const rest = ALL_ITEMS.filter(
    (i) => allowed(i) && !hidden.has(i.key) && !order.includes(i.key)
  );
  const byKey = new Map(ALL_ITEMS.filter(allowed).map((i) => [i.key, i]));
