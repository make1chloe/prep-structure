...
const RIDERS = Object.freeze([
  { after: "/schedule", href: "/schedule/grid", name: "내신", key: "page.schedule" },   // 06c 학교별 표
  { after: "/schedule", href: "/schedule/todo", name: "할 일", key: "page.schedule" },   // 05 내 할 일
]);
export function menuFor(role, rows) {
  ...
  for (const k of KEYS) {
    ...
    for (const r of RIDERS) if (r.after === k.href) out.push({ href: r.href, name: r.name, key: r.key });   // 얹은 탭은 태운 탭이 열릴 때만 뜬다
  }
  return out;
}
