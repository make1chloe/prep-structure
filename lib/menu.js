...
export const SECTIONS = [
  ...
  },
  {
    ...
    label: "일정",
    href: "/menu/calendar",
    keys: ["tasks", "schedule", "schools", "neis"],
    items: [
      { href: "/tasks", key: "tasks", label: "할일 · 달력" },
      { href: "/schedule", key: "schedule", label: "회차 관리", desc: "휴강 · 회차" },
      { href: "/schools", key: "schools", label: "학교 · 시험" },
      /**
       * **바꾸기 전을 보는 자리** (원장님, 2026-08-09 — 「오류가 난 건지
       * 입력이 안 된 건지 알 수가 없네」). 다른 화면은 앱이 정리한 뒤라,
       * 없는 것의 까닭을 가릴 수가 없다.
       */
      { href: "/neis", key: "neis", label: "나이스 원본", desc: "학교가 올린 그대로" },
    ],
  },
  {
    /**
  ...
  },
  {
    ...
    label: "설정",
    href: "/menu/settings",
    keys: ["settings", "messages", "screen", "admin", "sql", "import"],
    items: [
       ...
       */
      { href: "/settings", key: "settings", label: "연동 · 설정", desc: "키 · 앱 알림 · 운영 규칙", only: "principal" },
      /**
      ...
      /**
       * **다 만들고 나면 안 여는 것들** (2026-08-07 — 「노션이관과 sql db등
       * 웹앱 자체가 완성되고 나면 안쓰는 기능들은 따로 모아줘」).
       *
       * 없애지는 않는다 — 새 기능을 넣을 때마다 SQL 은 한 번씩 돌려야 한다.
       * 매일 쓰는 것과 같은 줄에서 한 칸 뒤로 옮길 뿐이다.
       */
      { href: "/settings/admin", key: "admin", label: "관리자", desc: "SQL · 노션 이관 · 아이콘", only: "principal" },
    ],
  },
];
