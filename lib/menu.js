...
/** (어28) /ops 가 곧 학생 14 다(app/ops/page.js 가 학생 화면을 그대로 낸다) · 「학생」 탭 하나 · 수강료 13 은 그 안 갈래(?v=fee) · 상담 18·자료함 20 은 그 아래 주소라 같은 탭이 파랗다 */
...
const RIDERS = Object.freeze([
  { after: "/ops", href: "/ops/files", name: "자료함" },   // 20 — 아이·학부모가 날마다 사진을 보낸다. 2번 → 1번
  { after: "/schedule", href: "/schedule/grid", name: "내신" },   // 06c 학교별 표
  { after: "/schedule", href: "/schedule/todo", name: "할 일" },   // 05 내 할 일
  { after: "/schedule", href: "/scores", name: "성적" },   // 16 — 탭이 아예 없어 시험 회차·할 일 안에서만 닿았다. 2번 → 1번
]);
