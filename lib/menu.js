 ...
 *
 *   대시보드 — 오늘수업, 재원생, 발송, 숙제검사  ※ 발송을 일일/월간으로 나누기
 *   일정     — 출결, 시험, 달력, 할일
 *   학습     — 내신범위, 교재, 영상
 *   성장     — 신규상담, 상담일지, 성적
 *   관리자   — 회차, 수납, 설정, 문구, 화면
...
export const SECTIONS = [
  {
    ...
    label: "대시보드",
    href: "/",
    keys: ["home", "today", "students", "report", "monthly", "check"],
    items: [
      { href: "/today", key: "today", label: "오늘 수업" },
      { href: "/students", key: "students", label: "재원생" },
      /**
       * **발송을 일일 / 월간으로 나눈다** (원장님, 2026-08-28).
       *
       * 화면은 원래부터 둘이었다(`/report` · `/monthly`) — 메뉴에서 한쪽만
       * 「발송」 이라고 불려서, 「월간리포트」 가 그 아래 딸린 것처럼 보였다.
       * 이름을 나란히 맞춘 것이지 화면을 쪼갠 것이 아니다.
       */
      { href: "/report", key: "report", label: "일일 리포트", desc: "데일리리포트 · 하원 · 안내" },
 ...
 * (「발송」 은 2026-08-28 에 대시보드 안의 두 칸이 되었다 — 위 이야기는
 *  그 묶음이 있던 때의 기록이다)
 */
