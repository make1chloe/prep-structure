...
const RIDERS = Object.freeze([
  { after: "/ops", href: "/ops/files", name: "자료실" },   // 20 · 아이·학부모가 날마다 사진을 보낸다. 2번 → 1번
  { after: "/schedule", href: "/schedule/grid", name: "내신" },   // 06c 학교별 표
  { after: "/schedule", href: "/schedule/todo", name: "업무" },   // 05 내 업무
  { after: "/schedule", href: "/scores", name: "성적" },   // 16 · 탭이 아예 없어 시험 회차·업무 안에서만 닿았다. 2번 → 1번
]);
export function menuFor(role, rows) {
  ...
  for (const k of KEYS) {
    ...
    for (const r of RIDERS) if (r.after === k.href) out.push({ href: r.href, name: r.name, key: k.key });   // 얹은 탭은 태운 탭이 열릴 때만 뜨고, **키도 그 탭의 것 그대로**(키 글자를 두 벌로 안 적는다 · 검사-perm)
  }
  return out;
}
