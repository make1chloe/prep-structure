...
export function lateReasons(r = {}, rule = {}) {
  ...
  if (rep.word_total > 0) {
    const ok = passed(rep.word_correct, rep.word_total, rule.wordWrongPct ?? 10);
    if (ok === false) {
      out.push({
        key: "retest",
        label: "단어 재시험",
        detail: `${rep.word_correct ?? 0}/${rep.word_total}`,
      });
    }
  }

  // 남아서 채우고 갈 것 (오늘 마무리에 아직 todo 로 남은 것)
  ...
  if (todo.length > 0) {
    ...
    // 오늘 마무리에 아직 안 올렸어도, 미제출·미흡이 있으면 늦어질 수 있다
