...
import { isMockExam } from "./examList.js";
...
/**
 * **쪼개져 남은 옛 줄** — 이번에 받아온 기간이 통째로 삼키는 나이스 줄들.
 *
 * 원장님 (2026-08-09) — 「대부분의 학교들이 내신 시험 시작 날짜만 나오고
 * 나머지 날짜가 아예 표시가 안 돼. 시험을 하루만 보는 건 모의고사가 그런
 * 거야, 내신은 아니야」
 *
 * 학년 때문에 날마다 쪼개졌던 시절의 줄이 DB 에 그대로 남아 있다. 이제
 * 사흘짜리 한 줄로 제대로 들어오는데, 옛 줄이 그 안에 그대로 있으면 같은
 * 시험이 네 줄로 보인다.
 *
 * **좁게 잡는다.** 지우는 일은 되돌릴 수 없다 —
 *   1. 나이스가 만든 줄일 것 (원장님이 손으로 적으신 줄은 절대 안 건드린다)
 *   2. 같은 학교일 것
 *   3. 내신은 내신끼리 · 모의고사는 모의고사끼리
 *   4. 이름이 같고, 학년이 같거나 안 적혀 있을 것 — 고3 기말이 고1·2 기말
 *      기간 안에 따로 있는 학교가 있다. 그건 진짜로 다른 회차다
 *   5. 성적·범위가 하나도 안 붙어 있을 것
 *   6. 기간이 새 줄 안에 온전히 들어앉을 것
 *
 * @param cand   이번에 받아온 후보 { school, name, grade, from_date, to_date }
 * @param pool   지금 DB 에 있는 줄들
 * @param keepId 남길 줄 (붙였거나 방금 만든 줄)
 * @param inUse  성적·범위가 붙어 있는 회차 id 들 (Set)
 */
export function absorbable(cand, pool = [], keepId = null, inUse = new Set()) {
  if (!cand?.school || !cand?.from_date || !cand?.to_date) return [];
  const flat = (v) => (v || "").toString().replace(/\s/g, "");
  const mock = isMockExam(cand);
  return pool.filter(
    (x) =>
      x.id !== keepId
      && (x.source || "") === "neis"
      && flat(x.school) === flat(cand.school)
      && isMockExam(x) === mock
      && flat(x.name) === flat(cand.name)
      && (!flat(x.grade) || flat(x.grade) === flat(cand.grade))
      && !inUse.has(x.id)
      && d(x.from_date) >= d(cand.from_date)
      && d(x.to_date) <= d(cand.to_date)
  );
}
