/** 발송 판단 한 벌(순수, 목업 10) · 올린 기록(🕘 지금 · 📨 마감하면 · 🔔 저절로 · 📢 예약) + 오늘 나간 것 · 읽음 셈 · 예약 시각(확정-㉕) · 치환 칸(뼈대-11).
...
import { labelOf } from "./notify-plan.js";
import { ymLabel } from "./report-plan.js";
import { hhmm } from "./late-plan.js";
/** 큐 일의 유형 · 넣는 자리(lib/late·arrival·plan·send)와 손(lib/send)이 같은 글자를 쓴다 */
export const KINDS = Object.freeze({ daily: "daily_report", late: "late_notice", arrival: "arrival_notice", leave: "leave_notice", plan: "attend_plan_notice", give: "give_notice" });   // (어96) 배부 안내 — 칸반 칸의 스위치가 켜져 있을 때만
export const JOB_NAME = Object.freeze({ daily_report: "데일리리포트", late_notice: "하원 지연 안내", arrival_notice: "등원 안내", leave_notice: "하원 안내", attend_plan_notice: "결석·지각 예정 안내", give_notice: "학습지 배부 안내" });
export const jobName = (kind) => JOB_NAME[kind] ?? kind;
/** 발송 이력 유형 → 이름 · **여기는 LABEL 과 말이 다른 것만**(데일리리포트) · 나머지는 LABEL 한 곳에서 온다(원칙-1 · (커)) */
export const LOG_NAME = Object.freeze({ daily: "데일리리포트", late: "하원 지연 안내", arrival: "등원 안내", leave: "하원 안내", plan_absent: "결석 예정 안내", plan_late: "지각 예정 안내", monthly: "월간 리포트", notice: "공지사항", score: "성적 입력 안내", fee: "수강료 안내" });
export const logName = (kind) => LOG_NAME[kind] ?? labelOf(kind) ?? kind;   // (커) 여기 없는 유형은 LABEL 의 말 그대로 · welcome·video·schedule·guide 가 키로 보이던 것(게이트 98)
/** 안 채운 치환 칸 · {{ 가 남아 있으면 못 나간다(뼈대-11). 이름 목록을 돌려준다 */
export const unfilled = (text) => [...String(text ?? "").matchAll(/\{\{\s*([^}]*?)\s*\}\}/g)].map((m) => m[1] || "빈 칸");
...
/** 발송 이력 한 줄의 상태 글 · **줄에는 그 줄만의 것**을 적는다(2026-09-12 실측: 발송 화면 설명문 1503자 중 768자가
 *  줄마다 반복된 같은 문장이었다 · 「리허설(off) · 실제로는 안 나감」 16줄 · 「못 보냄 · …이어져 있지 않습니다」 10줄).
 *  왜 안 나갔나(리허설인가 · 무엇이 없어서인가)는 **카드 머리에서 한 번** 말한다(failGroups · 머리 알약). */
export function logStatus(l) {
  if (l.failed_at) return { icon: "⚠️", text: "못 보냄", why: l.fail_why ?? "", bad: true, read: false, unread: false, rehearsal: false };
  if (!l.sent_at) return { icon: "🧪", text: seoulTime(l.created_at), bad: false, read: false, unread: false, rehearsal: true };
...
/** 못 보낸 까닭을 **한 번씩만** · [{why, n}] 많은 순. 줄마다 같은 문장을 반복하지 않기 위해(2026-09-12) */
export function failGroups(logs = []) {
  const m = new Map();
  for (const l of logs) if (l.failed_at) { const w = String(l.fail_why ?? "").trim() || "까닭 없음"; m.set(w, (m.get(w) ?? 0) + 1); }
  return [...m].map(([why, n]) => ({ why, n })).sort((a, b) => b.n - a.n);
}
...
/** 🕘 지금 · 오늘 수업 일지의 하원 지연(약속이 있는 것). 보내는 자리는 오늘 카드(01) 하나(확정-㊿) · 여기서는 상태만 */
...
/** 📨 마감하면 나갑니다 · 오늘 수업 일지마다 open(마감 전) · ready(보낼 것) · holes(치환 칸 빔) · scheduled · queued · sent */
export function dailyRows(sheets = [], logs = [], scheduled = [], jobs = []) {
  return sheets.map((s) => {
    ...
    const state = !closed ? "open" : holes.length ? "holes" : sch ? "scheduled" : job ? "queued" : log ? "sent" : "ready";   // 살아 있는 예약·큐가 나간 발송 이력보다 앞선다(다시 예약한 판은 ⏰)
...
export function scheduledRows(scheduled = [], jobs = [], sheets = [], nowIso, today) {
  const a = scheduled.filter((x) => !x.sent_at && !x.cancelled_at).map((x) => ({ id: x.id, jobId: null, name: x.student_name ?? sheets.find((s) => s.id === x.sheet_id)?.student_name ?? "?", what: x.kind === "daily" ? "데일리리포트" : `${labelOf(x.kind) ?? x.kind}${["monthly", "fee"].includes(x.kind) && x.body ? ` ${ymLabel(x.body)}` : ""}`, at: x.at, when: whenLabel(x.at, today), cancellable: true }));   // (어) 월간·수강료 예약은 아이 이름(send_board) · 달
...
/** 오늘 나간 것 · 발송 이력(최근 것부터) */
export const sentRows = (logs = []) => logs.map((l) => ({ id: l.id, name: l.student_name ?? "?", what: `${logName(l.kind)}${l.channel === "sms" ? ` · 문자${l.to_phone ? ` ${l.to_phone}` : ""}` : ""}`, status: logStatus(l), resendable: !logStatus(l).read && l.channel !== "sms" }));   // (커) 문자 발송 이력은 길과 가린 번호 · 다시 보내기는 앱 알림만
/** 머리 알약 · 🌙 지금 보낼 것(약속은 있는데 아직 안 보낸 하원 지연) · 📨 마감 n / N */
...
/** 뼈대-8 치환 칸 줄 · 표(v2.placeholder)의 설명을 「{{학생명}} · 아이 이름(예: 강민서)」 꼴로. 안 채운 자리는 unfilled 가 잡아 못 나간다(뼈대-11) */
