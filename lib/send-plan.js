...
/** 자취 갈래 → 이름 — **여기는 LABEL 과 말이 다른 것만**(데일리리포트) · 나머지는 LABEL 한 곳에서 온다(원칙-1 · (커)) */
...
export const logName = (kind) => LOG_NAME[kind] ?? labelOf(kind) ?? kind;   // (커) 여기 없는 갈래는 LABEL 의 말 그대로 — welcome·video·schedule·guide 가 열쇠로 보이던 것(게이트 98)
...
/** 자취 한 줄의 상태 글 — **줄에는 그 줄만의 것**을 적는다(2026-09-12 실측: 발송 화면 설명문 1503자 중 768자가
 *  줄마다 되풀이된 같은 문장이었다 — 「리허설(off) — 실제로는 안 나감」 16줄 · 「못 보냄 — …이어져 있지 않습니다」 10줄).
...
/** 못 보낸 까닭을 **한 번씩만** — [{why, n}] 많은 순. 줄마다 같은 문장을 되풀이하지 않기 위해(2026-09-12) */
...
/** 오늘 나간 것 — 자취(최근 것부터) */
export const sentRows = (logs = []) => logs.map((l) => ({ id: l.id, name: l.student_name ?? "?", what: `${logName(l.kind)}${l.channel === "sms" ? ` · 문자${l.to_phone ? ` ${l.to_phone}` : ""}` : ""}`, status: logStatus(l), resendable: !logStatus(l).read && l.channel !== "sms" }));   // (커) 문자 자취는 길과 가린 번호 · 다시 보내기는 앱 알림만
