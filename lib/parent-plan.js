/** 학부모 화면 판단 한 벌(순수, 목업 09) · 오늘 수업 꼬리표(시험 N/N · 숙제 %) · 오늘 하원 지연 안내 · 보낸 것 줄 · 등원·하원 한 줄. 세는 것은 세어 나온다(원칙-5). 빈 카드는 숨긴다(확정-⑮) */
import { seoulTime } from "./day-plan.js";
import { attendText, REASON_ON } from "./day-plan.js";   // (어44) 「지각(진료)」 글 한 벌
...
/** 보낸 것 · 하원 지연 안내들(최근 것부터) */
export const sentLines = (rows = [], today) => [...rows].filter((x) => x.on_date !== today).sort((a, b) => String(b.on_date).localeCompare(String(a.on_date))).slice(0, 5).map((x) => ({ id: x.id, text: `${md(x.on_date)} 하원 지연 안내 · ${hhmm(x.until_at)} 예정${x.left_at ? ` · ${hhmm(x.left_at)} 하원` : ""}`, small: x.reason ?? "" }));
...
export function todayArrival(rows = [], sheet = null) {
  ...
  const pill = REASON_ON.includes(sheet?.attend) ? attendText(sheet.attend, sheet.attend_reason) : st.arrived || sheet ? "정시 등원" : null;   // (어44) 「지각(진료)」
...
/** 📨 내게 온 알림 발송 이력(4단계-2a · 학부모 「보낸 것」이 발송 이력을 읽는다) · 실제로 나간 것만(sent_at) · 「M/D 수업 안내 · 읽음/안 읽음」 · 누르면 그 자리로 */
export function noticeLines(notices = []) {
  return (notices ?? []).filter((l) => l.sent_at).map((l) => ({ id: `n:${l.id}`, text: `${md(String(l.sent_at).slice(0, 10))} ${logName(l.kind)}${l.channel === "sms" ? " · 문자" : ""}`, small: l.channel === "sms" ? `문자로 받음${l.to_phone ? ` ${l.to_phone}` : ""}` : l.opened_at ? `읽음 ${hhmm(String(l.opened_at).slice(11, 16))}` : "안 읽음", url: l.url || "/parent" }));   // (커) 문자 발송 이력은 「문자로 받음」(읽음은 모른다)
}
