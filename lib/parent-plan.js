...
export function noticeLines(notices = []) {
  return (notices ?? []).filter((l) => l.sent_at).map((l) => ({ id: `n:${l.id}`, text: `${md(String(l.sent_at).slice(0, 10))} ${logName(l.kind)}`, small: l.opened_at ? `읽음 ${hhmm(String(l.opened_at).slice(11, 16))}` : "안 읽음", url: l.url || "/parent" }));
}
