import { unitsText } from "./item-plan.js";   // 교재 · 대단원 › 소단원 글은 항목 줄과 한 벌((어27))
...
const bookScope = (sc) => unitsText(sc?.units ? [{ ...sc.units, books: sc.books }] : [], { pages: false, count: false }) || sc?.books?.name || "";   // 교재 · 대단원 › 소단원. 항목 줄과 같은 글 한 벌(lib/item-plan · (어27))
export const scopeLabel = (sc, examName = "") => `${examName ? examName + " " : ""}내신 범위 · ${sc?.books?.name ? bookScope(sc) : sc?.free_note || "범위 없음"}`;
/** 방식 고치기(학생 × 교재 × 회독 × 유형 · 이 아이만) 양식 읽기 · 단어는 네 비율 합 100 · 문장은 방식 하나 · 통과선 0~100. DB 제약(style_pct·style_sentence)과 같은 문 */
...
export const scopeText = (q) => q.source === "book" ? (q.books?.name ? bookScope(q) : `교재${q.units ? ` · ${unitsText([q.units], { pages: false, count: false })}` : ""}`) : q.source === "prep" ? scopeLabel(q.prep_scope, q.prep_scope?.exams?.name ?? "") : q.free_note || "범위 없음";
/** 하원 지연 사유 꼬리표 · 「단어 재시험이 남음 · 범위 83%」. 떼는 쪽은 머리(「단어 재시험이 남음」)로 찾는다 */
/** 아이 화면 07 의 시험 줄 꼬리표((가)-⑨ · 남긴 것 12) — 오늘 건너뛴 재시험 · 재시험 · 없음 */
export const quizTag = (q) => (q?.state === "skipped" ? "오늘 건너뜀" : q?.retry_of ? "재시험" : null);
