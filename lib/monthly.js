...
export function summarize(reports = [], exams = []) {
  ...
  const wordRate = each.length
    ...
    : null;
  // 몇 개 중 몇 개 틀렸는지는 그대로 같이 적는다 (평균만 있으면 감이 안 온다)
...
export function buildMonthlyText(r, academy = "클로이영어", msg = {}) {
  ...
  if (saidAbsence) {
    ...
    L.push(
      "· 다만 진도가 이어질 때 훨씬 잘 붙어서, 가능하시면 정규 수업일에 " +
        "맞춰주시면 아이에게 도움이 됩니다."
    );
  }
  ...
  if (sum.word.count > 0) {
    ...
    // 평균만 보면 무너진 날이 안 보인다. 차이가 클 때만 덧붙인다
    if (sum.word.worst !== null && sum.word.rate - sum.word.worst >= 20) {
      L.push(`· 가장 낮았던 회차는 ${sum.word.worst}% 였습니다.`);
    }
  }
