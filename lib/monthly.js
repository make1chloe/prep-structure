 ...
 *  3 (2026-09-02) — 진도 줄의 `asOf` 가 「today」라는 **말**에서 **그 달 마지막날**로 바뀌었다.
 *    ⚠️ 뜻이 통째로 다르다 — 2판의 `asOf:"today"` 는 「굳힌 그날 누적」이고 3판은 「그 달 마지막날까지」다.
 *       안 올리면 옛 줄을 새 뜻으로 읽는다. (올릴 때 `v2.monthly_report` 는 0줄이었다 — 실측) */
export const FROZEN_V = 3;
 ...
 *      · `after_month` — **그 달이 끝난 뒤에 찍힌 `done` 줄 수**(날짜를 모르는 줄도 여기 센다).
 *        0 이 아니면 `book_progress()` 의 오늘 누적에 **그 달에 없던 진도가 섞여 있다.**
 *
 * ⚠️⚠️ **`idx` 를 왜 같이 뽑나 — 한 교재에 그 달 배정 줄이 둘일 수 있다.**
 *    `student_book` 의 유일키는 (student_id, book_id, from_date) 라 「1회독 끝내고 2회독 시작」이면
 *    같은 교재가 그 달에 **두 줄**이다(스키마가 허락하는 정상 모양). 그때 이 질의는 교재마다가 아니라
 *    **배정 줄마다** 한 줄씩 제대로 돌려주는데, 받는 쪽이 `book_id` 로만 맞대면 뒤엣것이 앞엣것을 덮어
 *    **남의 줄 값을 읽는다.** 실측 재현 — 구도은·「일관성 있는 기준 영문법」을 1회독(진도 27줄) +
 *    2회독(진도 0줄)으로 갈라 넣으면, 2회독 줄이 1회독의 `marks=27` 을 읽고 `marks === 0` 방벽을
 *    그냥 지나 **0% 가 학부모에게 굳어 나갔다.** 되돌아오는 차례는 보장이 없으므로 **번호를 붙여 맞댄다.**
 *    ⚠️ `with ordinality` 를 빼지 마라 — 빼면 그 자리가 다시 열린다
 *
...
select b.i as idx, b.book_id, p.done, p.skipped, p.total,
       ...
       (select count(*)::int from v2.progress pr
          join v2.units u on u.id = pr.unit_id
         where pr.student_id = $1::uuid and u.book_id = b.book_id and pr.round = b.round
           and pr.status = 'done'
           and (pr.done_on is null or pr.done_on > $4::date)) as after_month
  from unnest($2::uuid[], $3::int[]) with ordinality as b(book_id, round, i)
...
export async function buildReport(db, studentId, ym, opts = {}) {
  ...
  //    ⚠️⚠️ 방벽이 **회독만** 보면 모자란다 — 회독이 그대로여도 숫자는 여전히 **오늘 누적**이라
  //       지난 달 리포트에 **그 달에 있지도 않았던 진도**가 실린다. 그래서 「그 달이 끝난 뒤에
  //       찍힌 줄이 있나」(`after_month`)를 같이 묻고, 있으면 **숫자를 안 낸다**(대전제 0).
  ...
  const prog = books.length
    ? (await db.query(SQL_PROGRESS,
        [studentId, books.map((b) => b.book_id), books.map((b) => num(b.round)), last])).rows ?? []
    : [];
  // ⚠️⚠️ **`book_id` 로 맞대지 마라.** 한 교재에 그 달 배정 줄이 둘이면(1회독 끝내고 2회독 시작)
  //    뒤엣것이 앞엣것을 덮어 **남의 줄 `marks` 를 읽고** 거짓 0% 가 방벽을 그냥 지난다.
  //    보낸 차례 그대로 번호(`idx`)를 붙여 맞댄다 — `SQL_PROGRESS` 의 `with ordinality` 가 그 번호다
  const byIdx = new Map(prog.map((r) => [String(r.idx), r]));
  for (const [i, b] of books.entries()) {
    const label = `progress:${b.book_name}`;
    const p = byIdx.get(String(i + 1));                 // ⚠️ ordinality 는 1부터다
    ...
    const afterMonth = num(p.after_month) ?? 0;
    ...
    // ⚠️⚠️ **여기가 「그 달에 있지도 않았던 진도」가 실리던 자리다.** `v2.book_progress()` 는
    //    날짜를 안 받아 **오늘 누적**을 돌려준다. 그 달이 끝난 뒤에 찍힌 줄이 하나라도 있으면
    //    그 숫자는 그 달 것이 아니다 — 실측(2026-09-02) 2026-07 진도 줄 5개 중 4개 ·
    //    2026-06 은 3개 중 3개가 그랬다(김소현·기적의 영어문장 트레이닝 60/130 은
    //    2026-07-31 까지 실제로 **0단원**이었다). 굳으면 못 고친다 → **숫자를 안 낸다**(대전제 0).
    //    ⚠️ 날짜를 모르는 `done` 줄도 여기 걸린다 — 언제 찍었는지 모르면 그 달 것인지도 모른다
    if (afterMonth > 0) {
      drop(label, `그 달이 끝난 뒤에 찍힌 진도가 ${afterMonth}줄 있다 — 진도는 오늘 기준으로만 셀 수 있어 `
                + "그 달 진도를 모른다 (오늘 숫자를 그 달 것처럼 싣지 않는다)");
      continue;
    }
  ...
  if (bookLines.length)
    // ⚠️ 여기까지 온 줄은 **그 달이 끝난 뒤에 찍힌 진도가 하나도 없다**(`after_month === 0`).
    //    그러니 오늘 누적 = 그 달 마지막날 누적이다 — 그때서야 「그 달 마지막날까지」라고 말할 수 있다.
    //    ⚠️ 셀 수 없는 줄은 위에서 이미 다 빠졌다. 여기서 「오늘 기준」이라 얼버무리지 마라
    lines.push({ key: "progress", label: "교재 진도", from: `${last} 까지`,
                 asOf: last, books: bookLines });
...
export async function sentView(db, studentId, ym) {
  ...
  // ⚠️⚠️ **옛 모양으로 굳은 글을 새 모양으로 읽지 않는다.** `FROZEN_V` 를 올리는 까닭이 이것인데
  //    앞서는 올려 놓고 **읽는 쪽이 번호를 안 봤다** — 뜻이 달라진 칸(2판의 `asOf:"today"` 는
  //    「굳힌 그날 누적」, 3판은 「그 달 마지막날까지」)을 조용히 새 뜻으로 그렸다.
  //    막지 않고 **밝힌다** — 그때 나간 글은 그대로 보여야 한다(대전제 6). 숫자를 지어 고치지 않는다
  //    ⚠️ 여기에 **글을 담지 않는다.** 이 값은 학부모 화면이 그대로 읽는 자리라 ⚠️ 표시나
  //       안쪽 말을 담으면 그게 그대로 뜬다(`frozen` 과 같은 규칙). **숫자만** 낸다 —
  //       무슨 말을 띄울지는 화면이 정하고, 지금 판인 줄에는 이 칸이 아예 안 붙는다
  const 판 = Number(f.v ?? 0);
  const 옛판 = 판 !== FROZEN_V ? { oldShape: true, frozenV: 판 || null } : null;
  // ⚠️ 차례가 중요하다 — **굳은 것(`f`)이 먼저 깔리고 그 위를 칸이 덮는다.**
  //    글은 굳은 것이 이기고(`f.body`), 「보낸 때」는 칸이 이긴다(굳은 것엔 아예 없다)
  return { ...f, ym, studentId, sentAt: r.sent_at, body: f.body ?? r.body ?? null, ...(옛판 ?? {}) };
}
...
export async function monthlyBoard(db, ym) {
  ...
  const students = rows.map((r) => {
    ...
    return {
      ...
      // ⚠️⚠️ **`sent_at` 을 여기 섞지 마라.** 앞서 `ready`(보낼 수 있나)를 `allClosed`(마감 다 됐나)로
      //    **이름만 바꾸고 `!r.sent_at` 을 그대로 뒀다** — 「보낸 것」은 마감과 아무 상관이 없다.
      //    실측 재현(2026-09-02, 진짜 DB) — 왕희연의 2026-08 판 7개를 전부 마감하면
      //    {sheets:7, closed:7, open:0, allClosed:true}·집계 「마감 다 됨」 1명인데,
      //    **그 아이 리포트를 보내기만 하면** 마감은 그대로인데 allClosed:false·집계 0명이 됐다.
      //    원장님 화면이 판 7개를 다 마감한 아이를 「마감 안 됨」으로 읽는다.
      //    「안 보낸 아이」는 바로 옆 `sent`·`notSent` 가 따로 센다 — 한 칸이 두 가지를 말하지 않는다
      allClosed: closedN > 0 && sheets - closedN === 0,
    };
  });
