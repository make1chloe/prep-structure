...
export const handlers = {};   // kind → async (payload, ctx) => void | { deferUntil, why }(미룸 — 방해금지)
export const beforeRun = [];  // 한 바퀴 앞에 도는 것(예약 → 큐, lib/send.js) — 크론은 새 셈을 안 만든다(뼈대-9)
...
export async function claim(sb, limit = 20) {
  ...
  for (const row of data ?? []) {
    const { data: locked } = await db(sb).from("job_queue").update({ state: "taking", locked_at: now, tries: row.tries + 1 })
      .eq("id", row.id).eq("state", row.state).eq("tries", row.tries).select("*").maybeSingle();   /* 0줄 허용 — 같은 줄을 둘이 집지 않게, 읽은 그대로일 때만(0-3) · 남이 먼저 집었으면 0줄이 정상 */
    if (locked) got.push(locked);
  }
  return got;
}
export async function done(sb, id) { changed(await db(sb).from("job_queue").update({ state: "done", locked_at: null }, { count: "exact" }).eq("id", id), "큐 끝냄을 못 적음", { zero: "ok" }); }   /* 0줄 허용(부기) — 일은 이미 됐다 */
...
export async function fail(sb, row, err) {
  ...
  changed(await db(sb).from("job_queue").update({ state: "fail", locked_at: null, last_error: String(err?.message ?? err).slice(0, 500), next_at: new Date(Date.now() + (gaveUp ? 365 * 24 * 60 : minutes) * 60 * 1000).toISOString() }, { count: "exact" }).eq("id", row.id), "큐 실패를 못 적음", { zero: "ok" });   /* 0줄 허용(부기) */
  return gaveUp;
}
/** 미룸 — 실패가 아니다(시도 횟수를 되돌린다). 방해금지가 끝나는 때에 다시 집는다 */
export async function defer(sb, row, untilIso, why) {
  changed(await db(sb).from("job_queue").update({ state: "wait", locked_at: null, next_at: untilIso, tries: Math.max(0, row.tries - 1), last_error: String(why ?? "미룸").slice(0, 500) }, { count: "exact" }).eq("id", row.id), "큐 미룸을 못 적음", { zero: "ok" });   /* 0줄 허용(부기) */
}
/** 한 바퀴 — 크론이 「학원의 오늘」과 함께 부른다(뼈대-10). 손이 없는 kind 는 실패로 남긴다(조용히 삼키지 않는다). 손이 { deferUntil } 을 돌려주면 미룬다 */
export async function runDue(sb, today) {
  for (const h of beforeRun) await h(sb, today);
  const rows = await claim(sb); let ok = 0, bad = 0, gaveUp = 0, deferred = 0;
  for (const row of rows) {
    const h = handlers[row.kind];
    try {
      if (!h) throw new Error(`손이 없는 일: ${row.kind}`);
      const r = await h(row.payload ?? {}, { sb, today, row });
      if (r?.deferUntil) { await defer(sb, row, r.deferUntil, r.why); deferred++; } else { await done(sb, row.id); ok++; }
    }
    catch (e) { bad++; if (await fail(sb, row, e)) gaveUp++; }
  }
  return { claimed: rows.length, ok, bad, gaveUp, deferred };
}
