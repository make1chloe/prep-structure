...
export async function defer(sb, row, untilIso, why) {
  await db(sb).from("job_queue").update({ state: "wait", locked_at: null, next_at: untilIso, tries: Math.max(0, row.tries - 1), last_error: String(why ?? "미룸").slice(0, 500) }).eq("id", row.id);
}
