...
export async function done(sb, id) { changed(await db(sb).from("job_queue").update({ state: "done", locked_at: null }, { count: "exact" }).eq("id", id), "큐 완료를 못 적음", { zero: "ok" }); }   /* 0줄 허용(부기) — 일은 이미 됐다 */
