...
function cleanTitle(s = "") {
  return String(s)
    ...
    .replace(/^\s*\d{1,2}\s*[.)]\s+/, "")
    // 날짜를 걷어낸 자리에 점·쉼표가 남는다 (「2026.12.08.~12.10.」)
    .replace(/^[\s\-–~·|>».,]+/, "")
    .replace(/[\s\-–~·|.,]+$/, "")
    .replace(/\s+/g, " ")
    .trim();
...
export function readSchedule(text = "", year = null) {
  const lines = String(text)
    .split("\n")
    // 칸 사이의 탭은 남긴다 — 표를 복사해 붙이면 칸이 탭으로 갈린다
    .map((s) => s.replace(/^[ \t]+|[ \t]+$/g, ""))
    .filter(Boolean);
  const rows = [];
  const unread = [];
  const RANGE = /[~∼-]\s*((?:20\d{2}\s*[-./년]\s*)?\d{1,2}\s*[-./월]\s*\d{1,2})/;
  const endOf = (s, from) => {
    const m = String(s).match(RANGE);
    if (!m) return null;
    const t = readDate(m[1], year);
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    /**
     * **표를 복사해 붙여넣은 줄** (원장님, 2026-08-11 — 「이 방식은 오류가 많을
     * 거 같음」 → 붙여넣기를 기본 길로 두었다). 칸이 탭으로 갈려 온다.
     *
     * 칸별로 보면 정확하다 — 날짜 칸을 찾고, 나머지 칸에서 이름을 고른다.
     * 글자로 뭉개 놓고 정규식으로 긁으면 **번호 칸의 「1」 이 이름에 붙는다**
     * (실제로 「1 2학기 중간고사」 가 됐다).
     */
    if (line.includes("\t")) {
      const cells = line.split("\t").map((s) => s.trim()).filter(Boolean);
      const di = cells.findIndex((c) => readDate(c, year));
      if (cells.length > 1 && di >= 0) {
        const from = readDate(cells[di], year);
        // 끝날은 같은 칸에 있기도 하고(「10.13~10.16」) 옆 칸에 있기도 하다
        let endDate = endOf(cells[di], from);
        cells.forEach((c, j) => {
          if (j === di || endDate) return;
          const t = readDate(c, year);
          if (t && t > from) endDate = t;
        });
        /**
         * 이름은 **가장 긴 칸**으로 고른다 — 번호 칸(「1」)·대상 칸(「3학년」)
         * 보다 일정 이름이 길다. 칸 순서가 학교마다 달라도 견딘다.
         */
        const title = cleanTitle(
          cells
            .filter((c, j) => j !== di && !readDate(c, year) && /[^\d\s.,)(~-]/.test(c))
