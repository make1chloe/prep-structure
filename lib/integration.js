/** 연동 설정의 손((터) 확정-72) · 표는 v2.integration 하나(0031). **원장만** 쓴다.
 *  읽기는 서버 자신(service role)으로 읽되 **가려서** 돌려준다 · 진짜 키는 화면·발송 이력·오류 글 어디에도 안 나온다(대전제-9).
 *  쓰기도 서버 자신 — 사람은 insert·update 가 revoke 되어 있다(0031). 판단은 lib/integration-plan.js(순수) */
import { db, serviceClient } from "./supabase.js";
import { KEYS, keyDef, keyRows, mergeConfig, changedText, fieldWhyBad } from "./integration-plan.js";
import { sms, smsReady } from "./notify.js";
import { phoneDigits } from "./sms-plan.js";
import { changed, rows } from "./sqlError.js";
/** 설정 화면이 그리는 판 · 유형마다 가린 값 · 다 찼나 · 마지막으로 됐던 때·오류 */
...
export async function saveKeys(id, form = {}, svc = serviceClient()) {
  ...
  for (const f of def.fields) { const why = fieldWhyBad(f, form?.[f.k]); if (why) throw new Error(why); }   // 틀린 꼴은 저장하지 않는다 — 그 자리에서 말한다(원장님 9/10 이메일을 API Key 자리에)
  ...
  if (cur.data) changed(await db(svc).from("integration").update({ config: next, updated_at: new Date().toISOString(), last_error: null }).eq("id", id).select("id"), "키를 못 적음");
  else changed(await db(svc).from("integration").insert({ id, config: next }).select("id"), "키를 못 적음");
  return { id, text, ready: keyRows({ [id]: next }).find((r) => r.id === id)?.ready ?? false };
}
/** ✉️ 시험 문자 · 원장님 번호로 한 통. 됐으면 last_ok_at, 안 됐으면 last_error(까닭만 · 키는 안 적는다) */
export async function testSms(to, svc = serviceClient()) {
  if (!phoneDigits(to)) throw new Error("받을 번호를 010… 으로 적어 주세요");
  if (!(await smsReady(svc))) throw new Error("솔라피 키가 다 차야 보냅니다. API Key · Secret · 발신번호");
  const r = await sms(svc, { kind: "notice", to, text: "[클로이영어] 문자 길 시험입니다. 이 글이 오면 연결이 된 것입니다.", url: "/settings" });
  const now = new Date().toISOString();
  await db(svc).from("integration").update(r.sent ? { last_ok_at: now, last_error: null } : { last_error: String(r.why ?? (r.sink === "off" ? "리허설(NOTIFY_SINK=off) · 발송 이력만 남았습니다" : "안 나갔습니다")).slice(0, 300) }).eq("id", "solapi");   /* 0줄 허용(부기) */
  return r;
}
