...
export function parseReportRow(row, year) {
  ...
  return {
    ...
    wordCorrect: toInt(pick(row, ["-단어T", "단어맞은개수"])),
    wordTotal: toInt(pick(row, ["/단어T", "단어전체개수"])),
    sentCorrect: toInt(pick(row, ["-문장T", "\b-문장T"])),
    sentTotal: toInt(pick(row, ["/문장T"])),
    notice: pick(row, ["공지"]),
    result: pick(row, ["결과"]),
