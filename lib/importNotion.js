 ...
 * 으로 건너뛰었다. 이름이 이렇게 들어와 있었다:
 *
 *     서한결 (https://app.notion.com/p/1cce8b8e...?pvs=21)
 *
 * 노션에서 **관계(relation) 열**을 CSV 로 내보내면 이름 뒤에 그 페이지 주소가
 * 괄호로 붙는다. 우리는 그 통짜 글자로 학생을 찾았으니 하나도 못 찾았다.
 * **오류는 안 난다** — 「이 학생이 없습니다」 라고 얌전히 건너뛸 뿐이다.
 *
 * 여럿이면 쉼표로 이어진다. 주소 안에는 쉼표가 없으므로 쉼표로 갈라도 된다.
 */
export function relName(v) {
  const first = (v ?? "").toString().split(",")[0] || "";
  return first
    .replace(/\((?:https?:)?\/\/[^)]*\)/g, " ")   // (https://…) 통째로
    .replace(/\(\s*\)/g, " ")
    .trim();
}
 ...
 */
export function parseDateInfo(v, fallbackYear, past = true) {
  const s = (v ?? "").toString().trim();
  if (!s) return { date: null, guessed: false };
  const out = parseDateRaw(s, fallbackYear, past);
...
export function parseDate(v, fallbackYear, past = true) {
  return parseDateInfo(v, fallbackYear, past).date;
}

function parseDateRaw(s, fallbackYear, past = true) {
  ...
  if (md) {
    ...
    /**
     * **미래가 되면 한 해 앞으로 당긴다 — 연도 칸을 정하셨더라도.**
     *
     * 예전에는 `fallbackYear` 가 있으면 여기서 그냥 돌려줬다. 그래서 연도
     * 칸(기본값 올해)에 기대는 모든 줄이 **미래 방지 규칙을 안 탔다.**
     *
     * 2026-08-06 원장님 화면에서 그 결과를 봤다 — 수업 기록 1000건이 전부
     * 2026년이고 그중 **233건이 미래**였다. 「08/14」 가 2026-08-14(금)로
     * 들어갔는데, 이 학원은 월·수 / 화·목이고 금요일은 보강일이다.
     * 한 해 앞으로 당긴 2025-08-14 는 목요일 — 화·목반과 정확히 맞는다.
     *
     * **지난 일을 적은 기록은 미래일 수 없다.** 수업도, 출결도, 상담도.
     * 앞으로 잡는 것(일정·보강)은 이 규칙을 안 쓴다 — `past: false`.
     */
    const now = new Date();
    const today = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(
      now.getDate()
    ).padStart(2, "0")}`;
    const y = fallbackYear || now.getFullYear();
    const guess = `${y}-${mm}-${dd}`;
    if (past === false) return guess;
...
export function parseTaskRow(row, year) {
  ...
  // 일정·할일은 **앞으로 잡아둔 것이 맞다** — 미래 방지 규칙을 안 쓴다
  const fromI =
    parseDateInfo(pick(row, ["날짜", "date:날짜:start", "마감일", "시작", "due", "일자"]), year, false);
  const from = fromI.date || null;
  const yearGuessed = fromI.guessed;
  const to =
    parseDate(pick(row, ["끝날", "date:날짜:end", "종료", "끝", "end"]), year, false) || null;
...
export function parseAbsenceRow(row, year) {
  ...
  const name = relName(rel) || fromTitleName;

  const createdI = parseDateInfo(pick(row, ["결석날짜", "생", "생성 일시"]), year);
  const created = createdI.date;
  // 보강은 **앞으로 잡는 것**이라 미래여도 맞다
  const makeupI = parseDateInfo(pick(row, ["보강날짜", "date:보강날짜:start"]), year, false);
