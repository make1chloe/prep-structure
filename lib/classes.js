...
export async function setSchedule(sb, classId, f) {
  ...
  if (cur && cur.from_date >= p.fromDate) {
    ...
    changed(await db(sb).from("class_schedule").update({ weekdays: p.weekdays, start_time: p.start, end_time: p.end }).eq("id", cur.id).select("id"), "시간표를 못 고침");
...
export async function setClassName(sb, classId, nickname, kind) {
  ...
  const r = changed(await db(sb).from("classes").update({ nickname: p.nickname, kind: p.kind }).eq("id", classId).select("id"), "반을 못 고침");
...
export async function setClassFee(sb, classId, f) {
  ...
  if (cur && cur.from_date >= p.fromDate) { changed(await db(sb).from("fee_rule").update({ amount: p.amount, per_session: p.perSession }).eq("id", cur.id).select("id"), "단가 줄을 못 고침"); return { id: cur.id }; }
