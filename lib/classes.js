/** 🏫 반 손(4단계-3a) · 판 · 만들기(반 + 시간표) · 시간표 바꾸기(이 날부터 · 옛 줄은 닫는다, 지우지 않는다) · 이름·유형 · 명단 넣기·빼기(기간) · 반 단가 줄 · 반 닫기 · 그 시각 아이 수. 판단은 lib/class-plan */
...
export async function classBoard(sb, date) {
  ...
  if (error) throw new Error(`반 화면을 못 읽음: ${error.message}`);
...
export async function setSchedule(sb, classId, f) {
  ...
  if (cur && cur.from_date >= p.fromDate) {
    ...
    changed(await db(sb).from("class_schedule").update({ weekdays: p.weekdays, start_time: p.start, end_time: p.end }).eq("id", cur.id).select("id"), "시간표를 수정 못 함");
    return { id: cur.id, replaced: true };
  }
  if (cur) changed(await db(sb).from("class_schedule").update({ to_date: dayBefore(p.fromDate) }).eq("id", cur.id).select("id"), "옛 시간표를 못 닫음");
...
export async function setClassName(sb, classId, nickname, kind) {
  ...
  const r = changed(await db(sb).from("classes").update({ nickname: p.nickname, kind: p.kind }).eq("id", classId).select("id"), "반을 수정 못 함");
...
export async function closeClass(sb, classId, on) {
  ...
  const r = changed(await db(sb).from("classes").update({ state: "closed" }).eq("id", classId).select("id"), "반을 못 닫음"); if (!r?.length) throw new Error("반이 없습니다");
  ...
  for (const x of sch) changed(await db(sb).from("class_schedule").update({ to_date: x.from_date > last ? dayBefore(x.from_date) : last }).eq("id", x.id).select("id"), "시간표를 못 닫음");
  const mem = row(await db(sb).from("class_member").select("student_id,from_date").eq("class_id", classId).is("to_date", null), "명단을 못 읽음") ?? [];
  for (const m of mem) changed(await db(sb).from("class_member").update({ to_date: m.from_date > last ? dayBefore(m.from_date) : last }).eq("class_id", classId).eq("student_id", m.student_id).eq("from_date", m.from_date).select("class_id"), "명단을 못 닫음", { zero: "ok" });
  ...
  for (const f of fees) changed(await db(sb).from("fee_rule").update({ to_date: f.from_date > last ? f.from_date : last }).eq("id", f.id).select("id"), "단가 줄을 못 닫음");
}
...
export async function removeMember(sb, classId, studentId, on) {
  ...
  const r = changed(await db(sb).from("class_member").update({ to_date: dayBefore(on) }).eq("class_id", classId).eq("student_id", studentId).is("to_date", null).select("class_id"), "명단에서 못 뺌", { zero: "ok" });
...
export async function setClassFee(sb, classId, f) {
  ...
  if (cur && cur.from_date >= p.fromDate) { changed(await db(sb).from("fee_rule").update({ amount: p.amount, per_session: p.perSession }).eq("id", cur.id).select("id"), "단가 줄을 수정 못 함"); return { id: cur.id }; }
  if (cur) changed(await db(sb).from("fee_rule").update({ to_date: dayBefore(p.fromDate) }).eq("id", cur.id).select("id"), "옛 단가 줄을 못 닫음");
...
/** 고른 반·명단 여럿을 한 번에((어28)-④ · 대전제-20) · 한 줄의 손을 차례로 · 막히면 그 줄부터 멈춘다 */
export async function closeClassMany(sb, ids = [], on) { const list = [...new Set((ids ?? []).filter(Boolean))]; if (!list.length) throw new Error("고른 반이 없습니다"); let n = 0; for (const id of list) { await closeClass(sb, String(id), on); n++; } return { n }; }
export async function removeMemberMany(sb, pairs = [], on) { const list = (pairs ?? []).filter((x) => x?.classId && x?.studentId); if (!list.length) throw new Error("고른 아이가 없습니다"); let n = 0; for (const x of list) { await removeMember(sb, String(x.classId), String(x.studentId), on); n++; } return { n }; }
