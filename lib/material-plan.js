/** 받을 교재·학습지 판단 한 벌(순수, 목업 07 📚 · 07-2 ①) · 단계 넷(아직·받음·하는 중·완료) · 유형(자료 종류)별 올린 기록 · 끝낸 것 접기 · 스스로 정한 마감 글. 원장이 나눠 준 것(handed_at)과 아이가 받았다고 찍은 것(got_at)은 다른 사실(0013) */
...
/** 유형별 올린 기록 · 자료 종류 이름 차례(sort → 이름), 끝낸 것은 따로 */
export function groupGives(rows = []) {
  const open = rows.filter((r) => r.stage !== "done"), done = rows.filter((r) => r.stage === "done");
  const groups = new Map();
  for (const r of open) { const k = r.material?.material_type?.name ?? "기타"; if (!groups.has(k)) groups.set(k, { name: k, sort: r.material?.material_type?.sort ?? 0, items: [] }); groups.get(k).items.push(r); }
