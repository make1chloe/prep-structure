...
import { myLinks, mySent } from "./files.js";
import { myAssigns, myProgress } from "./video.js";
...
export async function meDay(sb, user, date) {
  ...
  const [people, arrival, sheetR, lastR, quizzes, abs, late, books, todo, memos, days, access, gives, asks, scoresR, examsR, linksR, rules, assignsR, progressR, sentR] = await Promise.all([
    ...
    myLinks(sb, id, lo),   // 📎 내 숙제에 붙은 파일(지난 60일 — 1달 창은 규칙 file.child_days 로 자른다)
    ruleMap(sb, ["file.", "video."]),
    myAssigns(sb, id),   // 🎬 내 영상 배정
    myProgress(sb, id),   // 🎬 지나간 구간(Postgres 가 합친 것)
    mySent(sb, user.id, lo),   // 📎 내가 보낸 것(원장님 답 한 줄)
  ]);
  ...
  return {
    ...
    links: rows(linksR, "붙임"), rules, uploads: rows(sentR, "내가 보낸 것"),
    videos: videoRows(rows(assignsR, "영상 배정"), rows(progressR, "영상 구간"), Number(rules["video.done_pct"] ?? 95), date),
