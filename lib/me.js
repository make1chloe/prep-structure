/** 아이 화면 07(「나」) 한 벌 — 층: 로그인 확인 → 오늘 → 제 학생 줄 → 나머지 한 파도 → 그리기(4단, 조회 16). 전부 아이 자격(RLS 가 제 것만 준다 — 0084 sheet_visible_to: 제 판은 날이 오면 마감 전에도).
...
export async function meDay(sb, user, date) {
  ...
  const id = st.id;
  const [people, arrival, sheetR, lastR, quizzes, abs, late, books, todo, memos, days, access] = await Promise.all([
