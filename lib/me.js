...
import { studentById } from "./asview.js";
import { arrivalState, classChoice, futureLines, classSteps, homeSteps , liveArrival } from "./arrival-plan.js";
...
import { myCc } from "./mine.js";   // (어72) 제 클래스카드 줄 — 파도 안에서 한 조회
import { fillFields, filledLines } from "./mine-plan.js";
...
export { SCORE_SEL };
/** 성적 줄 → 카드 한 줄(회차의 학교급·이름을 붙여) + 추이(같은 유형 지난 회차 대비 ▲▼ · 5단계-⑤) · 학부모 09 도 이 한 벌 */
...
export async function meDay(sb, user, date, asStudentId = null) {
  const st = asStudentId ? await studentById(sb, asStudentId) : await myStudent(sb, user.id);   // 👁 보는 중이면 그 아이 줄 — **이 층 안에서** 집는다(화면에 await 를 더하지 않는다 · 속도-1). 직원인지는 화면이 lib/asview 로 이미 걸렀다
  const id = st.id;
  const byProfile = st.profile_id ?? user.id;   // 「💬 내가 남긴 말」은 **사람**으로 고른다 — 보는 중에 원장님 것이 뜨지 않게
  const lo = plusDays(date, -60);
  const [people, arrival, sheetR, hR, cR, quizzes, abs, late, books, todo, memos, days, access, gives, asks, scoresR, examsR, rules, mine, gridR, cc, giveWay] = await Promise.all([
    rosterPeople(sb, date),   // 오늘 내 반(RLS 로 제 것만) ∥ 예정
    liveArrival(() => db(sb).from("arrival").select("id,step,at").eq("student_id", id).eq("date", date).order("step")),
    ...
    db(sb).from("request").select("id,kind,body,at,answer,answered_at,state").eq("by_profile", byProfile).order("at", { ascending: false }).limit(5),   // 💬 내가 남긴 말
    ...
    ruleMap(sb, ["file.", "video.", "me.fill."]),   // (어72) 「아이가 채울 칸」 스위치도 이 한 조회에 얹는다
    ...
    myCc(sb, id),   // 🃏 내 클래스카드 아이디 — 줄이 없으면 「빈 칸」((어72))
    /* (어96) 「어플 목록에 띄울까」 — 원장님 2026-09-19 「알림만주거나 어플목록에 띄우거나 둘다 하거나 선택가능하게해야함」.
       칸 이름을 조회 글자에 안 적는다(`*` · 대전제-27 ①) · 표·칸이 없는 DB 면 여태처럼 보인다. 이 파도에 태운다(속도-1). */
    db(sb).from("todo_kind").select("*").eq("kind", "hand").maybeSingle(),
  ]);
  /* ⚠️ 표(0179)나 칸(0185)이 없는 DB 에서는 **여태처럼 보인다** — 못 읽었다고 아이 숙제를 감추는 쪽으로 기울지 않는다(대전제-27).
     could not find the table · does not exist 면 스위치가 없는 것이고, 없으면 켜진 것으로 본다. */
  const noKindTbl = /could not find the table|PGRST205|does not exist/i.test(`${giveWay?.error?.code ?? ""} ${giveWay?.error?.message ?? ""}`);
  const listOn = noKindTbl || !giveWay?.data || giveWay.data.notify_way === undefined || ["list", "both"].includes(giveWay.data.notify_way);
  const sheet = shape(rows(sheetR, "오늘 수업 일지")[0] ?? null);
  ...
  return {
    ...
    gives: groupGives(listOn ? rows(gives, "받을 학습지") : []),   // (어96) 목록을 끄셨으면 카드가 아예 안 선다(알림만 가는 칸)
    asks: rows(asks, "남긴 말"),
    scores: scoreLines(rows(scoresR, "성적")),
    links: mine.links, rules, uploads: mine.sent,
    fill: fillFields(st, cc, rules), filled: filledLines(st, cc), progressEdit: st.progress_edit ?? "follow",   // (어72) 채울 칸(켠 것 **그리고** 빈 것) · 이미 찬 것 · 진도 체크 상태
    videos: videoRows(mine.assigns, mine.progress, Number(rules["video.done_pct"] ?? 95), date),
    entry: examsForEntry(rows(examsR, "시험").filter((e) => takes({ ...st, level: st.schools?.level }, e)), rows(scoresR, "성적"), date).map((e) => ({ id: e.id, name: e.name, school: e.schools?.name ?? "전국", on: e.english_on ?? e.term_from })),
  };
}
