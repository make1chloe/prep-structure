 ...
 *  키(v2.integration 'solapi')는 서명에만 쓰고 어디에도 안 적는다 · 짐·답 읽기는 lib/sms-plan.js(순수 · 화면도 가져오는 파일이라 node:crypto 가 없다) */
import { createHmac, randomBytes } from "node:crypto";
import { smsBody, readSolapi } from "./sms-plan.js";
export const SOLAPI_URL = "https://api.solapi.com/messages/v4/send";
/** 솔라피 서명 · Authorization: HMAC-SHA256 apiKey=…, date=…, salt=…, signature=HMAC_SHA256(secret, date + salt). 키를 만지므로 서버 쪽(화면은 lib/sms-plan.js 만 가져온다) */
export function solapiAuth({ key, secret, date = new Date().toISOString(), salt = randomBytes(16).toString("hex") }) {
  if (!key || !secret) throw new Error("솔라피 키가 없습니다");
