...
export function cardsOf(b) {
  ...
  // (버) 저절로 선 단원평가 — 루틴 11 의 「단원평가 본다」(대단원마다 · 소단원 N개마다)가 켜진 교재에서 그 묶음이 끝나면(todo_board unit_test_due · 0149) 카드가 선다. 「출제함」이 묶음을 새겨 made 로(같은 묶음은 다시 안 선다)
  ...
  for (const q of b.retests ?? []) out.push({ id: `q:${q.id}`, quizId: q.id, paperAt: q.paper_at ?? null, kind: "retest", title: `${q.student} · ${QKIND[q.kind] ?? q.kind}${q.pct != null ? ` ${q.pct}%` : ""}`, due: q.assigned_on ?? null, dueText: q.assigned_on === today ? "오늘 · 그날 남아서 봅니다" : q.assigned_on ? `${monthDay(q.assigned_on)} 낸 것 · 남아서 봅니다` : "", state: q.paper_at ? "doing" : "todo", why: "못 넘긴 시험에서 저절로", school: q.school ?? null, schoolId: q.school_id ?? null, student: q.student, studentId: q.student_id, n: 1, book: [q.book, q.unit ?? q.free_note].filter(Boolean).join(" · "), style: q.style ?? null, prep: false });
 ...
 *  단계는 flowOf 한 벌 · 날짜는 여기서 생긴 할 일(todo.material_id)에서 가져온다 — 새 셈을 만들지 않는다(원칙-1) */
...
/** (어21) 01 의 「내신 자료」 업무 — today_prep 한 아이 몫(시험마다 멈춘 교재 · 자료 · 이 아이 give)을 그린 줄로. 멈췄나는 routine-plan stopOn 한 벌 — 01 의 「교재 멈춤」 머리와 같은 판단 */
