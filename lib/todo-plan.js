...
export const KINDS = Object.freeze([
  ["make", "📄 자료 만들기", "nb-orange"], ["print", "🖨 인쇄", "nb-blue"], ["hand", "📤 배부", "nb-yellow"],
...
/** 자료의 단계 체크 — 만들기(만듦·♻️) · 인쇄 · 배부(다 줬나) · 풀이(다 풀었나) · 채점(아직 자리 없음 — 남긴 것). 종류에 없는 단계는 안 센다 */
export function stepChecks(m) {
  const steps = (m.steps ?? []).filter((s) => STEPS.some(([k]) => k === s));
  const gives = Number(m.gives ?? 0), handed = Number(m.handed ?? 0), solved = Number(m.solved ?? 0);
  const done = { make: Boolean(m.reuse_of) || ["made", "printed", "done"].includes(m.state), print: ["printed", "done"].includes(m.state), hand: m.state === "done" || (gives > 0 && handed >= gives), solve: gives > 0 && solved >= gives, score: false };
  return steps.map((s) => ({ step: s, name: stepName(s), done: Boolean(done[s]), text: s === "hand" && gives ? `${handed}/${gives}` : s === "solve" && gives ? `${solved}/${gives}` : null }));
}
...
export function cardsOf(b) {
  ...
  for (const q of b.retests ?? []) out.push({ id: `q:${q.id}`, quizId: q.id, kind: "retest", title: `${q.student} · ${QKIND[q.kind] ?? q.kind}${q.pct != null ? ` ${q.pct}%` : ""}`, due: q.assigned_on ?? null, dueText: q.assigned_on === today ? "오늘 — 그날 남아서 봅니다" : q.assigned_on ? `${monthDay(q.assigned_on)} 낸 것 — 남아서 봅니다` : "", state: "todo", why: "못 넘긴 시험에서 저절로(9/5 ⑪)", school: q.school ?? null, schoolId: q.school_id ?? null, student: q.student, studentId: q.student_id, n: 1, book: [q.book, q.unit ?? q.free_note].filter(Boolean).join(" · "), style: q.style ?? null, prep: false });
...
export function repeatText(th = {}) { const lead = Number(th.lead ?? 0); const tail = lead > 0 ? ` · ${lead}일 전부터` : ""; if (th.day != null) return `매달 ${th.day}일${tail}`; if (th.weekday != null) return `매주 ${WD[Number(th.weekday)] ?? "?"}${tail}`; return "언제인지 없음"; }
/** 되풀이 규칙 읽기 — every: month(day 1~31) · week(weekday 0~6) · lead(기본 규칙 줄) */
export function parseRepeat({ every, day, weekday, lead }) {
  const th = {};
  ...
  else throw new Error("매달인지 매주인지 고르세요");
