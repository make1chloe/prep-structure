...
export async function saveBody(sb, studentId, ym, body) {
  ...
  const { error } = await db(sb).from("monthly_report").upsert({ student_id: studentId, ym, body: String(body ?? "").trim() || null }, { onConflict: "student_id,ym" });
  if (error) throw new Error(`한마디를 못 적음: ${error.message}`);
  return { saved: true };
}
