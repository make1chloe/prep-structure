...
export async function monthlyBoard(sb, ym) {
  ...
  if (error) throw new Error(`월간 화면을 못 읽음: ${error.message}`);
...
/** 📨 보내기 · 고른 아이(없으면 아직 안 보낸 아이 전부): 그 달 숫자를 굳혀 리포트 줄에 적고(sent_at) 학부모 알림(monthly). 리포트 줄이 먼저 · 알림이 실패해도 리포트는 서고 발송 이력이 ✕ 로 말한다 */
...
export async function sendMonthly(svc, sb, ym, studentIds = null) {
  ...
  for (const r of rows) {
    if (r.report?.sent_at) continue;   // 이미 보낸 아이는 건너뛴다(다시 보내기는 발송 이력에서)
