 ...
 *  전화 다듬기·가리기 · 글자 수(통신사 바이트) → SMS/LMS · 치환(빈 칸은 못 나간다 · 「덧붙임」처럼 비어도 되는 자리는 줄째 사라진다) · 짐 · 답 읽기(서명·주소는 서버 쪽 lib/solapi.js · 이 파일은 화면도 가져온다).
 *  망을 타지 않는다 · 부르는 자리는 lib/solapi.js 하나(대전제-7 · 검사-①) · 키는 v2.integration 'solapi'(key · secret · from). 화면(글자 수 미리보기)과 서버가 같은 것을 본다 */
/** 비어도 되는 자리 — 값이 없으면 그 줄째 사라진다(원장님이 아이마다 덧붙이는 말) */
export const OPTIONAL = Object.freeze(["덧붙임", "한 줄"]);   // 비면 그 줄이 통째로 사라진다(뼈대-11 의 반대쪽 · 치환 칸이 남으면 못 나간다)
...
/** 가린 번호 「010-****-5678」 · 발송 이력(notify_log.to_phone)에는 이것만 남긴다(전화번호를 그대로 안 쌓는다) */
...
export function fill(template, vars = {}, optional = OPTIONAL) {
  ...
  const lines = String(template ?? "").split("\n").map((line) => {
    ...
    const out = line.replace(/\{\{\s*([^}]*?)\s*\}\}/g, (m, k) => {
      ...
      missing.push(k || "빈 칸"); return m;
    });
    return drop && !out.trim() ? null : out;
  }).filter((l) => l !== null);
  return { text: lines.join("\n").replace(/\n{3,}/g, "\n\n").trim(), missing: [...new Set(missing)] };
}
/** 한 통의 짐 · 받는 번호 · 발신번호(솔라피에 등록된 것) · 글 · 유형(길이가 정한다) · LMS 면 제목 */
...
/** 솔라피 답 읽기 · 2xx + statusCode 2000 대면 접수 · 아니면 까닭 한 줄(키는 절대 안 적는다) */
...
/** 문구 유형 · 화면(발송 10 「✉️ 문자 문구」)이 고치는 것 둘. 표는 v2.msg_template(0154) */
export const TEMPLATES = Object.freeze([
  ["sms_welcome", "첫 등원 안내", "18 등록 전환을 누르면 학부모 전화로 · 앱 주소 · 아이디 · 처음 비밀번호 · 규정 · 교재 · 시간표"],
  ["sms_guide", "상담 안내", "18 「📨 안내 문자」 · 문의한 집에 한 통"],
  ["sms_late", "하원 지연 안내", "앱 알림과 함께 · 발송 10 에서 켠 유형만(하원 지연)"],
  ["sms_fee", "수강료 안내", "앱 알림과 함께 · 발송 10 에서 켠 유형만(수강료)"],
  ["sms_monthly", "월간 리포트", "앱 알림과 함께 · 발송 10 에서 켠 유형만(월간)"],
]);
/** (뎌-2) 앱 알림과 **함께** 문자로도 갈 수 있는 유형 · 규칙 notify.sms_kinds 가 켠 것만 나간다(비면 등록·상담 안내만) */
/** (뎌-2) 유형마다의 「한 줄」 · 문구의 {{한 줄}} 에 들어간다. 비면 그 줄이 사라진다(OPTIONAL).
 *  하원 지연은 예상 시각과 사유 · 수강료는 달과 금액 · 월간은 그 달. 셈이 아니라 **글**이라 여기(순수)에 둔다 */
...
export const SMS_ALSO = Object.freeze([["late", "하원 지연"], ["fee", "수강료"], ["monthly", "월간 리포트"]]);
export const smsAlsoName = (k) => SMS_ALSO.find(([x]) => x === k)?.[1] ?? k;
/** 이 유형을 문자로도 보내나 · 규칙 목록에 들어 있고, 보낼 수 있는 유형일 때만 */
