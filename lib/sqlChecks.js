...
export const CHECKS = [
  ...
  // 함수 하나뿐이라 표·칸으로는 확인이 안 된다 — 표식이 pg_proc 을 실제로 본다.
  // 없으면 아이가 스스로 찍은 등원만 조용히 안 세어진다 (오류도 안 난다)
  { id: "0184", label: "출결을 어디서 찍든 그날 판에 같이 (월간 수업일수)", rpc: "attend_mirror_on" },
  // 0177 은 비어 있다 — 「마감 게이트 전면화」가 쓰기로 잡아둔 번호라 건너뛴다.
  // 표식은 카탈로그를 실제로 보고, **정책의 무한 재귀 냄새**까지 본다.
  // 다만 표식이 초록이어도 확인이 끝난 것이 아니다 — 진짜 확인은
  // 「학생 권한으로 앉아서 자료가 실제로 읽히나」다 (실행계획서 §6-2).
  { id: "0178", label: "내신 자료 수령 체크 (학생이 누름)", rpc: "prep_receipt_on" },
