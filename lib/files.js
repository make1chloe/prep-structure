...
export async function attachConsult(sb, fileId, consultId) { row(await db(sb).from("file_link").upsert({ file_id: fileId, consult_id: consultId }, { onConflict: "file_id,bin_id,day_item_id,notice_id,consult_id", ignoreDuplicates: true }), "상담에 못 붙임"); }
