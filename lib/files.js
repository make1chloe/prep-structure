 ...
 *
 * ⚠️⚠️ **버킷이 mime 을 걸고 있다 — 축이 다르다.**
 *    `supabase/migrations/9000_switch_day.sql` 의 `storage.buckets.allowed_mime_types` 는
 *    **mime 목록**이고 여기 `OK_EXT` 는 **확장자 목록**이다. 두 목록은 짝이 안 맞아도
 *    **아무 데서도 안 터진다** — 앱은 「받았습니다」라고 말한 뒤 Storage 가 400 으로 거절하고,
 *    학부모 화면에는 아무 오류도 안 뜬다. 원장님은 그 종이가 안 온 줄도 모른다.
 *    실측(2026-09-02): `.hwpx` 하나가 어긋나 있었다 — 여기는 `application/hwp+zip`,
 *    버킷은 `application/vnd.hancom.hwpx`. **한글 hwpx 가정통신문이 통째로 안 올라간다.**
 *    → 한글 mime 은 표준이 없어 **어느 쪽이든 우리가 고른 값**이므로, **버킷이 이미 받는 글자**로
 *      맞췄다(`application/vnd.hancom.hwpx`). DB 를 안 건드리고 그 자리에서 닫힌다.
 *    → 다시 어긋나는 것은 `scripts/check-files.mjs` 가 **그 SQL 파일을 진짜로 읽어** 막는다.
 *      확장자를 하나 더할 때 mime 만 더하고 버킷을 안 고치면 그 검사가 빨개진다.
 *    ⚠️ 두 벌을 한 벌로 줄이는 진짜 답은 **버킷이 mime 을 안 거는 것**이다(needsDb 에 SQL 을 냈다).
 *      그것이 들어가면 이 짝맞춤은 저절로 필요 없어진다 — 검사는 둘 다 초록으로 받는다.
 */
export const MIME_FOR = {
  ...
  pdf: "application/pdf",
  // ⚠️ 이 두 줄의 글자는 **버킷 목록과 같아야 한다** (위 ⚠️⚠️). 바꾸려면 9000 도 같이 본다
 ...
 *
 *   `file.recursion`      ⚠️⚠️ **지금 자료함은 아무도 못 읽는다 — 원장님도 포함.**
 *                         `own_file` 이 `v2.file_link` 를 직접 읽고, 그 표의 `own_link` 가 다시
 *                         `v2.file` 을 직접 읽는다. 정책 안에서 읽는 표는 **그 표의 정책이 또 돈다** —
 *                         `infinite recursion detected in policy for relation "file"` 가 나고
 *                         **select 가 통째로 터진다.** 2026-09-02 실측(아이·원장 둘 다 재현):
 *                           `set local role authenticated` + `select 1 from v2.file limit 1` → 터진다.
 *                         ⚠️ **정책 글자만 읽는 검사로는 절대 안 잡힌다.** 글자는 멀쩡하다.
 *                         푸는 법: 표를 건너 읽는 자리를 **security definer 함수**로 뺀다
 *                         (`v2.sheet_visible` 이 이미 그 꼴이라 안 돈다). needsDb 에 SQL 을 냈다.
 ...
 *                         ⚠️ 더 나쁜 쪽은 지우는 것이 아니라 **옮기는 것**이다 —
 *                         `file_id` 를 **남의 파일 id** 로 바꾸면 그 줄이 자기 숙제 줄에 붙어 있으므로
 *                         `own_file` 이 「보여도 된다」고 답한다 → **다른 집 자료를 그대로 연다.**
 *                         `authenticated` 가 `file_link` 의 **모든 칸**에 update 권한을 갖고 있어
 *                         (2026-09-02 실측: file_id·bin_id·day_item_id·notice_id·consult_id 전부)
 *                         정책만 고치면 **반만 고친 것**이다. 검사가 둘을 같이 본다.
 */
export const RLS_NOT_YET = new Set(["file.recursion", "file.notice", "file_link.child_seen"]);
 ...
 *
 * ⚠️⚠️ **되풀이가 안 풀린 동안은 「누가 보나」 자체가 뜻이 없다 — 아무도 못 연다**
 *    (`RLS_NOT_YET` 의 `file.recursion`). 그때는 그 말을 **앞에 붙여서** 내놓는다.
 *    안 붙이면 원장님은 「원장님만 봅니다」를 읽고 눌렀다가 오류를 본다 (대전제 0).
 */
export function seenByLabel(link = {}) {
  ...
  // ⚠️ 되풀이가 살아 있으면 **어느 자리든** 못 연다. 자리 판단보다 먼저 말한다
  if (RLS_NOT_YET.has("file.recursion"))
    return "⚠️ 지금은 아무도 못 엽니다 — 자료함 접근 규칙이 서로를 불러 터집니다(원장님도 포함)";
