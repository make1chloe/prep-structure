...
export async function attachFile(sb, fileId, itemId) { changed(await db(sb).from("file_link").upsert({ file_id: fileId, day_item_id: itemId }, { onConflict: "file_id,bin_id,day_item_id,notice_id,consult_id", ignoreDuplicates: true , count: "exact" }), "숙제에 못 붙임", { zero: "ok" }); }
/** 상담에 붙이기(4단계-6 · 14 상담 줄 📎) · 짝 표 한 줄. 상담은 원장만 보므로 파일도 원장 자료실 것 */
export async function attachConsult(sb, fileId, consultId) { changed(await db(sb).from("file_link").upsert({ file_id: fileId, consult_id: consultId }, { onConflict: "file_id,bin_id,day_item_id,notice_id,consult_id", ignoreDuplicates: true , count: "exact" }), "상담에 못 붙임", { zero: "ok" }); }
