...
export async function loadPicks(db, table) {
  // ⚠️⚠️ **표의 제약만 보면 안 된다 — 목록이 도메인에 살 수도 있다.**
  //    0079 에서 영역 목록을 `v2.area_name` 도메인 한 벌로 묶었더니, 표의 낱개 제약이 없어져
  //    여기가 그 칸을 **「제약 없음」으로 읽었다.** 그러면 엑셀이 없는 영역을 그대로 통과시킨다 —
  //    엑셀은 화면 제약을 뚫는 유일한 길이라(규칙 6), 이 자리가 비면 오타가 새 영역이 된다.
  //    → 표 제약과 **도메인 제약을 같이** 읽는다. 도메인 쪽 이름은 `value` 라 칸 이름으로 바꿔 준다.
  const r = await db.query(
    `select null::text as col, pg_get_constraintdef(c.oid) as def
       from pg_constraint c
       join pg_class t on t.oid = c.conrelid
       join pg_namespace n on n.oid = t.relnamespace
      where n.nspname = 'v2' and t.relname = $1 and c.contype = 'c'
     union all
     select a.attname::text as col, pg_get_constraintdef(dc.oid) as def
       from pg_attribute a
       join pg_class t on t.oid = a.attrelid
       join pg_namespace n on n.oid = t.relnamespace
       join pg_type dt on dt.oid = a.atttypid and dt.typtype = 'd'
       join pg_constraint dc on dc.contypid = dt.oid and dc.contype = 'c'
      where n.nspname = 'v2' and t.relname = $1 and a.attnum > 0 and not a.attisdropped`,
    [safeName(table)]
  );
  const out = {};
  for (const row of r.rows || []) {
    const def = String(row.def || "");
    // ⚠️ 도메인 제약은 칸 이름 자리에 **대문자 `VALUE`** 를 쓴다 — 소문자만 물면 한 줄도 안 걸린다
    const m = /\(?([A-Za-z_]+)\s*=\s*ANY\s*\(ARRAY\[(.+?)\]\)/s.exec(def);
    if (!m) continue;
    const vals = [...m[2].matchAll(/'((?:[^']|'')*)'/g)].map((x) => x[1].replace(/''/g, "'"));
    // 도메인 제약은 칸 이름이 `value` 다 — 그 칸에 붙은 이름으로 바꾼다
    const col = row.col ? row.col : m[1];
