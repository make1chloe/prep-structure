 ...
 * ─────────────────────────────────────────────────────────────
 *
 * ⚠️ **이 파일은 v2.excel_run · v2.excel_row 두 표를 쓴다.** 아직 마이그레이션에 없다 —
 *    그 SQL 을 돌리기 전에는 `apply()` 가 그 자리에서 **실패한다**(조용히 넘어가지 않는다).
...
/** SQL 에 표 이름을 끼우기 전에 한 번 본다 (우리 정의에서만 오지만 자물쇠는 건다) */
function safeName(t) {
  if (!/^[a-z_]+$/.test(String(t))) throw new Error(`표 이름이 이상하다 — ${t}`);
  return t;
}
...
async function loadRows(db, table, scopeCol, scopeVals) {
  ...
  if (scopeCol && scopeVals && scopeVals.length) {
    const r = await db.query(`select * from v2.${t} where ${safeName(scopeCol)} = any($1)`, [scopeVals]);
    return r.rows || [];
  }
...
async function loadRef(db, ref) {
  ...
  if (ref.alias) {
    const a = await db.query(
      `select ${safeName(ref.aliasIdCol)} as id, alias from v2.${safeName(ref.alias)}`, []);
...
export async function apply(db, sheetKey, p, opts = {}) {
  ...
  for (const ch of p.change) {
    ...
    const sets = ch.diffs.map((d, i) => `${d.col} = $${i + 2}`).join(", ");
...
export async function undo(db, runId) {
  ...
  return { runId, back, downed, cannot,
           note: cannot.length
             ? `⚠️ 상태 칸이 없어 **못 내린 줄 ${cannot.length}개** — 지우지 않는다(대전제 6). 손으로 봐야 한다`
             : null };
}
