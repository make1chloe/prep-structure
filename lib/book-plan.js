 ...
 *  교재 맞추기(교재ID › 이름 › 다른 이름 › 다듬은 키 · 옛 앱 lib/bookName 의 키 그대로) · 올리면 이렇게 됩니다(새로 생김 · 바뀜 · 손대지 않음 · 파일에 없는 기존 줄 · 보류) · 엑셀로 내보낼 줄 ·
 *  시트 유형(단원/교재 · 첫 줄 열 이름으로) · 교재 시트 읽기·계획(새로 · 고침 · 같음 · 보류 · 고칠 줄 · 옛 앱 importTextbook 의 열) · 올린 기록(덮어쓰기가 적을 것 · 올린 기록 글 · 📦 줄 · 되돌린 글 · 0057·0142, 5단계-④). 세는 것은 세어 나온다(원칙-5) */
...
export const MODE = Object.freeze([["unit", "단원"], ["passage", "지문"], ["set", "세트"], ["word", "단어"]]);   // 진행 방식(목업 15 · 0144 books.mode · (가)-⑥) — 교재마다 한 번 · 모르면 단원
export const modeName = (k) => MODE.find(([x]) => x === k)?.[1] ?? MODE[0][1];
export const MODES = Object.freeze([["overwrite", "덮어쓰기"], ["replace", "지우고 새로 올리기"], ["skip", "이 교재는 건너뛰기"]]);
// ── 같은 교재인지 알아보는 키(옛 앱 lib/bookName.js 그대로) · 판·연도·기호만 없앤다, 숫자와 글자는 절대 안 지운다
...
const COLS = Object.freeze([
  ...
  ["page_start", ["시작페이지", "시작p", "시작쪽", "시작"]], ["page_end", ["끝페이지", "끝p", "끝쪽", "종료페이지", "끝"]], ["q_count", ["문항수", "문제수", "문항개수", "문제개수", "문항"]], ["q_range", ["문항범위", "문제범위", "범위"]], ["gist", ["핵심내용", "학습내용", "내용", "요약"]], ["workbook", ["워크북", "유형"]],
]);
...
/** 파일의 교재 이름 → 앱의 교재 · 교재ID › 이름 그대로 › 다른 이름 › 다듬은 키. 후보가 둘이면 못 고른다(보류) */
...
/** 단원 줄의 올린 기록 · 이 교재의 기존 줄이 이관(import)이면 새 줄도 이관으로 적어 한 교재에 두 출처가 섞이지 않게(검사-⑰) · 없으면 excel */
...
/** 단원 한 줄 손으로 수정 읽기(15 · 4단계-4) — 쪽 「10-12」/「10」(p. 는 떼고) · 문항(0 이상 정수) · 요지. 비우면 없앤다(null). 이름·차례는 엑셀로(전체 차례가 얽힌다) */
...
/** 시트 유형 · 대단원·중단원이 있거나 「소단원/단원명/단원」 열이 그대로 있으면 단원 시트 · 교재명 + (영역·레벨·교재비·구매링크·출판사·연도·교재ID) 가 있으면 교재 시트 · 아니면 모름. 「단원수」(⬇ 교재 시트의 셈 열)는 단원 시트로 안 읽힌다 */
...
/** 교재 시트 줄 읽기 · 교재명 없는 줄은 버린다 · 영역 밖 · 링크 꼴 · 연도 꼴 · 같은 교재 두 줄(키가 같으면)은 「고칠 줄」로 빼고 저장하지 않는다 · 값이 빈 칸은 「모름」(지우지 않는다) · 단원수는 셈 열이라 안 읽는다 */
...
/** 교재 시트가 고칠 수 있는 칸(이름은 못 바꾼다 · 이름이 곧 키) · 시트 값이 있고 지금 값과 다른 칸만(빈 칸은 지우지 않는다) */
...
// ── 올린 기록(0057 excel_run · excel_row · 0142 부터 15b 가 적는다 · 되돌리기는 SQL 한 곳 undo_excel_run)
...
/** 올린 기록 한 줄의 글(excel_run.note) · 저장이 끝나면 적는다 · 📦 카드가 그대로 보여준다 */
...
export const bookRunNote = ({ created = 0, updated = 0, same = 0 } = {}) => `교재 시트 · 새로 ${created}권 · 수정 ${updated}권 · 같음 ${same}`;
export const RUN_TBL = Object.freeze({ units: "단원", books: "교재" });
const seoulAt = (ts) => (ts ? `${seoulDate(ts).slice(5)} ${seoulTime(ts)}` : "");
/** 📦 올린 기록 한 줄 · 「#12 단원 · 파일」 · 「09-07 23:10 · 원장」 · 글(없으면 셈으로) · 상태(살아 있음 · 되돌림) · 되돌릴 수 있나는 SQL 이 준 대로(can_undo · 같은 표의 마지막 살아 있는 올린 기록만) */
export function runLine(r = {}) {
  return { id: r.id, title: `#${r.id} ${RUN_TBL[r.tbl] ?? r.tbl}${r.sheet ? ` · ${r.sheet}` : ""}`, when: [seoulAt(r.at), r.who].filter(Boolean).join(" · "),
           note: r.note ?? `새로 ${r.n_insert ?? 0} · 수정 ${r.n_update ?? 0}${r.n_delete ? ` · 지움 ${r.n_delete}` : ""}`, state: r.undone_at ? "undone" : "live", canUndo: Boolean(r.can_undo), undoneAt: r.undone_at ? seoulAt(r.undone_at) : null };
}
