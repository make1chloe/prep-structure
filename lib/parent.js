...
export async function parentDay(sb, user, student, date) {
  const id = student.id;
  const [fam, arrival, lastR, todayR, abs, late, memos, asks, access, days, scoresR, linksR, rules, sentR] = await Promise.all([
    ...
    myLinks(sb, id, plusDays(date, -60)),   // 📎 아이 숙제에 붙은 파일(마감한 판만 — RLS own_link)
    ruleMap(sb, ["file."]),
    mySent(sb, user.id, plusDays(date, -60)),   // 📎 내가 보낸 것(형제 이름 · 원장님 답)
  ]);
  ...
  return {
    ...
    links: rows(linksR, "붙임"), rules, uploads: rows(sentR, "내가 보낸 것"),   // ⚠️ sent 는 📨 보낸 것(알림 자취) — 이름을 겹치지 않는다
  };
}
