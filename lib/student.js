/** 학생 14 의 손 · 판 읽기(한 벌 student_board) · + 학생 · 수정 · 퇴원·복귀(줄은 남는다) · 반 옮기기(이력) · 학생별 금액(fee_rule 이력) · 상담 적기 · 형제 묶기 · 계정 발급(학생 아이디 · 학부모 전화 · 첫 비밀번호 FIRST_PW · 바꿔야 들어감 · (어36) auth 에 이미 있는 아이디면 새로 안 만들고 잇는다) · 학부모 계정 이름 수정 · 비밀번호 초기화(앱이 발급한 계정만, 대전제-12).
...
import { parseStudent, parseLoginId, nextLoginId, canReset, parseParentName, suggestLoginId, FIRST_PW, RESET_KID } from "./student-plan.js";
...
export async function setStudent(sb, studentId, f, seenAt = null) {
  ...
  const r = row(await q.select("id"), "학생을 수정 못 함");
...
/** 학부모 계정 이름 수정((가)-⑩ · 남긴 것 14) — 이 아이에게 이어진 계정만(parent_student) · 형제가 같은 계정을 쓰면 둘 다 그 이름(줄이 하나다) · 비밀번호는 안 건드린다(대전제-12 는 비밀번호 얘기 — 이관된 계정도 이름은 고친다) · 헤더 알약(shell 의 me.name)과 14 학부모 줄이 이 값을 읽는다 */
export async function setParentName(sb, studentId, profileId, name) {
  ...
  const r = changed(await db(sb).from("profiles").update({ name: nm }).eq("id", profileId).eq("role", "parent").select("id"), "이름을 수정 못 함");
...
/** 비밀번호를 첫 비밀번호로 되돌린다 — 앱이 발급(또는 원장님이 켜신) 계정만(대전제-12). must_change_pw 를 켜는 것이 곧 초기화(0032).
 *  (어74) 선생님·조교도 **이 손 하나**를 쓴다 — 부르는 화면이 allow 로 제 몫을 말한다(14 는 아이·학부모 · 설정 👤 는 선생님·조교).
 *  원장 줄은 allow 에 없어서 어느 쪽에서도 안 된다. */
export async function resetPassword(svc, sb, profileId, allow = RESET_KID) {
  ...
  if (!allow.includes(p.role)) throw new Error(`${ROLE_NAME[p.role] ?? p.role} 계정은 이 화면에서 되돌리지 않습니다`);
  if (!canReset(p)) throw new Error("앱이 낸 계정만 되돌립니다 · 이어 쓰는 계정은 쓰던 비밀번호 그대로입니다");
