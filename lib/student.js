/** 학생 14 의 손 — 판 읽기(한 벌 student_board) · + 학생 · 고치기 · 퇴원·복귀(줄은 남는다) · 반 옮기기(이력) · 학생별 금액(fee_rule 이력) · 상담 적기 · 형제 묶기 · 계정 발급(학생 아이디 · 학부모 전화 — 첫 비밀번호 0000 · 바꿔야 들어감) · 비밀번호 초기화(앱이 발급한 계정만, 대전제-12).
...
import { parseStudent, parseLoginId, canReset } from "./student-plan.js";
...
export async function studentBoard(sb, studentId, date) {
  const { data, error } = await db(sb).rpc("student_board", { p_student: studentId ?? null, p_on: date });
...
export async function setStudent(sb, studentId, f) {
  const p = parseStudent(f);
  const r = row(await db(sb).from("students").update(p).eq("id", studentId).select("id"), "학생을 못 고침"); if (!r?.length) throw new Error("고쳐진 줄이 없습니다");
}
...
export async function setState(sb, studentId, state, on = null) {
  ...
  row(await db(sb).from("students").update({ state, left_on: state === "left" ? on : null }).eq("id", studentId).select("id"), "상태를 못 바꿈");
  if (state === "left") row(await db(sb).from("class_member").update({ to_date: on }).eq("student_id", studentId).is("to_date", null).select("class_id"), "반 줄을 못 닫음");
}
...
export async function setClass(sb, studentId, classId, fromDate) {
  ...
  for (const c of cur) row(await db(sb).from("class_member").update({ to_date: c.from_date > before ? c.from_date : before }).eq("student_id", studentId).eq("class_id", c.class_id).eq("from_date", c.from_date).select("class_id"), "반 줄을 못 닫음");
...
export async function setFee(sb, studentId, amount, fromDate) {
  ...
  for (const c of cur) row(await db(sb).from("fee_rule").update({ to_date: c.from_date > before ? c.from_date : before }).eq("id", c.id).select("id"), "단가 줄을 못 닫음");
...
export async function issueStudentAccount(svc, sb, studentId, loginId) {
  ...
  row(await db(svc).from("students").update({ profile_id: u.id }).eq("id", studentId).select("id"), "학생에 계정을 못 붙임");
...
export async function issueParentAccount(svc, sb, studentId, phone, rel = null) {
  ...
  row(await db(svc).from("students").update({ parent_phone: loginId }).eq("id", studentId).select("id"), "학부모 전화를 못 적음");
...
export async function resetPassword(svc, sb, profileId) {
  ...
  row(await db(svc).from("profiles").update({ must_change_pw: true }).eq("id", profileId).select("id"), "표시를 못 켬");
