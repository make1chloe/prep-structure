/** 학생 14 의 손 · 판 읽기(한 벌 student_board) · + 학생 · 고치기 · 퇴원·복귀(줄은 남는다) · 반 옮기기(이력) · 학생별 금액(fee_rule 이력) · 상담 적기 · 형제 묶기 · 계정 발급(학생 아이디 · 학부모 전화 · 첫 비밀번호 0000 · 바꿔야 들어감 · (어36) auth 에 이미 있는 아이디면 새로 안 만들고 잇는다) · 학부모 계정 이름 고치기 · 비밀번호 초기화(앱이 발급한 계정만, 대전제-12).
 ...
 *  학생 줄이 있는데 어느 아이에게도 안 이어졌으면 잇는다 · 다른 아이의 것이거나 학생이 아닌 역할이면 막는다 */
export async function issueStudentAccount(svc, sb, studentId, loginId) {
  ...
  const had = u.created ? null : row(await db(svc).from("profiles").select("id,role,login_id").eq("id", u.id).maybeSingle(), "사람 줄을 못 읽음");
  if (had) {
    if (had.role !== "student") throw new Error(`${id} 은 ${ROLE_NAME[had.role] ?? had.role} 계정입니다 · 다른 아이디로`);
    const owner = row(await db(svc).from("students").select("id,name").eq("profile_id", had.id).maybeSingle(), "학생을 못 읽음");
    if (owner) throw new Error(`이미 ${owner.name} 의 아이디입니다: ${id} · 다른 아이디로`);
  } else {
    const taken = row(await db(svc).from("profiles").select("id").eq("login_id", id).neq("id", u.id).maybeSingle(), "사람 줄을 못 읽음");
    if (taken) throw new Error(`아이디 ${id} 는 다른 사람 줄이 쓰고 있습니다 · 다른 아이디로`);
    row(await db(svc).from("profiles").insert({ id: u.id, role: "student", name: st.name, login_id: id, must_change_pw: u.issuedByApp, issued_by_app: u.issuedByApp }).select("id"), "사람 줄을 못 넣음");
  }
  changed(await db(svc).from("students").update({ profile_id: u.id }).eq("id", studentId).select("id"), "학생에 계정을 못 붙임");
  return { login_id: id, password: u.issuedByApp ? FIRST_PW : null, adopted: !u.created };
}
