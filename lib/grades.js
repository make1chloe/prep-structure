...
/** 학교 이름으로 학년의 앞글자를 짐작한다 — 「신정중」 이면 중 */
export function levelFromSchool(school = "") {
  const s = String(school || "");
  if (/고(등학교)?$/.test(s) || /고등학교/.test(s)) return "고";
  if (/중(학교)?$/.test(s) || /중학교/.test(s)) return "중";
  if (/초(등학교)?$/.test(s) || /초등학교/.test(s)) return "초";
  return "";
}
