...
export async function stayDone(sb, itemId) {
  ...
  const { error } = await db(sb).from("day_item").update({ status: "done" }).eq("id", itemId); if (error) throw new Error(`못 적음: ${error.message}`);
}
...
export async function stayCarry(sb, sheetId) {
  ...
  let n = 0;
  for (const r of rows ?? []) { await carryRest(sb, r.id, "home"); const { error: e2 } = await db(sb).from("day_item").update({ status: "missing" }).eq("id", r.id); if (e2) throw new Error(`못 넘김: ${e2.message}`); n++; }
  return { n };
}
