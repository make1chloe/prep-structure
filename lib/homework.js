...
import { layRoutine } from "./routine.js";
...
async function sheetOf(sb, itemId) {
  const { data } = await db(sb).from("day_item").select("id,sheet_id,slot,range_note,memo,unit_id,item_id,status,done_note,carry_of").eq("id", itemId).maybeSingle();
...
export async function checkItem(sb, itemId, status, doneNote = null) {
  ...
  const { data: left, error: e2 } = await db(sb).from("day_item").select("id,status").eq("sheet_id", it.sheet_id).eq("slot", "check");
  if (e2) throw new Error(`검사 줄을 못 읽음: ${e2.message}`);
  if (!(left ?? []).some(isUnchecked)) await layRoutine(sb, it.sheet_id);
}
...
export async function addItem(sb, sheetId, slot, text) {
  if (!["class", "home", "stay"].includes(slot)) throw new Error(`자리가 아닙니다: ${slot}`);
...
export async function moveItem(sb, itemId, slot) {
  ...
  changed(await db(sb).from("day_item").update({ slot }, { count: "exact" }).eq("id", itemId), "못 옮김");
}
...
/** 대전제-19(원장님 2026-09-15 「기본적으로 모든 항목을 추가/수정/삭제가 가능한게 기본이야」): 손으로 더한 줄(그 밖에 · 남 · 나머지 조각)은 글 고치기 · 빼기 · 되살리기.
 *  루틴이 깐 줄(교재 반쪽)은 「조절」과 같은 손(pickWave)으로 뺀다. 지우지 않는다(대전제-6): 빼기 = off · 되살리기 = off 풀기. 검사 줄(지난 판)은 못 건드린다 */
...
export async function removeItem(sb, itemId) {
  const it = await sheetOf(sb, itemId);
  if (!["class", "home", "stay"].includes(it.slot)) throw new Error("검사 줄은 못 뺍니다");
