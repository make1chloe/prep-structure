...
export async function carryUndo(sb, itemId, where) {
  ...
  changed(await db(sb).from("day_item").update({ off: true }, { count: "exact" }).eq("id", data.id), "줄을 못 내림");
  return { undone: true };
}
...
/** 대전제-19(원장님 2026-09-15 「기본적으로 모든 항목을 추가/수정/삭제가 가능한게 기본이야」): 손으로 더한 줄(기타 · 남 · 나머지 조각)은 글 고치기 · 빼기 · 복구.
