...
/** 모드 · 키(확장이 보내는 이름) · 우리 말 · 단위(pct 백분율 · count 개수) · 어느 세트 유형에 뜨나 */
export const MODES = Object.freeze([
  ["memorize", "암기", "pct", 1], ["recall", "리콜", "pct", 1], ["match", "매칭", "count", 1], ["spell", "스펠", "pct", 1],
  ["read", "낭독", "pct", 2], ["rec", "녹음", "pct", 2], ["sent", "문장암기", "pct", 2],
  ["scramble", "스크램블", "pct", 0], ["drill", "드릴 순서배열", "pct", 0],   // 확정-55 · 유형을 안 가린다(0 = 둘 다)
]);
export const MODE_NAME = Object.freeze(Object.fromEntries(MODES.map(([k, name]) => [k, name])));
/** 확정-⑩ · 3초훈련은 판정하지 않는다. 짐에 와도 여기서 걸러 낸다(화면·발송 이력 어디에도 안 남는다) */
export const DROPPED = Object.freeze(["speed", "speed3", "3sec"]);
/** 클래스카드가 사는 곳 — 아이 07 의 「🃏 클래스카드」 단추가 여기로 보낸다((어77)).
 *  확장(extension/background.js CC)도 같은 곳을 본다 — 확장은 크롬 안에서 도는 딴 짐이라 이 파일을 못 읽는다.
 *  그래서 두 벌이 되는데, 어긋나지 않게 check-cc 가 둘을 견준다(원칙-1 의 차선). */
export const CC_URL = "https://www.classcard.net";
/** (뎌-3) 클래스카드가 쓰는 이름 → 우리 키. **확장은 긁은 이름을 그대로 보낸다** · 옮김은 여기 한 곳뿐이다(원칙-1).
...
export function tidyScores(o = {}) {
  ...
  for (const [k, v] of Object.entries(o ?? {})) {
    const key = modeKey(k); if (!key) continue;   // 3초훈련·모르는 이름은 버린다 · 클래스카드 이름은 우리 키로 옮긴다(ALIAS)
...
export function modeLines(goals = {}, got = {}) {
  ...
  return keys.map((k) => {
    ...
    const fmt = (v) => (v === null ? "없음" : unit === "pct" ? `${Math.round(v)}%` : v.toLocaleString("ko-KR"));
    const ok = goal === null || actual === null ? null : actual >= goal;
    return { key: k, name, unit, goal, actual, ok, actualText: fmt(actual), goalText: goal === null ? null : `목표 ${fmt(goal)}`,
      text: `${name} ${fmt(actual)}${goal === null ? "" : ` · 목표 ${fmt(goal)}`}` };
  });
}
/** 못 넘긴 것만 — 「스펠 82% · 목표 100%」. 하나도 없으면 빈 목록(다 넘었다) · 확정-⑱ 앱은 넘기지 않는다, 보여만 준다 */
export const shortOf = (lines) => lines.filter((l) => l.ok === false);
/** 못 넘긴 것 한 줄 — 「스펠 · 매칭 다시 돌립니다」 · 다 넘었으면 「다 넘었습니다」 · 잴 것이 없으면 null(거짓말 안 한다) */
export function shortText(lines) {
  const ls = lines ?? []; if (!ls.some((l) => l.ok !== null)) return null;
  const short = shortOf(ls);
  return short.length ? `${short.map((l) => l.name).join(" · ")} 다시 돌립니다` : "다 넘었습니다";
}
