...
export const KEYS = Object.freeze([
  { id: "solapi", emo: "✉️", name: "문자 (솔라피)", why: "등록 안내·상담 안내 문자가 이 열쇠로 나갑니다. 로그인 아이디·비밀번호가 아니라 솔라피에서 **발급받은 API Key·Secret** 을 넣으세요. 발신번호는 솔라피에 미리 등록해 두셔야 합니다.", site: "https://solapi.com",
  ...
  { id: "classcard", emo: "🃏", name: "클래스카드 확장", why: "확장이 이 열쇠로 앱에 받아 적습니다(POST /api/cc). 아무 긴 글자나 되고, 확장 설정에 같은 값을 넣습니다.", site: null,
    fields: [{ k: "token", label: "확장 열쇠", peek: true, hint: "아무 긴 글자나 됩니다 — 20자 넘게, 남이 못 맞출 것으로" }] },
]);
...
/** 칸 값이 그 칸의 꼴인가 — 아니면 까닭 한 줄(저장을 막는다). 열쇠 자리에 이메일·비밀번호를 넣으신 일이 있다(원장님 2026-09-10) */
export function fieldWhyBad(field, value) {
  ...
  if (v.includes("@")) return `${field.label} 자리에 이메일이 들어왔습니다 — 로그인 아이디가 아니라 발급받은 열쇠를 넣어 주세요`;
  if (v.length < 8) return `${field.label} 가 너무 짧습니다 — 발급받은 열쇠를 그대로 붙여 넣어 주세요`;
  return null;
}
