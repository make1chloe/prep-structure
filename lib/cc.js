 ...
 *  키는 연동(v2.integration) 'classcard' 줄의 token · 원장님이 설정 🔌 연동 설정에서 넣고 고친다(확정-72 · 코드·SQL 로 안 넣는다).
...
/** 키 견주기 · 길이가 달라도 시간이 안 새게 해시로 견준다(둘 다 없으면 거짓) */
...
export async function ccToken(svc = serviceClient()) {
  const r = row(await db(svc).from("integration").select("config").eq("id", "classcard").maybeSingle(), "클래스카드 키를 못 읽음");
...
/** 클래스카드 아이디 읽기 — 빈 것·너무 긴 것·**앱 아이디(chloe…)**를 막는다. 원장(14)과 아이(07)가 **이 하나**를 같이 쓴다((어72) · 원칙-1) */
/** 아이 ↔ 클래스카드 아이디 연결(학생 14) · 아이마다 한 줄(student_id 가 키) · **다시 이으면 덮어쓴다**(대전제-6 · 줄을 지우지 않는다 · 잘못 이었으면 바른 아이디로 다시 잇는다) */
export function parseCcIdx(ccUserIdx) {
  ...
  if (/^chloe[0-9]/i.test(idx)) throw new Error(`${idx} 는 앱 로그인 아이디입니다 · 여기는 클래스카드(외부 사이트) 아이디 칸 · 앱 계정은 ✎ 수정 › 🔑 계정 · 학생에서 발급·연결`);   // (어46) 원장님 9/16 · 앱 아이디를 🃏 칸에 적으셨다
  return idx;
}
export async function linkCc(sb, studentId, ccUserIdx, ccLoginId = null) {
  const idx = parseCcIdx(ccUserIdx);
