...
import { addDays, dowOf, endOfMonth, WEEK_ORDER as DOW } from "./day.js";
...
export const KINDS = [
  ...
  { key: "retest", label: "단원평가 막힘", when: "event",
    hint: "같은 단원을 세 번째 다시 봐도 못 넘으면 그 학생·그 단원마다 한 번" },
];
 ...
 */
function autoKey(routineId, date) {
  ...
  for (const r of routines) {
    ...
    for (const d of occurrences(r, today, to)) {
      ...
      out.push({
        ...
        note: r.note || null,
        // 하위목록도 그대로 복사 — 생긴 할일마다 **따로** 체크된다 (0117)
...
/**
 * **단원평가 막힘 할일의 열쇠** — 학생 · 단원 · 몇 번째.
 *
 * 「몇 번째」 가 열쇠에 들어간다. 교재 끝나감에 회독이 들어간 것과 같은
 * 까닭이다 — 세 번째에 만들어 드린 재시험지로 네 번째를 봤는데 또 못
 * 넘었으면 **재시험지를 또 만들어야 한다.** 그건 같은 일이 다시 뜬 것이
 * 아니라 새로 생긴 일이다.
 *
 * 반대로 **같은 번째는 몇 번을 열어도 한 줄**이다 (auto_key 는 통째로
 * 유일하다 — 0028·0061). 원장님이 체크해 넘기신 것이 화면을 다시 열었다고
 * 되살아나지 않는다. 통과하면 그 단원은 아예 막힌 것이 아니게 되어
 * (unitProgress 의 passed) 다음 번째도 안 생긴다.
 *
 * 단원 이름은 원장님이 손으로 적으신 그대로다 — 열쇠에 들어가므로 이름을
 * 고쳐 적으면 새 줄이 된다. 이름이 곧 그 단원의 신원이라 어쩔 수 없다.
 */
export function retestKey(routineId, studentId, unit, tries) {
  return `routine:${routineId}:r:${studentId}:${unit}:${tries}`;
}
...
export function describe(r = {}) {
  ...
  if (r.repeat_kind === "retest") return "같은 단원을 세 번째 다시 봐도 못 넘으면";
  return "";
}
