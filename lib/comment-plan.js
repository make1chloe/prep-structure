 ...
 *  유형 다섯 · 길이 넷 · **상황이 길이를 먼저 고른다** · 그날 상태에서 유형이 저절로 · 글자 세기 · 넘으면 **문장 끝에서** 자른다(글자 중간에서 안 자른다)
...
/** 그날 그 아이 상태에서 유형이 저절로 골라진다 · 늦은밤(서울 시각 ≥ lateFrom) › 시험후 › 시험전 › 숙제안함(검사에 ✕) › 보통. 원장님은 바꾸고 싶을 때만 바꾼다.
...
/** 규칙 값 꾸러미({key: value}, v2.rule comment.*) → 글 설정. 줄이 없으면 던진다(뼈대-5 — 조용히 기본값으로 돌지 않는다) */
export function commentCfg(map, hour) {
  const caps = Object.fromEntries(KIND.map(([k]) => [k, parseInt(map?.[`comment.cap.${k}`], 10)]));
  for (const [k, n] of Object.entries(caps)) if (!n) throw new Error(`규칙 줄이 없다: comment.cap.${k} · 0111 의 씨앗을 본다`);
  if (!map?.["comment.late_from"] || !map?.["comment.retry"]) throw new Error("규칙 줄이 없다: comment.late_from · comment.retry · 0111 의 씨앗을 본다");
  return { caps, lateFrom: parseInt(map["comment.late_from"], 10), retry: parseInt(map["comment.retry"], 10), hour };
}
...
/** 글 밑에 저절로 붙는 줄 · 다음 시간 시험(**전체 개수가 있어야** 붙는다 · 리포트 10 도 같은 문) · 하원 지연(예상 귀가 약속) · 반성문 처분. on 이 꺼진 줄은 「왜 안 붙나」를 말한다 */
...
/** 학부모가 볼 글 = 원장님 글 + 붙는 줄(켜진 것만). 01 미리보기는 preview(한 글), 09 수업 카드는 글 + previewLines(줄마다) — 같은 attached() 한 벌((서) 2026-09-09) */
export const previewLines = (lines = []) => lines.filter((l) => l.on).map((l) => l.text);
export const preview = (comment, lines = []) => [norm(comment) ? String(comment).trim() : "", ...previewLines(lines)].filter(Boolean).join("\n");
const ATTEND_NAME = { present: "출석", late: "지각", absent: "결석", early: "조퇴", online: "온라인", makeup: "보강으로 옴", off: "휴강" };
const STATUS_NAME = { done: "○ 해옴", weak: "△ 일부", missing: "✕ 안 해옴", none: "아직 안 봄", inclass: "학원에서 함" };
const itemName = (it) => it?.text ?? itemTitle(it);   // 항목 줄 글 한 벌((어27))
...
export function facts({ student, sheet, quizzes = [], keys = "", lines = [] } = {}) {
  ...
  if (sheet?.late?.until_at) out.push(`하원 지연: 오늘 ${String(sheet.late.until_at).slice(0, 5)}까지 남아서${sheet.late.reason ? ` · ${sheet.late.reason}` : ""}`);
  for (const m of sheet?.memos ?? []) if (String(m?.memo ?? "").trim()) out.push(`${m.area} 메모(아이에게 그대로 나감): ${String(m.memo).trim()}`);
...
export function promptFor({ kind, cap, samples = [], rules = "", factLines = [] }) {
  const system = [
    ...
    samples.some((s) => s.startsWith("[이 아이")) ? "- 「[이 아이 …]」 로 표시된 것은 **이 아이에게 지난번에 보낸 글**입니다. 같은 아이니 호칭·톤을 그대로 잇되, 내용은 오늘 사실만 씁니다." : "",
