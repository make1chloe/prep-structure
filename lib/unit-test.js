...
export async function unitTestsOf(sb, studentIds, date) {
  ...
  const { data, error } = await db(sb).from("unit_test").select("id,student_id,topic_id,assigned_on,taken_on,q_count,correct,state,book_id,round,chapter,seq,covers,grammar_topics(name),books(name)")
...
export async function scoreUnitTest(sb, id, correct, date) {
  ...
  changed(await db(sb).from("unit_test").update({ correct: n, state: "scored", taken_on: date }, { count: "exact" }).eq("id", id), "단원평가 점수를 못 씀");
}
