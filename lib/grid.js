/** 학교별 표 06c 의 손 · 판 읽기(한 벌 grid_board) · + 새 표(본에서) · 표 삭제 · 칸(더하기 · 고치기 · 차례 · 삭제 · 보드 축) · 줄(더하기 · 차례 · 삭제) · 셀(손 떼면 저장 · 값은 lib/grid-plan parseCell 한 곳) · 따로 챙길 아이 메모.
...
export async function retireGrid(sb, gridId) { const r = changed(await db(sb).from("grid").update({ state: "retired" }).eq("id", gridId).eq("state", "active").select("id"), "표를 못 내림"); if (!r?.length) throw new Error("이미 삭제한 표입니다"); }
...
/** 칸 · 더하기(끝에) · 고치기(이름·종류·선택지 · 종류나 선택지가 바뀌면 값을 옮겨 담는다: 되는 것만 · 안 되는 것은 그대로 두고 알린다 · 5단계-⑥) · 차례 · 삭제 · 보드 축 */
...
export async function setCol(sb, colId, f) {
  ...
  const r = changed(await db(sb).from("grid_col").update(c).eq("id", colId).select("id"), "칸을 못 고침"); if (!r?.length) throw new Error("고쳐진 칸이 없습니다");
...
export async function retireCol(sb, colId) { const r = changed(await db(sb).from("grid_col").update({ state: "retired" }).eq("id", colId).eq("state", "active").select("id"), "칸을 못 내림"); if (!r?.length) throw new Error("이미 삭제한 칸입니다"); }
...
export async function retireRow(sb, rowId) { const r = changed(await db(sb).from("grid_row").update({ state: "retired" }).eq("id", rowId).eq("state", "active").select("id"), "줄을 못 내림"); if (!r?.length) throw new Error("이미 삭제한 줄입니다"); }
