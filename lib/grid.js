/** 학교별 표 06c 의 손 · 판 읽기(한 벌 grid_board) · + 새 표(본에서) · 표 삭제 · 칸(더하기 · 수정 · 차례 · 삭제 · 보드 축) · 줄(더하기 · 차례 · 삭제) · 셀(손 떼면 저장 · 값은 lib/grid-plan parseCell 한 곳) · 따로 챙길 아이 메모.
...
export async function addGrid(sb, { template, label = null }, by = null) {
  ...
  if (axis) changed(await db(sb).from("grid").update({ board_col: axis.id }).eq("id", g.id).select("id"), "보드 축을 못 붙임");
  return { id: g.id, cols: made.length, board: Boolean(axis) };
}
export async function retireGrid(sb, gridId) { const r = changed(await db(sb).from("grid").update({ state: "retired" }).eq("id", gridId).eq("state", "active").select("id"), "표를 못 삭제함"); if (!r?.length) throw new Error("이미 삭제한 표입니다"); }
export async function reviveGrid(sb, gridId) { const r = changed(await db(sb).from("grid").update({ state: "active" }).eq("id", gridId).eq("state", "retired").select("id"), "표를 못 되살림"); if (!r?.length) throw new Error("살아 있는 표입니다"); }
export async function renameGrid(sb, gridId, label) { const nm = String(label ?? "").trim(); if (!nm) throw new Error("표 이름을 적으세요"); changed(await db(sb).from("grid").update({ label: nm }).eq("id", gridId).select("id"), "표 이름을 못 바꿈"); }
...
export async function moveGrid(sb, gridId, dir) {
  ...
  for (const o of order) changed(await db(sb).from("grid").update({ sort: o.sort }).eq("id", o.id).select("id"), "표 차례를 못 바꿈");
  return { moved: true };
}
/** 칸 · 더하기(끝에) · 수정(이름·종류·선택지 · 종류나 선택지가 바뀌면 값을 옮겨 담는다: 되는 것만 · 안 되는 것은 그대로 두고 알린다 · 5단계-⑥) · 차례 · 삭제 · 보드 축 */
...
export async function setCol(sb, colId, f) {
  ...
  const r = changed(await db(sb).from("grid_col").update(c).eq("id", colId).select("id"), "칸을 수정 못 함"); if (!r?.length) throw new Error("고쳐진 칸이 없습니다");
  ...
  for (const u of plan.updates) changed(await db(sb).from("grid_cell").update({ value: u.value }).eq("row_id", u.row_id).eq("col_id", colId).select("row_id"), "값을 못 옮김");
...
export async function moveCol(sb, gridId, colId, dir) {
  ...
  for (const o of order) changed(await db(sb).from("grid_col").update({ sort: o.sort }).eq("id", o.id).select("id"), "칸 차례를 못 바꿈");
  return { moved: true };
}
export async function retireCol(sb, colId) { const r = changed(await db(sb).from("grid_col").update({ state: "retired" }).eq("id", colId).eq("state", "active").select("id"), "칸을 못 삭제함"); if (!r?.length) throw new Error("이미 삭제한 칸입니다"); }
export async function setBoardCol(sb, gridId, colId) {
  ...
  changed(await db(sb).from("grid").update({ board_col: colId || null }).eq("id", gridId).select("id"), "보드 축을 못 바꿈");
}
...
export async function moveRow(sb, gridId, rowId, dir) {
  ...
  for (const o of order) changed(await db(sb).from("grid_row").update({ sort: o.sort }).eq("id", o.id).select("id"), "줄 차례를 못 바꿈");
  return { moved: true };
}
export async function retireRow(sb, rowId) { const r = changed(await db(sb).from("grid_row").update({ state: "retired" }).eq("id", rowId).eq("state", "active").select("id"), "줄을 못 삭제함"); if (!r?.length) throw new Error("이미 삭제한 줄입니다"); }
...
export async function setCell(sb, rowId, colId, raw, seenAt = null) {
  ...
  changed(await db(sb).from("grid_cell").upsert({ row_id: rowId, col_id: colId, value }, { onConflict: "row_id,col_id" }).select("row_id"), "값을 못 적음");
  return { value };
}
...
/** (너) 아이·학부모 공개 스위치 — 학교 줄 표만 켤 수 있다(학생 줄 표는 다른 아이 이름이 있다) */
export async function setShare(sb, gridId, on) {
  const g = row(await db(sb).from("grid").select("id,rows").eq("id", gridId).single(), "표를 못 읽음");
  if (on && g.rows !== "school") throw new Error("학교 줄 표만 아이·학부모에게 공개할 수 있습니다");
  changed(await db(sb).from("grid").update({ share: Boolean(on) }).eq("id", gridId).select("id"), "공개 스위치");
}
export async function setWatch(sb, studentId, note) {
  const n = String(note ?? "").trim() || null;
  changed(await db(sb).from("student_watch").upsert({ student_id: studentId, note: n }, { onConflict: "student_id" }).select("student_id"), "메모를 못 적음");
  return { note: n };
}
