...
export async function retireGrid(sb, gridId) { const r = changed(await db(sb).from("grid").update({ state: "retired" }).eq("id", gridId).eq("state", "active").select("id"), "표를 못 내림"); if (!r?.length) throw new Error("이미 내린 표입니다"); }
...
export async function retireCol(sb, colId) { const r = changed(await db(sb).from("grid_col").update({ state: "retired" }).eq("id", colId).eq("state", "active").select("id"), "칸을 못 내림"); if (!r?.length) throw new Error("이미 내린 칸입니다"); }
...
export async function retireRow(sb, rowId) { const r = changed(await db(sb).from("grid_row").update({ state: "retired" }).eq("id", rowId).eq("state", "active").select("id"), "줄을 못 내림"); if (!r?.length) throw new Error("이미 내린 줄입니다"); }
