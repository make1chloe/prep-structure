...
export async function examBoard(sb, date) {
  ...
  if (error) throw new Error(`학교 시험 판을 못 읽음: ${error.message}`);
...
export async function releaseStops(sb, examId) {
  ...
  return { released: 0, why: bound.length ? `이 시험에 묶인 교재 ${bound.length}권이 이미 다 돌아가는 중입니다` : "이 시험에 묶인 교재가 없습니다. 「지금 멈춤」을 눌러야 묶입니다" };
}
...
/** 여럿 한 번에((어28)-③ · 대전제-20) · 숨기기·되살리기 · 📡 봤음. 하나씩의 손을 차례로 · 막히면 그 시험부터 멈춘다 */
