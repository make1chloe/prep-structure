 ...
 *  교재 보류를 회차에 맞춘다(syncStops: 영어 시험일 − N주 ~ 시험 끝 · 보는 아이의 정규 교재 전부 · 내신 교재는 돈다 · 손으로 멈춘 것은 안 건드린다) · 지금 보류 · 풀기.
 *  (저) 「봤음」(나이스가 옮긴 기간 · 0152 · 확정-69) · 새벽 정리(syncStopsDaily · 크론 한 바퀴 앞에 앞으로의 회차 보류를 다시 맞춘다).
 *  판단은 lib/exam-plan.js(순수) · 보류 판정은 lib/routine-plan.js stopOn 한 곳. 지우지 않는다(대전제-6) */
import { db } from "./supabase.js";
import { ruleMap } from "./rule.js";
import { weeksFor, stopWindow, LEVELS, examEnd, SKIP_WHY } from "./exam-plan.js";
...
export async function examBoard(sb, date) {
  ...
  if (error) throw new Error(`학교 시험 화면을 못 읽음: ${error.message}`);
  if (!data) throw new Error("학교 시험은 학원 사람의 화면입니다");
  return data;
}
...
export async function addScope(sb, examId, { unitIds = [], freeNote = null, date }) {
  if (!examId) throw new Error("시험이 없습니다"); if (!isDate(date)) throw new Error(`날짜가 아닙니다: ${date}`);
...
/** 「안 봄」 · 아이 × 회차. 되돌리면 skipped=false. 보는 아이가 달라지니 보류도 다시 맞춘다 */
export async function setSkip(sb, examId, studentId, skipped, date) {
  if (!examId || !studentId) throw new Error("시험과 아이가 있어야 합니다");
...
/** 「이 학년 안 봄 한 번에」((가)-⑨ · 남긴 것 18) · 남은 아이 전부를 한 문장으로 안 봄 · 보류는 한 번만 다시 맞춘다 */
...
export async function setHidden(sb, examId, hidden, date) {
  ...
  if (!r.length) throw new Error("시험이 없습니다");
  return syncStops(sb, examId, date);
}
...
export async function setChangeSeen(sb, examId) {
  ...
  if (!r.length) throw new Error("시험이 없거나 이미 봤습니다");
  return { seen: r.length };
}
/** 몇 주 전부터(학교급) · 규칙 줄을 고친다(뼈대-5). 바뀌면 앞으로의 회차 보류를 다시 맞춘다 */
export async function setStopWeeks(sb, level, weeks, date) {
  ...
  const r = changed(await db(sb).from("rule").update({ value: String(n) }).eq("key", `prep.stop_weeks.${level}`).select("key"), "규칙을 수정 못 함", { zero: "ok" }) ?? [];
...
/** 교재 보류를 회차에 맞춘다 · 보는 아이의 살아 있는 배정 교재(내신 교재 빼고)를 「영어 시험일 − N주 ~ 시험 끝」으로 book_off.
 *  건드리는 줄: 돌아가는 줄(묶인 데 없음) · 이 회차에 묶인 줄 · 기한이 지난 줄. 손으로 멈춘 줄과 「풀기」로 푼 줄(stop_exam_id 는 남고 running)은 안 건드린다 · force(지금 보류)면 푼 줄도 다시.
 *  영어 시험일이 없거나 숨겼거나 물린 회차면 묶인 줄을 전부 푼다 */
export async function syncStops(sb, examId, date, { from = null, force = false } = {}) {
  if (!isDate(date)) throw new Error(`날짜가 아닙니다: ${date}`);
  const e = row(await db(sb).from("exams").select("id,scope,school_id,grade,name,term_from,term_to,english_on,state,hidden,schools(level)").eq("id", examId).maybeSingle(), "시험을 못 읽음");
  if (!e) throw new Error("시험이 없습니다");
  const bound = row(await db(sb).from("student_book").select("id,student_id,stop_mode").eq("stop_exam_id", examId), "묶인 교재를 못 읽음") ?? [];
  const release = async (ids) => { if (!ids.length) return 0; changed(await db(sb).from("student_book").update({ stop_mode: "running", stop_exam_id: null, stop_from: null, stop_until: null }).in("id", ids).select("id"), "보류를 못 풂", { zero: "ok" }); return ids.length; };
  if (e.state !== "active" || e.hidden || !e.english_on) return { bound: 0, kept: 0, skip: [], released: await release(bound.map((b) => b.id)), why: e.state !== "active" ? "물린 회차" : e.hidden ? "숨긴 회차" : "영어 시험일 없음" };
  ...
  let n = 0; const keep = new Set(), skipped = new Map();
  const note = (why) => skipped.set(why, (skipped.get(why) ?? 0) + 1);   // 안 건드린 까닭을 센다 — 화면이 「왜 0권인가」를 말해야 한다(대전제-0)
  if (!ids.length) note(SKIP_WHY.none);
  for (const b of books) {
    if (b.books?.area === "내신") { note(SKIP_WHY.area); continue; }   // 내신 교재는 시험철에 돈다
    const st = students.find((s) => s.id === b.student_id), level = st?.schools?.level ?? e.schools?.level;
    const win = stopWindow(e, weeksFor(level, rules, st)); if (!win) { note(SKIP_WHY.rule); continue; }   // 규칙 줄이 없으면 안 세운다(화면이 말한다)
    ...
    if (!eligible) { note(mine ? SKIP_WHY.freed : b.stop_exam_id ? SKIP_WHY.other : SKIP_WHY.hand); continue; }   // 원장님이 손으로 멈춘 것 · 다른 회차에 묶인 것은 안 건드린다
    ...
    changed(await db(sb).from("student_book").update(patch).eq("id", b.id).select("id"), "보류를 못 맞춤"); n++;
  }
  const released = await release(bound.filter((b) => !keep.has(b.id) && b.stop_mode !== "running").map((b) => b.id));
  return { bound: n, kept: keep.size, skip: [...skipped].map(([why, cnt]) => ({ why, n: cnt })).sort((a, b) => b.n - a.n), released, why: null };
}
/** 지금 바로 멈추기 — 날짜와 상관없이 오늘부터(풀어 둔 줄도 다시) */
export const stopNow = (sb, examId, date) => syncStops(sb, examId, date, { from: date, force: true });
/** 풀기 · 이 회차에 묶인 줄을 돌아가게. stop_exam_id 는 남겨 「손으로 풀었다」를 기억한다(다음 맞추기가 다시 멈추지 않게 · 지금 보류는 다시 멈춘다) */
export async function releaseStops(sb, examId) {
  const r = changed(await db(sb).from("student_book").update({ stop_mode: "running", stop_from: null, stop_until: null }).eq("stop_exam_id", examId).neq("stop_mode", "running").select("id"), "보류를 못 풂", { zero: "ok" }) ?? [];
  if (r.length) return { released: r.length, why: null };
  const bound = row(await db(sb).from("student_book").select("id").eq("stop_exam_id", examId), "묶인 교재를 못 읽음") ?? [];   // 0권일 때만 한 번 더 읽는다 — 화면이 「왜 0인가」를 말해야 한다(대전제-0 · 검사-86)
  return { released: 0, why: bound.length ? `이 시험에 묶인 교재 ${bound.length}권이 이미 다 돌아가는 중입니다` : "이 시험에 묶인 교재가 없습니다. 「지금 보류」를 눌러야 묶입니다" };
}
/** 앞으로의 회차 전부 다시 맞추기 — 규칙·아이 주 수가 바뀌었을 때 · 받아온 뒤(기간이 바뀌었을 수 있다) */
export async function syncAllStops(sb, date) {
  const exams = row(await db(sb).from("exams").select("id,term_from,term_to,english_on").eq("state", "active").eq("hidden", false).not("english_on", "is", null), "시험을 못 읽음") ?? [];
...
/** (저) 크론 한 바퀴 앞 · 앞으로의 회차 보류를 다시 맞춘다(새로 이은 교재 · 학교·학년이 바뀐 아이 · 끝난 창 풀기 · 06b 남긴 것 · 뼈대-9: 새 셈이 아니라 같은 손 syncAllStops).
...
/** 여럿 한 번에((어28)-③ · 대전제-20) · 숨기기·복구 · 📡 봤음. 하나씩의 손을 차례로 · 막히면 그 시험부터 멈춘다 */
export async function setHiddenMany(sb, ids = [], hidden, date) { const list = [...new Set((ids ?? []).filter(Boolean))]; if (!list.length) throw new Error("고른 시험이 없습니다"); let n = 0; for (const id of list) { await setHidden(sb, id, hidden, date); n++; } return { n }; }
export async function setChangeSeenMany(sb, ids = []) { const list = [...new Set((ids ?? []).filter(Boolean))]; if (!list.length) throw new Error("고른 시험이 없습니다"); let n = 0; for (const id of list) { await setChangeSeen(sb, id); n++; } return { n }; }
