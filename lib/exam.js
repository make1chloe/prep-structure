...
export async function addScope(sb, examId, { unitIds = [], freeNote = null, date }) {
  ...
  if (ids.length) {
    ...
    if (back.length) row(await db(sb).from("prep_scope").update({ removed_on: null, added_on: date }).in("id", back).select("id"), "범위를 못 되살림");
...
export async function removeScope(sb, ids = [], date) {
  ...
  const r = row(await db(sb).from("prep_scope").update({ removed_on: date }).in("id", list).is("removed_on", null).select("id"), "범위를 못 뺌") ?? [];
...
export async function setSkip(sb, examId, studentId, skipped, date) {
  ...
  row(await db(sb).from("exam_skip").upsert({ exam_id: examId, student_id: studentId, skipped: Boolean(skipped) }, { onConflict: "exam_id,student_id" }).select("exam_id"), "안 봄을 못 적음");
...
export async function setSkipAll(sb, examId, studentIds = [], date) {
  ...
  row(await db(sb).from("exam_skip").upsert(ids.map((sid) => ({ exam_id: examId, student_id: sid, skipped: true })), { onConflict: "exam_id,student_id" }).select("exam_id"), "안 봄을 못 적음");
...
export async function setHidden(sb, examId, hidden, date) {
  const r = row(await db(sb).from("exams").update({ hidden: Boolean(hidden) }).eq("id", examId).select("id"), "숨김을 못 적음") ?? [];
...
export async function setStopWeeks(sb, level, weeks, date) {
  ...
  const r = row(await db(sb).from("rule").update({ value: String(n) }).eq("key", `prep.stop_weeks.${level}`).select("key"), "규칙을 못 고침") ?? [];
...
export async function setStudentWeeks(sb, studentId, weeks, date) {
  ...
  const r = row(await db(sb).from("students").update({ stop_weeks: n }).eq("id", studentId).select("id"), "아이의 주 수를 못 적음") ?? [];
...
export async function syncStops(sb, examId, date, { from = null, force = false } = {}) {
  ...
  const release = async (ids) => { if (!ids.length) return 0; row(await db(sb).from("student_book").update({ stop_mode: "running", stop_exam_id: null, stop_from: null, stop_until: null }).in("id", ids).select("id"), "멈춤을 못 풂"); return ids.length; };
  ...
  for (const b of books) {
    ...
    row(await db(sb).from("student_book").update(patch).eq("id", b.id).select("id"), "멈춤을 못 맞춤"); n++;
  }
...
export async function releaseStops(sb, examId) {
  const r = row(await db(sb).from("student_book").update({ stop_mode: "running", stop_from: null, stop_until: null }).eq("stop_exam_id", examId).neq("stop_mode", "running").select("id"), "멈춤을 못 풂") ?? [];
