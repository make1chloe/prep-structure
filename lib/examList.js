...
export function isMockExam(exam = {}) {
  if (isSuneung(exam)) return false;             // 대수능은 모의고사가 아니다
  const t = `${exam.name || ""} ${exam.neis_name || ""}`;
  return /전국\s*연합|학력\s*평가|모의\s*고사|모평|수능/.test(t);
}

/**
 * **수능은 모의고사도, 내신도 아니다** (원장님, 2026-08-09 — 「수능이 내신
 * 시험으로 잡혀 있으면 안 되는 거잖아. 수능 모의고사인데, 수능은 그냥
 * 대수능!!」).
 *
 * 셋을 다 갈라야 한다 —
 *   내신      학교가 내고, 범위가 있고, 대비 자료를 만든다
 *   모의고사  교육청·평가원이 내고, 범위가 없고, 성적만 붙는다
 *   대수능    한 해에 하루. 대비도 성적 입력도 여기서 할 일이 아니다
 *
 * ── **「수능」 두 글자로는 못 잡는다** ───────────────────
 *
 * 대「학수학능력」시험 — 수와 능이 붙어 있지 않다. 그래서 위 isMockExam 의
 * `수능` 이 「대학수학능력시험」 을 **한 번도 못 잡았고**, 그 줄은 조용히
 * 내신으로 분류돼 「범위를 안 넣으셨어요」 를 영영 재촉했다. 두 낱말을
 * 다 적어둔다.
 *
 * 「수능 모의평가」 는 평가원 모의고사다 — 모의라는 말이 붙으면 대수능이 아니다.
 */
export function isSuneung(exam = {}) {
  const t = `${exam.name || ""} ${exam.neis_name || ""}`;
  return /대학\s*수학\s*능력|대수능|수능/.test(t) && !/모의|연합|학력\s*평가/.test(t);
}

/**
 * 이 회차가 셋 중 무엇인가 — "school" | "mock" | "suneung".
 *
 * **판단을 한 곳에 모아둔다.** 예전에는 「모의고사인가」 만 물었고, 그
 * 반대편이 곧 내신이었다. 그 사이에 대수능이 끼자 어디에도 안 맞는 것이
 * 조용히 내신 쪽으로 흘러들었다.
 */
export function examKind(exam = {}) {
  if (isSuneung(exam)) return "suneung";
  if (isMockExam(exam)) return "mock";
  return "school";
}

/** 범위 자료를 넣어야 하는 시험인가 — **학교 내신만** 그렇다 */
export const needsScope = (exam) => examKind(exam) === "school";
...
export function filterExams(list = [], { school = "", year = "", kind = "all", q = "" } = {}) {
  ...
  return list.filter((e) => {
    ...
    // 「내신」 을 고르면 대수능도 빠진다 — 그것도 학교가 내는 시험이 아니다
    if (kind === "mock" && examKind(e) === "school") return false;
    if (kind === "school" && examKind(e) !== "school") return false;
    if (!kw) return true;
