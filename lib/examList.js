...
import { classifyExam } from "./examKind.js";

/**
 * **갈래 규칙은 lib/examKind 한 곳에 있다** (2026-08-09 전면 재검토).
 *
 * 예전에는 이 파일과 lib/neis.js 가 **저마다의 정규식**으로 같은 이름을
 * 갈랐고, 「6월 모의평가」 를 한쪽은 전국, 한쪽은 내신이라고 했다. 여기는
 * 그 규칙을 **회차 한 줄(exam_periods)** 모양에 맞춰 감싸기만 한다.
 *
 * 회차 이름은 원장님이 손으로 적으신 것일 수도 있어서(「중간」 「10월 시험」),
 * 갈래가 안 잡히면 **내신으로 본다** — 원장님이 회차를 만드셨다는 것 자체가
 * 대비할 시험이라는 뜻이다.
 */
function classify(exam = {}) {
  const k = classifyExam(`${exam.name || ""} ${exam.neis_name || ""}`);
  return k === "" ? "school" : k;
}

/** 전국연합학력평가·모의평가·모의고사인가 (대수능은 아니다) */
export const isMockExam = (exam = {}) => classify(exam) === "mock";

/** 대수능인가 — 「수능 모의평가」 처럼 모의가 붙으면 아니다 */
export const isSuneung = (exam = {}) => classify(exam) === "suneung";

/**
 * 이 회차가 무엇인가 — "school" | "mock" | "suneung" | "assess".
 *
 * assess 는 수행평가·학업성취도평가처럼 **지필 회차가 아닌 평가**다. 옛
 * 코드가 회차로 만들어 둔 줄이 남아 있을 수 있어서 갈래가 필요하다 —
 * 내신으로 보면 「범위를 안 넣으셨어요」 가 영영 켜져 있고, 진짜 내신이
 * 그 줄에 가서 붙는다.
 */
export const examKind = (exam = {}) => classify(exam);

/**
 * 범위 자료를 넣어야 하는 시험인가 — **학교 내신 지필만** 그렇다.
 * 모의고사는 그동안 배운 전부가 범위고, 대수능·수행평가는 대비 자료를
 * 만들 일이 없다. 늘 켜진 재촉은 안 보이는 재촉이 된다.
 */
export const needsScope = (exam) => classify(exam) === "school";

/**
 * **수행평가도 대비한다** (원장님 2026-08-24 — 「시험종류는 1학기중간,
 * 1학기기말, 2학기중간, 2학기기말, **수행평가**」).
 *
 * 2026-08-09 에 수행평가를 「지필 회차가 아닌 평가(assess)」 로 갈라 두었다.
 * 그 판단은 **회차·재촉** 쪽에서는 지금도 맞다 — 수행평가로 월 수업 회차를
 * 만들면 안 되고, 「범위를 안 넣으셨어요」 를 영영 켜 두어서도 안 된다.
 *
 * 그런데 **내신 대비 화면**은 다르다. 원장님은 수행평가에도 자료를 만들어
 * 아이들에게 배부하신다. 그래서 여태 수행평가를 더해도 저장은 되는데 목록에
 * 안 보였다 — 오류도 없이 사라지니 「추가가 안 된다」 로 보인다.
 *
 * 그래서 갈래는 그대로 두고, **대비 화면이 보는 잣대만** 따로 둔다.
 * 학업성취도·진단평가까지 끌어오지 않게 「수행」 만 집어 온다.
 */
export const isPerf = (exam = {}) => /수행/.test(exam.name || "");
export const needsPrep = (exam = {}) => needsScope(exam) || isPerf(exam);
...
export function termLabel(exam = {}) {
  ...
  /**
   * **지필 회차가 아닌 평가는 학기·중간·기말로 고쳐 쓰지 않는다**
   * (원장님 2026-08-24 — 「수행평가가 회차가 2학기중간고사로 보여」).
   *
   * termOf 는 이름에 「중간·기말」 이 없으면 **달로 짐작한다.** 그래서
   * 「수행평가1」 이 「26년 2학기 기말」 이 되어 화면에 나갔다 — 그냥 못생긴
   * 이름이 아니라 **거짓말이다.** 진짜 기말고사 줄과 나란히 서면 구별이 안 되고,
   * 내신 대비 · 학사일정 · 달력이 전부 같은 거짓 이름을 쓴다.
   *
   * 짐작이 통하는 것은 학교가 「2회고사」 「제2차 지필평가」 라고 적는
   * **지필 회차**뿐이다. 수행평가·학업성취도는 애초에 회차가 아니다 —
   * 적힌 이름 그대로 두는 것이 맞다.
   */
  if (classify(exam) === "assess") return "";
...
export function examTitle(exam = {}) {
  ...
  // 수행평가·학업성취도는 적힌 이름 그대로 (termLabel 이 "" 를 주므로 아래로 떨어진다)
 ...
 */
function groupKey(e) {
  if (isSuneung(e)) return "9999";                   // 대수능은 맨 뒤, 혼자
  if (isMockExam(e)) return "9998";                  // 모의고사는 그 앞
  const { year, half, round } = termOf(e);
  if (!year) return "9990";                          // 언제 것인지 모르는 것
  return `${year}${half || 9}${round === "중간" ? 1 : round === "기말" ? 2 : 9}`;
}

function groupLabel(e) {
  if (isSuneung(e)) return "대수능";
  if (isMockExam(e)) return "모의고사";
  return termLabel(e) || "때를 모르는 시험";
}
...
export function groupExams(list = []) {
  ...
  sortExams(list, { key: "term", dir: "asc" }).forEach((e) => {
    ...
    if (!at.has(k)) {
      at.set(k, out.length);
      out.push({ key: k, label: groupLabel(e), rows: [] });
    }
 ...
 */
/**
 * @param past  지난 시험도 볼 것인가. **기본은 안 본다** (원장님, 2026-08-13 —
 *   「지난건 안 보는걸 기본값으로 하고, 체크박스로 보게해줘」).
 *   나이스에서 한 해치를 받으면 지난 3~7월 시험이 통째로 쌓여서, 앞으로
 *   무슨 시험이 있는지가 그 아래로 묻힌다. 할일 화면이 이미 같은 규칙이다.
 * @param today "YYYY-MM-DD" — 없으면 지난 것을 가리지 않는다 (모르면 안 가린다)
 */
export function filterExams(
  list = [],
  { school = "", year = "", kind = "all", q = "", past = true, today = "" } = {}
) {
  const kw = (q || "").trim().toLowerCase();
  return list.filter((e) => {
    // **끝난 날**로 본다 — 내신은 여러 날이라 시작일로 가리면 시험 중인 것이 사라진다
    if (!past && today && (e.to_date || e.from_date || "") < today) return false;
    if (school && (e.school || "") !== school) return false;
    if (year && String(termOf(e).year || "") !== String(year)) return false;
    // 「내신」 을 고르면 모의고사·대수능이 빠진다. 수행평가류(assess)는 학교
    // 일이니 내신 쪽에 둔다 — 어차피 다음 받아오기가 치울 옛 줄이다
    const k = examKind(e);
    if (kind === "mock" && k !== "mock" && k !== "suneung") return false;
    if (kind === "school" && (k === "mock" || k === "suneung")) return false;
    if (!kw) return true;
