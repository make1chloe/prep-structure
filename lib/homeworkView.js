...
import { maskRows, GATE_COLS, GATE_COLS_OLD } from "./closeGate.js";

/** 학생·학부모 화면이 함께 쓰는 수업 기록 칸 (마감 판정 칸은 아래에서 붙인다) */
const REPORT_BASE =
  "id, date, own_progress, notice, word_correct, word_total, sent_correct, sent_total, attendance_kind";
const REPORT_COLS = `${REPORT_BASE}, ${GATE_COLS}`;

/**
 * **진짜 수업인가.** 선행 거절 제거(2026-08-26) 뒤에는 출결 없이 검사·
 * 배정만 얹힌 판이 생길 수 있다 — 그 판을 「최근 수업」 으로 그리면
 * 학부모 화면에 날짜만 있는 빈 수업이 뜨고 진짜 마지막 수업을 밀어낸다.
 * 기준은 월간 집계(#16 — attendance_kind)와 동일. 칸이 없어 못 읽은
 * 옛 DB(undefined)는 수업으로 친다 — 다 숨기는 쪽이 더 나쁘다.
 */
export const isLesson = (r) => r?.attendance_kind !== null;
...
export async function loadReports(supabase, studentId, todayStr, limit = 6) {
  ...
  if (error) {
    ...
    ({ data, error } = await q(REPORT_COLS));
  }
  if (error) {
    // 0169 전이면 closed_at 이 없다 — report_written 만으로도 게이트는 산다
    ({ data, error } = await q(`${REPORT_BASE}, ${GATE_COLS_OLD}`));
  }
  if (error) {
    // 그보다 옛날이면 출결 미러 칸도 없다 — isLesson 은 undefined 를 수업으로 친다
    ({ data } = await q(REPORT_BASE.replace(", attendance_kind", "")));
  }
  /**
   * **리포트 부분은 마감 전 통째 비노출** (원장 확정 8/28 — 「무조건 마감된
   * 것만 학생·학부모에게 공개한다」. 8/27 의 「점수는 공개」 를 뒤집은 것).
   *
   * 공지·점수·진도는 판 **칸**이라 RLS 로 못 가린다 (행 단위). 그래서 읽는
   * 이 한 곳에서 가린다 — /me 성장 기록·흐름·공지, /parent 최근 수업이
   * 한꺼번에 낫는다. 무엇을 가리는지의 판정은 lib/closeGate 한 벌.
   */
...
/**
 * 체크리스트 한 칸(줄바꿈으로 나눈 글, 0045)을 **줄 목록**으로.
 *
 * 아래 toView 가 쓰던 규칙을 그대로 이름만 붙였다 — 빈 줄과 앞뒤 공백을
 * 버린다. 교재 인쇄물(app/textbooks/print)도 같은 줄을 세어 네모를 그려야
 * 해서, 두 벌로 풀면 화면에는 세 줄인데 종이에는 네 줄이 되는 날이 온다.
 */
  ...
  return (x) => {
    ...
    return {
      ...
      name: item?.quick && (x.range_note || "").trim()
        ...
        : item?.name || "숙제",
      method: item?.method || "",
      checklist: checklistLines(item?.checklist),
...
/**
 * 지난 검사에서 ✕(안 해옴) 받은 항목 id 들 (#30).
 *
 * ✕는 「지난 숙제 검사」 에는 미제출로 뜨는데, 오늘 할 것 줄에는 아무
 * 표시가 없었다 — 아이가 어떤 숙제를 다시 봐야 하는지 알 수 없다.
 * 모집단은 그 화면과 같다: **최근 수업(isLesson)의 검사 줄** 가운데 ✕.
 * 오늘 할 것 줄은 이 집합을 받아 조용히 구분만 한다 (원칙 1 — 셈은 여기 한 곳).
 */
export function missedItemIds(reports = [], dri = []) {
  const from = (reports || []).find(isLesson);
  if (!from) return new Set();
  return new Set(
    dri
      .filter((x) => x.daily_report_id === from.id && x.status === "missing")
