...
export async function loadReportItems(supabase, reportIds = []) {
  ...
  let { data, error } = await q(
    `${BASE}, textbook_unit_id, textbook_unit_ids, range_note, student_done_at, changed_at, check_note, inclass_sort`
  );
  if (error) {
    // 0140 전이면 차례 없이
    ({ data, error } = await q(
...
export async function loadHomeworkItems(supabase) {
  ...
  // tool 은 0116, unit_test 는 0106 에서 붙는다 — 없으면 그 앞 것으로 물러난다
  let { data, error } = await q(
