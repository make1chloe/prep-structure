...
const REPORT_COLS =
  "id, date, own_progress, notice, word_correct, word_total, sent_correct, sent_total, attendance_kind, report_written";
...
export async function loadReports(supabase, studentId, todayStr, limit = 6) {
  ...
  if (error) {
    ...
    ({ data } = await q(REPORT_COLS.replace(", attendance_kind", "").replace(", report_written", "")));
  }
  // **리포트 부분은 마감 전 통째 비노출** (0169 — 원장 확정 8/27 「입력은
  // 실시간, 노출은 마감 후」). 공지(notice)는 판 칸이라 RLS 로 못 가리니
  // 여기 한 곳에서 가린다 — /me 공지 목록·/parent 최근 수업이 같이 낫는다.
  // 칸을 못 읽은 옛 DB(report_written undefined)는 공개로 친다(다 가리는
  // 쪽이 더 나쁘다) — 배포는 0169 실행 뒤로 게이트되어 있다.
  return (data || []).map((r) =>
    r.report_written === false ? { ...r, notice: null } : r
  );
}
