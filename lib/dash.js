...
export async function dashboard(sb, date, { principal = false } = {}) {
  ...
  const [books, routines, todo, late, warns, unitTodo, retests, opsR, makeupTodo, exams, requests, inquiries, unreadR] = await Promise.all([   // (어87) 권한 설정 조회를 걷어 13 + 위 4 + 오늘 1 + 로그인 1 = 19 ≤ 20(상한 — 다음 조회는 판으로 묶는다)
    booksOf(sb, ids, date),
