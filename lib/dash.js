...
import { seoulHour, seoulStamp } from "./day-plan.js";
import { bookGaps, gapText, areaCount, gapSummary, examLines, ccText, queueText, progressBand, confirmLine, memoCalls } from "./dash-plan.js";
import { shortClasses } from "./schedule-plan.js";   // (처) 「8회를 못 채웁니다」 — 12 와 같은 판단 한 벌
import { rows } from "./sqlError.js";
import { accessAllQuery } from "./access.js";
export async function dashboard(sb, date, { principal = false } = {}) {
  ...
  const [books, routines, todo, late, warns, unitTodo, retests, opsR, makeupTodo, exams, requests, inquiries, access, unreadR] = await Promise.all([   // 14 + 위 4 + 오늘 1 + 로그인 1 = 20 ≤ 20(상한 — 다음 조회는 판으로 묶는다)
    booksOf(sb, ids, date),
    ...
    db(sb).from("exams").select("id,name,scope,state,hidden,school_id,english_on,term_from,term_to,prev_term_from,prev_term_to,changed_at,changed_seen_at,schools(name)").eq("state", "active").eq("hidden", false).or(`english_on.gte.${date},term_from.gte.${date},and(changed_at.not.is.null,changed_seen_at.is.null)`),   // 학교 회차 임박 + (저) 나이스가 기간을 옮긴 회차(봤음 전) · 유형 가르기는 examLines(숨긴 회차는 뺀다)
    ...
    principal ? accessAllQuery(sb) : { data: null },   // 읽는 자리는 lib/access.js 한 곳(원칙-1)
    db(sb).from("notify_log").select("student_id").eq("kind", "daily").not("sent_at", "is", null).is("opened_at", null).gte("sent_at", seoulStamp(date, "00:00")),   // 📨 오늘(서울 자정부터) 나간 데일리리포트 중 안 읽은 집(발송 이력 opened_at · 4단계-2a)
  ]);
  ...
  const counts = rows(todo, "안 한 소단원 수"), rules = ops?.rules ?? {};   // todo_counts: 안 한 소단원 · 마지막 진도 표시 · 교재 시작 · 메모로만 연속(0140) · 규칙 dash.*(dash_ops)
  const gaps = bookGaps({ books, routines: rows(routines, "루틴 있는 영역"), todo: counts, date, rules });
  const unread = new Set(rows(unreadR, "안 읽은 집").map((r) => r.student_id)).size;
  return {
    unread,
    date, people: p,
    ...
    calls: memoCalls({ books, todo: counts, rules, date }).map((c) => ({ ...c, name: nameOf(c.student_id) })),   // 메모로만 N회 연속 — 앱이 부른다(5단계-①)
    late: rows(late, "하원 지연").filter((l) => l.until_at).map((l) => ({ id: l.id, name: l.day_sheet?.students?.name ?? "", sent: Boolean(l.sent_at), until: String(l.until_at).slice(0, 5) })),
    ...
    progress: progressBand(ops, date),   // 진도 체크 열림 띠(확정-㊶ 「N일째 열려 있습니다」) — dash_ops 한 조회에 얹었다
    ...
    pref: ops?.pref ?? null,   // 카드 순서(확정-⑮ · 4단계-6 — dash_ops 한 조회에 얹었다)
    confirm: confirmLine(ops?.confirm_next, date),   // 다음 달 일정 확정 줄(4단계-3b · ㉚ — dash_ops 한 조회에 얹었다)
    due: ops?.due ?? null,   // (어84) ⏰ 마감 필요 — 오늘 안 끝난 아이 줄 + 오늘까지가 마감인 업무(dash_ops 한 조회에 얹었다 · 0181). 0181 전이면 null → 카드가 스스로 「아직 못 셉니다」라고 말한다(대전제-27)
    short: shortClasses(ops?.classes),   // (처) 「화·목반 7회 — 8회를 못 채웁니다」 — dash_ops.classes(이 달 정규반 회차 · 0153) · 판단은 12 와 같은 sessionsOf
  };
}
