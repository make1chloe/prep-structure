...
export async function setNotice(sb, id, f) { const p = parseNotice(f); const r = row(await db(sb).from("notice").update(p).eq("id", id).is("sent_at", null).select("id"), "공지를 못 고침"); if (!r?.length) throw new Error("보낸 공지는 못 고칩니다 — 먼저 본 집과 달라집니다(새 공지로)"); }
...
export async function attachNotice(sb, fileId, noticeId) {
  ...
  row(await db(sb).from("file_link").upsert({ file_id: fileId, notice_id: noticeId }, { onConflict: "file_id,bin_id,day_item_id,notice_id,consult_id", ignoreDuplicates: true }), "공지에 못 붙임");
}
...
export async function sendNotice(svc, sb, noticeId, date) {
  ...
  row(await db(sb).from("notice").update({ sent_at: new Date().toISOString() }).eq("id", noticeId).select("id"), "보낸 때를 못 적음");
