/** 📢 공지의 손(4단계-6) · 판 읽기(notice_board) · 만들기·수정(보내기 전만) · 📎 붙이기(보내기 전만 · 보낸 뒤에 붙이면 먼저 본 집은 못 본다, 목업 20) · 보내기(받는 쪽·반·학교로 대상 아이를 고르고 아이마다 notify 유형 notice · 발송 이력은 notify_log · 끝에 sent_at) · 읽음 찍기(정의자 함수)
...
export async function noticeBoard(sb, date) { const data = await rpc(sb, "notice_board", { p_on: date }, "공지 화면을 못 읽음"); if (!data) throw new Error("공지는 학원 사람의 화면입니다"); return data; }
...
export async function setNotice(sb, id, f) { const p = parseNotice(f); const r = changed(await db(sb).from("notice").update(p).eq("id", id).is("sent_at", null).select("id"), "공지를 수정 못 함", { zero: "ok" }); if (!r?.length) throw new Error("보낸 공지는 못 고칩니다. 먼저 본 집과 달라집니다(새 공지로)"); }
/** 📎 · 보내기 전에만. 같은 파일을 두 번 붙여도 한 줄(짝 표 키) */
export async function attachNotice(sb, fileId, noticeId) {
  ...
  await linkOnce(sb, { file_id: fileId, notice_id: noticeId }, "공지에 못 붙임");   /* ⚠️ (어78) 유일 짝의 칸 이름을 코드가 외우면 붙는 자리가 하나 늘 때마다 앱이 먼저 깨진다(0178 이 짝에 todo_id 를 더하자 공지 붙이기가 죽었다 — 걷기가 잡았다) · 대전제-27 */
}
/** 보내기 · 대상 = 재원생 중 학교·반이 맞는 아이 · 받는 쪽대로 기기(학생 → 아이 기기만 · 학부모 → 학부모 기기 · 둘 다) · 발송 이력은 notify 가 남긴다.
...
/** 고른 공지 여럿을 한 번에 보낸다((어28)-④ · 대전제-20) · sendNotice 를 차례로 · 막히면 그 공지부터 멈춘다(보낸 것은 sent_at 이 있어 다시 안 나간다) */
export async function sendMany(svc, sb, ids = [], date) { const list = [...new Set((ids ?? []).filter(Boolean))]; if (!list.length) throw new Error("고른 공지이 없습니다"); let n = 0, to = 0, sent = 0, failed = 0, sink = null; for (const id of list) { const r = await sendNotice(svc, sb, id, date); n++; to += r.n; sent += r.sent; failed += r.failed; sink = r.sink ?? sink; } return { n, to, sent, failed, sink }; }
