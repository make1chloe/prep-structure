/** 올린 기록 되돌리기 검사(검사-69 · 5단계-④) · SQL 한 곳 undo_excel_run 이 맞게 도나(진짜 DB · 한 트랜잭션 안에서 만들고 끝에 되돌린다):
 *  update → before 로 · insert → 지움 · 걸린 insert(문법 분류가 이어진 단원) → 숨김(지우지 않는다) · ② 지우고 새로(replace_book_units p_run)가 적은 delete(before = 줄 전체) → 같은 id 로 되살림(같은 키의 줄이 살아 있으면 그 줄을 before 내용으로) ·
 *  뒤 올린 기록이 살아 있으면 막힘 · 이미 되돌린 올린 기록은 막힘 · before 가 비면 막힘(0087 파기) · 교재 시트 올린 기록(insert → 지움 · 단원이 걸린 교재는 안 씀으로 · update → before) · book_board.runs 의 can_undo 는 같은 표의 마지막 살아 있는 올린 기록만 · 학원 사람이 아니면 막힘 */
...
try {
  ...
  // ① 덮어쓰기 올린 기록 · 1-2 고침(before = 줄 전체) · 1-3 새 줄 · 1-4 새 줄(분류를 이어 못 지우게)
  ...
  // ② 뒤 올린 기록이 살아 있으면 막힌다 · 판의 can_undo
  const run2 = (await q(`insert into v2.excel_run(tbl, sheet, who) values ('units', 'zz2.xlsx', $1) returning id`, [STAFF]))[0].id;
  ok("뒤에 올린 단원 기록이 살아 있으면 앞 기록은 못 되돌린다(DB 글은 아직 「묶음」)(「뒤에 올린 묶음 #N 을 먼저」)", new RegExp(`뒤에 올린 묶음 #${run2}`).test(await fails(`select v2.undo_excel_run($1)`, [run1])));
  ...
  ok("book_board.runs · 최근 올린 기록에 둘 다 · 앞 올린 기록 can_undo false · 뒤 올린 기록 true · 앞 올린 기록 셈 새로 2 · 고침 1 · 올린 사람 이름", Boolean(rr && r2) && rr?.can_undo === false && r2?.can_undo === true && Number(rr?.n_insert) === 2 && Number(rr?.n_update) === 1 && rr?.who === "zz_시험_원장", J((board?.runs ?? []).slice(0, 2)));
  const res2 = (await q(`select v2.undo_excel_run($1) r`, [run2]))[0].r;
  ok("빈 올린 기록(줄 0)을 되돌리면 셈 0 · undone_at 이 선다", res2.restored === 0 && res2.removed === 0 && (await q(`select undone_at from v2.excel_run where id = $1`, [run2]))[0].undone_at !== null, J(res2));
  ok("이미 되돌린 올린 기록은 다시 못 되돌린다", /이미 되돌린/.test(await fails(`select v2.undo_excel_run($1)`, [run2])));
  ...
  // ③ 지우고 새로(replace_book_units p_run) · delete(before 줄 전체) + insert 를 적는다 → 되돌리면 같은 id 로 되살아난다 · 같은 키의 새 줄이 걸려 있으면 그 줄을 before 내용으로
  ...
  ok("② 지우고 새로 · 파일 2줄 · 올린 기록에 delete 3(before = 줄 전체) · insert 2", cnt === 2 && J(ops3) === J([{ op: "delete", n: 3 }, { op: "insert", n: 2 }]) && (await q(`select before->>'sub' s from v2.excel_row where run_id = $1 and op = 'delete' order by id`, [run3])).map((x) => x.s).join() === "1-1,1-2,1-4", J(ops3));
  ...
  ok("before 가 비워진 올린 기록은 못 되돌린다(90일 파기 · 「비워져 있습니다」)", /비워져/.test(await fails(`select v2.undo_excel_run($1)`, [run4])));
  // ⑤ 교재 시트 올린 기록 · 새 교재 지움 · 단원이 걸린 새 교재는 안 씀으로 · 고친 교재는 before 로
  ...
  ok("단원 올린 기록 #4(before 없음)가 살아 있어도 교재 올린 기록은 되돌릴 수 있다. 표마다 따로 센다", (await q(`select v2.book_board(current_date, $1) b`, [bk]))[0].b?.runs?.find((r) => r.id === Number(run5))?.can_undo === true);
  ...
  ok("교재 시트 올린 기록 되돌리기 · 새 교재는 지워지고(1) · 단원이 걸린 새 교재는 「안 씀」으로(숨김 1 · 지우지 않는다) · 고친 교재는 before 로(레벨·교재비 없음) · 되살림 1", res5.removed === 1 && res5.hidden === 1 && res5.restored === 1 && (await q(`select 1 from v2.books where id = $1`, [bkb])).length === 0 && (await q(`select state from v2.books where id = $1`, [bk3]))[0].state === "stopped" && bkNow.level === null && bkNow.price === null, J([res5, bkNow]));
