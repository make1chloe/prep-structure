 ...
 *    5. **키마다 소비처가 있다**((서2) 2026-09-10) · 원장님이 켜고 끄는 칸이 아무것도 안 바꾸면 화면이 거짓말을 한다(대전제-0).
 *       ME.x · PARENT.x · KEYS 의 page 키(메뉴 BUILT)로 실제로 쓰이는지 센다. 안 쓰는 키는 lib/perm.js 의 그 줄에
...
for (const f of [...files("app"), ...files("lib")].filter((f) => /\.(js|jsx)$/.test(f) && !f.endsWith("lib/perm.js"))) {
  ...
  for (const m of s.matchAll(/["'`](page|ops|me|parent)\.[a-z]+["'`]/g)) bad.push(`${f}: 키 글자 ${m[0]} · lib/perm.js 의 KEYS 로`);
...
if (new Set(KEYS.map((k) => k.key)).size !== KEYS.length) bad.push("키가 겹친다");
...
// 6. 아이·학부모의 **모든** 화면이 키로 잠긴다((서2)) · 카드는 끄면 사라지는데 주소로 들어가면 그대로 열리던 곳이 둘 있었다(/me/book · /me/videos).
//    07·09 의 카드와 **같은 키**여야 한다: 화면마다 decide(…) 가 한 번은 나온다
for (const f of [...files("app/me"), ...files("app/parent")].filter((f) => f.endsWith("page.js"))) {
  const s2 = strip(readFileSync(f, "utf8"));
  if (!/\bdecide\s*\(/.test(s2)) bad.push(`${f}: 아이·학부모 화면인데 키를 안 본다. 카드를 끄면 사라지지만 주소로 들어가면 열린다. 07·09 의 그 카드와 같은 키로 decide(…) 를 한 번 부른다`);
}
// 5. 키마다 소비처 · 이름(ME.books · PARENT.recent …)이 app·lib 어딘가에서 불리나. page 키는 lib/menu.js 의 BUILT 가 소비처다
...
console.log(`check-perm ✓ 키 ${KEYS.length} · 칸 ${CELLS} · 키 글자는 lib/perm.js 에만 · v2.can fail closed · principal 은 못 들어감 · 소비처 없는 키 ${KEYS.filter((k) => k.unused).length}(화면이 그렇게 말한다)`);
