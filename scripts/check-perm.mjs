/** 권한 검사 — 「누가 무엇을 보나」는 lib/perm.js 한 벌 + v3.role_access 의 값. 기본값은 코드에 없다.
 *    1. 열쇠(page.* ops.* me.* parent.*) 글자가 lib/perm.js 밖에 없다 — 화면이 열쇠를 지어내지 않는다
 *    2. 칸 수 = 32 (원장님이 2026-09-03 에 정하신 칸과 같다 — 이름이 바뀌면 그 답이 사라진다)
 *    3. DB 의 v3.can 은 줄이 없으면 거짓(fail closed) · principal 은 role_access 에 못 들어간다
...
if (!/coalesce\(\(select a\.allowed[\s\S]*?\), false\)/.test(sql)) bad.push("v3.can 이 fail closed 가 아니다");
if (!/role_access \([\s\S]*?check \(role in \('instructor', 'assistant', 'student', 'parent'\)\)/.test(sql)) bad.push("role_access 가 principal 을 안 막는다");
if (bad.length) { console.log("check-perm ✗\n  " + bad.join("\n  ")); process.exit(1); }
console.log(`check-perm ✓ 열쇠 ${KEYS.length} · 칸 ${CELLS} · 열쇠 글자는 lib/perm.js 에만 · v3.can fail closed · principal 은 못 들어감`);
