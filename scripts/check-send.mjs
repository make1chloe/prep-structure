/** 발송 판단 검사(검사-㊾) · lib/send-plan.js · lib/notify-plan.js 순수 셈: 치환 칸(뼈대-11) · 예약 때(확정-㉕) · 올린 기록 넷의 상태 · 읽음 셈 · 발송 이력 상태(리허설은 그렇게 말한다) · 방해금지 · 잠금화면 문구 · 기기 고르기 · 스위치 기본 off */
...
ok("뼈대-11 치환 칸 · {{다음달수업일}} 과 빈 {{}} 를 잡는다 · 없으면 빈 목록", JSON.stringify(unfilled("안녕 {{다음달수업일}} 에 {{ }}")) === JSON.stringify(["다음달수업일", "빈 칸"]) && unfilled("치환 없음").length === 0);
...
ok("🕘 지금 · 약속 있는 판만 · 보낸 것은 「HH:MM에 보냄」 + 발송 이력 · 사유 빈 것은 noReason", late.length === 2 && late[0].sent === "19:00" && late[0].log?.rehearsal === true && late[1].sent === null && late[1].noReason === true && nowCount(late) === 1, JSON.stringify(late));
const daily = dailyRows(sheets, logs, sch, jobs);
ok("📨 마감하면 · ready(기본 고름) · open(마감 전) · holes(치환 칸) · scheduled · sent", daily.map((d) => d.state).join(",") === "ready,open,holes,scheduled,sent" && daily[0].checked === true && daily[2].holes[0] === "다음달수업일" && daily[3].scheduledId === "c1" && daily[4].logId === 1 && daily[4].log.read === true, daily.map((d) => d.state).join(","));
...
ok("(커) 발송 이력 유형 이름이 키 그대로인 것이 없다. LOG_NAME 에 없으면 LABEL 의 말로(welcome 「첫 등원 안내」 · schedule 「수업 일정 안내」 · guide 「상담 안내」)", Object.keys(LABEL).every((k) => logName(k) !== k) && logName("welcome") === "첫 등원 안내" && logName("guide") === "상담 안내" && logName("daily") === "데일리리포트", Object.keys(LABEL).filter((k) => logName(k) === k).join());
...
ok("발송 이력 한 줄은 **그 줄만의 것** · 스위치 off 로 안 나간 줄은 **시각만**(HH:MM) 적고 rehearsal 로 표시한다. 「리허설 · 실제로는 안 나감」은 카드 머리가 한 번 말한다(2026-09-12 · 같은 문장 16줄 · 448자였다)",
...
ok("제목 · [클로이영어] 수업 안내 · 이미 붙어 있으면 다시 안 붙인다 · 유형 열여섯(첫 등원 안내는 3단계-8 등록 전환 · 영상 안내는 3단계-9 알림)", titleFor("daily", "클로이영어") === "[클로이영어] 수업 안내" && titleFor("late", "클로이영어") === "[클로이영어] 하원 지연 안내" && titleFor("welcome", "클로이영어") === "[클로이영어] 첫 등원 안내" && titleFor("video", "클로이영어") === "[클로이영어] 영상 안내" && Object.keys(LABEL).length === 16);
ok("유형 열여섯 · + score 「성적 입력 안내」(16 알림 · 아이 기기도) · fee 「수강료 안내」(13 안내) · 4단계-2a · schedule 「수업 일정 안내」(12 확정 → 학부모 · 4단계-3b ㉚) · guide 「상담 안내」((커) 18 안내 문자 · 솔라피) · 제약(0154)과 같은 열", Object.keys(LABEL).length === 16 && titleFor("guide", "클로이영어") === "[클로이영어] 상담 안내" && titleFor("schedule", "클로이영어") === "[클로이영어] 수업 일정 안내" && titleFor("score", "클로이영어") === "[클로이영어] 성적 입력 안내" && titleFor("fee", "클로이영어") === "[클로이영어] 수강료 안내" && titleFor("reject", "클로이영어") === "[클로이영어] 숙제 다시 내기 안내" && titleFor("give", "클로이영어") === "[클로이영어] 학습지 배부 안내");   // (어76) reject — 반려하면 아이에게 · (어96) give — 배부하면 아이에게(칸의 스위치가 켜져 있을 때만)
threw = false; try { titleFor("nope"); } catch { threw = true; } ok("모르는 유형은 던진다", threw);
...
ok("뼈대-8 치환 칸 줄 · 표의 설명을 「{{학생명}} · 아이 이름(예: 강민서)」 꼴로 · 예가 없으면 설명만 · unfilled 가 같은 {{ }} 를 잡는다", phr[0].tag === "{{학생명}}" && phr[0].text === "{{학생명}} · 아이 이름(예: 강민서)" && phr[1].text === "{{내용}} · 안내 본문" && unfilled(phr[0].tag)[0] === "학생명");
console.log(`\n■ 발송 검사 ${n}건 · 실패 ${bad}`);
