 ...
 *  **장원우의 오늘 수업 일지에 검사 자료 52줄**을 남겼고, 2028년 날짜에도 8줄이 굳었다.
...
ok("앞날 수업 일지에 숙제 줄이 없다", future.map(x => `${x.name} ${x.date} · ${x.n}줄`),
...
ok("올린 기록이 안 붙은 판이 없다 (진짜 학생 쪽)", orphan.map(x => `${x.name} ${x.date}`),
