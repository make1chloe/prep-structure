/** 학교 시험 판단 검사(검사-54 · (어18) 「회차」는 수업 회차만) · lib/exam-plan.js 순수 셈: 보는 아이(학교·학년·안 봄·전국은 고등) · 그날·끝나는 날 · 몇 주 전부터(아이 따로 › 학교급 규칙) · 보류 창(영어 시험일 − N주 ~ 끝) · 시험전·시험후 · 범위 올린 기록 · 알약 · 머리 · 보류 글 · 단원 묶기 + routine-plan stopOn 의 「시작 전이면 진행중」 */
...
console.log("■ 날 · 몇 주 · 보류 창");
ok("그날 = 영어 시험일(없으면 기간 시작) · 끝나는 날 = 가장 늦은 것", examOn(mid) === "2026-10-16" && examOn(anyGrade) === "2026-10-14" && examEnd(mid) === "2026-10-17" && examEnd({ term_from: "2026-10-14", english_on: "2026-10-20" }) === "2026-10-20" && examEnd({}) === null);
...
ok("보류 창 · 영어 시험일 10/16 − 4주 = 9/18 부터 시험 끝 10/17 까지 · 6주면 9/4 · 영어 시험일 없으면 null(모르면 안 세운다) · 주 수 없으면 null", J(stopWindow(mid, 4)) === J({ from: "2026-09-18", until: "2026-10-17" }) && stopWindow(mid, 6).from === "2026-09-04" && stopWindow(anyGrade, 4) === null && stopWindow(mid, null) === null, J(stopWindow(mid, 4)));
ok("stopOn · 창에 묶인 교재는 시작 전이면 진행중 · 안에서는 교재 보류 · 지나면 진행중 · 손으로 멈춘 것(stop_from 없음)은 그대로", (() => { const sb = { stop_mode: "book_off", stop_from: "2026-09-18", stop_until: "2026-10-17" }; return stopOn(sb, "2026-09-17") === "running" && stopOn(sb, "2026-09-18") === "book_off" && stopOn(sb, "2026-10-17") === "book_off" && stopOn(sb, "2026-10-18") === "running" && stopOn({ stop_mode: "hw_off" }, "2026-09-01") === "hw_off"; })());
console.log("■ 시험전 · 시험후");
ok("영어 시험일 10/16~10/9~10/15 는 시험전 · 10/16~10/19 는 시험후 · 10/8 · 10/20 은 아무것도 · 여러 회차면 가까운 것", examPhase([mid], "2026-10-09") === "before" && examPhase([mid], "2026-10-15") === "before" && examPhase([mid], "2026-10-16") === "after" && examPhase([mid], "2026-10-19") === "after" && examPhase([mid], "2026-10-08") === null && examPhase([mid], "2026-10-20") === null && examPhase([mid, { ...nat, english_on: "2026-10-10" }], "2026-10-09") === "before" && examPhase([], "2026-10-09") === null);
...
ok("알약 · 회차 4(숨긴 것 빼고) · 숨김 1 · 범위 4줄(산 것만) · 영어 시험일 없음 1(학교 회차 · 아직 안 끝난 것 · 숨긴 것·지난 것은 안 센다)", cnt.exams === 4 && cnt.hidden === 1 && cnt.scopes === 4 && cnt.missing === 1 && J(cnt.missingIds) === J(["e2"]), J(cnt));
...
ok("보류 글 · 아직(9/18부터 보류 · 10/17에 저절로 풀림) · 중(보류 중) · 지남(10/17에 풀림) · 창 없으면 null", stopText(win, "2026-09-01").state === "soon" && stopText(win, "2026-09-01").text === "9/18부터 보류 · 10/17에 저절로 풀림" && stopText(win, "2026-10-01").state === "on" && stopText(win, "2026-10-18").state === "past" && stopText(win, "2026-10-18").text === "10/17에 풀림" && stopText(null, "2026-10-01") === null);
ok("손으로 넣은 회차의 출처 키 · 학교·학년·이름·시작날로(둘째 회차가 (manual, null) 에 걸리던 사고) · 학년·학교 비면 all", manualKey({ scope: "school", schoolId: "s1", grade: 2, name: " 2학기 중간 ", termFrom: "2026-10-14" }) === "manual:school:s1:2:2학기 중간:2026-10-14" && manualKey({ scope: "national", name: "10월 학평", termFrom: "2026-10-14" }) === "manual:national:all:all:10월 학평:2026-10-14" && manualKey({ scope: "school", schoolId: "s1", name: "a", termFrom: "d" }) !== manualKey({ scope: "school", schoolId: "s1", name: "b", termFrom: "d" }));
...
// 「지금 보류」가 0권이면 **까닭**까지 · 2026-09-12 내신대비 한 판에서 「교재 0권」만 뜨고 까닭이 없어 막혔다(대전제-0)
ok("지금 보류 끝말 · 멈춘 것이 있으면 권수만 · 0권이면 까닭과 갯수 · 가장 많은 까닭에 업무까지", stopDone({ kept: 3, skip: [] }) === "지금 멈췄습니다. 교재 3권"
  && stopDone({ kept: 0, skip: [{ why: SKIP_WHY.other, n: 4 }, { why: SKIP_WHY.area, n: 2 }] }) === "지금 멈췄습니다. 교재 0권 · 안 건드린 것 · 다른 시험에 묶임 4 · 내신 교재 2 · 그 시험에서 「풀기」를 먼저"
...
ok("회차가 멈출 수 없는 꼴이면(물림·숨김·영어 시험일 없음) 그 까닭 한 줄 · 빈 것도 안 터진다", stopDone({ kept: 0, skip: [], why: "영어 시험일 없음" }) === "지금 멈췄습니다. 교재 0권 · 영어 시험일 없음" && stopDone(undefined) === "지금 멈췄습니다. 교재 0권" && stopDone({ kept: 0, skip: [{ why: SKIP_WHY.none, n: 1 }] }).includes("보는 아이 없음"));
ok("풀기 끝말도 짝을 맞춘다. 푼 것이 있으면 권수 · 0권이면 까닭부터(묶인 것이 없다 / 이미 다 돌아간다)", releaseDone({ released: 2 }) === "풀었습니다. 교재 2권"
  && releaseDone({ released: 0, why: "이 시험에 묶인 교재가 없습니다. 「지금 보류」를 눌러야 묶입니다" }) === "푼 교재가 없습니다. 이 시험에 묶인 교재가 없습니다. 「지금 보류」를 눌러야 묶입니다"
  && releaseDone(undefined) === "푼 교재가 없습니다. 까닭 없음", J(releaseDone({ released: 0, why: "x" })));
// 글자로도 · 판단은 한 벌(화면이 같은 문장을 또 짓지 않는다 · 손 syncStops 는 여섯 유형을 다 센다)
...
ok("손 syncStops 가 안 건드린 까닭 여섯 유형을 다 센다. 새 유형이 생기면 여기서 잡는다", Object.values(SKIP_WHY).length === 6 && Object.keys(SKIP_WHY).every((k) => 손.includes(`SKIP_WHY.${k}`)) && /skip: \[\.\.\.skipped\]/.test(손), Object.keys(SKIP_WHY).filter((k) => !손.includes(`SKIP_WHY.${k}`)).join());
console.log(`\n■ 학교 시험 검사 ${n}건 · 실패 ${bad}`);
