...
{ const checks = [{ id: 1, status: "missing", learn_items: { name: "워크북" } }, { id: 2, status: "done", learn_items: { name: "단어" } }, { id: 3, status: "missing", range_note: "CHAPTER 1 · 10-18번", learn_items: { name: "교재" } }, { id: 4, status: "missing", learn_items: { name: "워크북" } }];
  ...
  ok("✕ 인 항목만 「이름 미제출」(검사 카드가 보이는 글 · 제목 · 손 글 · 같은 글은 하나) + 처분이 남아서면 「반성문 · 오늘 남아서」(고정) · 글에 든 조각은 on", ch.map((c) => c.text).join("|") === "워크북 미제출|교재 · CHAPTER 1 · 10-18번 미제출|반성문 · 오늘 남아서" && ch[0].on === true && ch[1].on === false && ch[2].fixed === true, JSON.stringify(ch));
