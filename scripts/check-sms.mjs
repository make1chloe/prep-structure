/** 문자(솔라피) 판단 검사(검사-75 · (커) 확정-71) · lib/sms-plan.js 순수 셈을 본보기로: 전화 다듬기·가리기 · 통신사 바이트 · SMS/LMS · 치환(빈 칸은 못 나간다 · 「덧붙임」은 줄째 사라진다) · 솔라피 서명(lib/solapi.js · 고정 값으로) · 짐 · 답 읽기 */
...
import { solapiAuth } from "../lib/solapi.js";   // 서명은 서버 쪽(키를 만진다) · 화면은 sms-plan 만 가져온다
let n = 0, bad = 0;
...
ok("발송 이력에는 가려서 「010-****-5678」 · 짧으면 빈 글", maskPhone("01012345678") === "010-****-5678" && maskPhone("123") === "");
...
console.log("■ 치환 · 빈 칸은 못 나간다(뼈대-11) · 「덧붙임」은 비면 줄째 사라진다");
...
ok("비어도 되는 자리는 둘 · 「덧붙임」(등록 안내)과 「한 줄」((뎌-2) 유형마다의 한 줄) · 빈 칸(공백)도 빈 것으로 본다", OPTIONAL.join() === "덧붙임,한 줄" && fill("a\n{{덧붙임}}\nb", { 덧붙임: "   " }).text === "a\nb");
console.log("■ 솔라피 · 서명 · 짐 · 답");
const a = solapiAuth({ key: "K", secret: "S", date: "2026-09-10T00:00:00.000Z", salt: "abc" });
ok("서명 = HMAC-SHA256(secret, date + salt) · 머리글에 키·때·소금·서명", a.signature === "b26f7025c99a7dcc6efcb9e3bc0d8b240e40493c4f884b0843a1c9afb06281b5" && a.header === `HMAC-SHA256 apiKey=K, date=2026-09-10T00:00:00.000Z, salt=abc, signature=${a.signature}`, a.header);
let threw = false; try { solapiAuth({ key: "", secret: "S" }); } catch { threw = true; }
ok("키가 없으면 던진다(조용히 안 보낸 척하지 않는다)", threw);
...
ok("답 · 2xx + 2000 대면 접수(messageId) · 아니면 까닭 한 줄(키는 안 적는다)", readSolapi({ statusCode: "2000", messageId: "M1" }, 200).ok === true && readSolapi({ statusCode: "2000", messageId: "M1" }, 200).id === "M1" && readSolapi({ errorCode: "ValidationError", errorMessage: "발신번호가 등록되지 않았습니다" }, 400).why === "솔라피 400 ValidationError · 발신번호가 등록되지 않았습니다" && readSolapi(null, 500).ok === false, readSolapi({ errorCode: "ValidationError", errorMessage: "발신번호가 등록되지 않았습니다" }, 400).why);
console.log("■ 문구 · 화면이 고치는 것 다섯");
ok("문구 다섯 · 등록·상담 안내 둘((커))과 (뎌-2) 하원 지연·수강료·월간 리포트 셋 · 이름 · 모르는 유형은 키 그대로", TEMPLATES.length === 5 && templateName("sms_welcome") === "첫 등원 안내" && templateName("sms_guide") === "상담 안내" && templateName("nope") === "nope");
console.log("■ (뎌-2) 앱 알림과 함께 문자로도 · 켠 유형만");
ok("문자로도 갈 수 있는 유형은 셋(하원 지연 · 수강료 · 월간 리포트) · 규칙에 든 것만 나간다 · 아무 유형나 켜지지 않는다",
  SMS_ALSO.map(([k]) => k).join() === "late,fee,monthly" && smsAlso(["late", "fee"], "late") === true && smsAlso(["late"], "fee") === false && smsAlso(["daily"], "daily") === false);
ok("유형마다의 「한 줄」 · 하원 지연은 시각·사유 · 수강료는 달·금액(세 자리 쉼표) · 월간은 그 달 · 재료가 없으면 빈 글(그 줄이 사라진다)",
...
ok("「한 줄」과 「덧붙임」은 비면 그 줄이 통째로 사라진다(치환 칸이 남으면 못 나간다. 뼈대-11 의 반대쪽)", OPTIONAL.includes("한 줄") && OPTIONAL.includes("덧붙임")
...
{ const notify = strip(readFileSync("lib/notify.js", "utf8")), send = strip(readFileSync("lib/send.js", "utf8")), fee = strip(readFileSync("lib/fee.js", "utf8")), rep = strip(readFileSync("lib/report.js", "utf8"));
  ...
  ok("문자가 안 돼도 앱 알림은 이미 나갔다. smsAlong 은 던지지 않고 까닭만 돌려준다(발송 이력은 sms 가 남긴다)", /catch \(e\) \{ return \{ skipped:/.test(notify));
  ok("어느 유형을 문자로 보낼지는 **규칙 한 줄**(send.sms_kinds)에서 읽는다. 코드에 안 박는다(뼈대-5)", /ruleList\(svc, "send\.sms_kinds"\)/.test(notify) && /"send\.sms_kinds"/.test(send));
  ok("켤 수 있는 유형 목록은 한 벌(SMS_ALSO) · 화면·손·검사가 같은 것을 본다(원칙-1)", (() => { const tpl = strip(readFileSync("app/send/templates.js", "utf8"));
    return /SMS_ALSO/.test(tpl) && /SMS_ALSO/.test(send) && !/\[\s*\[\s*"late"/.test(tpl); })());
  ok("걷기가 유형 토글을 눌러본다. 켜고 · 새로 읽어도 켜져 있고 · 다시 눌러 끈다", (() => { const w = strip(readFileSync("scripts/e2e/today.mjs", "utf8"));
    return /data-act=sms-kind/.test(w) && /문자로도: 하원 지연/.test(w) && /문자로도 보내는 유형이 없습니다/.test(w); })());
