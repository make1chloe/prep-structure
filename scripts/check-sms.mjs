...
import { phoneDigits, maskPhone, smsBytes, smsType, lenText, fill, smsBody, readSolapi, SMS_MAX_BYTES, TEMPLATES, templateName, OPTIONAL } from "../lib/sms-plan.js";
...
ok("비어도 되는 자리는 「덧붙임」 하나 · 빈 칸(공백)도 빈 것으로 본다", OPTIONAL.join() === "덧붙임" && fill("a\n{{덧붙임}}\nb", { 덧붙임: "   " }).text === "a\nb");
...
console.log("■ 문구 — 화면이 고치는 것 둘");
ok("문구 둘(sms_welcome 첫 등원 안내 · sms_guide 상담 안내) · 이름 · 모르는 갈래는 열쇠 그대로", TEMPLATES.length === 2 && templateName("sms_welcome") === "첫 등원 안내" && templateName("sms_guide") === "상담 안내" && templateName("nope") === "nope");
console.log(`\n■ 문자 판단 검사 ${n}건 · 실패 ${bad}`);
