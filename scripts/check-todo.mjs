...
ok("카드 10장 = 할 일 7 + 단원평가 1 + 재시험 1 + 성적 받기 1(다 낸 회차는 안 선다) · 종류 열(풀이·채점은 준 자료가 없어 0)", cards.length === 10 && cards.filter((c) => c.kind === "score").length === 1 && cards.find((c) => c.kind === "score").title === "신정중 · 중2 — 3명" && KINDS.length === 10, J(cards.map((c) => c.kind)));
...
ok("칸 열 — 자료 만들기 1(하는 것만) · 인쇄 「20장」(4항목 × 5명) · 배부 1 · 풀이 0 · 채점 0 · 단원평가 1 · 재시험 1 · 성적 받기 1 · 되풀이 1 · 메모 1 · 만들기 칸의 🔥 1", cols.length === 10 && colOf("make").count === "1" && colOf("print").count === "20장" && colOf("hand").count === "1" && colOf("solve").count === "0" && colOf("grade").count === "0" && colOf("unit_test").count === "1" && colOf("retest").count === "1" && colOf("score").count === "1" && colOf("repeat").count === "1" && colOf("note").count === "1" && colOf("make").hot === 1, J(cols.map((c) => c.count)));
...
console.log("■ 4단계-5 — 재시험지 만들었음 · 되풀이 사건 갈래(신규 학생 · 교재 끝나감)");
{
...
{ const due = (over) => ({ student_id: "a", student: "김서은", school: "신정중학교", school_id: "s2", book_id: "b1", book: "중등3800제3", round: 1, mode: "per_chapter", chapter: "CHAPTER 8", seq: null, n: null, covers: "CHAPTER 8", topics: ["관계사"], topic_id: "g1", next_class: "2026-10-06", ...over });
  ...
  const cd = only(due())[0];
  ok("저절로 선 카드 — 「김서은 · 관계사」 · 다음 수업 10월 6일 · 왜 「📚 중등3800제3 · CHAPTER 8 — 대단원 끝 · 저절로」 · 출제함이 새길 것(아이·교재·회독·대단원·덮는 것·분류·25문항)", cd && cd.kind === "unit_test" && cd.state === "todo" && cd.title === "김서은 · 관계사" && cd.dueText === "다음 수업 · 10월 6일" && cd.why.startsWith("📚 중등3800제3 · CHAPTER 8 — 대단원 끝") && J(cd.dueUnitTest) === J({ studentId: "a", bookId: "b1", round: 1, chapter: "CHAPTER 8", seq: null, covers: "CHAPTER 8", topicId: "g1", qCount: 25 }), J(cd));
  const c2 = only(due({ mode: "per_n_sub", chapter: null, seq: 2, n: 3, covers: "1-4 · 1-5 · 2-1", topics: [], topic_id: null, next_class: null }))[0];
  ok("소단원 N개마다 — 「소단원 3개」 · 묶음 번호 2 · 분류 없는 묶음은 「분류 없음」(15 에서 단원에 분류를) · 다음 수업 없으면 그 말", c2.why.includes("소단원 3개") && c2.dueUnitTest.seq === 2 && c2.dueUnitTest.chapter === null && c2.title === "김서은 · 분류 없음" && c2.extra.startsWith("분류 없음") && c2.dueText === "다음 수업이 아직 없어요", J(c2));
...
console.log("■ ✍️ 풀이 · ✅ 채점 — 할 일이 아니라 배정에서 세어 나온다((가)-⑧ 원장님 「학생어플에서 제출」 · 0145)");
const b2 = { today: T, rules: {}, todos: [], materials: [mat({ id: "m4", title: "지문", type: "너른터", exam, created_at: "2026-09-01T00:00:00Z", gives: 2, handed: 2, got: 2, submitted: 1, scored: 0, students: [{ id: "a", name: "김서은", handed: true, stage: "done", submitted_at: "2026-10-01T00:00:00Z", scored_at: null }, { id: "b", name: "강민서", handed: true, stage: "doing", submitted_at: null, scored_at: null }] }),
  mat({ id: "m5", exam, handed: 0, gives: 3, submitted: 0, scored: 0, students: [] })] };   // 판의 materials(준 자료만 — 할 일이 없어도 · 0145)
