 ...
 *  docs/말-사전.md 의 「지금」 말이 화면 글(String · Template · JSXText 토큰 · 주석 뺌 · app(api 뺌) + lib 전체)에 남아 있나 센다.
...
const files = [...walk("app").filter((p) => !p.includes("/api/")), ...readdirSync("lib").filter((f) => /\.js$/.test(f)).map((f) => "lib/" + f)];   // (어40) lib 전체 · 오류 글도 화면에 닿는다
...
const MAX = 0;   // (어39) 실측 270(app + lib/*-plan) → (어40) 전수 치환 뒤 0(app + lib 전체)
