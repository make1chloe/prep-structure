...
p.once("dialog", (dg) => dg.accept()); await rm2.locator("[data-g=book]").filter({ hasText: "독해책" }).locator("button[data-act=book-end]").click(); await p.waitForFunction(() => document.querySelector("[data-g=msg]")?.textContent?.startsWith("끝냈습니다"), null, { timeout: 15000 }); await p.waitForTimeout(1200);
...
await gArea.locator("[data-g=line]").first().locator("button[data-act=edit]").click(); p.once("dialog", (dg) => dg.accept()); await gArea.locator("[data-g=item-form] button[data-act=item-retire]").click(); await p.waitForFunction(() => document.querySelector("[data-g=msg]")?.textContent?.startsWith("항목을 내렸습니다"), null, { timeout: 15000 }); await p.waitForTimeout(1200);
...
{ const cf = sc.locator("[data-g=confirm]"), mN = Number(ymNext.slice(5, 7));   // 4단계-3b 확정 도장(㉚ 9/7 「예상수업일정보내기 기능 필요없음 무조건확정후 알림」)
  ...
  p.once("dialog", (dg) => dg.accept()); await cf.locator("button[data-act=confirm-month]").click(); await p.waitForSelector("[data-g=confirm] [data-g=msg]", { timeout: 15000 }); const cmsg = await cf.locator("[data-g=msg]").textContent(); await p.waitForTimeout(1200);
...
{ const cf = sc.locator("[data-g=confirm]"), mN = Number(ymNext.slice(5, 7));
  ...
  await p.goto(`${APP}/schedule?m=${ymNext}&d=${d10}`); await p.waitForLoadState("networkidle").catch(() => {});
  p.once("dialog", (dg) => dg.accept()); await cf.locator("button[data-act=confirm-month]").click(); await p.waitForSelector("[data-g=confirm] [data-g=msg]", { timeout: 15000 }); await p.waitForTimeout(1200);
...
p.once("dialog", (dg) => dg.accept()); await nrow.locator("button[data-act=send]").click(); await p.waitForFunction(() => document.querySelector("[data-g=msg]")?.textContent?.includes("보냈습니다"), null, { timeout: 20000 }); await p.waitForTimeout(1200);
...
ok("폴더 고르개(datalist — 양식 안에만 있다)에 씨앗 폴더 zz_문법 하나", (await vb.locator("datalist#video-folders option").count()) === 1 && (await vb.locator("datalist#video-folders option").first().getAttribute("value")) === "zz_문법", `${await vb.locator("datalist#video-folders option").count()}`);
...
p.once("dialog", (dg) => dg.accept()); await fe.locator("button[data-act=pay-all]").click(); await p.waitForFunction(() => /줄 받음\(/.test(document.querySelector("[data-g=msg]")?.textContent ?? ""), null, { timeout: 15000 }); await p.waitForTimeout(1200);
