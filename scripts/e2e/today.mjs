...
{ const sm2 = p.locator("main"), h = sm2.locator("[data-g=sent-head]"); if ((await h.getAttribute("aria-expanded")) !== "true") { await h.click(); await p.waitForTimeout(400); }
  ok("(뎌-2) 리허설이면 **켜는 법**을 그 자리에서 말한다(원장님 9/10 「문자 안보내져」 — 앱이 아니라 Vercel 스위치였다) — NOTIFY_SINK=live · Production · Redeploy", (await p.locator("main [data-g=sink-how]").count()) === 1 && (await p.locator("main [data-g=sink-how]").textContent()).includes("Redeploy") && (await p.locator("main [data-g=sink]").textContent()).includes("리허설"), (await p.locator("main [data-g=sink-how]").textContent().catch(() => "없음")).replace(/\s+/g, " ").slice(0, 160));
