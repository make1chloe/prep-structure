...
{ await p.setViewportSize(VIEWS[1].viewport); await p.waitForTimeout(300);   // (거) 폰 상단 띠 — 원장님 9/9 「로그아웃을 첫줄에 배치해 여백없이」
  ...
  ok("(거)(려) 폰 상단 띠 · 로그아웃이 첫 줄(이름·칩) 오른끝 · 탭은 그 아래 꽉 차게", !!h && h.sameRow && h.logoutRight && h.tabsBelow && h.tabsWide && h.tabs === 12, JSON.stringify(h));
...
{ await p.goto(`${APP}/`); await p.waitForLoadState("networkidle").catch(() => {});
  ...
  ok("상단 탭 열둘 · 대시보드 · 오늘 · 발송 · 일정 · **내신** · **할 일** · **성적** · 교재 · 운영 · **학생** · **자료함** · 설정(날마다 여는 것은 한 번에 · 원장님 2026-09-11)", (await tabs.allTextContents()).map((t) => t.trim()).join(",") === "대시보드,오늘,발송,일정,내신,할 일,성적,교재,운영,학생,자료함,설정", (await tabs.allTextContents()).join(" · "));
...
ok(`이 달 · 머리 「${Number(ymNow.slice(0, 4))}년 ${Number(ymNow.slice(5, 7))}월」 · 재원생 줄 ${activeN} ≥ 2 · 단가 줄·학년 기준이 없어 전부 「금액 없음」(0원이 아니다) · 합계 0원 · 저장 잠김 · **학생은 이제 상단 탭이라 여기 카드가 없다**(원칙-1) · 신규 상담 카드는 있다`, (await fe.locator("[data-g=month]").textContent()) === `${Number(ymNow.slice(0, 4))}년 ${Number(ymNow.slice(5, 7))}월` && activeN >= 2 && (await fe.locator("[data-g=fee-row][data-state=none]").count()) === activeN && (await fe.locator("[data-g=none-count]").textContent()) === `금액 없음 ${activeN}` && (await fe.locator("[data-g=sum] b").textContent()) === "0원" && (await fe.locator("button[data-act=save]").isDisabled()) && (await fe.locator("[data-card=students]").count()) === 0 && (await fe.locator("[data-card=inquiry]").count()) === 1, (await fe.textContent()).replace(/\s+/g, " ").slice(0, 300));
...
await fe.locator("button[data-act=pay-all]").click(); await fe.locator("[data-g=sure] button[data-act=sure-yes]").click(); await p.waitForFunction(() => /줄 받음\(/.test(document.querySelector("[data-g=msg]")?.textContent ?? ""), null, { timeout: 15000 }); await p.waitForTimeout(1200);
ok(`✔ 안 받음 2줄 다 받음(묻고) → 「2줄 받음(${Number(todayText.slice(5, 7))}/${Number(todayText.slice(8, 10))}) · 저장했습니다」 · 학생·학생둘 받음 · 받은 날 ${todayText} · 안 받음 0 · 0원 · 단추 잠김 · 금액 없음 ${noneNext} 그대로 · 단가 줄 새로 없음`, (await fe.locator("[data-g=msg]").textContent()) === `2줄 받음(${Number(todayText.slice(5, 7))}/${Number(todayText.slice(8, 10))}) · 저장했습니다` && (await feeRow(S1).getAttribute("data-state")) === "paid" && (await feeRow(S1).locator("input.dt").inputValue()) === todayText && (await feeRow(S2_9).getAttribute("data-state")) === "paid" && (await feeRow(S2_9).locator("input.dt").inputValue()) === todayText && (await fe.locator("[data-g=unpaid-count]").textContent()) === "안 받음 0" && (await fe.locator("[data-g=unpaid-sum]").textContent()) === "안 받음 0원" && (await fe.locator("button[data-act=pay-all]").isDisabled()) && (await fe.locator("[data-g=fee-row][data-state=none]").count()) === noneNext, (await fe.locator("[data-g=msg]").textContent()) + " | " + (await fe.locator("[data-g=fee-bar]").textContent()));
