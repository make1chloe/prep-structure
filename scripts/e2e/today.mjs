...
ok("칸 열 · 풀이 1(2과 단어 — 아직 zz_시험_학생) · 채점 1(대의파악 — 07 에서 제출) · 자료 만들기 2(지난 회차 분석지 · 클카 — ♻️ 는 끝난 채라 안 섬) · 인쇄 「6장」(3항목 × 1명 × 2) · 배부 3 · 성적 받기 1(다음 달 회차 — 안 낸 1명) · 메모 1(12 의 zz_수납 안내) · 🔥 마감 지남 ≥ 3(지난 회차의 셋)", (await td.locator("[data-g=col]").count()) === 10 && (await colCount("solve")) === "1" && (await colCount("grade")) === "1" && (await colCount("make")) === "2" && (await colCount("print")) === "6장" && (await colCount("hand")) === "3" && (await colCount("score")) === "1" && (await colCount("note")) === "1" && Number((await td.locator("[data-g=overdue]").textContent()).replace(/\D/g, "")) >= 3, [await colCount("make"), await colCount("print"), await colCount("hand"), await colCount("score"), await colCount("note"), await td.locator("[data-g=overdue]").textContent()].join(" | "));
