...
{ // 동선이 실제로 줄었나 — 탭 한 번으로 그 화면이 열린다(전엔 운영·일정을 거쳐 두 번이었다)
  ...
  ok("일정 머리의 👉 는 셋 · 탭에 이미 있는 「업무」·「학교별 표」는 뺐다(원칙-1)", (await p.locator("main [data-g=head] a[href^='/schedule/']").count()) === 3 && (await p.locator("main [data-g=head] a[href='/schedule/todo']").count()) === 0 && (await p.locator("main [data-g=head] a[href='/schedule/grid']").count()) === 0, "머리 👉 " + (await p.locator("main [data-g=head] a[href^='/schedule/']").count()));
