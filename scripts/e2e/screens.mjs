...
ok("판이 섰다(원장 손과 같은 길) — 「오늘 낼 숙제 3」(어제 둘 + 그저께 하나 — 판이 서면 검사 줄 전부 · (카))는 이제 검사 줄에서 · 오늘 할 것은 「선생님이 검사하면」", (await meMain.locator("[data-card=due] .h .pill").textContent()) === "3" && !(await meMain.locator("[data-card=due]").textContent()).includes("에 받은 숙제") && (await meMain.locator("[data-card=todo]").textContent()).includes("검사하면"), (await meMain.locator("[data-card=due]").textContent()).replace(/\s+/g, " ").slice(0, 200) + " / " + (await meMain.locator("[data-card=todo]").textContent()).slice(0, 100));
