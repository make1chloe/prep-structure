...
ok("업무 단추의 클래스는 .tk · CSS 가 .tk 로 있고(.tk · aria-pressed · .n · .tb) 아이 화면 카드 .task 에 nav 규칙이 안 섞였다(9/14 겹쳐서 카드가 flex 줄로 깨졌던 것)", /className="tk" data-g="task"/.test(row) && /\.tk\{display:flex/.test(css) && /\.tk\[aria-pressed="true"\]/.test(css) && /\.tk \.tb\{/.test(css) && !/\.task\{display:flex/.test(css));
