/** 받을 교재·학습지 검사(검사-㊼ · 목업 07 📚 · 07-2 ①) · 순수 판단 lib/material-plan.js: 단계 넷 · 유형별 올린 기록(종류 차례) · 끝낸 것은 따로 · 스스로 정한 마감 글(까지 · 오늘까지 · 지났어요) · 달력의 🚩(끝낸 것은 안 뜬다) */
...
ok("유형별 올린 기록 · 클카 문장훈련(2과 단어 · 2과 본문) · 너른터(대의파악) · 종류 차례 · 끝낸 것 1 은 따로", g.groups.map((x) => `${x.name}:${x.items.map((i) => i.material?.title).join("+")}`).join(" / ") === "클카 문장훈련:2과 단어+2과 본문 / 너른터:대의파악" && g.done.length === 1 && g.done[0].material_id === "c", JSON.stringify(g.groups.map((x) => x.name)));
