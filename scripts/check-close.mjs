...
const bad = [];
for (const f of ["lib/day.js", "lib/homework.js", "lib/late.js", "lib/routine.js"]) {
  ...
  for (const m of s.matchAll(/export (?:async )?function (\w+)\s*\([^)]*\)\s*\{/g)) {
    ...
    if (writes && !["ensureSheet", "assertOpen"].includes(name) && !/assertOpen\(|sheetOf\(/.test(body)) bad.push(`${f} ${name}(): 판에 쓰면서 마감을 안 본다`);
  }
}
