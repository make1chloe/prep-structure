 ...
 *  판을 세우는 ensureSheet(새 판은 마감이 없다)와 assertOpen 자신, 달 정리 resetWarnings · 아이 기준 setLimit · 예정 planSave·planNotify(판이 아니라 예정 표다), 검사·마감 안에서만 불리는 markFromCheck·autoDoneOnClose(부르는 쪽이 이미 봤다)만 예외 — 안 내보내는 helper 는 안 본다(손은 내보낸 것만 부른다). 화면의 손(app/today/actions.js)은 lib 만 부른다 — DB 를 직접 만지지 않는다 */
...
for (const f of ["lib/day.js", "lib/homework.js", "lib/late.js", "lib/routine.js", "lib/quiz.js", "lib/warn.js", "lib/progress.js", "lib/plan.js", "lib/comment.js", "lib/area-memo.js"]) {
  ...
  for (const m of s.matchAll(/export (?:async )?function (\w+)\s*\([^)]*\)\s*\{/g)) {
    ...
    if (writes && !["ensureSheet", "assertOpen", "resetWarnings", "setLimit", "markFromCheck", "autoDoneOnClose", "planSave", "planNotify"].includes(name) && !/assertOpen\(|sheetOf\(|sheetRow\(/.test(body)) bad.push(`${f} ${name}(): 판에 쓰면서 마감을 안 본다`);
  }
}
