 ...
 *  판을 세우는 ensureSheet(새 판은 마감이 없다)와 assertOpen 자신, 달 정리 resetWarnings · 아이 기준 setLimit · 🔤 시험 카드 자리 setQuizPos((가)-① — 아이 표 students.quiz_pos 이지 판이 아니다) · 예정 planSave·planNotify(판이 아니라 예정 표다) · 교재 잇기·줄 손(4단계-5 customizeBook · resetBook · endBook 은 루틴·교재 줄이지 판이 아니다) · 실제 하원 setLeft(판이 아니라 등원 표 — 아이가 마감 뒤에 나간다) · 루틴 11 의 손 열(addItem…assignBook · retireItem — 판이 아니라 루틴·교재 표), 검사·마감 안에서만 불리는 markFromCheck·autoDoneOnClose(부르는 쪽이 이미 봤다)만 예외 — 안 내보내는 helper 는 안 본다(손은 내보낸 것만 부른다). 화면의 손(app/today/actions.js)은 lib 만 부른다 — DB 를 직접 만지지 않는다 */
import { readFileSync } from "node:fs";
const strip = (s) => s.replace(/\/\*[\s\S]*?\*\//g, "").replace(/(^|[^:\\])\/\/.*$/gm, "$1");
const bad = [];
for (const f of ["lib/day.js", "lib/homework.js", "lib/late.js", "lib/routine.js", "lib/quiz.js", "lib/warn.js", "lib/progress.js", "lib/plan.js", "lib/comment.js", "lib/area-memo.js"]) {
