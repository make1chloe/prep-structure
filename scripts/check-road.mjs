 ...
 *  머리 꼬리표(회독 · 소단원씩 · 다 한 단원 · 이대로면 · 앞으로의 수업일에서) · 내 교재 상태 글(진행중 · 숙제 보류(수업만) · 교재 보류 · M/D에 풀림 · 기한 지나면 진행중) · ❗ 줄 · 열림 띠 · 원장 쪽(N일째 · 아이별 셈 · ❗ 처분 후보) */
...
const r = roadOf(b);
ok("(어43) 대단원마다 상태 하나(status · 08 이 말과 색으로) · CH1 다 했어요 · CH2 하고 있어요(커서) · CH3 하고 있어요(내가 찍음) · CH4 아직", r.chapters.map((c) => c.status).join() === "done,doing,doing,none", r.chapters.map((c) => c.status).join());
ok("끝냄 1(CH1 = done + skip) · 하는 중 2(CH2 커서 · CH3 아이가 찍음) · 아직 1(CH4) · 다 한 단원 1/4 · 확인 기다리는 중 2", r.done.map((c) => c.chapter).join() === "CH1" && r.doing.map((c) => c.chapter).join() === "CH2,CH3" && r.todo.map((c) => c.chapter).join() === "CH4" && r.finished === 1 && r.total === 4 && r.now === "CH2" && r.pending === 2, J([r.done.map((c) => c.chapter), r.doing.map((c) => c.chapter), r.todo.map((c) => c.chapter)]));
const ch2 = r.doing[0];
...
ok("「3회독」 · 「소단원씩」 · 「다 한 단원 1/4」 · 남은 4 ÷ 1덩어리 = 4번째 수업일 11/4 → 「이대로면 11월 초」", h.round === "3회독" && h.basis === "소단원씩" && h.finished === "다 한 단원 1/4" && h.end === "이대로면 11월 초" && h.sessions === 4 && h.endDate === "2026-11-04", J(h));
...
const bt = bookTags([{ book_id: "b1", name: "중등3800제3", stop_mode: "running" }, { book_id: "b2", name: "수능딥독1", stop_mode: "hw_off" }, { book_id: "b3", name: "능률보카", stop_mode: "book_off", stop_until: "2026-10-17" }, { book_id: "b4", name: "지난 보류", stop_mode: "book_off", stop_until: "2026-10-01" }, { book_id: "b5", name: "아직 안 보류", stop_mode: "book_off", stop_from: "2026-10-20" }], T);
ok("교재 글 · 「중등3800제3 · 진행중」 · 「수능딥독1 · 숙제 보류 (수업만)」 · 「능률보카 · 교재 보류 · 10/17에 풀림」 · 기한 지난 보류·아직 안 온 보류는 진행중(stopOn 한 곳)", bt[0].text === "중등3800제3 · 진행중" && bt[1].text === "수능딥독1 · 숙제 보류 (수업만)" && bt[2].text === "능률보카 · 교재 보류 · 10/17에 풀림" && bt[3].state === "running" && bt[4].state === "running", J(bt.map((x) => x.text)));
...
ok("❗ 줄 · 「CH4 › PSS 4-2」 「이거 아직 안 했어요」 · 9/1에 달았어요 · 기다리는 중 / 기타는 적은 말 · 그대로 두기로 했어요 / 바꿨어요", fl[0].title === "CH4 › PSS 4-2" && fl[0].said === "이거 아직 안 했어요" && fl[0].when === "9/1에 달았어요" && fl[0].waiting === true && fl[0].state === "기다리는 중" && fl[1].said === "두 번 나왔어요" && fl[1].state === "그대로 두기로 했어요" && fl[2].state === "바꿨어요", J(fl));
ok("띠 · 열림 · 닫힘(학원) · 닫힘(이 아이만) · 유형 셋 · 모드 셋", editBand({ can_edit: true }).open === true && editBand({ can_edit: false }, { progress_edit: "follow" }).small.includes("지금은 보기만") && editBand({ can_edit: false }, { progress_edit: "off" }).small.includes("내 것만") && FLAG_KIND.length === 3 && EDIT_MODE.length === 3);
...
}
{
  ...
  ok("(어43) 08 대단원 머리에 「이 단원 다 했어요」 네모(펴지 않아도) · 소단원 줄엔 조각 글", /data-g="chapter-ck"/.test(me) && /markChapter\(c\.markable/.test(me) && /data-g="sub-parts"/.test(me));
