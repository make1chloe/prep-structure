...
ok("16일 · 영어 시험일만(그날은 시험 기간 줄을 겹쳐 안 쓴다)", eventsOf("2026-10-16", b).map((e) => e.text).join() === "🅰️ 영어 시험일 · 신정중");
...
{ const strip = (x) => x.replace(/\/\*[\s\S]*?\*\//g, "").replace(/(^|[^:\\])\/\/.*$/gm, "$1");
  ...
  ok("결석 예정은 02c 와 같은 손 — addAbsence 가 planSave 를 부른다(표를 직접 안 만진다) · 화면 손 absenceAct · 아이 목록은 파도에(속도-1)", /export async function addAbsence/.test(lib) && /addAbsence[\s\S]{0,240}planSave\(/.test(lib) && !/addAbsence[\s\S]{0,240}from\("makeup"\)/.test(lib) && /export async function absenceAct/.test(act) && /Promise\.all\(\[scheduleBoard\(/.test(page) && /studentPicks\(sb, date\)/.test(page));   // (어83) 그날 다니는 반을 알아야 반 단추가 목록을 고른다
  ok("(어83) 반 보강일도 한 부품(ClassMakeup) · 반 카드(반이 정해짐)와 최상단(고르개로 아이를 고름)이 같은 손 · 아이 목록이면 makeupManyAct, 반이면 classMakeupAct", /export function ClassMakeup\(\{ classId = null/.test(panel) && (panel.match(/classMakeupAct\(/g) ?? []).length === 1 && (panel.match(/makeupManyAct\(/g) ?? []).length === 1); }
console.log(`\n■ 일정 검사 ${n}건 · 실패 ${bad}`);
