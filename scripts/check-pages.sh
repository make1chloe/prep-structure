...
echo
echo "== 5-5-3) 이어져 있어야 하는 자리가 끊어지지 않았나 =="
# 같은 사실이 두 곳에 저장되는데 한쪽만 고쳐지는 자리들 (2026-08-28 감사 ⑥).
# 아이디를 고쳤는데 그 아이디로 로그인이 안 되는 종류 — 원장님은 고쳐진 줄 안다
runjs scripts/check-links.mjs
