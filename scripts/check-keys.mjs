/** 연동 설정 검사(검사-76 · (터) 확정-72 · 대전제-9) · 키가 화면으로 새지 않나 · 가리기 셈 · 저장 합치기:
 *  ① 판단(lib/integration-plan.js, 순수): 유형 셋 · 비밀은 길이만 · 알아보기는 앞 넉 자 · 빈 칸이 있으면 안 켜짐 · 비운 칸은 그대로 · 「-」 면 지운다 · 바뀐 것 글에 값이 안 들어간다
 *  ② 글자 검사: 연동 표를 읽는 곳은 손(lib/*.js)뿐 · 화면(app/**)이 v2.integration 을 직접 읽지 않는다 · 설정 화면은 keyBoard(가린 판)만 쓴다 · 키 값이 코드에 안 적혀 있다(대전제-9) */
...
ok("유형 넷 · 솔라피(문자) · 나이스(학사일정) · 앤트로픽(AI 초안) · 클래스카드 확장(받는 길 키) · 키 이름은 v2.integration 의 id 그대로", KEYS.map((k) => k.id).join() === "solapi,neis,anthropic,classcard" && keyDef("solapi").fields.map((f) => f.k).join() === "key,secret,from" && keyDef("nope") === null);
...
ok("가린 값에 진짜 키가 없다. 비밀 칸은 어떤 글자도 안 내보낸다", (() => { const secret = "supersecretvalue"; const r = keyRow(sol, { key: "K", secret, from: "010" }); return !r.fields.some((f) => f.secret && String(f.shown).includes(secret.slice(0, 6))); })());
...
ok("**키 값**(config)을 쓰는 곳은 lib/integration.js 하나 · 키를 고치는 길이 하나다(확정-72). 받은 때·까닭(last_ok_at · last_error)은 그 길의 손도 적는다(문자 시험 · 클래스카드 받기)", badWriters.length === 0, badWriters.join(", "));
...
ok("코드에 진짜 키 꼴이 안 적혀 있다(대전제-9)", hard.length === 0, hard.join(", "));
console.log("■ (퍼) 자동완성이 키를 덮지 않나 · 원장님 2026-09-10 「저장이안돼 뭔가 자동완성같은데 · 고쳐도 다시 보면 계속 저 화면으로바뀌어」");
const keysCode = strip(readFileSync("app/settings/keys.js", "utf8"));
ok("키 칸에 type=password 를 **안** 쓴다. 짝(아이디+비밀번호)으로 보이면 크롬이 저장된 로그인을 넣어 키를 덮는다 · 가리기는 -webkit-text-security 로 한다", !/["'`]password["'`]/.test(keysCode) && /WebkitTextSecurity/.test(keysCode), keysCode.split("\n").filter((l) => /password/.test(l)).join(" / "));
...
ok("(퍼) 넣어 둔 값이 틀린 꼴이면 꼬리표가 「수정 필요」 · 까닭을 그 줄에 적는다(key-bad · 값은 안 적는다)", /수정 필요/.test(keysCode) && /data-g="key-bad"/.test(keysCode) && /r\.bad/.test(keysCode));
