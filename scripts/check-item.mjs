...
{ const U = (id, book) => ({ id, book_id: book, chapter: "C", short: id, sort: 1, books: { name: book, area: "문법" } });
  ...
  ok("(어51) 검사할 숙제가 0줄이면 그 자리에서 숙제 주기·교재 배정(원장님 9/16 「검사'할' 숙제가 없는게 제일큰 문제야」 · 「오늘줄에서 숙제검사 페이지에서부터 배정할 수 있어야해」)", /data-g="check-none"/.test(rj) && /data-act="give-here"/.test(rj) && /data-act="assign-here"/.test(rj) && /<AssignModal studentId=\{sheet\.student_id\}/.test(rj) && /<GiveModal sheet=\{sheet\} slot=\{giveHere\}/.test(rj)); }
