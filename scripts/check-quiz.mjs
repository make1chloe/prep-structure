...
import { parseStyle, scopeLabel, scopeText, S_WAY, quizTag } from "../lib/quiz-plan.js";
...
// ── 순수((어12)) — 카드 자리는 없다: 🔤 는 무조건 숙제 검사 카드 안 맨 끝
ok("quiz_pos 를 읽는 자리 0 — quiz-plan 에 QUIZ_POS 가 없고 · 오늘 조회(lib/day · lib/plan)·lib/quiz·01 줄·루틴 11 화면·손이 quiz_pos 를 안 읽는다((어12) — 자리는 설정이 아니라 기본값, 대전제-14)", !/QUIZ_POS|quizPos/.test(strip(readFileSync("lib/quiz-plan.js", "utf8"))) && ["lib/day.js", "lib/plan.js", "lib/quiz.js", "app/today/row.js", "app/settings/routine/board.js", "app/settings/routine/actions.js"].every((f) => !/quiz_pos|quizPos|QUIZ_POS/.test(strip(readFileSync(f, "utf8")))));
