...
/**
 * **연도 칸을 지난 해로 두고 본다** (2026-08-07).
 *
 * 전에는 `("25/08", 2026)` → `2026-08-25` 를 바랐는데, 이 검사를 돌리는
 * 날이 8월 25일보다 앞이면 그 날짜는 **미래**라 한 해 앞으로 당겨진다
 * (그게 맞다 — 지난 일을 적은 기록은 미래일 수 없다). 그래서 이 줄은
 * **날짜가 바뀌면 틀리는 검사**였다. 여기서 보려는 것은 「앞자리가 12를
 * 넘으면 날로 읽는가」 뿐이므로, 미래가 될 일이 없는 해로 물어본다.
 */
eq(parseDate("25/08", 2025), "2025-08-25", "연도 없이 일/월");
...
eq(parseDate("8/25", 2025), "2025-08-25", "연도 없이 월/일");
