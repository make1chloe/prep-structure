...
const KNOWN = {
  "lib/book.js:applyUpload": "올린 기록(excel_run)이 먼저 서고 교재마다 단원 줄을 올리며 바꾼 줄(excel_row · before = 줄 전체)을 적는다. 반쯤 돼도 그 올린 기록까지 되돌릴 수 있다(되돌리기가 곧 이어지는 길 · 0142) · ②는 SQL 한 곳(replace_book_units)이 지운 줄·새 줄을 적는다",
  "lib/book.js:applyBookSheet": "같은 결(교재 시트) · 교재 한 줄 + 올린 기록 줄 · 반쯤 돼도 올린 기록으로 되돌린다(5단계-④)",
  ...
  "lib/todo.js:dropMaterial": "자료 내림 + 업무 내림 · sync_material_todos(SQL) 가 다시 맞춘다",
  "lib/todo.js:handOut": "나눠 줌 + 업무 hand 끝 · 같은 결(sync)",
