 ...
 *  createServerClient(요청 쿠키 읽기 · 응답 쿠키 쓰기) · getSession(getUser 아님 · 속도-2) · 정적 파일은 안 건드림 · 열쇠 읽기는 lib/supabase-keys 한 벌(next/headers 를 안 문다) · 화면 쪽 setAll 은 조용히 넘긴다 */
...
ok("열쇠 읽기는 lib/supabase-keys.js 한 벌 · 미들웨어는 next/headers 를 안 문다 · lib/supabase.js 도 그것을 쓴다", /from "\.\/lib\/supabase-keys\.js"/.test(m) && !/next\/headers/.test(m) && /from "\.\/supabase-keys\.js"/.test(readFileSync("lib/supabase.js", "utf8")));
